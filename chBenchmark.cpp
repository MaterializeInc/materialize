...
int main(int argc, char* argv[]) {
    ...
    // 2. Run main chBenchmark
        ...
        unsigned int count = Config::getAnalyticalClients() +
                             Config::getTransactionalClients() + 1;
        pthread_barrier_t barStart;
        ...
        unsigned long long qphh =
            analyticalResults * 3600 / Config::getTestDurationInS();
        unsigned long long tpmc =
            transcationalResults * 60 / Config::getTestDurationInS();

        ofstream resultStream;
        resultStream.open((Config::getOutputPath() + "/Results.csv").c_str());
        resultStream << "System Under Test;" << Config::getDataSourceName()
                     << endl;
        resultStream << "Analytical Clients;" << Config::getAnalyticalClients()
                     << endl;
        resultStream << "Transactional Clients;"
                     << Config::getTransactionalClients() << endl;
        resultStream << "Warmup Duration in [s];"
                     << Config::getWarmupDurationInS() << endl;
        resultStream << "Test Duration in [s];" << Config::getTestDurationInS()
                     << endl;
        ...
        resultStream.close();
        Log::l2() << Log::tm()
                  << "-results written to: " << Config::getOutputPath()
                  << "/Results.csv\n";
