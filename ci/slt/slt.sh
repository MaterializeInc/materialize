...
if [[ "${SQLLOGICTEST_FAST-}" ]]; then
    ...
    args=(-v sqllogictest/test/ test/*.slt test/cockroach/*.slt)
fi

RUST_BACKTRACE=full sqllogictest --json-summary-file=target/slt-summary.json "${args[@]}" "$@" \
    | tee target/slt.log
