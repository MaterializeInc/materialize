...
cargo miri nextest run -j"$(nproc)" --partition "count:$PARTITION/$TOTAL" --no-fail-fast --workspace --exclude 'mz-environmentd*' --exclude 'mz-compute-client*' --exclude 'mz-compute-types*' --exclude 'mz-ssh-util*' --exclude 'mz-rocksdb*' --exclude 'fivetran_destination*'
