...
function maintainedObjectsUrl(): string {
  return `${BASE_URL}/regions/${REGION_SLUG}/maintained-objects`;
}
...
const SQL_LABELS: [RegExp, string][] = [
  ...
  [/mz_object_arrangement_sizes/, "objectArrangementSizes (unified)"],
  [/mz_object_fully_qualified_names.*mz_wallclock_global_lag/, "maintainedObjectsList"],
...
test.describe("Console Scalability", () => {
  ...
  /**
   * Test 4: Maintained Objects Query Timing
   *
   * Opens a single Maintained Objects page tab and observes every /api/sql
   * request for 2 minutes. Captures which queries fire, how often, and
   * whether any fail or are slow. Use this to compare before/after when
   * switching to mz_internal.mz_object_arrangement_sizes.
   */
  test("query timing audit on maintained objects", async ({ browser }) => {
    const context = await createAuthenticatedContext(browser);
    const records: QueryRecord[] = [];
    const url = maintainedObjectsUrl();
    ...
    await assertNoErrors(page, "maintained objects audit");
    printQuerySummary(records);

    const slowQueries = records.filter((r) => r.durationMs > 10_000);
    if (slowQueries.length > 0) {
      console.warn(
        `\n⚠ ${slowQueries.length} queries took >10s on maintained objects page.`
      );
    }

    const failedQueries = records.filter(
      (r) => r.failed || r.status !== 200
    );
    console.log(
      `\nMaintained objects: ${records.length} total queries, ${failedQueries.length} failures`
    );
  ...
  /**
   * Test 5: Maintained Objects Multi-Tab Stress
   *
   * Opens 3 tabs on Maintained Objects and 3 tabs on Cluster Detail
   * simultaneously. Holds for 2 minutes. Tests whether arrangement size
   * queries from both pages compete for resources.
   */
  test("maintained objects + cluster detail multi-tab", async ({ browser }) => {
    const context = await createAuthenticatedContext(browser);
    const records: QueryRecord[] = [];

    const urls = [
      { url: maintainedObjectsUrl(), label: "Maintained Objects" },
      { url: clusterDetailUrl(CLUSTER_ID, CLUSTER_NAME), label: "Cluster Detail" },
      { url: maintainedObjectsUrl(), label: "Maintained Objects 2" },
      { url: clusterDetailUrl(CLUSTER_ID, CLUSTER_NAME), label: "Cluster Detail 2" },
      { url: maintainedObjectsUrl(), label: "Maintained Objects 3" },
      { url: clusterDetailUrl(CLUSTER_ID, CLUSTER_NAME), label: "Cluster Detail 3" },
    ];
    ...
    console.log("\nHolding 6 tabs open for 2 minutes...");
    for (let check = 0; check < 4; check++) {
      await pages[0].waitForTimeout(30_000);
      for (let j = 0; j < pages.length; j++) {
        await assertNoErrors(pages[j], `${urls[j].label} (check ${check + 1}/4)`);
      }
      console.log(`Check ${check + 1}/4: all tabs OK`);
    }
