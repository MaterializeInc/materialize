...
import {
  buildCriticalPathPMaxInWindowQuery,
  CriticalPathRow,
} from "./criticalPath";
...
const seedCatalogSql = (testSchema: string) => `
      ...
      > CREATE TABLE ${testSchema}.mz_compute_dependencies (
...
const runQuery = async (
  ...
  const compiled = buildCriticalPathPMaxInWindowQuery(probeId, lookbackMinutes);
...
describe("buildCriticalPathPMaxInWindowQuery", () => {
  /**
  ...
  it("walks a linear chain back to the source", async () => {
    ...
    await testdrive(`${seedCatalogSql(testSchema)}
      > INSERT INTO ${testSchema}.mz_compute_dependencies VALUES
          ('u3', 'u2'),
          ('u2', 'u1');
