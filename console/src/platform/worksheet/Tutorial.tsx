...
import { currentTutorialStepAtom } from "./store";
...
const Runnable = ({ insertAndExecute, value, title }: RunnableProps) => {
    ...
    render: ({ insertAndExecute, title }) => (
      <>
        ...
        <OrderedList spacing={3}>
          ...
          <ListItem textStyle="text-base">
            Switch to the new schema. From the top of the SQL Worksheet, select
            your schema from the namespace dropdown.
          </ListItem>
        </OrderedList>
      </>
    ),
...
const Steps = forwardRef(
  (
    ...
    const [currentTutorialStep] = useAtom(currentTutorialStepAtom);
    const { colors } = useTheme<MaterializeTheme>();
...
const Tutorial = ({ insertAndExecute, ...rest }: TutorialProps) => {
  const { colors } = useTheme<MaterializeTheme>();
  const [step, setStep] = useAtom(currentTutorialStepAtom);
