...
```json
{
  ...
  "fields" : [
    {"name": "c1", "type": "string", "doc": "this is the meaning of column a"},
    {"name": "c2", "type": "int"}
  ]
}
```
...
```sql
CREATE MATERIALIZED VIEW v AS SELECT 'text' AS c1, 'int'::integer as c2;
...
The option will be located in a new parenthesized option list immediately
following the existing `FORMAT AVRO` clause. After this change, `[KEY|VALUE] DOC
ON [TYPE|COLUMN]` will be the only supported option, but we expect to add
further options to control the Avro schema in the future.
...
be attached to a single field in the Avro schema. A `KEY DOC ON ...` option
applies only to the key schema; a `VALUE DOC ON ...` option applies only to the
value schema; a `DOC ON ...` option applies to both types of schemas.
...
Materialize will automatically use [comments] on the columns of materialized
views, or its constituent types, as the default documentation for a field.
Specifically, for a given field in the Avro schema, Materialize will consider
the following sources of documentation in order:
  ...
  * A comment on the column that produced the field. If the field is at the top
    ...
    If the field is nested, this will be a comment on a column of a record type.
  * If the key schema, a `KEY DOC ON TYPE` option specifically naming the field's
    type. If the value schema, a `VALUE DOC ON TYPE` option specifically naming the
    field's type.
  * A `DOC ON TYPE` option specifically naming the field's type.
  * A comment on the field's type.
...
For `c1`, Materialize will consider the following sources of documentation
in order:
...
For `c1.x`, Materialize will consider the following sources of documentation
in order:
  ...
  * A `KEY DOC ON TYPE` or `VALUE DOC ON TYPE` option naming `integer`.
  * A `DOC ON TYPE` option naming `integer`.
  * A comment on type `integer`.
...
explain Avro schema node specifiers and the order in which Materialize searches
...
`docs: BTreeMap<String, String>` parameter, where the key represents the
field specifier and the value represents the requested documentation for that
field.
...
the Avro schema by their Avro names.  [Brennan](https://github.com/umanwizard)
invented a way of unambiguously naming Avro schema nodes in the `kgen` load
generator; [Brennan](https://github.com/umanwizard) invented a way of
