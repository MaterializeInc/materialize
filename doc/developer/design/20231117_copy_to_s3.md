...
* For upsert, we read configured amount of data from persist, but we eventually consolidate in rocksdb (or in memory if disk is not enabled) which maintains the upsert state.
...
and I don't think it will be worth the effort. Also, given that we might
have arrangements spilling to disk in the future, we'll probably get this for free.

TODO: Check on the status of consolidation on read from persist, if it can be used here.

Since backpressure is not feasible right off the bat when reading from materialized view as I had
assumed, there's no reason to limit the scope to only support materialized views/tables and
...
under the configured MAX FILE SIZE. In the initial version, we can have a single timely worker
doing the [multi part uploads](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html)
to S3.
* A single worker starts a multi part upload with a `CreateMultipartUpload` API call to start
writing data to a S3 file in the given path.
* The worker should keep a counter, say `bytes_to_write` to keep track of the aggregate bytes it will write.
...
* The worker receives data to be written to S3, adds it to `bytes_to_write` to see if it will exceed
MAX FILE SIZE.
...
#### S3 file names
The name of the individual S3 files can follow a pattern like `part-0001.csv`, `part-0002.csv` etc.
...
## Open Questions
### Can we do `COPY TO S3` with bounded memory?
Look at the [Bounded Memory](#bounded-memory) section above.

### If we support arbitrary queries, should we support ORDER BY?
TODO: Look into compute's support for `ORDER BY`.

Say, we manage to write the records in an ordered fashion in multiple part files.
When the data is eventually used in an external data warehouse like say an external table,
is the order preserved? If not, then it's probably of not much use.
