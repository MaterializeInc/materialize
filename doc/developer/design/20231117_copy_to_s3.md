...
their [config settings](https://www.postgresql.org/docs/current/config-setting.html#CONFIG-SETTING-NAMES-VALUES), they accept user specified units.
...
So eventually the `MAX FILE SIZE` can be either an integer or a parseable string.
