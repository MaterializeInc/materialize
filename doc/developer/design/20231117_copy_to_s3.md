...
* We should pick a minimum number of part files, and tell the compute sink how many parts it should have.
* The number should be the number of workers in a replica.
* Assign records based on a hash function to parts, exchange the data based on the part to workers.
* Upload the individual parts, name them by their part ID.
