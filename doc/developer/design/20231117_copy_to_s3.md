...
```sql
COPY <mv_name or select_query> -- mv or table name or a valid SELECT query
...
very expensive. As a workeraound when users are using the data as an external
...
under the configured MAX FILE SIZE. Multiple timely workers can do writes in paralle, where each
...
like `<prefix>/part-<worker_idx>-<0001>.csv`, `<prefix>/part-<worker_idx>-<0001>.csv`.
