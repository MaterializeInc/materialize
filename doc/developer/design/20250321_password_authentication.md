...
4. Integrate with third-party identity providers (IDPs) like SCIM, SAML, or OAuth.
5. Auth integration at the balancer layer.
...
1. **Protocol Layer Authentication**: Authentication will occur at the protocol layer of Materialize, ensuring that the coordination layer does not need to be involved in the authentication process. This approach allows for minimal changes to existing code while also enabling a scalable and efficient authentication mechanism. Sessions will only validate the password on initial login.


2. **Synchronization Mechanism**: Since the protocol and coordinator layers run in the same process, synchronization between them can be handled using `Arc<Mutex<Userlist>>`. This will allow the protocol layer to access the synchronized object containing password hashes securely stored in the coordinator.
...
The hashed passwords must be stored in secure, read-only system tables in the catalog. This table will store the hashed password along with the associated user role. The passwords must not be stored in plain text. 
...
- **Access Control**: Hashed passwords should not be extracted from the catalog and should not be accessible through queryable tables. Passwords must not be logged or output in stack traces.
