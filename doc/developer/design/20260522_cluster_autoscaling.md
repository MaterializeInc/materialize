...
- **Hydration signal is available in-process from the controller(s).** Per-replica, per-collection hydration state is tracked in-memory by the controller(s) and updated reactively as frontier information flows in from replicas. An in-process API already exists for asking whether target replicas are hydrated and, optionally, within a lag allowance of the furthest output frontier in a specified reference set. For graceful reconfiguration, that reference set contains the outgoing replicas.
...
- **Graceful reconfiguration.** Engaged when `ALTER` writes a durable `reconfiguration` record with `status = InProgress`. The strategy desires the target's `replication_factor` replicas at its **config shape** (size, logging, and availability-zone list). That many target-shape replicas must be ready before cut-over. Extra target-shape replicas do not block it.
  ...
  Readiness probes log unhydrated collections, finite gaps in `lagging_ticks`, and collections `awaiting_completion` separately. Each finite gap is the distance from the closest hydrated target's output frontier to the reference, in timestamp ticks. Ticks are milliseconds on the epoch-milliseconds timeline. A completed reference requires target completion and has no finite gap.
