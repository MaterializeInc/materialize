...
### Changelog algebra and state profiles

Distilled from design discussion (2026-06-03); this frames the consolidation, the `mz_diff` column, and the physical shapes available to the optimizer.

**Deltas vs accumulations.**
The input's *accumulations* are non-negative at every time — it is a SQL-compatible relation throughout.
What `CHANGES` reifies is the per-time *delta*, and a delta may be negative: `mz_diff < 0` occurs exactly when the input retracts a row at a time after the changelog start (`DELETE`, or the old row of an `UPDATE`).
The snapshot can never be negative — it is the input's contents at the start, a valid relation, with zero-netting rows vanishing entirely.

**The algebraic definition.**
Let `Changes_raw` be the changelog with the diff left in the *multiplicity*: `(cols, mz_timestamp)` as a Z-set, a deletion being multiplicity `-1` at its `mz_timestamp`.
Then the surfaced form is exactly

```sql
SELECT <cols>, mz_timestamp, count(*) AS mz_diff
FROM Changes_raw GROUP BY <cols>, mz_timestamp
```

and the identity is faithful in all cases: projecting `mz_diff` away from the count output yields one `+1` row per `(row, ts)` group, which is precisely what packing the netted diff produces.
`Changes_raw` itself is *not* a relation — a deletion's group accumulates to `-1` permanently, because once `mz_timestamp` is reified into the data nothing can ever re-net it — so it can never surface at SQL level; the count-reduce is load-bearing for validity, not just for the column.
(Exception: over an append-only input the raw form never goes negative and is itself a valid relation.)
The governing principle: **net while the time is still the time coordinate; reify it into data last.**

**Three state profiles** for physically realizing the count:

1. *Reified-time reduce* (`count(*) BY cols, mz_timestamp` as a literal `Reduce`): `ts` in the group key makes every group unique — no trace entries ever share data, compaction merges nothing, and the reduce's input and output traces grow with the full changelog for the dataflow's lifetime. Never lower to this.
2. *Frontier-sealed netting* (`consolidate_pact`, what rendering does today): the same batcher an arrangement builds, minus the trace. Each `(row, ts)` group is final once the frontier passes `ts` — seal, emit, drop; retained state is in-flight updates only. This is the time-in-key specialization of profile 1.
3. *Fused diff-linear aggregate* (`sum(mz_diff [* f(cols)]) GROUP BY k` over `Changes_raw`): skips reification entirely; the recurring group key lets accumulations net under compaction, so state is `~#keys`. This is exactly the introspection pattern — `mz_message_counts` is `count(*)` over a diff-as-multiplicity raw log, planned as an accumulable `GroupAggregate` over the arranged raw stream. An optimizer rule recognizing diff-linear aggregates over `Changes` and rewriting to a single reduce over the raw stream is the corresponding `CHANGES` optimization (the canonical net-change-per-key view collapses to one small reduce). Aggregates not linear in the diff (`count(*)` of changelog entries, `min/max(mz_diff)`) and predicates on `mz_diff` still need profile 2 first.

**Arranging the raw stream.**
Arranged *before* reification (row in the data, `ts` still the time coordinate), the trace is bounded: deletes cancel inserts, so it accumulates to the input's current contents (`~|input|`, not `~|changelog|`), while the arrangement's *batch stream* is already the netted `(row, ts, diff)` sequence the pack step needs (readable via a batch cursor — the only place a diff is legible).
In the per-consumer design this makes one arrangement serve both consumer kinds: changelog consumers tap the batch stream, direct readers tap the trace.
Residual wrinkle: the import reads at `start < as_of`, so direct readers need their accumulation advanced to the `as_of` at read-out, and per-time batches below the `as_of` must not leak progress violations downstream.

**Implementation shape.**
Prefer composing these from idiomatic differential pieces — `SemigroupVariable` + `negate` + `concat` + `arrange` — over bespoke unary operators, so the shapes stay representable and reusable by the optimizer.
...
* **Per-consumer reinterpretation.** Prototyped on this branch; the paragraphs below record the design and where the prototype deviates. Rendering used to reinterpret a changelog import *once, at the import site*, and bind only the packed stream under the input's id — which forced two restrictions. First, reading a collection both directly and via `CHANGES` in one dataflow is rejected (`ChangesMixedRead`): both reads lower to indistinguishable LIR `Get`s of the same id, and the direct read would consume packed changelog rows (a real bug before the rejection: `SELECT (SELECT count(*) FROM r), (SELECT count(*) FROM CHANGES (r AS OF ...))` returned the changelog count twice). Second, multiple one-off reads of one input share the *earliest* start outright. Neither restriction is information-theoretic: the raw import stream read at the earliest start determines everything — advance times to `as_of` for a direct read, advance to a read's own `start` + consolidate + pack for its changelog (post-*packing* filtering cannot re-derive a later snapshot collapse, but pre-packing time advancement can). Lifting both means moving the reinterpretation from the import site to the consumer side. What that entails:
  ...
  * *Costs.* One netting per changelog read instead of one per import (each over the same in-flight window); the snapshot is fetched whenever any consumer needs it; `LirId`-keyed spec plumbing through the controller. Alternatively, a single arrangement of the raw stream serves both consumer kinds at `~|input|` trace cost — see "Changelog algebra and state profiles" above, including the preference for idiomatic differential composition (`SemigroupVariable`/`negate`/`concat`/`arrange`) over bespoke operators.
  * *Prototype deviations.* The per-read start travels as `resolved_start` on the MIR `Changes` node (filled by the peek optimizer at timestamp resolution; deliberately not displayed by EXPLAIN, whose `bound` stays as written) and is copied into `GetPlan::Changelog { start }` at lowering — instead of `LirId`-keyed import metadata. This works because one-off starts are resolved *before* lowering; maintained starts stay on the import (`ChangelogMode::Maintained`), where the three resolution sites already operate, and maintained reads carry `start: None` (their deltas replay strictly after the import start, nothing collapses per read). The consumer-site netting uses `consolidate_pact` (the arrangement batcher without a trace) followed by the pack; the import-level `SnapshotMode` is unchanged (one-off `Include`, maintained `Exclude`), so the all-maintained snapshot-fetch skip is preserved but a maintained import coexisting with a direct read is *not yet* handled snapshot-wise (the direct read of such an import sees deltas only — currently unreachable, as maintained dataflows import each collection once and MV bodies mixing both reads read one-off-style at creation… to be revisited when indexes land). Direct reads advance via `Collection::delay` on the raw binding, keyed off `Context::changelog_imports`.
