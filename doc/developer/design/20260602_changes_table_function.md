...
* **Tests.** `test/sqllogictest/changes.slt` (gating, creation, rejection cases), `test/testdrive/changes.td` (maintained data round-trip: ages in empty, appends deltas, aggregates), `as_of_selection` unit tests.

### Remaining work

Recorded with enough detail to be picked up in a later session.

* **Restart verification (highest value).** The restart-exact-reproduction claim is verified only at the as-of-selection unit level. End-to-end: create a maintained `CHANGES` MV, write data, restart `environmentd`, and assert the window contents are identical post-restart — pre-restart rows still present with their original `mz_timestamp`s, no spurious snapshot rows, no correction churn. Natural home: a platform check (`misc/python/materialize/checks/all_checks/`), which also exercises 0dt upgades (the leased-handle path); the feature flag must be enabled in the check's initialize. Watch for the zero-slack interaction below.
* **Zero slack in restart as-of selection.** The lagged hold sits at exactly `output_read_frontier - window`, and restart needs `as_of >= since + window` with `as_of <= output_upper - 1` — a single valid point. Any extra `since` slippage degrades to the advisory fallback (short window + sink correction). If restart tests show churn in practice, lag the holds by `window + slack` (a small constant or dyncfg) in `apply_read_hold_change`; everything else recomputes from the hold.
* **Window cap.** The window directly sizes the compaction hold on the input, and nothing bounds it today (`mz_now() - INTERVAL '10 years'` pins ten years of history). A system var (e.g. `changes_max_window`, default something safe like a day or an hour) checked in `changelog_window` would bound the cost; rejected-by-default with an opt-up knob matches the "explicit, attributable cost" success criterion.
* **Month intervals.** Rejected because their millisecond lag varies with the calendar while the hold/start machinery needs a constant. Could be supported later with a conservative constant over-approximation (31 days per month) for the hold and start, while the temporal filter keeps the calendar-correct arithmetic — the hold may then over-retain, never under-retain. Only worth doing after the window cap exists.
...
* **Sharp edges to keep in mind.** Multiple changelog reads of one input use the *widest* window for the import's hold and start while each read's filter enforces its own window. A `Maintained { start: None }` import reaching the controller is a `soft_panic_or_log` that degrades to reading at the `as_of` (empty window aging in). Cached physical plans may carry a stale or `None` start; every decision site recomputes by `join`, which is monotone and therefore safe.
