...
One thing is not. The shard upper coming back from each persist sink is concatenated into a
single feedback edge, so its frontier is the meet across exports and every export reports the
...
- Definiteness is preserved. The contents of every collection at each readable timestamp remain a
  deterministic function of durable shared facts.
...
- Memory on the replica during hydration is bounded by the unflushed tail of each open builder,
  independent of snapshot duration.
- The number of batches the sink appends is significantly lower than 1 per tick per worker.
- A restart during a snapshot remains correct.
...
unchanged. Kafka would need no reconciliation at all, since a backfill feed and a steady feed
read disjoint offset ranges. Its second consumer is deferred, see the Kafka section and Rollout.
...
This step preserves existing behavior. All port capabilities are downgraded in lockstep, so every
export observes the same frontier as today.

For existing operators using a single output handle, `handle.give_fueled()` provides a means of
...
**Replication operator:** ports of hydrated exports downgrade with the data upper. A hydrating
export's port holds at the minimum until the data upper passes its snapshot offset, because events
at or below the snapshot offset must still be emitted negated at the minimum for that export. Once
the upper passes the snapshot offset, the port downgrades with the data upper like the rest. The
emission of rewind negations remains unchanged.
...
Kafka reads real offsets and reaches the same place, see the Kafka section. The minter keeps the
...
are deleted rather than appended as truncated.
...
the meet across exports, which has to respect the slowest one, and `mz_source_statistics` reports
the parent source as `MIN(offset_committed)` over its exports, so the source-level number keeps its
old meaning while the per-export rows tell the truth about each table.
...
#### Handling for large transactions

If an upstream transaction contains a large number of rows, they all land at a single timestamp. The
builder holding them spills its parts to blob storage once it passes `persist_blob_target_size`, so
memory is bounded by that rather than by the transaction. A description covering that timestamp puts
the transaction in the same batch, and the same append, as everything else the description covers.
...
consumers emit concurrently for hydrating exports. As today with the transition from hydration to
...
logical choice for `B` is the minimum `resume_upper` for all non-hydrating exports in the
mixed-export case. For the case where all exports are hydrating, `B` keeps its existing definition,
...
each partition in the topic, sum them, and treat the sum as `snapshot_records_known`. In the
mixed-export case, snapshot and steady-state records are being processed concurrently. To
accurately record statistics, metric definitions are updated.
...
correctness, so it is best handled as a separate effort.
...
- **Reader distribution versus pipelining.**
  ...
  - The one-worker shape no longer starves the minter, since every worker exchanges its largest
...
- **Recovering accumulated work across restarts.**
  - A restart during hydration discards the hydrating export's snapshot and CDC data. This will
    affect the freshness of already hydrated exports, as the CDC stream has to be re-read.
...
- **DBLog-style watermark snapshots.**
  ...
  violates definiteness, and all sources would require upsert.
...
- **Shadow pipeline with catch-up and handoff.**
  ...
  to an agreed LSN, and hands off. Rejected as this does not improve the slot's WAL retention, adds
  connection cost on the upstream database, and requires an exactly-once handoff protocol.

The following were considered for the persist sink, and all share a root cause. Each tries to
