...
becoming `hydrated_at IS NOT NULL`. The `complete_mvs` UNION branch for
...
What `hydration_time` should mean is a separate question, discussed below.

### What `hydration_time` measures today, and what would change

Today `hydration_time` is `time_ns` rendered as an interval, and `time_ns` runs
from the `Export` event to the `Hydration` event. In the new terms that is
`hydrated_at - installed_at`, which spans three things:
...
Only the third is hydration in the sense the column name suggests. Narrowing to
`hydrated_at - started_at` would drop 1 and 2, which move to `queue_time`.

The change is one-directional: a narrowed `hydration_time` is always less than or
equal to today's value, never greater. How much smaller depends entirely on the
case, and the two cases from the sequence above bracket it:

| Scenario | Today | Narrowed |
| --- | --- | --- |
| index created on a running cluster with available inputs | queueing is approximately zero, so this is already the work interval | unchanged in practice |
| replica rehydration with a hydration concurrency limit | an object at the back of the queue reports mostly the time other dataflows spent hydrating ahead of it | drops to that object's own work, potentially by orders of magnitude |
| new materialized view over a source that is far behind | includes waiting for the source to reach the as-of, which is not compute work at all | drops to the work interval |

The rehydration row is worth dwelling on, because it makes today's number
actively misleading in an unobvious way. Queueing intervals of different objects
overlap, so the sum of per-object `hydration_time` on a rehydrating replica can
greatly exceed the wall-clock duration of the replica's whole hydration. After
narrowing, the per-object numbers stop double-counting the same wall-clock
seconds.

Three ways to land this:

1. **Narrow `hydration_time`, add `queue_time`.** The column finally means what
   its name says. Existing queries silently return smaller numbers.
2. **Leave `hydration_time` as it is, add `queue_time`.** No breakage, but the
   column keeps a name that overstates what it measures, and the number the PRD's
   history table wants has no name at all.
3. **Leave `hydration_time` as it is, add both `queue_time` and a narrow column.**
   No breakage, every interval named, at the cost of three columns where two
   would do and a `hydration_time` that stays permanently misnamed.

Note that the compat view means the old measurement survives under all three, so
this is purely about which name points at which interval. The relevant risk is
that option 1 changes an existing documented column silently: a user's query
keeps working and returns different numbers, with no error to prompt them to look.

The prediction question the PRD opens with, "how long will hydration take", is
not answered by any per-object column. It is answered by the episode rollup,
`max(hydrated_at) - min(started_at)` over an object set. The per-object column's
job is attribution, which is to say identifying the expensive object, and
attribution wants the work interval rather than the queue wait. That argues for
naming the narrow interval `hydration_time`, and it is the reason this design
initially proposed option 1.
...
1. **`hydration_time` narrow or wide.** The three options are laid out in "What
   `hydration_time` measures today, and what would change". The trade is a
   silently changed column against a permanently misnamed one. Keeping the compat
   view rather than deprecating it was decided on the grounds of not breaking
   existing consumers, and option 1 breaks one in the harder-to-notice way, by
   changing a number rather than removing a relation. Consistency with that
   decision favours option 2 or 3, against the argument from the column's name.
