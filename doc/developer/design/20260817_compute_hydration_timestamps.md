...
7. Each row carries a key that identifies its hydration episode, so a consumer
   ...
   from a new one.
8. No existing relation changes shape or semantics. Every query against
   `mz_compute_hydration_times` and `mz_compute_hydration_statuses` returns what
   it returns today, including the values, so monitoring and analytics built on
   them are unaffected.
...
Storing `installed_at` rather than only reporting a corrected duration is
deliberate, for two reasons. It is what makes the queueing interval derivable at
all, which is the distinction today's single measurement loses. And it is what
the existing relations are rebuilt from: both `mz_compute_hydration_times` and
`mz_compute_hydration_statuses` report `hydrated_at - installed_at`, so without
this column they could not be preserved unchanged.
...
`mz_internal.mz_compute_hydration_statuses` is likewise preserved exactly. Its
`(object_id, replica_id, hydrated, hydration_time)` shape and its semantics both
stay as they are: `hydrated` is `hydrated_at IS NOT NULL` and `hydration_time` is
`hydrated_at - installed_at`, which is precisely the interval `time_ns` measures
today. The `complete_mvs` UNION branch for materialized views that have advanced
to the empty frontier stays as it is, with NULL timestamps, since those objects
have no dataflow installed.

No column is added to it, and no column changes meaning. New consumers should read
`mz_compute_hydration_timestamps` and derive whatever interval they want.

### Why `hydration_time` is left alone

This is worth spelling out, because the narrow reading is tempting and this design
deliberately declines it.

Today `hydration_time` is `time_ns` rendered as an interval, and `time_ns` runs
from the `Export` event to the `Hydration` event, which in the new terms is
`hydrated_at - installed_at`. That spans three things:

1. waiting for input frontiers to advance, because the controller has not sent
   `Schedule` yet,
2. waiting for a hydration concurrency slot, because the interceptor is
   withholding a `Schedule` it already has,
3. the actual hydration work.

Only the third is hydration in the sense the column name suggests, so the column
overstates, sometimes by a lot. How much depends on the case, and the two cases
from the sequence above bracket it:

| Scenario | What `hydration_time` reports |
| --- | --- |
| index created on a running cluster with available inputs | queueing is approximately zero, so the value is already just the work interval |
| replica rehydration with a hydration concurrency limit | mostly the time other dataflows spent hydrating ahead of this one |
| new materialized view over a source that is far behind | includes waiting for the source to reach the as-of, which is not compute work at all |

The rehydration row has a non-obvious consequence. Queueing intervals of
different objects overlap, so the sum of per-object `hydration_time` on a
rehydrating replica can greatly exceed the wall-clock duration of the replica's
whole hydration. Anyone totalling the column is double-counting the same
wall-clock seconds.

Narrowing the column to `hydrated_at - started_at` would fix that, and the
column's name would finally match its contents. We are not doing it, because
monitoring and analytics already depend on the current values and narrowing
breaks those consumers in the worst available way: the query keeps working and
silently returns smaller numbers, with no error to prompt anyone to look. A
misnamed column that has been stable for years is a smaller problem than a
number that quietly changes underneath existing dashboards.

Nor do we add a `queue_time` column to soften it. Every interval anyone could
want is derivable from the timestamps relation, and adding columns to the old
view invites new consumers to build on the compat surface instead of the one we
want them on. The intended migration path is that the views stay frozen and new
work reads `mz_compute_hydration_timestamps`.

One consequence to record: the overstating behavior is now deliberate and load
bearing, so it needs a comment on the view saying so. Otherwise the next person
to notice that `hydration_time` includes queueing will "fix" it.

The prediction question the PRD opens with, "how long will hydration take", is
not answered by any per-object column in any case. It is answered by the episode
rollup, `max(hydrated_at) - min(started_at)` over an object set.
...
- `src/catalog/src/builtin/mz_internal.rs`: the new storage-managed collection
  and its index, `mz_compute_hydration_times` recast as a compat view, and
  `mz_compute_hydration_statuses` rebased onto the new relation with its output
  unchanged, plus a comment recording that its `hydration_time` deliberately
  includes queueing.
...
- Docs: the `mz_internal` and `mz_introspection` system catalog reference pages
  for the new relation. No release note for a behavior change, because there
  isn't one.
...
three timestamps survive an environmentd restart unchanged, and that a replica
restart yields entirely fresh timestamps for every object on it.

The most important test is a compatibility one: that
`mz_compute_hydration_times` and `mz_compute_hydration_statuses` return the same
values before and after the change. Since the whole design rests on those two
staying frozen, the existing assertions in
`test/testdrive/hydration-status.td` and the blue-green tests should be treated
as the contract and left alone rather than adjusted to fit.
...
**Narrow `hydration_time` to the interval its name claims.** Argued at length in
"Why `hydration_time` is left alone". Rejected because monitoring and analytics
depend on the current values, and the failure mode is a silent numeric change
rather than a loud break.

**Add a `queue_time` column to `mz_compute_hydration_statuses`.** Would make the
queueing interval visible without changing anything existing. Rejected because it
extends a surface we want frozen, and every interval it would expose is already
derivable from the timestamps relation that new consumers should be reading.
...
- **Relation naming.** Add the new relation and turn
  ...
  consumers untouched.
- **The queueing interval.** Store `installed_at` as a timestamp rather than only
  reporting a corrected duration. It makes the queueing interval derivable, and it
  is what the preserved views are rebuilt from.
...
- **The compat view stays.** `mz_compute_hydration_times` is kept as a view for
  now, reporting the old measurement. Deprecating it is deferred and is not part
  of this work.
- **`hydration_time` is left exactly as it is.** Monitoring and analytics depend
  on the current values, and narrowing it would change them silently. The view
  keeps a name that overstates what it measures, which is the cheaper problem.
- **No `queue_time` column.** All future consumers should read the timestamps
  relation and derive intervals themselves, rather than growing the compat
  surface.

The net effect of the last three is that this design adds relations and changes
none, so it carries no user-visible behavior change at all.

## Open questions

None outstanding for the compute half. Everything raised in review is recorded
above.

Two things are deliberately left to the adapter half rather than being open
questions here:

- How episodes are written down, deduplicated across repeated observations of the
  same `(replica_id, object_id, started_at)`, and retained.
- How an episode that ends without completing is captured, given that a replica
  crash resets all of its introspection and the only usable record is one taken
