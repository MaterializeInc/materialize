...
stays at the names above and an alias costs a client no context.
...
advertises. A client on an old route therefore sees its historical tools under
its historical names, while the same old name also works on `/api/mcp`.
...
Instead we add one tool, `get_session_info`, returning both halves of the
answer:

```json
{"grants": ..., "session_settings": ...}
```

`grants` is what the caller may reach. `session_settings` is the role settings
that change what it may do, starting with `restrict_to_user_objects` and the
default cluster. They are one call because an agent wants both at the start of
a session, and two tools would cost two calls and two entries in every tool
list.

An agent is expected to call this once per session, so the grants lookup runs
about as often as sessions are opened. It has to stay cheap at that rate, which
rules out a query that walks every object in a large catalog. That is a
constraint on the implementation, not a detail left to it.
...
privileges". That is the same idea this tool is built on.

It is also a constraint to design around, and it decides how the tool is built.
That list holds three OIDs today, the two MCP data product views and
`mz_show_my_cluster_privileges`. `mz_internal.mz_show_all_my_privileges` is the
obvious backing view for the grants half and it is not on the list, so a
SQL-backed implementation would be blocked in exactly the restricted sessions
that most need the answer. The same applies to the settings half if it reads
role defaults from `mz_catalog.mz_role_parameters`.

So the two halves are read differently, and that choice is part of this design.
`session_settings` reads effective values from session state, which needs no
catalog access and cannot be blocked. `grants` reads the catalog snapshot the
request already holds, through the privilege APIs in `src/sql/src/catalog.rs`.
Reading the snapshot is preferred over adding `mz_show_all_my_privileges` to the
exempt list, since it keeps that list small and avoids a per-session SQL query.
...
grants. `get_session_info` then lets the agent read back what that produced,
and say why a tool call failed. It reports, it does not configure.
...
-- query works on cluster prod. query_system_tables fails, because the
-- restriction rejects system catalog objects and only the MCP data product
-- views are exempt.
```
...
What is not reachable is withholding the tool itself. The role can still call
`query`, it just returns what its grants allow, which for catalog statements is
the same answer `query_system_tables` gives. Access is per object, not per tool,
so two roles cannot be given different tool sets. That is the subject of an open
question below.
...
- Clients on the old URLs keep working. They see the same tools under the same
  names.
- Clients that move to `/api/mcp` see the new names and `get_session_info`.
...
two old routes pointed at the same code. Add `get_session_info`. Leave the
audit table and the protocol uplift out.
...
1. Does one URL plus `get_session_info` actually answer "what can this person
   do" for an admin, or do they still need to read docs?
...
`GRANT MCP TOOL query_system_tables TO analyst`. Rejected. A new privilege type
