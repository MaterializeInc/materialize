...
see additional rows but no changed ones. The storage branch of
`mz_mappable_objects` maps each storage object only to its own global id,
not to every global id its dataflow renders. `EXPLAIN ANALYZE CLUSTER`
therefore attributes each stage to exactly one object, and the per-object
forms reach the other exports of a dataflow through
`mz_storage_dataflow_global_ids`.

`EXPLAIN ANALYZE CLUSTER` sums an object's cost over its stage ranges.
Operators outside any stage count toward no object there, so a source's
cluster total can be below the root row of `EXPLAIN ANALYZE ... FOR SOURCE`,
which sums the whole dataflow.
...
has both controllers connected in normal operation. The replica logs each
command it holds and the number it releases, so a source stuck in `starting`
because compute has not connected is visible in the replica's log.
...
situation. This design does not change that. `AS SQL` reads no introspection,
so it skips the check. The generated SQL identifies the explainee by its
global ids, not by name, because a qualified name that contains dots is
ambiguous.
...
caller must own the explainee. A source's report also lists its subsources
and tables, which other roles may own. With the restriction, the generated
SQL lists only the exports the caller owns. The shared stages and the root
row still cover the whole dataflow, which the source's owner owns. Running
the generated SQL reads system catalog objects that the restriction blocks,
so a restricted session can see the generated SQL through `AS SQL` but not
run it, the same as `EXPLAIN ANALYZE CLUSTER`.

## Rollout

The new explainees are gated behind `enable_explain_analyze_storage_objects`.
It is off in production and on in CI. `INGESTION` works on either cluster
topology. `CPU` and `MEMORY` return rows only on unified clusters, so the flag
stays off until `enable_unified_cluster` is on. The user docs mark the new
forms as private preview.

## Minimal Viable Prototype

The `INGESTION` property needs no storage changes and is the first slice. It
lands on its own, before the ordering fix, the storage logger, and the mapping
relations. `CPU` and `MEMORY` follow once those land. Validation for each
slice:
...
* **Reuse `mz_lir_mapping`.** Storage stages are not LIR nodes, and
  ...
  give that column two meanings. Where `EXPLAIN ANALYZE CLUSTER` reads both,
  it unions them into a neutral `node_id` column inside the query.
