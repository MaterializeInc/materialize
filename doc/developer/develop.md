...
Materialize uses the simplest git strategy that allows us to have high assurance that we
are providing excellent quality software to our users. Inside of git, we have the
following high-level strategy, with the goal of 100% automation of all steps that don't
mention a human:

1. A developer pushes an annotated tag that looks like `v<VERSION>-prerelease[<N>]`
1. More detailed testing takes place on the artifacts generated by this tag. For all
   ...
   The set of tests is:
   * Install apt/rpm packages in clean ubuntu/docker image
   * Full SLT (against auto-generated materialize/materialized image)
   * The chbench soak test is run for 48 hours (same image)
   * The protobuf soak test is run for 48 hours (same image)
1. A an annotated tag named `v<VERSION>-rc[<N>]` is pushed to that same git commit.
   (There are now at least two tags on that commit)
1. Soak testing continues for 3 more days, with the same requirements as step 2.
1. A human verifies that everything looks good, and pushes the `v<VERSION>` tag.

So an example of this is:

1. @anonymousHenchperson runs

        git tag -a v0.1-prerelease master
        git push origin v0.1-prerelease
1. Testing happens. If it all succeeds, move on to the step.
1. `git tag -a v0.1-rc v0.1-prerelease` and `git push origin v0.1-rc`
1. Further verification and testing happens.
