...
Generates the row-level diffs that keep the `mz_catalog` system tables (e.g. `mz_tables`, `mz_indexes`, `mz_clusters`) in sync with the in-memory `CatalogState`.
...
`pack_role_auth_update` writes a row into `mz_role_auth` containing the role's password hash and the timestamp it was last updated. `mz_roles` and `mz_role_parameters` are materialized views backed by `mz_internal.mz_catalog_raw`; role and role-parameter rows are not produced by `pack_*` functions here.
