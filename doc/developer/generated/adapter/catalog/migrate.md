...
`ast_rewrite_add_missing_doc_on_ids` backfills catalog item IDs into `DOC ON TYPE` and `DOC ON COLUMN` references stored in `CREATE SINK` statements. Prior to the fix that made name resolution print IDs for types, those references were persisted as bare qualified names. This migration rewrites them to include the ID, so `create_sql` reference extraction can recover the sink's dependency on the commented type. References that already carry an ID are skipped, making the migration idempotent and safe to run every boot.
