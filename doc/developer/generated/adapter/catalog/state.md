...
Connection inlining (used when resolving connection references for storage and compute) handles the `GlueSchemaRegistry` connection type alongside `Kafka`, `Postgres`, and `Csr`.
