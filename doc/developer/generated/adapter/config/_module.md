...
The four child modules divide responsibilities cleanly: `params` owns the data model, `frontend` owns the read side, `backend` owns the write side, and `sync` owns the loop. The `frontend` and `sync` modules are also re-exported for use by the coordinator's create-time synchronous scoped resolution path (`evaluate_scoped_parameters`).
