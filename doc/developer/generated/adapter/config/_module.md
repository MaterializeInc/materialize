...
`SystemParameterSyncClientConfig::LaunchDarkly` carries `sdk_key`, `now_fn`, and `base_uri: Option<String>`; when `base_uri` is set it overrides the LaunchDarkly streaming, polling, and events endpoints with a single base URL (for a relay proxy or mock server in tests).
