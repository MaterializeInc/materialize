...
The LaunchDarkly client is built with `ld::StreamingDataSourceBuilder` and `ld::EventProcessorBuilder`, both configured with their respective `MetricsTransport` instances, and is started via `start_with_default_executor` (without a per-event callback). Client initialization waits in a loop using `ld::Client::wait_for_initialization` with exponential backoff.
