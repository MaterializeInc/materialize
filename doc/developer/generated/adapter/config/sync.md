...
After each environment-wide pull-and-push, `sync_scoped_params` reconciles per-cluster and per-replica scoped parameter overrides. It reads the `ENABLE_SCOPED_SYSTEM_PARAMETERS` feature gate from the `SynchronizedParameters` working copy (avoiding a coordinator round-trip on disabled environments). When enabled, it takes a catalog snapshot, evaluates each live cluster and replica against LaunchDarkly, and pushes the desired sparse `ScopedParameters` to the coordinator via `Client::update_scoped_system_parameters`. When disabled, it performs a single clearing push the first time it observes the feature as disabled, then does no per-tick work until it is re-enabled.
