...
`catalog_transact_with_side_effects` supports DDL operations that need to run async side-effects (e.g. dropping persist shards) after the catalog transaction commits. It also tracks changes to the `OPTIMIZER_E2E_LATENCY_WARNING_THRESHOLD` system variable and propagates them to `optimizer_metrics.set_e2e_optimization_time_log_threshold` at transaction commit, and tracks changes to `DISABLED_METRIC_SINKS` to trigger `reconcile_metric_sinks` so the installed sink set converges on the new denylist. Both `catalog_transact_with_side_effects` and `catalog_transact_with_context` record fine-grained phase timings via `catalog_transact_phase_seconds` (labeled by phase: `prep`, `write_ts`, `transact`, `stage_builtin`, `finalize`, `side_effects`, `table_updates_wait`, `table_updates_residual_wait`, `consistency_check`, and DDL-transaction-specific labels such as `ddl_txn_snapshot_clone`, `ddl_txn_prep`, `ddl_txn_write_ts`, `ddl_txn_dry_run`). These phases overlap and do not sum to `catalog_transact_seconds`; their purpose is to attribute where transact time is spent.
`catalog_transact_inner` calls `scoped_overrides_create_op` on the incoming op list and appends the resulting `Op::UpdateScopedSystemParameters` when any cluster or replica is being created, so scoped override evaluation is an invariant of every catalog transaction path. `catalog_transact_with_ddl_transaction` does the same for the combined op list (accumulated DDL-transaction ops plus the current statement's ops), so a replica created in a later statement than its still-uncommitted cluster gets the correct override context.
