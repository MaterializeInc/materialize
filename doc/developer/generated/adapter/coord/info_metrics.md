...
`CatalogInfoMetrics` owns five `UIntGaugeVec` families — `mz_object_info` (labeled by `object_id`, `global_id`, `name`, `schema_name`, `database_name`, `type`; tagged `MetricTag::Environment`), `mz_cluster_info` (by `cluster_id`, `name`, `size`; tagged `MetricTag::Compute`), `mz_replica_info` (by `replica_id`, `cluster_id`, `name`, `size`; tagged `MetricTag::Compute`), `mz_source_info` (by `source_id`, `type`, `envelope_type`, `cluster_id`; tagged `MetricTag::Source`), and `mz_sink_info` (by `sink_id`, `type`, `envelope_type`, `cluster_id`; tagged `MetricTag::Sink`) — plus a `Vec<InfoGauge>` of live `DeleteOnDropGauge` handles and a `reconcile_seconds` histogram.
