...
`PeekDataflowPlan` describes a slow-path peek that requires a dataflow; its `desc` field carries a `DataflowDescription<mz_compute_types::plan::LirRelationExpr, ()>`. `FastPathPlan` covers constant results, direct arrangement reads (`PeekExisting`), and persist fast-path reads (`PeekPersist`).
