...
The module also contains shared utilities used across statement types: `statistics_oracle` (wraps the transform statistics oracle), `eval_copy_to_uri` (validates the COPY TO S3 URI), and `return_if_err!` macro (short-circuits on error, sending a response to the client).
