...
`sequence_copy_from` dispatches STDIN sources by calling `ctx.retire(Ok(ExecuteResponse::CopyFrom {...}))`, which hands control back to pgwire (which drives the CopyData/CopyDone exchange); the statement-logging obligation moves into the response, and `ctx` stays intact so `retire` can wait out any response barriers safely.
