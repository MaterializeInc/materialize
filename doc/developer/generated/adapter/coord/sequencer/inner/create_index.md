...
The optimizer config is built by layering cluster features (`ClusterConfig::features()`), then cluster-coherent scoped overrides (`Coordinator::cluster_scoped_optimizer_overrides`), over the base `OptimizerConfig`. This layering applies at both the initial `create_index_validate` stage and the finish stage.
