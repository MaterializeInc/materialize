...
In the `CopyToDataflow` stage, the dataflow is shipped via `ship_dataflow(df_desc, cluster_id, target_replica)`, where `target_replica` carries the optional `ReplicaId` stored in `PeekStageCopyTo`; when set, this routes the COPY TO execution to the targeted replica.
