...
`sequence_subscribe` (via `implement_subscribe`) constructs an `ActiveSubscribe` with `ActiveSubscribeOwner::Session` and `internal: false`; background and internal subscribes that should be hidden from `mz_subscriptions` use a separate code path that sets `ActiveSubscribeOwner::Background` or `internal: true`.
