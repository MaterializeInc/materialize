...
`add_active_compute_sink` and `remove_active_compute_sink` treat `ActiveSubscribe` entries with `internal: true` differently from regular subscribes: they skip the `mz_subscriptions` builtin table update and increment/decrement the `active_internal_subscribes` gauge rather than `active_subscribes`.
