...
`PeekClient::try_frontend_peek` is the main entry point; it receives an `ExecutionLogging` slot owning the end-of-execution obligation, threads the slot through `try_frontend_peek_inner` — dispatch sites that hand a statement to the coordinator for asynchronous completion defuse the slot — and retires any still-armed slot with the execution's outcome.
