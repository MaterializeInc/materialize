...
`PeekOptimizer` is an enum that carries either a `peek::Optimizer` (for `SELECT` and `EXPLAIN`) or a `copy_to::Optimizer` (for `COPY TO`) through the shared peek sequencing state machine. Both statement types go through identical surrounding stages (timestamp selection, read holds, off-thread optimization), and `PeekOptimizer` lets that shared machinery carry either optimizer without caring which one it is. The companion `PeekGlobalLirPlan` enum tags the optimization result with the same two variants so that downstream stages can recover the concrete plan type. The shared optimization pipeline steps (HIR-to-local-MIR, timestamp resolution, global-MIR-to-LIR) are factored into the `optimize_oneshot` helper to avoid duplication across variants.
