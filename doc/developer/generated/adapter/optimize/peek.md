...
After local optimization, the pipeline attempts to identify a `FastPathPlan` (direct arrangement read or constant result); if fast-path is not applicable it builds a full `DataflowDescription<LirRelationExpr>` for the slow path.
