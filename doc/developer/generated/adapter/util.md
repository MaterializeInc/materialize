...
Additional utilities include `index_sql` (generates a canonical `CREATE INDEX` SQL string), `describe` (invokes the SQL planner's describe path), `viewable_variables` (merges session and catalog system variables for SHOW), `sort_topological` (Kahn's algorithm for dependency-ordered iteration), and `verify_datum_desc` (checks that the first row of a `RowIterator` matches an expected `RelationDesc`).
