...
The `append_datum` implementation includes a lossless signed-to-signed widening path for `Datum::Int16` into `Int32Builder`, supporting destinations (e.g., Iceberg) that lack a smallint type. It also includes a `Datum::Uuid` → string path for both `StringBuilder` and `LargeStringBuilder`, rendering the UUID in lowercase hyphenated form (matching `uuid::text`) for destinations that map UUID to a string type rather than fixed binary. The array `dimensions` struct field is similarly widened: when the schema carries an `Int32Builder` for the `dimensions` field (as Iceberg does, having no narrow integer types), `ndims` (a `u8`) is promoted to `i32` via `i32::from`; a `UInt8Builder` is also accepted for non-Iceberg paths.
