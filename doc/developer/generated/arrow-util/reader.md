...
The reader supports a wider set of Arrow types than the builder produces (e.g., `Date64`, `Time32`, `Float16`, `Decimal256`, `BinaryView`) to allow reading Parquet files written by other tools, including `Map` columns (string-keyed maps) and `Interval` columns (year-month, day-time, and month-day-nano representations).
