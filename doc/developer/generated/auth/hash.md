...
* `HashError` / `VerifyError` — error types wrapping `aws-lc-rs` failures, malformed hash strings, and invalid passwords.

## Key functions

* `hash_password` — hashes a `Password` with a random salt and returns a `PasswordHash`.
