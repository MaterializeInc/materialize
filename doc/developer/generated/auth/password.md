...
The inner `String` is zeroized on drop via a custom `Drop` implementation using the `zeroize` crate, ensuring passwords do not linger in memory after the value is freed.
