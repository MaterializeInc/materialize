...
`DecimalValue` holds the unscaled two's-complement integer bytes, precision, and scale for a `Value::Decimal`.
`Scalar` is a copy-friendly enum covering the fixed-width primitive variants (null, boolean, int, long, float, double, date, timestamp); it converts into `Value` via `From<Scalar>`.
