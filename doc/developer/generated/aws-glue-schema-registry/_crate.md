...
* `RegisteredSchemaVersion` — `id: Uuid` and `lifecycle_status: Option<SchemaVersionLifecycleStatus>`; returned by the write-path methods (`get_schema_by_definition`, `register_schema_version`, `create_schema`). Glue validates versions asynchronously, so `lifecycle_status` is often `Pending`; callers must wait for `Available` before using the id to frame records.
