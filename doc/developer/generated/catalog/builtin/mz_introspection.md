...
- Resource usage: `MZ_CLUSTER_REPLICA_RESOURCE_USAGE` — per-process resource usage of a cluster replica, as reported by each measurement source (`LogVariant::Compute(ComputeLog::ResourceUsage)`); carries an `Ontology` annotation with `entity_name: "cluster_replica_resource_usage"`.
