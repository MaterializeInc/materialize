...
Handles catalog schema upgrades on open. `open_inner` returns `Result<Box<dyn DurableCatalogState>, CatalogError>`. On open, audit log entries are partitioned out of the snapshot and dropped (not returned); they are counted for metrics and then discarded, since audit events are served from `mz_internal.mz_catalog_raw` via the `mz_audit_events` materialized view. On a writable open, `remove_ephemeral_items()` is called before anything else reads the catalog; this reclaims temporary items owned by sessions served by previous catalog owners, which are presumed dead once the epoch fence fires.
