...
When `--enable-storage-introspection-logs` is set, per-worker `arc_event_link` bridges are created so that storage Timely logging events are forwarded to the compute logging dataflow, making storage operators visible in `mz_introspection.mz_dataflow_*` tables. This flag has no effect when `--unified-cluster` is set, because storage dataflows run on the compute Timely cluster and appear in its logging unconditionally.
