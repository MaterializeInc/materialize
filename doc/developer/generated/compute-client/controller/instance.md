...
`collections_hydrated_on_replicas` collects all non-hydrated collections before returning, so that when it logs at INFO it names every collection the caller is waiting on in a single log line rather than one line per collection.
