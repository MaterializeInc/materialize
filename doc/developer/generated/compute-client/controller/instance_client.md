...
Commands are sent as boxed closures (`Command<T>`) over an `mpsc` channel; `call` is fire-and-forget while `call_sync` awaits a `oneshot` response. Read hold changes are propagated via a separate `read_holds::ChangeTx` sender.
