...
`LogVariant` enumerates the three log families -- `TimelyLog` (operators, channels, scheduling, messages, reachability), `DifferentialLog` (arrangement batches, records, sharing, batcher stats), and `ComputeLog` (dataflows, frontiers, peeks, arrangement sizes, error counts, hydration, LIR mappings, Prometheus metrics, resource usage) -- and provides `index_by()` and `desc()` to return the index columns and `RelationDesc` for each variant, which must match the schema expected by the catalog and the logging dataflows.
`ComputeLog::OperatorHydrationStatus` has a three-column key `(export_id, lir_id, worker_id)` (columns 0, 1, 2).
`ComputeLog::ResourceUsage` exposes resource usage observations of each replica process with schema `(process_id UInt64, source String, metric String, value UInt64)` and key `(process_id, source, metric)` (columns 0, 1, 2).
