...
`CommandMetrics` provides a keyed metric set for each `ComputeCommand` variant (including `Hello`, `CreateInstance`, `CreateDataflow`, `Schedule`, `AllowCompaction`, `Peek`, `CancelPeek`, `InitializationComplete`, `UpdateConfiguration`, and `AllowWrites`). `ResponseMetrics` (private) provides keyed metrics per `ComputeResponse` variant (`Frontiers`, `PeekResponse`, `SubscribeResponse`, `CopyToResponse`, `Status`). `PeekMetrics` tracks peek outcomes by result type (rows, rows_stashed, error, canceled).
