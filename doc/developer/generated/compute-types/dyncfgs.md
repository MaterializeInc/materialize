...
- **Rendering**: `ENABLE_HALF_JOIN2`, `ENABLE_MZ_JOIN_CORE`, `LINEAR_JOIN_YIELDING`, `ENABLE_COMPUTE_TEMPORAL_BUCKETING`/`TEMPORAL_BUCKETING_SUMMARY`, `COMPUTE_FLAT_MAP_FUEL`, `ENABLE_COMPUTE_RENDER_FUELED_AS_SPECIFIC_COLLECTION`, `COMPUTE_APPLY_COLUMN_DEMANDS`, `ENABLE_COLUMN_PAGED_BATCHER` (replica-scoped), `ENABLE_COLUMN_PAGED_BATCHER_SPILL` (replica-scoped), `COLUMN_PAGED_BATCHER_BUDGET_FRACTION`, `COLUMN_PAGED_BATCHER_LZ4` (replica-scoped), `COLUMN_PAGED_BATCHER_SWAP_PAGEOUT`.
...
Constants that carry a `ParameterScope` declaration (via `.scoped(ParameterScope::Replica)`) are eligible for per-replica override through the scoped feature flags mechanism; those without a declaration default to environment-wide scope.
