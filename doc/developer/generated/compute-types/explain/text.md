...
Join nodes are labeled `→Differential Cross Join` when any stage is a cross product (empty lookup key), and `→Differential Join` otherwise, for linear joins. Delta joins use `→Delta Cross Join` or `→Delta Join`, and when reduced to a single path (one-shot joins) use `→One-Shot Delta Cross Join` or `→One-Shot Delta Join`. The private `fmt_join_chain` helper formats the join chain as `%pos:name[key]` entries, annotating each input with its relation name (if the underlying `LirRelationNode::Get` can be identified) and key columns.
