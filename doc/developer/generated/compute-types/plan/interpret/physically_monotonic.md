...
`SingleTimeMonotonic` extends this interpreter with a set of globally monotonic `GlobalId`s, allowing external knowledge (e.g. append-only sources) to seed the analysis. Method signatures accept `LirScalarExpr`-based types; temporal bound checks use `mfp.has_temporal_bounds()`.
