...
Aggregate expressions within reduce plans are represented as `LirAggregateExpr` (a stable, serializable struct with `func: AggregateFunc`, `expr: LirScalarExpr`, and `distinct: bool`) rather than `mz_expr::AggregateExpr` (which embeds `MirScalarExpr`, while LIR aggregates contain `LirScalarExpr`). `LirAggregateExpr::from_mir` converts from `AggregateExpr`, panicking on unmaterializable functions. `LirAggregateExpr::is_count_asterisk` detects `COUNT(*)` by checking `func == Count`, `!distinct`, and `expr.is_literal_true()`. `AccumulablePlan`, `SingleBasicPlan`, and the `BasicPlan::Multiple` variant all store `LirAggregateExpr` values.
