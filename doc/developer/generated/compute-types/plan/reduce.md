...
`ReducePlan` combines these into a single enum; `KeyValPlan` describes how rows are split into key and value columns before reduction. `KeyValPlan` now stores `key_plan: SafeMfpPlan<LirScalarExpr>` and `val_plan: SafeMfpPlan<LirScalarExpr>` (previously unparameterized `SafeMfpPlan`).
