...
`ArrangementBatcher` is an enum with three variants — `Columnation` (columnation stacks via `Col2ValBatcher`/`RowRowBuilder`), `Columnar` (resident `Column` chains via `Col2ValColBatcher`/`RowRowColPagedBuilder`), and `Chunked` (`Column` chains routed through the process buffer pool via `ChunkChunker`/`ChunkBatcher`/`UnchunkBuilder<RowRowColPagedBuilder>`; chunk bodies are spilled while the process chunk spill gate is set and stay resident otherwise) — resolved from dyncfg at arrange-site construction time via `ArrangementBatcher::from_config`. `ENABLE_COLUMN_PAGED_BATCHER` takes precedence over `ENABLE_COLUMNAR_MERGE_BATCHER`; with both false the `Columnation` path is used.
