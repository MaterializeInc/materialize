...
`arrange_collection` accepts a `use_paged_path: bool` flag controlled by the `ENABLE_COLUMN_PAGED_BATCHER` dyncfg: when true it uses `Col2ValPagedBatcher`/`RowRowColPagedBuilder`; when false it falls back to the legacy `Col2ValBatcher`/`RowRowBuilder` path.
