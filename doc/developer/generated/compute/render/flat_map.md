...
Renders `LirRelationNode::FlatMap` nodes, which expand each input row into zero or more output rows by calling a `TableFunc` (e.g., `generate_series`, `jsonb_array_elements`).
