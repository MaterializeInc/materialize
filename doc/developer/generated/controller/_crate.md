...
Provides the top-level `Controller` that unifies the storage controller, storage collections, and compute controller behind a single `ready` / `process` interface consumed by `environmentd`'s coordinator.
