...
Defines the key configuration and location types — `ClusterConfig`, `ReplicaConfig`, `ReplicaAllocation`, `ReplicaLocation` (`Managed` / `Unmanaged`), `ManagedReplicaLocation`, and `ClusterEvent` (which carries `replica_id`, `process_id`, `status`, a cumulative `restart_count` propagated from the orchestrator, and `time`) — as well as `ReplicaServiceName`, which encodes `(ClusterId, ReplicaId, generation)` as an orchestrator service-name string.
...
`provision_replica` calls `NamespacedOrchestrator::ensure_service` with a fully constructed `ServiceConfig` (including `app_name: "clusterd"`, Timely addresses, resource limits, labels, and availability zones) and spawns a background task that polls replica CPU/memory metrics every 60 seconds. Timely configuration parameters that carry `ParameterScope::Replica` (such as `ARRANGEMENT_EXERT_PROPORTIONALITY`, `ENABLE_TIMELY_ZERO_COPY`, and related flags) are resolved via `get_with_overrides` against the per-replica override map maintained by the `Controller`, so replica-specific LaunchDarkly targeting takes effect at provisioning time. Storage and compute arrangements share the same `ARRANGEMENT_EXERT_PROPORTIONALITY` value: the parameter is read once via `get_with_overrides` and applied to both the storage `TimelyConfig` and the compute `TimelyConfig` when provisioning a unified replica.
Dropping a replica also removes its entry from the `Controller`'s per-replica dyncfg override map, so stale overrides do not accumulate for replicas that no longer exist.
