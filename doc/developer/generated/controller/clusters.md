...
`provision_replica` calls `NamespacedOrchestrator::ensure_service` with a fully constructed `ServiceConfig` (including `app_name: "clusterd"`, Timely addresses, resource limits, labels, and availability zones) and spawns a background task that polls replica CPU/memory metrics every 60 seconds.
