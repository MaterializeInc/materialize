...
Values in the JSON file are keyed by config name and coerced to the type already registered in the `ConfigSet`; supported value types include `Bool`, `U32`, `Usize`, `OptUsize`, `F64`, `String`, `OptString` (nullable string), `Duration`, and `Json`; an `on_update` callback is invoked after each successful apply.
