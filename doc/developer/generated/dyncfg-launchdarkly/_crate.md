...
`sync_launchdarkly_to_configset` initializes an LD streaming client (via `hyper_tls::HttpsConnector` and `StreamingDataSourceBuilder`), waits up to a configurable timeout for the first sync (retrying with exponential backoff up to 60 seconds), and spawns a background task that periodically pulls flag values into the config set; an `on_update` callback is invoked after each sync with the set of applied updates.
