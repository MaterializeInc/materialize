...
`sync_launchdarkly_to_configset` initializes an LD polling client (via `launchdarkly_sdk_transport::HyperTransport` and `PollingDataSourceBuilder`), waits up to a configurable timeout for the first sync, and spawns a background task that periodically pulls flag values into the config set.
