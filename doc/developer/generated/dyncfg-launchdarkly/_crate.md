...
`sync_launchdarkly_to_configset` initializes an LD streaming client (via `launchdarkly_sdk_transport::HyperTransport` and `StreamingDataSourceBuilder`), waits up to a configurable timeout for the first sync, and spawns a background task that periodically pulls flag values into the config set.
