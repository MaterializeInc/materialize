...
Provides two endpoints: `/api/mcp/agents` (tools: `get_data_products`, `get_data_product_details`, `query`) for user data products discovered via `mz_internal.mz_mcp_data_products`, and `/api/mcp/observatory` (tool: `query_system_catalog`) for read-only access to system catalog tables in schemas from `SYSTEM_SCHEMAS` (excluding `mz_unsafe`).
