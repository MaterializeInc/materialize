...
Child modules `text` and `json` provide format-specific rendering; `text` is the primary output format and exports the humanization helpers used throughout the optimizer's explain pipeline, including `HumanizeDisplay` (re-exported from this module alongside the other humanization types).
