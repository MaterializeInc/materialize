...
`MfpPlan<MirScalarExpr>` has an `into_map_filter_project` method that reconstructs a `MapFilterProject` by folding temporal bounds back as `mz_now()` predicates: lower bounds become `mz_now() >= expr` and upper bounds become `mz_now() < expr`. This is the inverse of `MfpPlan::create_from`.
