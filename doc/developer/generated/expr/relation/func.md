...
`AnalyzedRegex` wraps a compiled regex and a `Vec<CaptureGroupDesc>` computed at construction time; each `CaptureGroupDesc` carries the capture index, optional name, and a `nullable` flag. The `nullable` flag is determined via `non_nullable_capture_groups()` on the underlying `Regex`: capture groups that unconditionally participate in every match are marked non-nullable.
