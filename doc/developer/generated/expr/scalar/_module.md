...
Also defines `EvalError` (the comprehensive error enum for runtime scalar evaluation failures, including `InvalidCatalogJson` for malformed catalog JSON, `MultipleRowsFromSubquery` for subqueries returning more than one row, `NegativeRowsFromSubquery` for subqueries returning a negative row count, and `TempStorageBudgetExceeded` for when a budgeted `RowArena` exceeds its limit during expression evaluation) and associated type-inference and simplification methods on `MirScalarExpr`.
