...
Child modules: `columns` defines the `Columns` trait for abstracting over column-reference operations; `eval` defines the `Eval` trait for abstracting over scalar evaluation; `func` provides all the concrete function implementations; `like_pattern` provides LIKE matching logic; `reduce` (private) holds the implementation of `MirScalarExpr::reduce`.
