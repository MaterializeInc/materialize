...
Also provides `parse_catalog_id` (converts catalog JSON-serialized IDs into string format like `u1`, `s2`, `p`), `parse_catalog_privileges` (converts catalog JSON-serialized privilege arrays into `mz_aclitem[]`), and `parse_catalog_create_sql` (parses a catalog `create_sql` string into a JSONB object with type and additional fields depending on the statement: for connections, a `connection_type` field; for materialized views, `cluster_id` and `definition`; for sources (`CreateSource`), `source_type`, `cluster_id`, `connection_id`, `envelope_type`, `key_format`, and `value_format`; for webhook sources, `source_type` and `cluster_id`; for subsources, `source_type` and `of_source_id`), with helper functions `jsonb_datum_to_u64` and `jsonb_datum_to_role_id` for the conversions.
