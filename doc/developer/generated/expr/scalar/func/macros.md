...
Each macro takes a list of variant-name/inner-type pairs and emits the enum definition (with standard derives), delegating methods (`eval`, `output_sql_type`, `output_type`, `propagates_nulls`, `introduces_nulls`, `could_error`, `is_monotone`, etc.), a `Display` impl, and `From<InnerType>` conversions for each variant.
