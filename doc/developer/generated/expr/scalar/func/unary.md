...
The `derive_unary\!` invocation includes variants for all built-in unary functions, including `ParseCatalogId`, `ParseCatalogPrivileges`, `ParseCatalogAclMode`, `ParseCatalogCreateSql`, and `ParseCatalogAuditLogDetails` for catalog JSON deserialization, and `RedactSql` for SQL redaction.
