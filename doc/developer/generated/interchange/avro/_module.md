...
Re-exports `Decoder`, `DiffPair` (decode), `AvroEncoder`, `AvroSchemaGenerator`, `encode_datums_as_avro`, `encode_debezium_transaction_unchecked`, `get_debezium_transaction_schema` (encode), and `AvroSchemaResolver`, `parse_schema`, `schema_to_relationdesc` (schema).
