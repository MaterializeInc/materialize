...
Provides three functions for walking arrangement batches and emitting `DiffPair`s:
...
* `for_each_diff_pair_async` — async, fallible variant of `for_each_diff_pair`. Accepts optional inclusive lower and exclusive upper time bounds and invokes an async fallible callback for each pair. Thin wrapper around `iter_diff_pairs`.
