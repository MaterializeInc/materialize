...
`ValidatedLicenseKey` carries the decoded claims including credit-consumption limits, expiration behavior, organization/environment binding, and an `entitlements` list of feature-flag strings (e.g. `"ory"`); the `has_entitlement` method tests membership.
