...
Both structs expose a `descs()` method returning `Vec<(String, String, Vec<String>)>` of `(name, help, label_keys)` triples for every metric they define; this is used by `mz-metrics-catalog` to document metrics whose names are assembled at macro-expansion time and invisible to its source scraper.
