...
The crate is organized into seven modules: `tunnel` (connection management with SSH/PrivateLink/direct transport and RDS IAM auth), `desc` (protobuf-serializable table/column/key descriptors), `schemas` (schema introspection via `information_schema`), `replication` (system-variable validation for CDC), `privileges` (source privilege checking), `decoding` (`pack_mysql_row`), and `probe` (`KeyProber`, for probing primary-key values on a live connection).
