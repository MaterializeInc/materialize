...
In `self-managed` mode the tool port-forwards to the `environmentd` SQL port using `kubectl`, collects Kubernetes resources (pods, services, CRDs, logs, etc.) via the kube API, and scrapes heap profiles, CPU profiles, and Prometheus metrics from internal HTTP endpoints.
