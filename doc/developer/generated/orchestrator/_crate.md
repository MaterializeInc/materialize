...
The crate also defines `ServiceConfig` (including resource limits, labels, and scheduling hints), `ServiceEvent` for status change notifications (carrying `service_id`, `process_id`, `status`, a cumulative `restart_count`, and `time`), and a `scheduling_config` module for topology-spread and anti-affinity tuning.
