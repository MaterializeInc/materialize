...
Heartbeat timestamps used in compare-and-append, since downgrade, and reader registration are sampled fresh inside each retry closure via `(cfg.now)()` rather than being passed in as parameters; this ensures that a long-running retry sequence commits a recent timestamp rather than one computed before the first attempt.
