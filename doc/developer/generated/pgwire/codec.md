...
`Codec` also holds a `max_frame_len` field that starts at `MAX_PREAUTH_FRAME_SIZE` (16 KiB) to limit pre-authentication credential frames. `FramedConn::allow_post_auth_frames` widens it to `mz_ore::netio::MAX_FRAME_SIZE` once authentication succeeds, permitting the bulk data that query traffic requires. This method must be called exactly once, immediately before `AuthenticationOk` is sent.
