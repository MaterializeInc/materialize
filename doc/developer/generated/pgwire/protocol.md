...
During startup, parameters that are successfully applied via `set()` are collected in `applied_params` and subsequently registered as session defaults via `set_default()`, after role defaults have been applied. This means `RESET` and `DISCARD ALL` restore to the startup parameter values rather than server defaults, matching PostgreSQL behavior and allowing connection poolers (e.g., pgbouncer) to rely on `DISCARD ALL` for session reset. Immediately after authentication succeeds and before `AuthenticationOk` is sent, `conn.allow_post_auth_frames()` is called to widen the codec's frame size ceiling from `MAX_PREAUTH_FRAME_SIZE` to the full `MAX_FRAME_SIZE`, permitting the bulk data that query traffic carries.
