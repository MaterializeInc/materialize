...
Modules are feature-gated: `replication` (WAL level checks, slot management, RLS validation), `schemas` (publication table and column introspection), `schema_change` (structured `SchemaChangeError` with optional recovery hints, used by `desc::PostgresTableDesc::determine_compatibility`), and `tunnel` (connection with SSH/PrivateLink support) are compiled only when their respective features are enabled; `query` is always available.
