...
When the `replication` feature is enabled, `fetch_max_lsn` (renamed from `get_current_wal_lsn`) is re-exported at the crate root; it queries `pg_current_wal_lsn()` and returns the current WAL LSN as a `PgLsn`, returning an error if the function returns no value.
