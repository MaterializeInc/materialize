...
All three descriptor types implement `RustType` for protobuf serialization via generated code included from `OUT_DIR`. The `from_proto` implementations for `PostgresColumnDesc` and `PostgresKeyDesc` reject `u32` wire values that exceed `u16::MAX` with a `TryFromProtoError` rather than panicking, making these paths safe against untrusted proto bytes.
