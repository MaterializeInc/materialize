...
`get_pg_major_version` reads `pg_catalog.current_setting('server_version_num')` and divides by 10000 to obtain the major version, using a schema-qualified call to avoid picking up any user-defined `current_setting` in the search path.
