...
`BaseListenerConfig` captures the common properties — bind address, `AuthenticatorKind`, `AllowedRoles`, and TLS flag — while `HttpListenerConfig` adds `HttpRoutesEnabled` to control which HTTP route groups are active (base, webhook, internal, metrics, profiling, `mcp_agents`, `mcp_developer`, and `console_config`).
