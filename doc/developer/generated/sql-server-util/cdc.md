...
`snapshot` acquires a consistent snapshot and LSN by: locking the table under READ COMMITTED on a fencing connection, starting a SNAPSHOT isolation transaction on a second connection, reading a single row from `cdc.change_tables` to force XSN assignment, creating a savepoint to generate an LSN, rolling back the fencing lock, then streaming the table rows. The lock-duration metric (`snapshot_table_lock_end`) is called after the locking phase completes regardless of whether an error occurred, preventing a gauge leak on failure.
