...
`CreateMetricSinkStatement` is handled in the rename and schema-rename visitors so that metric sink `FROM` references are rewritten alongside all other item references.
