...
The `MZ_INTERNAL_BUILTINS` table includes `parse_catalog_id` and `parse_catalog_privileges` for converting catalog JSON into display-format IDs and `mz_aclitem[]` arrays, `parse_catalog_acl_mode` (backed by `UnaryFunc::ParseCatalogAclMode`) for converting a catalog ACL mode JSONB value into a string representation, and `parse_catalog_audit_log_details` (backed by `UnaryFunc::ParseCatalogAuditLogDetails`, OID `FUNC_PARSE_CATALOG_AUDIT_LOG_DETAILS_OID`) which takes a `Jsonb` and returns a `Jsonb`, reshaping proto audit log data into the `as_json` format for use by the `mz_audit_events` materialized view.
