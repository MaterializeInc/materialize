...
`TryFromValue` is re-exported from the `with_options` submodule for callers that need to convert `WithOptionValue` items outside the planner. The public validation functions `validate_metric_sink_desc` and `validate_metric_sink_prefix` are re-exported from `statement::ddl`, along with `PlannedAlterRoleOption`, `PlannedRoleVariable`, `PlannedRoleAttributes`, `SqlServerConfigOptionExtracted`, `MySqlConfigOptionExtracted`, `PgConfigOptionExtracted`, and `AlterSourceAddSubsourceOptionExtracted`.
