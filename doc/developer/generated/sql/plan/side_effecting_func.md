...
`SideEffectingFunc::PgCancelBackend` carries `connection_id: Option<u32>`; a `None` value means the argument was `NULL`, in which case the function returns `NULL` without cancelling any connection. The return type of `pg_cancel_backend` is nullable to match PostgreSQL's behavior.
