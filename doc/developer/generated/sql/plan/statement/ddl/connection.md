...
`CreateConnectionType::GlueSchemaRegistry` is planned by resolving the `AwsConnection` reference and the `Registry` name string; planning is guarded by `ENABLE_GLUE_SCHEMA_REGISTRY` and rejects an empty registry name.
