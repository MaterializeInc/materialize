...
Plans `SHOW` statements that query catalog state: `SHOW CREATE TABLE/VIEW/SOURCE/SINK/METRIC SINK/INDEX/CONNECTION/MATERIALIZED VIEW/TYPE/CLUSTER`, `SHOW OBJECTS`, `SHOW METRIC SINKS`, `SHOW COLUMNS`, and related.
...
`show_clusters` selects `name`, `replicas`, `activity`, and `comment` from `mz_internal.mz_show_clusters`, projecting those four columns.
...
`plan_show_create_metric_sink` and `describe_show_create_metric_sink` implement `SHOW CREATE METRIC SINK`, gated by the `ENABLE_METRIC_SINK` feature flag. The output schema matches other `SHOW CREATE` statements: two columns `name` and `create_sql` (both non-nullable `String`). `show_metric_sinks` implements `SHOW METRIC SINKS` via a query against `mz_internal.mz_show_metric_sinks`, dispatched from `show_objects` on the `ShowObjectType::MetricSink { in_cluster }` variant.
