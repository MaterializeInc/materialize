...
`IcebergStorageProvider` implements `TryFromValue<WithOptionValue<Aug>>`: accepts case-insensitive `"s3"`, `"gcs"`, or `"adls"`, mapping them to the corresponding enum variants; any other string is rejected. `try_into_value` serializes via `as_str()`. `ImpliedValue` bails with an error requiring an explicit value.
