...
`AvroSchema::Glue` in a source format is handled by `purify_glue_connection_avro`: it validates the connection type, requires the `SCHEMA NAME` option, fetches the latest Avro schema version from AWS Glue (skipping the fetch when a seed is already present), and writes the result into `GlueAvroSeed`. Only Kafka sources are supported.
