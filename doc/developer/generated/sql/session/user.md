...
Defines the `User` struct (name plus optional external/internal metadata, an optional `AuthenticatorKind` tracking which authenticator authenticated the user, and an optional `groups: Option<Vec<String>>` field holding JWT group claims extracted during OIDC authentication), `RoleMetadata` (current/session/authenticated role IDs), and the static built-in user instances: `SYSTEM_USER` (`mz_system`), `SUPPORT_USER` (`mz_support`), `ANALYTICS_USER` (`mz_analytics`), `HTTP_DEFAULT_USER`, and their well-known `RoleId` constants.
