...
`set_default` (called for `ALTER ROLE ... SET`) bypasses `check_read_only` for variables returned by `allow_role_default`; `allow_role_default` currently admits only `restrict_to_user_objects`. `set_default` calls `self.parse` (which applies domain constraints) rather than `self.definition.parse` (which does not), so constraint violations are caught when setting defaults.
The public `check_transaction_isolation_feature_flag(name, input, system_vars)` function gates feature-flagged isolation levels (`strong session serializable` requires `ENABLE_SESSION_TIMELINES`) and is shared across all assignment paths so the gate cannot be bypassed by choosing a different syntax.
All dyncfg-backed `SystemVars` are internal-only and not accessible to environment superusers via `ALTER SYSTEM SET`.
`SessionVars::text_encode_settings()` returns a `mz_pgrepr::TextEncodeSettings` capturing the session's current text-encoding parameters (currently `extra_float_digits`) for forwarding to encoding calls.
