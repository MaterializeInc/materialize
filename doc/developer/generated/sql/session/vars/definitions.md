...
Feature flags include `enable_repeat_row_non_negative` (guards the `repeat_row_non_negative` table function), `enable_storage_introspection_logs` (guards forwarding storage timely logging events into the compute introspection dataflow), `enable_kafka_broker_matching_rules` (guards `MATCHING` broker rules in `BROKERS` for Kafka PrivateLink connections), and `enable_glue_schema_registry` (guards `CREATE CONNECTION ... TO AWS GLUE SCHEMA REGISTRY`).
`DEFAULT_TIMESTAMP_INTERVAL` carries a `NON_ZERO_DURATION` constraint, so setting it to zero raises an `InvalidParameterValue` error.
