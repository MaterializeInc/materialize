...
Defines the abstract syntax tree for sqllogictest files: `Record` (a single directive such as `statement`, `query`, `simple`, `halt`, `copy`, `user`, or `replace`), `Output` (expected results as literal values or an MD5 hash), `Sort` (no-sort, rowsort, or valuesort), `Type` (column type hint), and `Mode` (Standard vs Cockroach output format).
`QueryOutput` bundles sort order, column types, mode, and expected output for a `query` directive.
`Record::User` carries a `location` and a `user` name; it switches the session user for subsequent records (creating the role if it does not exist yet), as in CockroachDB's logic tests.
