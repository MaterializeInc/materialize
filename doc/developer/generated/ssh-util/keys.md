...
`SshKeyPair` generates keys via OpenSSL, exposes them as OpenSSH-formatted strings, wraps intermediate private-key bytes in `mz_ore::secure::Zeroizing` to ensure they are erased on drop, and implements custom serde serialization for backwards-compatible on-disk encoding.
