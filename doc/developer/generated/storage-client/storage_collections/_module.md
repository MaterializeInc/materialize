...
The `partition_finalizable_shards` helper separates the finalization WAL into shards that are still referenced by active collections (excluded from finalization to avoid panicking running dataflows) and shards that are safe to finalize. This reconciliation is performed both at bootstrap in `initialize_state` and during `update_collection_metadata`, using active collection metadata as the authoritative source.
