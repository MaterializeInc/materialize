...
`Instance` maintains per-object scheduling decisions — single-replica objects (most sinks and some sources) run on the lowest-ID replica, while multi-replica objects run on all replicas — and replays the command history to newly added or reconnected replicas via `CommandHistory`. It also stores per-replica dyncfg overrides in a `replica_dyncfg_overrides: BTreeMap<ReplicaId, ConfigUpdates>` field. When an `UpdateConfiguration` command is sent or replayed, `specialize_command_for_replica` merges the target replica's override (if any) into the command's `dyncfg_updates` field; the history records only the base (un-specialized) command so overrides are re-applied at replay time rather than baked in. `update_replica_dyncfg_overrides` replaces the stored override map for the instance. `drop_replica` removes the dropped replica's entry from `replica_dyncfg_overrides`; without this, the entry would persist until the next coordinator-driven override map update.
