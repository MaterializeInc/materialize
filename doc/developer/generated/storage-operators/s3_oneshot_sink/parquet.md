...
The `WriterProperties` for each file disable the writer's own row-group-size enforcement (`set_max_row_group_row_count(None)`) because flushing is driven by the byte-size threshold above.
