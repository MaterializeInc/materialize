...
* **MySQL**: `MYSQL_REPLICATION_HEARTBEAT_INTERVAL`, `MYSQL_SOURCE_SNAPSHOT_PARALLELISM` (gates PK-range splitting of snapshot reads across workers, defaults `false`), and `MYSQL_SOURCE_SNAPSHOT_EXACT_COUNT_MAX_ROWS` (tables whose optimizer row estimate exceeds this threshold report the estimate for the snapshot size gauge rather than running `COUNT(*)`; defaults to 1,000,000).
