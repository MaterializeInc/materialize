...
* `fuzz_exports` — re-exports internal upsert types (`FuzzUpsertParts`, `StateValue`, `UpsertValueAndSize`, `upsert_bincode_opts`, `upsert_value_to_row`, `datum_seq_to_upsert_value`) for the storage fuzz crate; only present under `#[cfg(feature = "fuzzing")]`.
