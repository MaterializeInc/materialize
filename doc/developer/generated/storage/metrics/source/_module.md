...
`SourceMetricDefs` bundles these general definitions with source-type-specific definitions for Kafka, Postgres, MySQL, and SQL Server, plus a cluster-wide `mz_bytes_read_total` counter. The `offset_commit_failures` counter carries `MetricTag::Source` for metric categorization.
