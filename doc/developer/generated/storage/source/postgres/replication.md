...
The `outputs_at` helper filters `SourceOutputInfo` entries for a given relation and commit LSN, excluding outputs for which `info.ignores(commit_lsn)` is true — that is, outputs whose `initial_lsn` exceeds `commit_lsn`. This prunes logical replication messages committed before each export's schema was captured during purification, which may have described an incompatible upstream schema.
