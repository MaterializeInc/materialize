...
The public `key_persist_feedback` function extracts `(UpsertKey, UpsertValue)` pairs from the two separate persist feedback collections (ok rows and error rows): ok rows are keyed directly, and only `UpsertError` instances from the error collection survive (those are the only errors this operator could have written, and therefore the only ones it can retract).
The `upsert` function delegates to the continual-feedback variant via `upsert_continual_feedback::upsert_inner`, managing rehydration from the persist feedback stream, snapshot consolidation, and ongoing updates using pluggable `UpsertStateBackend` implementations (memory or RocksDB). It takes the persist feedback as two separate collections (`previous_ok: VecCollection<Row>` and `previous_err: VecCollection<DataflowError>`).
The `upsert_v2` function provides an alternative implementation that uses a differential dataflow collection to hold key state, delegating to `upsert_continual_feedback_v2::upsert_inner`; it is selected at render time via the `ENABLE_UPSERT_V2` dyncfg. It accepts a `UpsertStashFlavor` resolved at operator construction time and likewise takes the persist feedback as two separate collections.
