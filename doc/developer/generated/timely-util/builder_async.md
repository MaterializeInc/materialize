...
Key types include `AsyncInputHandle` (a `Stream` of `Event<Data|Progress>` events with synchronous `next_sync` and async `ready`), `AsyncOutputHandle` (with optional back-pressure via `give_fueled` for columnation-backed outputs), `InputConnection` (controlling capability routing via `Disconnected`, `ConnectedToOne`, `ConnectedToMany`), `Button`/`ButtonHandle` for coordinated cross-worker shutdown, and `PressOnDropButton` as a deadman's-switch wrapper.
