...
`Column::is_empty()` returns whether the column holds no records; `Typed` answers directly from the container while `Bytes` and `Align` reconstruct a borrow.
The `SizableContainer::at_capacity` implementation uses `at_serialized_capacity`, a shared helper (also used by `ColumnBuilder`) that returns `true` once the serialized size reaches 10% under the 2 MiB (`SHIP_WORDS`) threshold. The signal is monotone: once fired it stays fired, aligning chunk-size decisions between the builder and merger paths.
