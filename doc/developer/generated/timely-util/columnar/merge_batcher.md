...
* `ColumnMergeBatcher<D, T, R>` — implements `differential_dataflow::trace::Batcher`. Maintains a stack of `VecDeque<PagedColumn>` chains. On `push_container` it consolidates input through a `ColumnChunker` and pages each output chunk. On `seal` it merges all chains, partitions by the frontier (`extract_chain`), returns the ship side to the caller's `Builder`, and retains the keep side paged for the next round. The pager is resolved lazily per call from `column_pager::global_pager`; tests can pin a specific pager via `set_pager`.
