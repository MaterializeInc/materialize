...
`CollectionExt` adds `map_fallible`, `flat_map_fallible`, `explode_one`, `ensure_monotonic`, `consolidate_named`, and `consolidate_named_if` to differential `Collection`s, along with a `empty` constructor.
