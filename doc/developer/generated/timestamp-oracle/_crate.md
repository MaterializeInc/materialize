...
Provides the `TimestampOracle<T>` trait and a Postgres/CockroachDB production implementation for linearizable timestamp allocation across Materialize timelines.
...
Key types: `TimestampOracle<T>` (trait), `WriteTimestamp<T>` (write timestamp plus lock token), `GenericNowFn` (clock abstraction for tests), `PostgresTimestampOracle<N>`, `BatchingTimestampOracle<T>`.

Key dependencies: `mz-postgres-client` (connection pool), `mz-adapter-types` (timeline types), `mz-repr` (timestamp type), `deadpool-postgres`.
