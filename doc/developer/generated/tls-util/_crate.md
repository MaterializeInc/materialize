...
* `Pkcs12Archive` — a struct holding the DER bytes and the (empty) password for the produced PKCS #12 archive. It implements `mz_ore::secure::Zeroize` and `Drop` so that sensitive key material is zeroed on drop.
