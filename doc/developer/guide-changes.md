...
are familiar with the areas of the codebase that are touched in the PR. Once a
PR is approved and merged to the `main` branch, CI will automatically deploy a
new `materialized` binary to <https://downloads.mtrlz.dev> and the [materialized
Docker image][docker].
...
Sometimes, two PRs are fine when tested isolation, but the combination of the
...
please try to jump on fixing the merge skew as soon as possible, or nominate
someone else to fix it if you are otherwise occupied.
...
The goals of code review are threefold:
...
Google's overriding principle is as follows:
...
addding the first special case is easy, but adding the tenth special case is
nearly impossible.
