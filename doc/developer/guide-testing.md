...
* Additionally all paramters described by `--help` can be provided as environment variables by
  converting them to `SCREAMING_SNAKE_CASE`, e.g. `--aws-region` --> `AWS_REGION`.
