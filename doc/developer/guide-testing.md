...
* `local-aws`: Use [LocalStack][] as an AWS stub. Use this when you want to run the esoteric tests
  but don't have AWS credentials:

  ```console
  $ TD_TEST=esoteric/*.td ./mzcompose run local-aws
  ```

* `ci`: Expect actual AWS credentials to be available, either in the process environment or via
  AWS EC2 Profiles:
