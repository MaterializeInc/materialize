...
* `INTERSECT ALL` and `EXCEPT ALL` on `UnifiedStream`. `UNION ALL` and `UNION` shipped in `Mz/SetOps.lean`; the bag-difference flavors require subtraction-aware multiplicity arithmetic on the diff semiring. Once landed, `INTERSECT` / `EXCEPT` derive via `consolidate`.
