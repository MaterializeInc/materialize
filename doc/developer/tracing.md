...
`mz_log_filter` system variable to control the level at a _per-crate_, _per-module_, and
even _per-span_ granularity.

For example:
```
ALTER SYSTEM SET mz_log_filter='my_interesting_module::sub_module=trace,error';
```
...
```
./bin/environmentd -- --system-parameter-default=mz_log_filter=my_interesting_module::sub_module=trace,error
```
