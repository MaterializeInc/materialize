...
If a command line flag named above takes an argument, you can alternatively set
that flag via an environment variable named after the flag. For example, to set
the data directory, you can set the `MZ_DATA_DIRECTORY` environment variable. If
both the environment variable and command line flag are specified, the command
line flag takes precedence.

The process for converting a flag name to an environment variable name is as
follows: convert all characters to uppercase, replace all hyphens with
underscores, and add an `MZ_` prefix.
