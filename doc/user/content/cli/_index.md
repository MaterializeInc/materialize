...
The `--logical-compaction-window` option specifies the size of the window behind
the current time for which Materialize is required to maintain full historical
detail in its [arrangements](/overview/api-components#indexes). Note that
compaction happens lazily, so Materialize may retain more historical detail than
requested, but it will never retain less.

The value of the option is a duration like `10ms` (10 milliseconds) or `1min
