...
{{< note >}}
**Available for organization administrators only.**
{{</ note >}}

The **Roles and Users** screen allows organization administrators to manage
role-based access control (RBAC) in Materialize: create roles, grant privileges
to roles, and assign roles to users.
...
- **Create new roles** by clicking **Create New Role** to grant privileges to the role.
- **Edit roles** by clicking **Edit Role** to grant or revoke privileges.
...
- View roles granted to users and remove roles from users.

![Remove User](/images/console/rbac/console-roles-remove-user.png "Remove User")

### Create role

Admin users can create new roles by filling out the Create Role form to grant
...
Edit an existing role to grant or revoke privileges, using the same form as
**Create Role**.
...
For more details, see [Drop a role](/security/cloud/access-control/manage-roles/#drop-a-role).
...
Click **Edit** on a user to add or remove role assignments.

![Add User](/images/console/rbac/console-add-role-to-user.png "Add Role User")

## See also

- [Manage database roles](/security/cloud/access-control/manage-roles/)
...
- [Users and Service Accounts](/security/cloud/users-service-accounts/)
