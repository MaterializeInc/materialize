...
If Kafka is not part of your stack, you can use the [Postgres source](/sql/create-source/postgres/#postgresql-source-details) to connect directly to Materialize. This source uses Postgres’ native replication protocol to continuously ingest changes resulting from `INSERT`, `UPDATE` and `DELETE` operations in the upstream database.
...
1. Set the replica identity value for the tables you want to replicate:
    ...
    This setting determines the amount of information that is written to the WAL in case of `UPDATE` and `DELETE` operations. By setting it to `FULL`, change events will include the previous values of all the table’s columns; as a heads-up, you should expect a performance hit in the database from increased CPU usage.
...
1. Create a [publication](https://www.postgresql.org/docs/current/logical-replication-publication.html) with the tables you want to replicate:
    ...
    The `mz_source` publication contains the set of change events generated from `table1` and `table2`. This publication will later be used to ingest the replication stream into Materialize.
...
```sql
CREATE VIEWS FROM SOURCE mz_source (view1, view2);
```
...
1. Grant enough privileges to ensure Debezium can operate in the database. This will depend on how much control you want to give to the replication user, so we recommend following the [Debezium documentation](https://debezium.io/documentation/reference/connectors/postgresql.html#postgresql-replication-user-privileges).

1. Set the replica identity value for any tables that you want to replicate but have **no primary key** defined:
...
Debezium emits change events using an envelope that contains detailed information about upstream database operations, like the `before` and `after` values for each record. To create a source that interprets the [Debezium envelope](/sql/create-source/avro-kafka/#debezium-envelope-details):
...
Enabling `UPSERT` will only work for tables with `REPLICA IDENTITY DEFAULT`/`INDEX` and, although this approach is less resource-intensive for the upstream database, it will require more memory in Materialize.
...
Debezium provides a [transaction metadata topic](https://debezium.io/documentation/reference/connectors/postgresql.html#postgresql-transaction-metadata) that can be used to preserve transactional boundaries downstream. We are working on using this topic to support transaction-aware processing in Materialize!
