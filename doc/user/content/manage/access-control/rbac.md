...
{{< private-preview />}}
 ...
 there is no one size fits all solution on how to design and implement an effective RBAC hierarchy. Having the ability to create a nearly unlimited amount of custom roles with varying object permissions and structural hierarchy that enforces privileges inheritance can be overwhelming.
...
## Step 1. Configure account security

## Step 1. Configure default database privileges
...
privileges available to all other roles. On creation, roles are automatically
granted membership in `PUBLIC`, and inherit the privileges assigned to it. By
default, members of this role have the following privileges:

* 

As an administrator, you can use the `GRANT` or `REVOKE` commands to modify the
`PUBLIC` role and change the default privileges available to all other roles in
...
## Step 2. Choose an RBAC strategy
...
# Step 2: grant usage on the database & schema to the read_only role
GRANT USAGE ON DATABASE d1 TO read_only;
GRANT USAGE ON SCHEMA d1.s1 TO read_only;
GRANT USAGE ON CLUSTER c1 to read_only;

# Step 3: grant select to all objects in the d1.s1 schema to the read_only role.
# Both for pre-existing objects (can skip first line if there are none) and for future objects in the schema.
GRANT SELECT ON ALL TABLES IN SCHEMA d1.s1 TO read_only;
ALTER DEFAULT PRIVILEGES FOR ALL ROLES IN SCHEMA d1.s1 GRANT SELECT ON TABLES TO read_only;

# Step 4: create the read_write role
CREATE ROLE read_write;

# Step 5: grant usage & create on the database & schema to the read_write role
GRANT USAGE, CREATE ON DATABASE d1 TO read_write;
GRANT USAGE, CREATE ON SCHEMA d1.s1 TO read_write;
GRANT USAGE, CREATE ON CLUSTER c1 to read_write;

# Step 6: grant all privileges to all objects in the d1.s1 schema to the read_write role
# Both for pre-existing objects (can skip first line if there are none) and for future objects in the schema.
...
GRANT read_write TO user3;
```

## Step 2. Invite new users


## Step 3. Set up advanced RBAC configuration
