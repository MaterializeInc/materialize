...
{{< warning >}}
**Storage and computation objects can't share a schema.** Within a single
schema, `mz-deploy` doesn't allow **storage objects** (tables and sinks) and
**computation objects** (views and materialized views) to coexist. This
prevents a view change from accidentally recreating a table or sink when a
deployment recreates the schema's computation objects.

Keep them in separate schemas — for example, storage objects under
`models/<database>/tables/` and computation objects under
`models/<database>/views/` (or `public`).
{{< /warning >}}
