...
- Validate `WITH` clauses in [`CREATE SOURCE`](/sql/create-source) and [`CREATE
  ...
  Upgrading to v0.6.1 may therefore fail if any of the sources or sinks within
...
- Permit specifying columns in the `USING` clause of a [join](/sql/join) when
  the columns are not of identical types, as long as there is an [implicit
  cast](/sql/types#casts) between the types {{% gh 5276 %}}.
