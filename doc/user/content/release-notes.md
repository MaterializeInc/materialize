...
- Add the [`map_length`](/sql/functions/#map-func) function, which computes the
  numer of entries in a [`map`] value.

- Fix a memory leak in [`TAIL`](/sql/tail) {{% gh 11540 %}}.

{{% version-header v0.25.0 %}}

- Fix `base64` decoding to support numerals in encoded strings. Previously, if
  the encoded string contained a numeral, decoding would fail.

- Fix a panic that would occur if an object or type was over-qualified (i.e.,
  the fully qualified object name consisted of more than three identifiers
  separated by a `.`).
