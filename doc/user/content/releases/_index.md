...
- **Row filtering by session role for restricted roles**: `mz_session_role_memberships()` has moved from `mz_internal` to `mz_catalog`, so your own views can depend on it to resolve the names of the roles the current session belongs to — including from a role with `restrict_to_user_objects` enabled, which cannot use the equivalent `pg_has_role` filter over `mz_catalog.mz_roles`.
