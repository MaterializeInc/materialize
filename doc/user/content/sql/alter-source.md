...
**ADD SUBSOURCE** ... | PostgreSQL sources only: Add the identified tables from the upstream database to the named source, with the option of choosing the name for the subsource in Materialize. Supports [additional options](#add-subsource-with_options)
**DROP SUBSOURCE** ... | PostgreSQL sources only: Drop the identified subsources from the source. Specifying **CASCADE** also drops all objects that depend on the subsource. **RESTRICT** (default) will not drop the subsource it it has any dependencies.
