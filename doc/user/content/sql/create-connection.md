...
want Materialize to read data from or write data to. Once created, a connection is **reusable**
...
### Authentication options {#kafka-auth}

- [SSL](#kafka-auth-ssl-options)
- [SASL](#kafka-auth-sasl-options)
...
### Network security options {#kafka-network-security}
...
- [SSH tunnel](#kafka-ssh-tunnel)

{{< note >}}
`SSH TUNNEL` can be configured at the top-level and/or per-broker, but 
`AWS PRIVATELINK` can be configured only per-broker.
{{< /note >}}
...
those brokers without any tunneling, or with a
[top-level `SSH TUNNEL`](/sql/create-connection#kafka-ssh-example),
if configured.
{{< /warning >}}
...
want to connect to via tunnel.

##### AWS PrivateLink options {#kafka-privatelink-options}
...
##### Permissions {#kafka-privatelink-permissions}
...
##### Accepting connection requests {#kafka-privatelink-requests}
...
##### SSH tunnel options {#kafka-ssh-options}
...
You can configure an SSH tunnel for each broker in the `BROKERS` clause:
...
However, if your Kafka cluster advertises brokers that are not in the `BROKERS` 
clause, Materialize will attempt to connect to those brokers without any 
tunneling. Therefore, to ensure that all brokers connect with tunneling, it's 
best to configure a **top-level SSH tunnel**:
...
It's also possible to use both default and per-broker SSH tunnels, for example, 
to override the tunnel for a specific broker:
...
CREATE CONNECTION ssh_connection_override TO SSH TUNNEL (
    HOST '<SSH_BASTION_HOST>',
    USER '<SSH_BASTION_USER>',
    PORT <SSH_BASTION_PORT>
);
...
CREATE CONNECTION kafka_connection TO KAFKA (
    BROKERS (
        'broker1:9092' USING SSH TUNNEL ssh_connection_override,
        'broker2:9092'
    ),
    SSH TUNNEL ssh_connection
);
...
##### Key pairs {#kafka-ssh-tunnel-keypairs}
...
### Authentication options {#csr-authentication}
...
### Network security options {#csr-network-security}
...
- [AWS PrivateLink](#csr-privatelink)
- [SSH tunnel](#csr-ssh-tunnel)
...
##### Permissions {#csr-privatelink-permissions}
...
### Authentication options {#postgres-authentication-options}
...
### Network security options {#postgres-network-security}
...
- [AWS PrivateLink](#postgres-privatelink)
- [SSH tunnel](#postgres-ssh-tunnel) 
...
##### Permissions {#postgres-privatelink-permissions}
...
##### Key pairs {#postgres-ssh-tunnel-keypairs}
