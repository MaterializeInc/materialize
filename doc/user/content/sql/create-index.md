...
[cluster](/concepts/clusters/), which provides future queries the data they
need in a format they can immediately use. In particular, creating indexes can
be very helpful for the [`JOIN`](/sql/select/join) operator, which needs to
build and maintain the appropriate indexes if they do not otherwise exist.
