...
The progress collection schema depends on your source type. For S3 sources, we
return the greatest `byte_offset` ([`uint8`](/sql/types/uint)) we have consumed
from the S3 bucket.

As long as as the `byte_offset` continues to increase, Materialize is consuming
data. Once Materialize consumes the entire object, the progress collection
