...
Name | Use
-----|-----
`element_type` | Creates a [`list`](../types/list) whose elements are are of `element_type`.

### MAP properties

Name | Use
-----|-----
`key_type` | Creates a [`map`](../types/map) whose keys are are of `key_type`. `key_type` must resolve to [`text`](../types/text).
`value_type` | Creates a [`map`](../types/map) whose values are are of `value_type`.
