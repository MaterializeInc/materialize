...
If the `TIMESTAMPS` option is specified (`WITH (TIMESTAMPS)`) an additional `done` column appears in the output.
It is `false` if there may be more rows with the same timestamp.
It is `true` if there willbe no more rows at or before the timestamp, in which case all further columns will be `NULL`.
