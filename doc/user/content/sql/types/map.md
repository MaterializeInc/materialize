...
Materialize supports the following map operators.
...
- `map` keys must be of type [`text`](../text).
- `map` values can be of any [type](../../types) as long as the type is uniform.
- `map` keys must be unique. If duplicate keys are present in a map, only one of the
...
Accessing a map's field returns the target field value or `NULL`.
