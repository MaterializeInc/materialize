...
[sources](/docs/concepts/sources/). For the sources, you need to
associate a [cluster](/docs/concepts/clusters/) to provide the compute
...
Console](/docs/console/data/#sample-source-overview). Alternatively, you
can manually keep track of using information from the system catalog.
See [Monitoring the snapshotting
progress](/docs/ingest-data/monitoring-data-ingestion/#monitoring-the-snapshotting-progress)
for guidance.
...
level](/docs/get-started/isolation-level/) of [strict
serializability](/docs/get-started/isolation-level/#strict-serializable).
...
When a cluster is restarted (such as after resizing), certain objects on
...
size](/docs/sql/alter-cluster/#alter-cluster-size) during snapshotting
for upsert sources. Once the snapshotting operation is complete, you can
downsize the cluster to align with the steady-state ingestion.
...
size](/docs/sql/alter-cluster/#alter-cluster-size) to complete the
operation.
...
[resize](/docs/sql/alter-cluster/#resizing) the cluster to align with
the volume of changes being replicated from your upstream in
steady-state.
...
Resizing a cluster with sources requires the cluster to restart. This
operation incurs downtime for the duration it takes for all objects in
the cluster to [hydrate](#hydration).
...
- [Troubleshooting data ingestion](/docs/ingest-data/troubleshooting)
