...
[RudderStack](https://rudderstack.com/) into Materialize using the
[Webhook source](/docs/sql/create-source/webhook/).
...
- A RudderStack
  [source](https://www.rudderstack.com/docs/sources/overview/) set up
  and running.
...
To validate requests between Rudderstack and Materialize, you must
create a [secret](/docs/sql/create-secret/):
...
from RudderStack. By default, the source will be created in the active
...
To configure the webhook endpoint as a destination in RudderStack,
follow the steps outlined below:

1.  **Select your RudderStack source**

    Identify the source you wish to add a webhook endpoint to. If you
    don’t have a source set up, follow the steps outlined in the
    Rudderstack [Getting
    Started](https://www.rudderstack.com/docs/dashboard-guides/sources/)
    guide.

2.  **Add a webhook destination and connect it to the Rudderstack
    source**
...
- **Webhook URL**: Define the endpoint where events will be dispatched
  by RudderStack. Use the URL from **Step 3.**.

- **URL method**: Use the `POST` method to send events to Materialize.

- **Headers**: These headers get added to the RudderStack request sent
  to your webhook. For this setup, ensure that the following headers are
  added:
...
configured in Rudderstack, you can now query the incoming data:
...
2.  Use SQL queries to inspect and analyze the incoming data:
    ...
    If you don’t see any data, head over to the [RudderStack
    console](https://app.rudderstack.com/) and try to sync your source
    to trigger a new data ingestion.
...
With Materialize ingesting your Rudderstack data, you can start
...
out the [Rudderstack documentation](https://rudderstack.com/docs/) and
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/ingest-data/webhooks/rudderstack.md"
class="btn-ghost"><img
