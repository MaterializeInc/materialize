...
[Stripe](https://stripe.com/) into Materialize using the [Webhook
source](/docs/sql/create-source/webhook/).
...
To validate requests between Stripe and Materialize, you must create a
[secret](/docs/sql/create-secret/):
...
from Stripe. By default, the source will be created in the active
...
The `CHECK` clause defines how to validate each request. For details on
the Stripe signing scheme, check out the [Stripe
documentation](https://stripe.com/docs/webhooks#verify-manually).
...
3.  Enter the webhook URL from the previous step in the *Endpoint URL*
    field.
...
2.  Use SQL queries to inspect and analyze the incoming data:
    ...
    You may need to wait for webhook-generating events in Stripe to
    occur.
...
computing real-time results that stay up-to-date as new data arrives,
and serving results efficiently. For more details, check out the [Stripe
documentation](https://stripe.com/docs/webhooks) and the [webhook source
reference documentation](/docs/sql/create-source/webhook/).
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/ingest-data/webhooks/stripe.md"
class="btn-ghost"><img
