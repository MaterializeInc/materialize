...
PostgreSQL as a metadata database; blob storage; specifically **block**
blob storage on Azure; and a license key.

The tutorial deploys Materialize to Azure Kubernetes Service (AKS) with
a PostgreSQL database as the metadata database and Azure premium block
blob storage for blob storage. The tutorial uses [Materialize on Azure
Terraform
modules](https://github.com/MaterializeInc/terraform-azurerm-materialize)
to:
...
If you do not have an Azure subscription to use for this tutorial,
create one.

### Azure CLI

If you don’t have Azure CLI installed, [install Azure
...
[Python.org](https://www.python.org/downloads/). If `pip` is not
included with your version of Python, install it.

### Helm 3.2.0+

If you don’t have Helm version 3.2.0+ installed, install. For details,
see to the [Helm documentation](https://helm.sh/docs/intro/install/).

### jq (Optional)

*Optional*. `jq` is used to parse the AKS cluster name and region from
...
3.  Select the subscription and tenant to use. After you have signed in,
...
for deploys a sample infrastructure on Azure with the following
components:
...
| `cert-manager` and a self-signed `ClusterIssuer`. `ClusterIssuer` is deployed on subsequent runs after the `cert-manager` is running. | [v0.3.0+](/docs/installation/appendix-terraforms/#materialize-on-azure-terraform-module) |
| Load balancers for each Materialize instance | [v0.3.1+](/docs/installation/appendix-terraforms/#materialize-on-azure-terraform-module) |
| OpenEBS and NVMe instance storage to enable spill-to-disk | [v0.4.0+](/docs/installation/appendix-terraforms/#materialize-on-azure-terraform-module) |
...
For more configuration options, you can run the `main.tf` file at the
[root of the
...
configuration](/docs/installation/install-on-azure/appendix-azure-configuration/).
...
2.  Fork the [Materialize’s sample Terraform
    repo](https://github.com/MaterializeInc/terraform-azurerm-materialize).

3.  Set `MY_ORGANIZATION` to your github organization name, substituting
...
4.  Clone your forked repo and checkout the `v0.6.4` tag. For example,

    - If cloning via SSH (substitute `YOUR_ORGANIZATION` with your
      organization’s name):

      <div class="highlight">

      ``` chroma
      git clone --depth 1 -b v0.6.4 git@github.com:${MY_ORGANIZATION}/terraform-azurerm-materialize.git
      ```
    ...
    - If cloning via HTTPS (substitute `YOUR_ORGANIZATION` with your
      ...
      ``` chroma
      git clone --depth 1 -b v0.6.4 https://github.com/${MY_ORGANIZATION}/terraform-azurerm-materialize.git
      ```

      </div>

5.  Go to the `examples/simple` folder in the Materialize Terraform repo
    directory.
    ...
    **💡 Tip:**
    ...
    For more configuration options, you can run the `main.tf` file at
    the [root of the
    repository](https://github.com/MaterializeInc/terraform-azurerm-materialize/)
    instead. When running with the root `main.tf`, see [Azure required
    configuration](/docs/installation/install-on-azure/appendix-azure-configuration/).
...
6.  Optional. Create a virtual environment, specifying a path for the
...
8.  Create a `terraform.tfvars` file (you can copy from the
    `terraform.tfvars.example` file) and specify:

    - The prefix for the resources. Prefix has a maximum of 12
      characters and contains only alphanumeric characters and hyphens;
      e.g., `mydemo`.
...
12. Configure `kubectl` to connect to your cluster:
    ...
    - `<cluster_name>`. Your cluster name has the form
      `<your prefix>-aks`; e.g., `mz-simple-aks`.
    ...
    name and resource group name from the Terraform output:
...
14. Once the Materialize operator is deployed and running, you can
    ...
    - **Starting in v26.0**, Self-Managed Materialize requires a license
      key. To get your license key:
    ...
    - **Starting in v0.3.0**, the Materialize on Azure Terraform module
      also deploys, by default, a self-signed `ClusterIssuer`. The
      `ClusterIssuer` is deployed after the `cert-manager` is deployed
      and running.

    - **Starting in v0.3.1**, the Materialize on Azure Terraform module
      also deploys, by default, [Load
    ...
    **💡 Tip:**
    ...
    See [Materialize on Azure
    releases](/docs/installation/appendix-terraforms/#materialize-on-azure-terraform-module)
    for notable changes.
...
16. If you are satisfied with the changes, apply.
    ...
    <span id="azure-terraform-output"></span>
...
18. Open the Materialize Console in your browser:
    ...
    Starting in v0.3.1, for each Materialize instance, Materialize on
    Azure Terraform module also deploys load balancers (by default,
    ...
    in the [Terraform output](#azure-terraform-output).
    ...
    **💡 Tip:** If you experience long loading screens or
    ...
    Unresponsiveness](/docs/installation/troubleshooting/#troubleshooting-console-unresponsiveness)
    guide.
...
- To start ingesting your own data from an external system like Kafka,
  ...
  [sources](/docs/sql/create-source/).
...
**💡 Tip:** If the `terraform destroy` command is unable to delete the
subnet because it is in use, you can rerun the `terraform destroy`
command.
...
- [Troubleshooting](/docs/installation/troubleshooting/)
- \[Appendix: Azure deployment
  guidelines\](/installation/install-on-azure/
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/installation/install-on-azure/_index.md"
class="btn-ghost"><img
