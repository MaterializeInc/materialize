...
- **Breaking change.** Use `SSL` as the default security protocol in
  Kafka connections when no `SSL...` or `SASL...` options are specified.
  Previously, `PLAINTEXT` was used as the default.

- Add support for the `PLAINTEXT` and `SASL_PLAINTEXT` security
  protocols for Kafka connections.

- Allow Kafka connections to enable the `SSL` security protocol without
  enabling TLS client authentication (i.e., using TLS only for
  encryption).

- Add the [`INCLUDE HEADER`
  option](/docs/sql/create-source/kafka/#headers) to Kafka sources,
  which allows extracting individual headers from Kafka messages and
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/releases/v0.78.md"
class="btn-ghost"><img
