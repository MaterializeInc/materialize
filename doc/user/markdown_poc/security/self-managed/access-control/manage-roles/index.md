...
[Home](/docs/)  /  [Security](/docs/security/)
 /  [Self-managed](/docs/security/self-managed/)  /  [Access control
(Role-based)](/docs/security/self-managed/access-control/)
...
(i.e., enforced) when using
[authentication](/docs/security/self-managed/authentication/#configuring-authentication-type).
...
- For [local installations using
  Kind/Minikube](/docs/installation/#installation-guides), set
  `spec.enableRbac: true` option when instantiating the Materialize
  object.

- For [Cloud deployments using Materialize’s
  Terraforms](/docs/installation/#installation-guides), set
  `enable_rbac_checks` in the environment CR via the
...
command](/docs/security/appendix/appendix-command-privileges/)
...
[`CREATE ROLE ... WITH LOGIN PASSWORD ...`](/docs/sql/create-role):
...
restrictions](/docs/sql/identifiers/#naming-restrictions).
...
using [`CREATE ROLE … WITH LOGIN PASSWORD`](/docs/sql/create-role).
...
[`PUBLIC`](/docs/security/appendix/appendix-built-in-roles/#public-role).
As such, every role includes inherited privileges from `PUBLIC`.

Once a role is created, you can:

- [Manage its current
  privileges](/docs/security/self-managed/access-control/manage-roles/#manage-current-privileges-for-a-role)
...
- [Manage its future
  privileges](/docs/security/self-managed/access-control/manage-roles/#manage-future-privileges-for-a-role)
  ...
  - By defining default privileges for objects. With default privileges
    ...
    privileges](/docs/security/appendix/appendix-privileges/) for that
...
[`CREATE ROLE`](/docs/sql/create-role):
...
[`SHOW PRIVILEGES`](/docs/sql/show-privileges) command, substituting
`<role>` with the role name (see
[`SHOW PRIVILEGES`](/docs/sql/show-default-privileges) for the full
syntax):
...
[`SHOW PRIVILEGES`](/docs/sql/show-privileges) on the user’s role. For
example, show the privileges for the
...
[`SHOW PRIVILEGES`](/docs/sql/show-privileges) on the service account’s
role. For example, show the privileges for the `sales_report_app` role
created in the [Create a role
section](#create-individual-userservice-account-roles).
...
To grant
[privileges](/docs/security/appendix/appendix-command-privileges/) to a
role, use the [`GRANT PRIVILEGE`](/docs/sql/grant-privilege/) statement
(see [`GRANT PRIVILEGE`](/docs/sql/grant-privilege/) for the full
syntax)
...
- [`SELECT`](/docs/sql/select/#privileges) from currently existing
  materialized views/views/tables/sources in the `mydb.sales` schema.
- [`CREATE MATERIALIZED VIEW`](/docs/sql/create-materialized-view/#privileges)
  in the `mydb.sales` schema on the `compute_cluster`.
- [`CREATE VIEW`](/docs/sql/create-view/#privileges) if using
  intermediate views as part of a stacked view definition (i.e., views
  whose definition depends on other views).
...
- [`CREATE INDEX`](/docs/sql/create-index/#privileges) in the
  `mydb.sales` schema on the `serving_cluster`.
...
- See [Grant a role to another
  role](/docs/security/self-managed/access-control/manage-roles/#grant-a-role-to-another-role)
  for details and example of granting `serving_cluster` role to
  `view_manager`.

- See [Change ownership of
  objects](/docs/security/self-managed/access-control/manage-roles/#change-ownership-of-objects)
...
- [`SELECT`](/docs/sql/select/#privileges) from all existing
  tables/materialized views/views/sources in the `mydb.sales` schema on
  the `serving_cluster`.
...
[`GRANT ROLE`](/docs/sql/grant-role/) statement (see
[`GRANT ROLE`](/docs/sql/grant-role/) for full syntax):
...
privileges](/docs/security/appendix/appendix-privileges/) for an object
they create; for example, the creator of a schema gets `CREATE` and
...
[`REVOKE <privilege>`](/docs/sql/revoke-privilege/) statement:
...
[`REVOKE <role>`](/docs/sql/revoke-role/) statement:
...
In Materialize, a role automatically gets all [applicable
privileges](/docs/security/appendix/appendix-privileges/) for an object
they create/own; for example, the creator of a schema gets `CREATE` and
`USAGE`; the creator of a table gets `SELECT`, `INSERT`, `UPDATE`, and
`DELETE`. However, for others to access the new object, you can either
manually grant privileges on new objects or use default privileges to
automatically grant privileges to others as new objects are created.
...
[`SHOW DEFAULT PRIVILEGES`](/docs/sql/show-default-privileges) command,
substituting `<role>` with the role name (see
[`SHOW DEFAULT PRIVILEGES`](/docs/sql/show-default-privileges) for the
full syntax):
...
[`SHOW DEFAULT PRIVILEGES`](/docs/sql/show-default-privileges) on the
user’s role. For example, show the defaultprivileges for the
[`blue.berry@example.com`](mailto:blue.berry@example.com) role created
in the [Create a role
section](#create-individual-userservice-account-roles).
...
[types](/docs/sql/types/#custom-types) created by any user
(`object_owner` `PUBLIC`).
...
- To use a [type](/docs/sql/types/#custom-types) created in a schema,
  the `USAGE` access is required on the containing schema as well.
...
[`SHOW DEFAULT PRIVILEGES`](/docs/sql/show-default-privileges) on the
service account’s role. For example, show the default privileges for the
...
[`ALTER DEFAULT PRIVILEGES`](/docs/sql/alter-default-privileges) command
(see [`ALTER DEFAULT PRIVILEGES`](/docs/sql/alter-default-privileges)
for the full syntax):
...
[`SHOW ROLES`](/docs/sql/show-roles/) command:
...
[`DROP ROLE`](/docs/sql/drop-role/) command:
...
You can use [`ALTER ROLE ... SET`](/docs/sql/alter-role/) to set various
...
object](/docs/security/appendix/appendix-command-privileges/) (such as
creating an index on a materialized view or changing owner of an object)
require ownership of the object itself (or *superuser* privileges).
...
privileges](/docs/security/appendix/appendix-privileges/) for the
object. To transfer ownership (and privileges) to another role (another
user role/service account role/functional role), you can use the [ALTER
… OWNER TO](/docs/sql/alter-owner/) command:
...
privileges
example](/docs/security/self-managed/access-control/manage-roles/#alter-default-privileges)).
...
- [Access control best
  practices](/docs/security/self-managed/access-control/#best-practices)
- [Manage privileges with
  Terraform](/docs/manage/terraform/manage-rbac/)
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/security/self-managed/access-control/manage-roles.md"
class="btn-ghost"><img
