...
[Snowflake](/docs/serve-results/snowflake/), Databricks or BigQuery.
...
assumption-based
authentication](/docs/sql/create-connection/#aws-permissions) to manage
access to the target bucket.
...
3.  In **Trusted entity type**, select **Custom trust policy**, and copy
    ...
    [AWS connection
    permissions](/docs/sql/create-connection/#aws-permissions)). For
    now, you’ll set this ID to a dummy value; later, you’ll update it
    with the unique identifier for your Materialize region.
...
1.  In the [SQL Shell](/docs/console/), or your preferred SQL client
    ...
    `<account-id>` with the 12-digit number that identifies your AWS
    account, and `<role>` with the name of the role you created in the
    previous step:
...
3.  In your AWS account, find the IAM role you created in [Create an IAM
    ...
    **Edit trust policy**. Use the `external_id` from the previous step
    to update the trust policy’s `sts:ExternalId`, then click **Update
    policy**.
...
4.  Back in Materialize, validate the AWS connection you created using
    the [`VALIDATE CONNECTION`](/docs/sql/validate-connection) command.
