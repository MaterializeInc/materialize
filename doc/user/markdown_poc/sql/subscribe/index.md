...
[`SELECT`](/docs/sql/select) statement. While a `SELECT` statement
computes a relation at a moment in time, a subscribe operation computes
how a relation *changes* over time.
...
- `<select_stmt>` is the [`SELECT` statement](/docs/sql/select) whose
  output you want to subscribe to.
...
href="/docs/sql/create-sink/kafka/#debezium-envelope">Debezium-style
...
subscriptions](/docs/transform-data/patterns/durable-subscriptions/#history-retention-period).
If `AS OF` is unspecified, the system automatically chooses an `AS OF`
timestamp.

The value in the `AS OF` clause is automatically [cast to
`mz_timestamp`](../../sql/types/mz_timestamp/#valid-casts) with an
assignment or implicit cast.
...
The value in the `UP TO` clause is automatically [cast to
`mz_timestamp`](../../sql/types/mz_timestamp/#valid-casts) with an
assignment or implicit cast.
...
[`FETCH`](/docs/sql/fetch) statement or `AS OF` and `UP TO` bounds to
fetch rows from `SUBSCRIBE` in batches. See the [examples](#examples)
for details.
...
As an example, we’ll create a [auction load
...
[`DECLARE`](/docs/sql/declare) and [`FETCH`](/docs/sql/fetch). These
must be used within a transaction, with [a single
`DECLARE`](/docs/sql/begin/#read-only-transactions) per transaction.
This allows you to limit the number of rows and the time window of your
requests. Next, let’s subscribe to the `bids` table of the `auction`
...
Then, use [`FETCH`](/docs/sql/fetch) in a loop to retrieve each batch of
results as soon as it’s ready:
...
[Debezium-style diff
envelope](/docs/sql/create-sink/kafka/#debezium-envelope), use
