...
Materialize exposes a system catalog that contains metadata about the
running Materialize instance.

The system catalog consists of several schemas that are implicitly
available in all databases. These schemas contain sources, tables, and
views that expose different types of metadata.
...
- [`information_schema`](information_schema), which presents the data in
  `mz_catalog` in the format used by the SQL standard’s
  information_schema.
...
These schemas contain sources, tables, and views that expose metadata
like:
...
`pg_catalog`. The mapping between Materialize concepts and PostgreSQL
concepts is not one-to-one, and so the data in `pg_catalog` cannot
accurately represent the particulars of Materialize.
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/sql/system-catalog/_index.md"
class="btn-ghost"><img
