...
[materialized views](/docs/sql/create-materialized-view/),
[tables](/docs/sql/create-table), and
...
view](/docs/sql/create-materialized-view/#with_options) or
[table](/docs/sql/create-table/#syntax) should be partitioned by the
listed columns. For example, a table that stores an append-only
collection of events may want to partition the data by time:
...
filter](/docs/transform-data/patterns/temporal-filters/) that matches
only rows with recent timestamps.
...
statement](/docs/sql/explain-plan/). In the following example, we can be
confident our temporal filter will be pushed down because it’s present
in the `pushdown` list at the bottom of the output.
