...
2.  Create the `bird_rules` table and insert a few rules.
    ...
    type](/docs/sql/types/jsonb) and adjust the logic in the upcoming
    `LATERAL` join to suit your needs.
...
1.  Subscribe to the changes of `birds_filtered`.
    ...
    **💡 Tip:** If running this example in a client, use
    `COPY(SUBSCRIBE...) TO STDOUT;`.
