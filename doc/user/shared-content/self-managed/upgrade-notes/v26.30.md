...
Adopting `v1` is **opt-in**. You may continue to apply `v1alpha1` CRs and your existing instances will behave exactly as before. We recommend you opt-in to `v1` at your convenience, as we will migrate to the `v1` behavior in the next major release. For step-by-step instructions, see [Adopting the v1 CRD](/self-managed-deployments/upgrading/adopting-the-v1-crd/).
...
run `terraform init && terraform plan && terraform apply`. To enable
