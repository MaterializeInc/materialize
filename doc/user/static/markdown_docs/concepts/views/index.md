...
| Type |  |
|----|----|
| [**Views**](#views) | Results are recomputed from scratch each time the view is accessed. You can create an **[index](/docs/concepts/indexes/)** on a view to keep its results **incrementally updated** and available **in memory** within a cluster. |
| [**Materialized views**](#materialized-views) | Results are persisted in **durable storage** and **incrementally updated**. You can create an [**index**](/docs/concepts/indexes/) on a materialized view to make the results available in memory within a cluster. |

## Views

A view saves a query under a name to provide a shorthand for referencing
the query. Views are not associated with a
[cluster](/docs/concepts/clusters/) and can be referenced across
clusters.
...
[index](/docs/concepts/indexes/) on a view to keep view results
**incrementally updated** in memory within a cluster. That is, with
**indexed views**, you do not recompute the view results each time you
access the view in the cluster; queries can access the already
...
- [`CREATE VIEW`](/docs/sql/create-view) for complete syntax information
- [`CREATE INDEX`](/docs/sql/create-index/) for complete syntax
  information

### Indexes on views

In Materialize, views can be [indexed](/docs/concepts/indexes/). Indexes
represent query results stored in memory. Creating an index on a view
executes the underlying view query and stores the view results in memory
within that [cluster](/docs/concepts/clusters/).
...
results in memory within that [cluster](/docs/concepts/clusters/).
Within the cluster, the **in-memory up-to-date** results are immediately
...
[clusters](/docs/concepts/clusters/).
...
- [`CREATE MATERIALIZED VIEW`](/docs/sql/create-materialized-view) for
...
in-memory state by reading data from Materialize’s storage layer;
...
- Views can be referenced across [clusters](/docs/concepts/clusters/).

- Materialized views can be referenced across
  [clusters](/docs/concepts/clusters/).

- [Indexes](/docs/concepts/indexes) are local to a cluster.
