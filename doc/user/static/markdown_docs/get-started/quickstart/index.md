...
consistency guarantees](/docs/get-started/isolation-level/). In
Materialize, both
[indexes](/docs/concepts/indexes/ "Indexes represents query results stored in memory
within a cluster") and [materialized
views](/docs/concepts/views/#materialized-views) **incrementally
update** results when Materialize ingests new data; i.e., work is
performed on writes. Because work is performed on writes, reads from
these objects return up-to-date results while being computationally
**free**.
...
- Create and query various [views](/docs/concepts/views/) on sample
  auction data. The data is continually generated at 1 second intervals
  to mimic a data-intensive workload.

- Create an
  [index](/docs/concepts/indexes "Indexes represents query results stored
...
Console](/docs/images/quickstart-console.png "Quickstart in the Materialize Console")

## Prerequisite

To get started with Materialize Cloud, you will need a Materialize
account. If you do not have an account, you can [sign up for a free
trial](https://materialize.com/register/?utm_campaign=General&utm_source=documentation).

Alternatively:

- You can [download the Materialize
  Emulator](/docs/get-started/install-materialize-emulator/). However,
...
- If you have a Materialize account, navigate to the [Materialize
  Console](/docs/console/) and sign in. By default, you should be in the
  SQL Shell. If you’re already signed in, you can access the SQL Shell
  in the left-hand menu.

- If you are using the Materialize Emulator, open the Materialize
  Console in your browser at <http://localhost:6874>.

- If you are running against your own \[Self-managed Materialize\], open
  your deployment’s Materialize Console.

## Step 1. Create a schema

By default, you are using the `quickstart` cluster, working in the
`materialize.public` [namespace](/docs/sql/namespaces/), where:

- A [cluster](/docs/concepts/clusters/) is an isolated pool of compute
  resources (CPU, memory, and scratch disk space) for running your
  workloads),
...
[Sources](/docs/concepts/sources/) are external systems from which
Materialize reads in data. This tutorial uses Materialize’s [sample
`Auction` load
generator](/docs/sql/create-source/load-generator/#auction) to create
the source.

1.  Create the
    [source](/docs/concepts/sources "External systems from which
    Materialize reads data.") using the
    [`CREATE SOURCE`](/docs/sql/create-source/) command.

    For the [sample `Auction` load
    generator](/docs/sql/create-source/load-generator/#auction), the
    quickstart uses [`CREATE SOURCE`](/docs/sql/create-source/) with the
    `FROM LOAD GENERATOR` clause that works specifically with
...
2.  Use the [`SHOW SOURCES`](/docs/sql/show-sources/) command to see the
    ...
    A [`subsource`](/docs/sql/show-subsources) is how Materialize refers
    to a table that has the following properties:
...
3.  Use the [`SELECT`](/docs/sql/select) statement to query `auctions`
    and `bids`.
...
A [view](/docs/concepts/views/) is a saved name for the underlying
`SELECT` statement, providing an alias/shorthand when referencing the
query. The underlying query is not executed during the view creation;
instead, the underlying query is executed when the view is referenced.
...
1.  Using the [`CREATE VIEW`](/docs/sql/create-view/) command, create a
    [**view**](/docs/concepts/views/ "Saved name/alias for a query") to
    find the winning (highest) bids.
    ...
    queries](/docs/transform-data/idiomatic-materialize-sql/top-k/)
    using the
    [`DISTINCT ON`](/docs/transform-data/idiomatic-materialize-sql/top-k/#for-k--1-1)
...
2.  [`SELECT`](/docs/sql/select/) from the view to execute the
    underlying query. For example:
    ...
    [**indexes**](/docs/concepts/indexes/) on views. Indexes provide
    always fresh view results in memory within a cluster by performing
    incremental updates as new data arrives. Queries can then read from
    the in-memory, already up-to-date results instead of re-running the
...
[indexes](/docs/concepts/indexes/) on views to provide always fresh,
up-to-date view results in memory within a cluster. Queries can then
read from the in-memory, already up-to-date results instead of
...
operations](/docs/transform-data/optimization/) like point lookups and
joins.

1.  Use the [`CREATE INDEX`](/docs/sql/create-index/) command to create
    the following index on the `winning_bids` view.
    ...
    operations](/docs/transform-data/optimization/) like point lookups
    and [delta
...
1.  Create a view to detect auction flipping activities.
    ...
    [`SELECT`](/docs/sql/select) command:
...
2.  Use [`CREATE TABLE`](/docs/sql/create-table) to create a
    `known_flippers` table that you can manually populate with known
    flippers. That is, assume that separate from your auction activities
    data, you receive independent data specifying users as flippers.
...
[`SUBSCRIBE`](/docs/sql/subscribe/) to `flippers` to see new flippers
appear as new data arrives (either from the known_flippers table or the
flip_activities view).

1.  Use [`SUBSCRIBE`](/docs/sql/subscribe/) command to see flippers as
    new data arrives (either from the `known_flippers` table or the
    `flip_activities` view). [`SUBSCRIBE`](/docs/sql/subscribe/) returns
    data from a source, table, view, or materialized view as they occur,
    in this case, the view `flippers`.
    ...
    option](/docs/sql/subscribe/#with-options) indicates that the
    command displays only the new flippers that come in after the start
    of the `SUBSCRIBE` operation, and not the flippers in the view at
...
1.  Use the [`DROP SOURCE ... CASCADE`](/docs/sql/drop-source/) command
    to drop `auction_house` source and its dependent objects, including
    views and indexes created on the `auction_house` subsources.
...
2.  Use the [`DROP TABLE`](/docs/sql/drop-table) command to drop the
    separate `known_flippers` table.
...
results stored in memory within a cluster. When you create an index on a
view, the index incrementally updates the view results (instead of
recalculating the results from scratch) as Materialize ingests new data.
These up-to-date results are then immediately available and
computationally free for reads within the cluster.
...
implications](/docs/administration/billing/#compute). For best practices
when creating indexes, see [Index Best
Practices](/docs/concepts/indexes/#best-practices).
...
[sources](/docs/sql/create-source/), and navigate to **Data** \>
**Sources** \> **New source** in the [Materialize
Console](/docs/console/) to create your first source.
