...
from Segment. By default, the source will be created in the active
...
the signature using the [`decode`](/docs/sql/functions/#decode)
function, getting the raw bytes, and generate our own HMAC using the
[`hmac`](/docs/sql/functions/#hmac) function. If the two values are
equal, then the request is legitimate!
