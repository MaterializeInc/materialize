...
(Self-Managed)](/docs/self-managed/v25.2/installation/)  /  [Install
locally on
...
`orchestratord` version, and the `environmentd` versions. The following
table presents the versions compatibility for the operator and the
applications:
...
7.  Create a new `upgrade-materialize.yaml` file with the following
    content:
    ...
    <td>Update the version to the new version.</td>
    </tr>
    <tr>
    ...
    spec:
      environmentdImageRef: materialize/environmentd:v0.147.20 # Update version
...
8.  Apply the upgrade-materialize.yaml file to your Materialize
    instance:
...
10. Open the Materialize Console. The Console should display the new
    version.
