...
pattern](/docs/manage/dbt/slim-deployments/) instead for quicker
...
[sinks](/docs/manage/dbt/get-started/#sinks), you **must** ensure that
these are created in a **dedicated schema and cluster**. Unlike other
objects, sinks must not be recreated in the process of a blue/green
deployment, and must instead cut over to the new definition of their
upstream dependencies after the environment swap.
...
3.  Run the dbt project containing the code changes against the new
    ...
    sinks](/docs/manage/dbt/development-workflows/#exclude-sources-and-sinks)
