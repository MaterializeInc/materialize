...
(Role-based)](/docs/security/cloud/access-control/)
...
**database objects** through privileges granted to database roles.
...
admins**](/docs/security/cloud/users-service-accounts/#organization-roles)
can manage roles (including overriding ownership requirements when
granting privileges on various objects).
...
To create a new role manually, use the
[`CREATE ROLE`](/docs/sql/create-role/) statement.
...
- The existence of a `mydb` database and a `sales` schema within the
  `mydb` database. For example:
...
- The existence of `items`, `orders`, and `sales_items` tables within
  the `mydb.sales` schema. For example:
...
[user](/docs/security/users-service-accounts/invite-users/), run
[`SHOW PRIVILEGES`](/docs/sql/show-privileges) on the role named after
the user’s email (automatically created when the account is activated;
i.e., first time the user logs in):
...
account](/docs/security/users-service-accounts/create-service-accounts/),
run [`SHOW PRIVILEGES`](/docs/sql/show-privileges) on the role named
after the service account user (automatically created when the account
is activated; i.e., first time the service account connects):
...
<div id="tab-manually-created-functional-roles" class="tab-pane"
title="Manually created functional roles">
...
For example, the following grants privileges to the manually created
functional roles.
...
- See [Grant a role to another
  role](/docs/security/cloud/access-control/manage-roles/#grant-a-role-to-another-role)
  for details and example of granting `serving_cluster` role to
  `view_manager`.

- See [Change ownership of
  objects](/docs/security/cloud/access-control/manage-roles/#change-ownership-of-objects)
...
individual user or service account roles (which are named after email
addresses or service account user). Instead, create reusable, functional
roles (e.g., `data_reader`, `view_manager`) with well-defined
...
After the `view_manager` role is granted to
[`blue.berry@example.com`](mailto:blue.berry@example.com),
[`blue.berry@example.com`](mailto:blue.berry@example.com) can create
objects in the `mydb.sales` schema on the `compute_cluster`.
...
schema and vice versa. For privileges on new objects created by
**others**, you can either:
...
role named after the user’s email:
...
account](/docs/security/users-service-accounts/create-service-accounts/),
run [`SHOW DEFAULT PRIVILEGES`](/docs/sql/show-default-privileges) on
the role named after the service account user:
...
To illustrate, the following adds a new member `lemon@example.com` to
the `view_manager` role and creates a new default privilege, specifying
...
require ownership of the object itself (or *superuser* privileges of an
Organization admin).
...
… OWNER TO](/docs/sql/#rbac) commands:
...
privileges
example](/docs/security/cloud/access-control/manage-roles/#alter-default-privileges)).
...
- [Access control best
  practices](/docs/security/cloud/access-control/#best-practices)
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/security/cloud/access-control/manage-roles.md"
class="btn-ghost"><img
