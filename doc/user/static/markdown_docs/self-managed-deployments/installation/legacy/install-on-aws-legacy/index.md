...
 /  [Install Guides
(Legacy)](/docs/self-managed-deployments/installation/legacy/)
...
PostgreSQL as a metadata database; blob storage; and a license key.
...
blob storage. The tutorial uses the [Legacy Materialize on AWS Terraform
...
See [AWS Deployment
guidelines](/docs/self-managed-deployments/deployment-guidelines/aws-deployment-guidelines/)
...
| AWS Load Balancer Controller and Network Load Balancers for each Materialize instance | [v0.3.0+](/docs/self-managed-deployments/appendix/legacy/appendix-legacy-terraform-releases/#materialize-on-aws-terraform-module) |
| OpenEBS and NVMe instance storage to enable spill-to-disk | [v0.3.1+](/docs/self-managed-deployments/appendix/legacy/appendix-legacy-terraform-releases/#materialize-on-aws-terraform-module) |
| `cert-manager` and a self-signed `ClusterIssuer`. `ClusterIssuer` is deployed on subsequent runs after the `cert-manager` is running. | [v0.4.0+](/docs/self-managed-deployments/appendix/legacy/appendix-legacy-terraform-releases/#materialize-on-aws-terraform-module) |
...
configuration](/docs/self-managed-deployments/appendix/legacy/appendix-configuration-legacy-aws/).
...
5.  Clone your forked repo and checkout the `v0.8.8` tag. For example,
    ...
    - If cloning via SSH (replace `YOUR_ORGANIZATION` with your
      ...
      ``` chroma
      git clone --depth 1 -b v0.8.8 git@github.com:${MY_ORGANIZATION}/terraform-aws-materialize.git
      ```
    ...
    - If cloning via HTTPS (replace `YOUR_ORGANIZATION` with your
      ...
      ``` chroma
      git clone --depth 1 -b v0.8.8 https://github.com/${MY_ORGANIZATION}/terraform-aws-materialize.git
      ```
...
13. By default, the example Terraform installs the Materialize Operator
    ...
    [Troubleshooting](/docs/installation/troubleshooting) guide.
...
14. Once the Materialize operator is deployed and running, you can
    ...
    **💡 Tip:**
    ...
    releases](/docs/self-managed-deployments/appendix/legacy/appendix-legacy-terraform-releases/#materialize-on-aws-terraform-module)
    for notable changes.
...
18. Open the Materialize Console in your browser:
    ...
    **💡 Tip:** If you experience long loading screens or
    ...
    Unresponsiveness](/docs/self-managed-deployments/troubleshooting/#troubleshooting-console-unresponsiveness)
    guide.
...
- To start ingesting your own data from an external system like Kafka,
  MySQL or PostgreSQL, see [Ingest data](/docs/ingest-data/).
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/self-managed-deployments/installation/legacy/install-on-aws-legacy.md"
class="btn-ghost"><img
