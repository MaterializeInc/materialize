...
You can use [Tableau Cloud](https://www.tableau.com/products/cloud-bi)
and [Tableau Desktop](https://www.tableau.com/products/desktop) to
...
[PostgreSQL database
connector](https://help.tableau.com/current/pro/desktop/en-us/examples_postgresql.htm)
...
For more details and troubleshooting, check the [Tableau
documentation](https://help.tableau.com/current/pro/desktop/en-us/examples_postgresql.htm).
...
1.  Download the [Java 8 JDBC driver for
    PostgreSQL](https://jdbc.postgresql.org/download/)

2.  Copy the `.jar` file to the following directory (which may have to
    be created manually):
...
Once you’ve set up the required driver, start Tableau and run through
the following steps:
...
Errors like the following indicate that the JDBC driver was not
...
The errors occur because Tableau falls back to a legacy PostgreSQL ODBC
driver that does not support connecting to Materialize. Follow the
[Setup](#setup) instructions again and ensure you’ve downloaded the
driver to the correct folder for your platform.
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/serve-results/bi-tools/tableau.md"
class="btn-ghost"><img
