...
examples, refer to
[`CREATE SINK`](/docs/self-managed/v25.2/sql/create-sink/kafka).
...
[connection
options](/docs/self-managed/v25.2/sql/create-sink/kafka#connection-options).

If the connection’s [progress
topic](/docs/self-managed/v25.2/sql/create-sink/kafka#exactly-once-processing)
does not exist, Materialize will attempt to create it with a single
partition, the broker’s default replication factor, compaction enabled,
and both size- and time-based retention disabled. The replication factor
can be overridden using the `PROGRESS TOPIC REPLICATION FACTOR` option
...
options](/docs/self-managed/v25.2/sql/create-sink/kafka#connection-options)
to set any relevant kafka [topic
...
using the <a
href="/docs/self-managed/v25.2/sql/create-sink/kafka#upsert">Upsert
...
href="/docs/self-managed/v25.2/sql/create-sink/kafka#exactly-once-processing">exactly-once
guarantees</a>.</td>
</tr>
<tr>
<td>Progress topic</td>
...
- Can refer to any column in the sink’s underlying relation when using
  the [upsert
  envelope](/docs/self-managed/v25.2/sql/create-sink/kafka#upsert-envelope).
- Can refer to any column in the sink’s key when using the [Debezium
  envelope](/docs/self-managed/v25.2/sql/create-sink/kafka#debezium-envelope).
...
partioning](/docs/self-managed/v25.2/sql/create-sink/kafka#custom-partitioning).
