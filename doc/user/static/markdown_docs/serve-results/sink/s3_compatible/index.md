...
[Home](/docs/self-managed/v25.2/)  /  [Serve
...
1.  In the [SQL Shell](https://console.materialize.com/), or your
    ...
    replacing `<ACCESS_KEY_ID>` and `<SECRET_ACCESS_KEY>` with the
    credentials for your bucket. The AWS connection can be used to
    connect to any S3 compatible object storage service, by specifying
    the endpoint and the region.
...
2.  Validate the connection you created using the
    ...
    If no validation error is returned, you’re ready to use this
    connection to run a bulk export from Materialize to your target
    bucket.
...
and the AWS connection you created in the previous step. Replace the
