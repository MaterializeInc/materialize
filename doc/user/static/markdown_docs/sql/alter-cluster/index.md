...
For completeness, the syntax for `SWAP WITH` operation is provided.
However, in general, you will not need to manually perform this
operation.
...
<td>The name of the cluster you want to alter.</td>
</tr>
<tr>
<td><code>SIZE</code></td>
<td><span id="alter-cluster-size"></span> The size of the resource
allocations for the cluster.
<ul>
...
<p>See <a href="#available-sizes">Size</a> for details as well as legacy
sizes available.</p>
<div class="warning">
...
<td><p>Optional.The number of replicas to provision for the cluster.
Each replica of the cluster provisions a new pool of compute resources
to perform exactly the same computations on exactly the same data. For
more information, see <a href="#replication-factor">Replication factor
considerations</a>.</p>
<p>Default: <code>1</code></p></td>
</tr>
<tr>
<td><code>MANAGED</code></td>
<td><p>Optional. Whether to automatically manage the cluster’s replicas
based on the configured size and replication factor.</p>
...
<td><p>Optional. The <a
href="/docs/sql/create-cluster/#scheduling">scheduling type</a> for the
cluster. Valid values are <code>MANUAL</code> and
<code>ON REFRESH</code>.</p>
...
<tr>
<td><code>REPLICATION FACTOR</code></td>
<td><p>Optional. The number of replicas to provision for the
cluster.</p>
<p>Default: <code>1</code></p></td>
</tr>
<tr>
...
**NOTE:** You cannot rename system clusters, such as `mz_system` and
`mz_catalog_server`.
...
To change the owner, you must have ownership of the cluster and
membership in the `<new_owner_role>`. See also [Required
privileges](#required-privileges).

</div>

<div id="tab-swap-with" class="tab-pane" title="Swap with">
...
You cannot rename system clusters, such as `mz_system` and
`mz_catalog_server`.
