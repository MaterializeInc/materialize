...
To change the owner of a materialized view, you must be the owner of the
materialized view and have membership in the `<new_owner_role>`. See
also [Privileges](#privileges).
...
- In addition, to change owners:
  ...
  - `CREATE` privileges on the containing schema if the materialized
    view is namespaced by a schema.
