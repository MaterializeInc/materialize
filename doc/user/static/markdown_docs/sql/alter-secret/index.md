...
<div id="tab-change-value" class="tab-pane" title="Change value">
...
To change the owner, you must be a current owner as well as have
...
- Future [`CREATE CONNECTION`](/docs/sql/create-connection/),
  [`CREATE SOURCE`](/docs/sql/create-source), and
  [`CREATE SINK`](/docs/sql/create-sink) commands will use the new value
  of the secret immediately.

- Running sources and sinks that reference the secret will **not**
  immediately use the new value of the secret. Sources and sinks may
  cache the old secret value for several weeks.

  To force a running source or sink to refresh its secrets, drop and
...
- In addition, to change owners:
  ...
  - `CREATE` privileges on the containing schema if the secret is
    namespaced by a schema.

## Related pages

- [`SHOW SECRETS`](/docs/sql/show-secrets)
- [`DROP SECRET`](/docs/sql/drop-secret)
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/sql/alter-secret.md"
class="btn-ghost"><img
