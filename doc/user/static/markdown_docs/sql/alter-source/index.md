...
<div id="tab-add-subsource" class="tab-pane" title="Add subsource">

### Add subsource

To add the specified upstream table(s) to the specified
PostgreSQL/MySQL/SQL Server source:
...
**NOTE:** When you add a new subsource to an existing source
...
<div id="tab-rename" class="tab-pane" title="Rename">
...
See also [Renaming
restrictions](/docs/sql/identifiers/#renaming-restrictions).

</div>

<div id="tab-change-owner" class="tab-pane" title="Change owner">
...
To change the owner of a source, you must be the owner of the source and
have membership in the `<new_owner_role>`. See also
[Privileges](#privileges).

</div>

<div id="tab-reset-retain-history-config" class="tab-pane"
title="(Re)Set retain history config">
...
change the schema of the PostgreSQL/MySQL/SQL Server tables that are
ingested.

<div class="important">

**! Important:** When you add a new subsource to an existing source
([`ALTER SOURCE … ADD SUBSOURCE …`](/docs/sql/alter-source/)),
Materialize starts the snapshotting process for the new subsource.
During this snapshotting, the data ingestion for the existing subsources
for the same source is temporarily blocked. As such, if possible, you
...
To drop a subsource, use the [`DROP SOURCE`](/docs/sql/drop-source/)
command:
...
- In addition, to change owners:
  ...
  - `CREATE` privileges on the containing schema if the source is
    namespaced by a schema.

## See also

- [`CREATE SOURCE`](/docs/sql/create-source/)
