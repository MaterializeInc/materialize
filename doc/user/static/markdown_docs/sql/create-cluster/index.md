...
- Executing [`SELECT`](/docs/self-managed/v25.2/sql/select) and
  [`SUBSCRIBE`](/docs/self-managed/v25.2/sql/subscribe) statements.
- Maintaining [indexes](/docs/self-managed/v25.2/concepts/indexes/) and
  [materialized
  views](/docs/self-managed/v25.2/concepts/views/#materialized-views).
- Maintaining [sources](/docs/self-managed/v25.2/concepts/sources/) and
...
<td>text</td>
<td>The size of the resource allocations for the cluster. See <a
href="/docs/self-managed/v25.2/sql/create-cluster#size">Size</a> for
details.</td>
</tr>
<tr>
...
<td>interval</td>
<td><p>The interval at which to collect introspection data. See <a
...
entirely disables the gathering of introspection data.</p>
<p>Default: <code>1s</code></p></td>
</tr>
<tr>
<td><code>INTROSPECTION DEBUGGING</code></td>
<td>bool</td>
<td><p>Indicates whether to introspect the gathering of the
...
and <a
href="/docs/self-managed/v25.2/sql/drop-cluster-replica"><code>DROP CLUSTER REPLICA</code></a>
...
named `quickstart` with a size of `25cc` and a replication factor of
`1`. You can drop or alter this cluster to suit your needs.
...
To show your session’s active cluster, use the
[`SHOW`](/docs/self-managed/v25.2/sql/show) command:
...
To switch your session’s active cluster, use the
[`SET`](/docs/self-managed/v25.2/sql/set) command:
...
The `SIZE` option determines the amount of compute resources (CPU,
memory, and disk) available to the cluster.
...
workload using
[`ALTER CLUSTER`](/docs/self-managed/v25.2/sql/alter-cluster). Depending
on the type of objects the cluster is hosting, this operation **might
incur downtime**.

See the reference documentation for
[`ALTER CLUSTER`](/docs/self-managed/v25.2/sql/alter-cluster#zero-downtime-cluster-resizing)
...
[`ALTER CLUSTER`](/docs/self-managed/v25.2/sql/alter-cluster/) to set a
nonzero replication factor.
...
[`ALTER CLUSTER`](/docs/self-managed/v25.2/sql/alter-cluster/) statement
provisions the replica and ends when an
[`ALTER CLUSTER`](/docs/self-managed/v25.2/sql/alter-cluster/) or
[`DROP CLUSTER`](/docs/self-managed/v25.2/sql/drop-cluster/) statement
deprovisions the replica.
...
refresh](/docs/self-managed/v25.2/sql/create-materialized-view/#querying-materialized-views-with-refresh-strategies)
...
query the
[`mz_internal.mz_cluster_schedules`](/docs/self-managed/v25.2/sql/system-catalog/mz_internal/#mz_cluster_schedules)
...
[`mz_catalog.mz_cluster_replicas`](/docs/self-managed/v25.2/sql/system-catalog/mz_catalog/#mz_cluster_replicas)
system catalog table:
...
- Clusters containing sources and sinks can only have a replication
  factor of `0` or `1`.

- Clusters containing sources and sinks cannot be resized without
  downtime.

- When a cluster of size `3200cc` or larger uses multiple replicas,
  those replicas are not guaranteed to be spread evenly across the
...
Disabling introspection can yield a small performance improvement, but
you lose the ability to run [troubleshooting
queries](/docs/self-managed/v25.2/ops/troubleshooting/) against that
cluster replica.
...
- [`ALTER CLUSTER`](/docs/self-managed/v25.2/sql/alter-cluster/)
- [`DROP CLUSTER`](/docs/self-managed/v25.2/sql/drop-cluster/)
