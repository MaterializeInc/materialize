...
`CREATE INDEX` creates an in-memory
[index](/docs/self-managed/v25.2/concepts/indexes/) on a source, view,
or materialized view.

In Materialize, indexes store query results in memory within a specific
[cluster](/docs/self-managed/v25.2/concepts/clusters/), and keep these
results **incrementally updated** as new data arrives. This ensures that
indexed data remains
[fresh](/docs/self-managed/v25.2/concepts/reaction-time), reflecting the
latest changes with minimal latency.

The primary use case for indexes is to accelerate direct queries issued
via [`SELECT`](/docs/self-managed/v25.2/sql/select/) statements. By
maintaining fresh, up-to-date results in memory, indexes can
significantly [optimize query
performance](/docs/self-managed/v25.2/transform-data/optimization/),
reducing both response time and compute load—especially for
resource-intensive operations such as joins, aggregations, and repeated
subqueries.
...
[materialized
view](/docs/self-managed/v25.2/sql/create-materialized-view), which also
...
maintain this index. If not specified, defaults to the active
cluster.</td>
</tr>
<tr>
<td><em>method</em></td>
<td>The name of the index method to use. The only supported method is <a
href="/docs/self-managed/v25.2/overview/arrangements"><code>arrangement</code></a>.</td>
</tr>
<tr>
