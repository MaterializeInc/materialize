...
A [source](/docs/self-managed/v25.2/concepts/sources/) describes an
external system you want Materialize to read data from, and provides
details about how to decode and interpret that data. To create a source,
you must specify a [connector](#connectors), a [format](#formats) and an
...
generator
source](/docs/self-managed/v25.2/sql/create-source/load-generator/) to
produce sample data that is suitable for demo and performance test
scenarios.
...
way](https://avro.apache.org/docs/current/spec.html#Schema+Resolution),
...
To avoid
[case-sensitivity](/docs/self-managed/v25.2/sql/identifiers/#case-sensitivity)
...
types](https://avro.apache.org/docs/current/spec.html), *except for*
...
with type `jsonb`. Refer to the [`jsonb`
type](/docs/self-managed/v25.2/sql/types/jsonb) documentation for the
supported operations on this type.

If your JSON messages have a consistent shape, we recommend creating a
parsing [view](/docs/self-managed/v25.2/concepts/views) that maps the
individual fields to columns with the required data types:
...
parsing widget**](/docs/self-managed/v25.2/sql/types/jsonb/#parsing)!
...
For details on casting, check the
[`text`](/docs/self-managed/v25.2/sql/types/text/) documentation.
...
[`bytea`](/docs/self-managed/v25.2/sql/types/bytea/) documentation.
...
The data in CSV sources is read as
[`text`](/docs/self-managed/v25.2/sql/types/text). You can then handle
the conversion to other types using explicit
[casts](/docs/self-managed/v25.2/sql/functions/cast/) when creating
views.
...
integration guide](/docs/self-managed/v25.2/integrations/debezium/).
...
- [`SHOW CREATE SOURCE`](/docs/self-managed/v25.2/sql/show-create-source/)
