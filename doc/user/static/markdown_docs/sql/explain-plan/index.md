...
present. The following three statements are equivalent:
...
If `PHSYICAL PLAN` is specified without an `AS`-format, we will provide
output similar to the above, but more verbose. The following two
statements are equivalent (and produce the more verbose output):
...
path):” heading before the explained query in the `EXPLAIN`,
...
Each operator can also be annotated with additional metadata. Some
details are shown in the default `EXPLAIN` output
(`EXPLAIN PHYSICAL PLAN AS TEXT`), but are hidden elsewhere.
...
join](/docs/transform-data/optimization/#optimize-multi-way-joins-with-delta-joins)
...
way](/docs/transform-data/optimization/#use-explain-to-verify-index-usage).
...
href="/docs/get-started/arrangements/#arrangements">arranged</a> in
memory and will be streamed directly.</p></li>
...
href="/docs/sql/functions/#table-functions">table function</a> to each
row in the input.</p>
...
href="/docs/sql/functions/#table-functions">table function</a>
used.<br />
<strong>Uses memory:</strong> No</td>
<td><div class="highlight">
...
<p>The <a href="/docs/transform-data/optimization/#join">two joins
differ in performance characteristics</a>.</p>
<br />
<br />
...
href="/docs/sql/system-catalog/mz_introspection/#mz_expected_group_size_advice">see
...
href="/docs/sql/system-catalog/mz_introspection/#mz_expected_group_size_advice"><code>mz_introspection.mz_expected_group_size_advice</code></a>.
...
href="/docs/sql/system-catalog/mz_introspection/#mz_expected_group_size_advice"><code>mz_introspection.mz_expected_group_size_advice</code></a>.</td>
<td><div class="highlight">
...
href="/docs/get-started/arrangements/#arrangements">arrangement</a> in
the dataflow engine, i.e., it will consume memory to cache
results.<br />
<br />
<strong>Can increase data size:</strong> No<br />
<strong>Uses memory:</strong> ✅ Uses memory proportional to the input
...
href="/docs/get-started/arrangements/#arrangements">arrangements</a> on
their inputs (shown by the <code>ArrangeBy</code> operator). These
arrangements use memory proportional to the input sizes. If an input has
an <a href="/docs/transform-data/optimization/#join">appropriate
...
<td>Indicates a point that will become an <a
href="/docs/get-started/arrangements/#arrangements">arrangement</a> in
the dataflow engine (each <code>keys</code> element will be a different
arrangement). Note that if an appropriate index already exists on the
input or the output of the previous operator is already arranged with a
key that is also requested here, then this operator will just pass on
that existing arrangement instead of creating a new one.<br />
<br />
<strong>Can increase data size:</strong> No<br />
...
<strong>Uses memory:</strong> ✅ Uses memory proportional to the input
sizes, unless <a href="/docs/transform-data/optimization/#join">the
...
that the operator is fused with its input, i.e., the operator below it.
That is, if you see a `Fused X` operator above a `Y` operator:
...
house load
generator](/docs/sql/create-source/load-generator/#creating-an-auction-load-generator)
...
Same explanation as above, but with the `EXPLAIN` expressed a bit more
verbosely:
...
The [`EXPLAIN ANALYZE`](/docs/sql/explain-analyze/) statement will let
you debug memory and cpu usage (optionally with information about worker
skew) for existing indexes and materialized views in terms of their
physical plan operators. It can also attribute [TopK
hints](/docs/transform-data/idiomatic-materialize-sql/top-k/#query-hints-1)
