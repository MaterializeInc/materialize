...
- Numbers in `jsonb` elements are all equivalent to
  [`numeric`](/docs/self-managed/v25.2/sql/types/numeric) in SQL.
...
from [`text`](/docs/self-managed/v25.2/sql/types/text).
...
syntax](/docs/self-managed/v25.2/sql/types/list#slicing-ranges) is not
supported. Note also that 0-indexed positions are at variance with
[`list`](/docs/self-managed/v25.2/sql/types/list) and
[`array`](/docs/self-managed/v25.2/sql/types/array) types, whose
subscripting operation uses 1-indexed positions.
...
which is a [text array](/docs/self-managed/v25.2/sql/types/array) where
each element is either a field key or an array element:
