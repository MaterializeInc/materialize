...
`UPDATE` changes values stored in [user-created
tables](../create-table).
...
- `UPDATE` can reference [user-created tables](../create-table) but not
  [sources](../create-source).
- **Low performance.** While processing an `UPDATE` statement,
...
The following `UPDATE` example includes a `WHERE` clause to specify
...
Only one row should be updated, namely the row with `b = 'hello'`. To
verify that the operation updated the `a` column only for that row, run
...
The returned results show that column `a` was updated only for the row
with `b = 'hello'`:
...
The following `UPDATE` example updates all rows in the table to set `a`
to 0 and `b` to `aloha`:
...
<a
href="//github.com/MaterializeInc/materialize/edit/main/doc/user/content/sql/update.md"
class="btn-ghost"><img
