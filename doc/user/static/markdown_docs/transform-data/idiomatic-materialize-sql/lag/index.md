...
href="/docs/sql/functions/#lag"><code>LAG(fieldZ) OVER (ORDER BY ...)</code></a>
...
value) ordered by some field that increases in a **regular** pattern,
use a self
[`LEFT JOIN/LEFT OUTER JOIN`](/docs/sql/select/join/#left-outer-join)
...
href="/docs/sql/select/join/#left-outer-join"><code>LEFT JOIN/LEFT OUTER JOIN</code></a>
...
<td>Anti-pattern ❌</td>
<td><p>Avoid the use of <a
href="/docs/sql/functions/#lag"><code>LAG(fieldZ) OVER (ORDER BY ...) window function</code></a>
...
`1 DAY`](/docs/sql/types/interval/#valid-operations). The query excludes
the first row in the results since the first row does not have a
previous row.
...
<td>Anti-pattern ❌</td>
<td><p>Avoid the use of <a
href="/docs/sql/functions/#lag"><code>LAG() OVER (ORDER BY ...)</code>
...
[`LEFT JOIN/LEFT OUTER JOIN`](/docs/sql/select/join/#left-outer-join) on
`orders_daily_totals`. The row ordering on the `order_date` field is
represented by an **equality condition** using an [interval of
`1 DAY`](/docs/sql/types/interval/#valid-operations). The query includes
the first row in the results, using `null` as the previous value.
...
- [Lead over](/docs/transform-data/idiomatic-materialize-sql/lead)
- [`INTERVAL`](/docs/sql/types/interval/)
- [`LEFT JOIN/LEFT OUTER JOIN`](/docs/sql/select/join/#left-outer-join)
