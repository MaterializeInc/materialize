...
the [MIN()](/docs/sql/functions/#min) or
...
<td>Anti-pattern ❌</td>
<td><p>Do not use <a
href="/docs/sql/functions/#last_value"><code>LAST_VALUE() OVER (PARTITION BY ... ORDER BY ... RANGE ...)</code>
...
[`MAX(price)`](/docs/sql/functions/#max) to find the highest price
(i.e., the last price if ordered by ascending price values):
...
[`MIN(price)`](/docs/sql/functions/#min) as the lowest price (i.e., last
price if ordered by descending price value)
...
[`MIN(price)`](/docs/sql/functions/#min) as the lowest price (i.e., last
value if ordered by descending price values) and
[`MAX(price)`](/docs/sql/functions/#max) as the highest price (i.e.,
last value if ordered by ascending price values).
