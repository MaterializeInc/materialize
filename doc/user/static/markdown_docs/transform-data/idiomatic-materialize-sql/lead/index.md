...
href="/docs/self-managed/v25.2/sql/functions/#lead"><code>LEAD(fieldZ) OVER (ORDER BY ...) window function</code></a>
...
The query excludes the last row in the results since the last row does
not have a next row.
...
<td>Anti-pattern ❌</td>
<td><p>Avoid the use of <a
href="/docs/self-managed/v25.2/sql/functions/#lead"><code>LEAD() OVER (ORDER BY ...)</code>
...
represented by an **equality condition** using an [interval of
`1 DAY`](/docs/self-managed/v25.2/sql/types/interval/#valid-operations)).
The query includes the last row in the results, using `null` as the next
row’s value.
...
- [Lag
  over](/docs/self-managed/v25.2/transform-data/idiomatic-materialize-sql/lag)
