...
subquery to [SELECT
DISTINCT](/docs/self-managed/v25.2/sql/select/#select-distinct) on the
grouping key and perform a
[LATERAL](/docs/self-managed/v25.2/sql/select/join/#lateral-subqueries)
join (by the grouping key) with another subquery that specifies the
ordering and the limit K.
...
href="/docs/self-managed/v25.2/sql/select/#select-distinct">SELECT
DISTINCT</a> on the grouping key (e.g., <code>fieldA</code>), and
perform a <a
href="/docs/self-managed/v25.2/sql/select/join/#lateral-subqueries">LATERAL</a>
join (by the grouping key <code>fieldA</code>) with another subquery
that specifies the ordering (e.g., <code>fieldZ [ASC|DESC]</code>) and
the limit K.</p>
<div class="highlight">
...
ON()](/docs/self-managed/v25.2/sql/select/#select-distinct-on) on the
grouping key (e.g., `fieldA`) and order the results first by the
`DISTINCT ON` key and then the Top-K ordering key (e.g.,
`fieldA, fieldZ [ASC|DESC]`).
...
[`EXPLAIN ANALYZE HINTS`](/docs/self-managed/v25.2/sql/explain-analyze/#explain-analyze-hints).
...
[LATERAL](/docs/self-managed/v25.2/sql/select/join/#lateral-subqueries)
join (by the grouping key) with another subquery that specifies the
ordering (`ORDER BY subtotal DESC`) and limits its results to 3
(`LIMIT 3`).
...
- [LATERAL
  subqueries](/docs/self-managed/v25.2/sql/select/join/#lateral-subqueries)
- [Query hints for Top
  K](/docs/self-managed/v25.2/transform-data/optimization/#query-hints)
- [Window
  functions](/docs/self-managed/v25.2/sql/functions/#window-functions)
