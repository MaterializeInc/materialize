...
[Subscriptions](/docs/sql/subscribe/) allow you to stream changing
results from Materialize to an external application programatically.
Like any connection over the network, subscriptions might get disrupted
for both expected and unexpected reasons. In such cases, it can be
useful to have a mechanism to gracefully recover data processing.
...
[sources](/docs/sql/create-source/), [tables](/docs/sql/create-table/),
and [materialized views](/docs/sql/create-materialized-view/), you can
either:

- Specify the `RETAIN HISTORY` option in the `CREATE` statement. The
  `RETAIN HISTORY` option accepts positive
  [interval](/docs/sql/types/interval/) values (e.g., `'1hr'`). For
  example:
...
[`mz_internal.mz_history_retention_strategies`](/docs/sql/system-catalog/mz_internal/#mz_history_retention_strategies)
...
4.  As results come in continuously, buffer the latest results in memory
    until you receive a [progress](/docs/sql/subscribe#progress)
    message. At that point, the data up until the progress message is
    complete, so you can:
...
5.  To resume the subscription in subsequent restarts, use the following
    ...
    [progress](/docs/sql/subscribe#progress) message. At that point, the
