...
- **Array element_reference ids** are injected silently at resolution
  (`names.rs:1385`, `:1444`) and never printed.
  **Decision (user-confirmed): drop these edges.** Small semantic change,
  nothing user-visible consumes them (pg_depend filters to relations/indexes).
  Document the recovery path (static array→element type map) in a comment.
...
pub struct ItemReferences {
    ...
    pub relation_names: BTreeSet<Vec<Ident>>,  // RawItemName::Name outside function/type position, CTE-excluded
    pub func_names: BTreeSet<Vec<Ident>>,      // Function<Raw>.name when RawItemName::Name
    pub type_names: BTreeSet<Vec<Ident>>,      // RawDataType::Other names when RawItemName::Name
}
...
RELATIONS: dict[str, RelationDiffConfig] = {
    "mz_internal.mz_object_dependencies": RelationDiffConfig(
        # element-ref edges dropped by design: old-only edge to type X where
...
1. Array element_reference edges dropped (user-approved): `int4[]` no longer
   also yields an edge to `int4` (edge to `_int4` survives via bracketed id).
