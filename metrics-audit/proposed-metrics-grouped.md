# Proposed Metrics — Grouped by Implementation Readiness

Companion to [`proposed-metrics.md`](./proposed-metrics.md), reorganized into three groups based on the **Existing Metric** column:

- **[A. Ready to implement](#a-ready-to-implement)** — an existing Prometheus metric (or concrete alternative) is identified. ~84 metrics.
- **[B. Unknown (`?`) with notes](#b-unknown-with-notes)** — marked `?` but the Notes column (or an inline qualifier like `? (data in ...)`) provides guidance or a partial path forward. ~52 metrics.
- **[C. Unknown (`?`) with no guidance](#c-unknown-no-guidance)** — marked `?` with no notes and no inline guidance; needs investigation. ~30 metrics.

Section-level "General notes" (design discussion, caveats) are **not** duplicated here — see the original [`proposed-metrics.md`](./proposed-metrics.md) for that context.
...
## B. Unknown, with notes

Rows where Existing Metric is `?` but guidance exists (either in the Notes column or as an inline `? (data in ...)` qualifier pointing to a SQL table / derivation).
...
## C. Unknown, no guidance

Rows where Existing Metric is just `?` and no notes are provided. These need investigation before we can plan implementation.
