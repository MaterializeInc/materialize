...
rename_view_macro = """
{% macro rename_view(from_name, to_name) %}
  {% set from_relation = api.Relation.create(
      database=target.database, schema=target.schema, identifier=from_name, type='view') %}
  {% set to_relation = api.Relation.create(
      database=target.database, schema=target.schema, identifier=to_name, type='view') %}
...
class TestRelationMacros:
    ...
    def test_rename_view(self, project):
        ...
        result = project.run_sql(
            ...
            WHERE v.name = 'my_renamed_view' AND s.name = '{}'
            """.format(
                project.test_schema
            ),
            fetch="one",
