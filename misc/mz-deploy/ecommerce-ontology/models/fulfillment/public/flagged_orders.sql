...
MOCK ontology.public.orders(order_id INTEGER, customer_id INTEGER, product_id INTEGER, order_date TIMESTAMP, total_amount NUMERIC, status TEXT, refund_amount NUMERIC, net_amount NUMERIC) AS (
  SELECT * FROM VALUES
...
EXPECTED(order_id INTEGER, customer_id INTEGER, customer_name TEXT, customer_email TEXT, total_amount NUMERIC, order_date TIMESTAMP, lifetime_orders BIGINT, refund_amount NUMERIC, flag_reason TEXT) AS (
  SELECT * FROM VALUES
