...
MOCK ontology.public.customers(customer_id INTEGER, email TEXT, name TEXT, signup_date DATE, last_order_date TIMESTAMP, lifetime_orders BIGINT, lifetime_spend NUMERIC) AS (
  SELECT * FROM VALUES
...
EXPECTED(customer_id INTEGER, name TEXT, email TEXT, signup_date DATE, lifetime_orders BIGINT, lifetime_spend NUMERIC, last_order_date TIMESTAMP, loyalty_tier TEXT) AS (
  SELECT * FROM VALUES
