...
# misc_mz_ontology: Catalog Ontology Schema

The `misc_mz_ontology` schema contains four system tables that describe the structure
...
|---|---|---|
| `misc_mz_ontology.entity_types` | `name, relation, properties, description` | What kinds of things exist. `properties` jsonb has `{"primary_key": ["id"]}`. |
| `misc_mz_ontology.semantic_types` | `name, sql_type, description` | Typed ID domains and other semantic column types (CatalogItemId, GlobalId, ByteCount, etc.) |
| `misc_mz_ontology.properties` | `entity_type, column_name, semantic_type, description` | Maps every column to its semantic type and describes what it means. |
| `misc_mz_ontology.link_types` | `name, source_entity, target_entity, properties, description` | Named relationships between entity types. |

### Link type kinds

The `properties` jsonb in `misc_mz_ontology.link_types` uses a `"kind"` field:
...
The `properties` jsonb in `misc_mz_ontology.link_types` is freeform but uses the
...
The `properties` jsonb in `misc_mz_ontology.entity_types` uses:
...
SELECT l.name, l.source_entity, l.target_entity,
       ...
       l.description
FROM misc_mz_ontology.link_types l
...
FROM misc_mz_ontology.link_types l
JOIN misc_mz_ontology.entity_types e ON e.name = l.source_entity
...
FROM misc_mz_ontology.properties p
...
the same semantic type in `misc_mz_ontology.properties`:

```sql
SELECT sp.semantic_type AS src_type, tp.semantic_type AS tgt_type
FROM misc_mz_ontology.properties sp, misc_mz_ontology.properties tp
...
The `name` column in `misc_mz_ontology.entity_types` is a short alias (e.g.,
...
`misc_mz_ontology.entity_types.relation`.** Never guess or abbreviate table
...
FROM misc_mz_ontology.entity_types
WHERE name = 'mv';
