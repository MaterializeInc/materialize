...
ROOT = Path(os.getenv("MZ_ROOT", os.getcwd()))
...
def run(duration: int, composition: str, tag: str, workflow: Optional[str]) -> None:
    ...
    if comp is not None:
        ...
        raise MzError(
            f"Unknown test {composition}, expected one of: {' '.join(Composition.known_demos())}"
        )
...
def help_workflows(composition: str) -> None:
    ...
    if comp is not None:
        ...
        for comp_ in Composition.known_demos():
            print(f"    {comp_}")
...
def down(composition: str) -> None:
    ...
    if comp is not None:
        ...
        raise UnknownItem("composition", comp, Composition.known_demos())
...
class Composition:
    ...
    def __str__(self) -> str:
        return (
            "Composition<{self.name}, {self._path}, {len(self._workflows)} workflows>"
        )
    ...
    @classmethod
    def known_demos(cls) -> Iterable[str]:
        if cls._demos is None:
            cls._demos = cls.load()
    ...
    def load() -> Dict[str, "Composition"]:
        ...
        for mzcompose in compose_files:
            ...
            raw_comp = mzcomp.get("x-mzconduct")
            workflows = {}
    ...
    def register(cls, name: str) -> Callable[[Type[T]], Type[T]]:
        ...
        def reg(to_register: Type[T]) -> Type[T]:
            if not issubclass(to_register, WorkflowStep):
                raise ValueError(
                    f"Registerd step must be a WorkflowStep: {to_register}"
                )
