...
class ArrayType(Check):
    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                """
                > CREATE MATERIALIZED VIEW array_type_view1 WITH (REFRESH EVERY '2 seconds') AS SELECT
                ...
                """,
                """
                > CREATE MATERIALIZED VIEW array_type_view2 WITH (REFRESH EVERY '2 seconds') AS SELECT
                  int_col, array_fill(2, ARRAY[2], ARRAY[2]),
                  text_col, array_fill('foo'::text, ARRAY[2]) AS array_fill2,
                  array_col, ARRAY[ARRAY[1,2], ARRAY[NULL, 4]]
                  FROM array_type_table;
