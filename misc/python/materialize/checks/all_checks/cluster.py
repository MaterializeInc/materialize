    ...
    def validate(self) -> Testdrive:
        return Testdrive(
            dedent(
                ...
                > CREATE MATERIALIZED VIEW create_cluster1_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM create_cluster1_table;
                ...
                > CREATE MATERIALIZED VIEW create_cluster2_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM create_cluster2_table;
    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                ...
                > CREATE MATERIALIZED VIEW drop_cluster1_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM drop_cluster1_table;

                > SET cluster=drop_cluster2
                > CREATE DEFAULT INDEX ON drop_cluster2_table;
                > CREATE MATERIALIZED VIEW drop_cluster2_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM drop_cluster2_table;
