    ...
    def initialize(self) -> Testdrive:
        return Testdrive(
            dedent(
                ...
                > CREATE MATERIALIZED VIEW debezium_view1 WITH (REFRESH EVERY '2 seconds') AS SELECT f1, f3, SUM(LENGTH(f4)) FROM debezium_source1 GROUP BY f1, f3;
    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                ...
                > CREATE MATERIALIZED VIEW debezium_view2 WITH (REFRESH EVERY '2 seconds') AS SELECT f1, f3, SUM(LENGTH(f4)) FROM debezium_source2 GROUP BY f1, f3;
                """,
                """
                $ postgres-execute connection=postgres://postgres:postgres@postgres
                BEGIN;
                ...
                > CREATE MATERIALIZED VIEW debezium_view3 WITH (REFRESH EVERY '2 seconds') AS SELECT f1, f3, SUM(LENGTH(f4)) FROM debezium_source3 GROUP BY f1, f3;
                """,
            ]
        ]
