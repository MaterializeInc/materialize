    ...
    def validate(self) -> Testdrive:
        return Testdrive(
            dedent(
                ...
                > CREATE MATERIALIZED VIEW create_managed_cluster1_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM create_managed_cluster1_table;
                ...
                > CREATE MATERIALIZED VIEW create_managed_cluster2_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM create_managed_cluster2_table;
    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                ...
                > CREATE MATERIALIZED VIEW drop_managed_cluster1_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM drop_managed_cluster1_table;

                > SET cluster=drop_managed_cluster2
                > CREATE DEFAULT INDEX ON drop_managed_cluster2_table;
                > CREATE MATERIALIZED VIEW drop_managed_cluster2_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM drop_managed_cluster2_table;
