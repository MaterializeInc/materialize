...
class NestedTypes(Check):
    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                """
                > CREATE MATERIALIZED VIEW nested_types_view1 WITH (REFRESH EVERY '2 seconds') AS SELECT
                ...
                """,
                """
                > CREATE MATERIALIZED VIEW nested_types_view2 WITH (REFRESH EVERY '2 seconds') AS SELECT
                  map_col, '{a => 1, b => 2}'::map_type,
                  list_col, '{{1,2},{3,4}}'::int4_list_list,
                  record_col, ROW(1, 'abc'),
                  array_col, ARRAY[ARRAY['a', 'b'], ARRAY['c', 'd']]
