...
class SaslAuth(Check):

    def _can_run(self, e: Executor) -> bool:
        return self.base_version >= MzVersion.parse_mz("v0.158.0-dev")
    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                ...
                > GRANT SELECT ON TABLE t1 TO user2;

                # Validate that the user can select from the table
                $ postgres-execute connection=postgres://user2:password@${testdrive.materialize-sasl-sql-addr}
                SELECT * FROM materialize.schema2.t1
                """,
                """
                > ALTER ROLE user2 PASSWORD 'password2';
                """,
            ]
        ]
