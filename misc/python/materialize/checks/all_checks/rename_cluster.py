    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                ...
                > CREATE MATERIALIZED VIEW rename_cluster1_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM rename_cluster1_table;

                > SET cluster=rename_cluster2
                > CREATE DEFAULT INDEX ON rename_cluster2_table;
                > CREATE MATERIALIZED VIEW rename_cluster2_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM rename_cluster2_table;
