    ...
    def initialize(self) -> Testdrive:
        return Testdrive(
            dedent(
                ...
                > CREATE MATERIALIZED VIEW rename_index_view1 WITH (REFRESH EVERY '2 seconds') AS SELECT f2 FROM rename_index_table WHERE f2 > 0;
                """
            )
        )
    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                ...
                > CREATE INDEX rename_index_index1 ON rename_index_table (f2);
                > CREATE MATERIALIZED VIEW rename_index_view2 WITH (REFRESH EVERY '2 seconds') AS SELECT f2 FROM rename_index_table WHERE f2 > 0;
                ...
                > CREATE MATERIALIZED VIEW rename_index_view3 WITH (REFRESH EVERY '2 seconds') AS SELECT f2 FROM rename_index_table WHERE f2 > 0;
