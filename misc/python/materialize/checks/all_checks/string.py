...
class String(Check):
    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                """
                > CREATE MATERIALIZED VIEW string_view1 WITH (REFRESH EVERY '2 seconds') AS SELECT
            ...
            """,
                """
                > CREATE MATERIALIZED VIEW string_view2 WITH (REFRESH EVERY '2 seconds') AS SELECT
                    f1 BETWEEN f2 AND f3 AS c1,
                    'foo' BETWEEN 'abc' AND 'xyz' AS c2,
                    f1 NOT BETWEEN f2 AND f3 AS c3,
                    substring(f1 FROM f4 FOR f5) AS c4,
