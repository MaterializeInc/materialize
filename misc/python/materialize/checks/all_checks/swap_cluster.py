    ...
    def initialize(self) -> Testdrive:
        return Testdrive(
            dedent(
                ...
                > CREATE MATERIALIZED VIEW swap_cluster1_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM swap_cluster1_table;

                > SET cluster=swap_cluster2
                > CREATE DEFAULT INDEX ON swap_cluster2_table;
                > CREATE MATERIALIZED VIEW swap_cluster2_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM swap_cluster2_table;

                > SET cluster=swap_cluster3
                > CREATE DEFAULT INDEX ON swap_cluster3_table;
                > CREATE MATERIALIZED VIEW swap_cluster3_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM swap_cluster3_table;

                > SET cluster=swap_cluster4
                > CREATE DEFAULT INDEX ON swap_cluster4_table;
                > CREATE MATERIALIZED VIEW swap_cluster4_view WITH (RETAIN HISTORY FOR '30s') AS SELECT SUM(f1) FROM swap_cluster4_table;
                """
            )
        )
