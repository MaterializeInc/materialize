    ...
    def initialize(self) -> Testdrive:
        return Testdrive(
            ...
            + dedent(
                ...
                > CREATE MATERIALIZED VIEW upsert_insert_view WITH (REFRESH EVERY '2 seconds') AS SELECT COUNT(DISTINCT key1 || ' ' || f1) FROM upsert_insert;
                """
            )
        )
    ...
    def initialize(self) -> Testdrive:
        return Testdrive(
            ...
            + dedent(
                ...
                > CREATE MATERIALIZED VIEW upsert_update_view WITH (REFRESH EVERY '2 seconds') AS SELECT LEFT(f1, 1), COUNT(*) AS c1, COUNT(DISTINCT key1) AS c2, COUNT(DISTINCT f1) AS c3 FROM upsert_update GROUP BY LEFT(f1, 1);
                """
            )
        )
    ...
    def initialize(self) -> Testdrive:
        return Testdrive(
            ...
            + dedent(
                ...
                > CREATE MATERIALIZED VIEW upsert_delete_view WITH (REFRESH EVERY '2 seconds') AS SELECT COUNT(*), MIN(key1), MAX(key1) FROM upsert_delete;
                """
            )
        )
