...
class WindowFunctions(Check):
    ...
    def manipulate(self) -> list[Testdrive]:
        return [
            ...
            for s in [
                """
                > CREATE MATERIALIZED VIEW window_functions_view1 WITH (REFRESH EVERY '2 seconds') AS
                  SELECT
                ...
                """,
                """
                > CREATE MATERIALIZED VIEW window_functions_view2 WITH (REFRESH EVERY '2 seconds') AS
                  SELECT
                  row_number() OVER (PARTITION BY f1 ORDER BY f2),
                  dense_rank() OVER (PARTITION BY f2 ORDER BY f1 DESC),
                  lag(f1, f2, f1) OVER (ORDER BY f2),
