...
WARN_RUST_LINTS = [
    # No-op numeric casts (e.g., `u32 as u32`) obscure where real
    # conversions happen.
    "trivial_numeric_casts",
    # Dead lifetime parameters can mask refactor mistakes.
    "unused_lifetimes",
    # Dead macro rules indicate stale code.
    "unused_macro_rules",
    # Catches `use foo::{bar}` that should be `use foo::bar`.
...
WARN_CLIPPY_LINTS = [
    ...
    # Catches copy-paste bugs where both branches of an if/else are identical.
    "if_same_then_else",
    # `.ok()` silently discards errors; this catches cases where the Result
    # itself is also unused, which is almost certainly a bug.
    "unused_result_ok",
    # Populated collections that are never read are dead code or a missed bug.
    "collection_is_never_read",
    # Match guards that duplicate the pattern are redundant and can be inlined.
    "redundant_guards",
    # Warns about confusing operator precedence in bitwise expressions
    # (e.g., `a & b == 0` is `a & (b == 0)`, not `(a & b) == 0`).
    "precedence_bits",
    # Holding a value with a significant Drop (e.g., MutexGuard) in a match
    # scrutinee keeps it alive for the entire match, which can cause deadlocks.
