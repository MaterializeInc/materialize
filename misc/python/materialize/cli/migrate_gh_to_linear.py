...
LINEAR_MAX_BODY = 100_000  # Linear rejects longer descriptions and comment bodies.
...
def clamp_body(body: str, url: str, kind: str) -> str:
    """Trim body to Linear's maximum length, linking to GitHub for the rest.

    Linear rejects descriptions and comment bodies longer than LINEAR_MAX_BODY
    characters. The full text stays on the linked GitHub issue, so trimming
    here loses nothing.
    """
    if len(body) <= LINEAR_MAX_BODY:
        return body
    note = f"\n\n---\n\n*{kind} truncated, see {url}*"
    return body[: LINEAR_MAX_BODY - len(note)] + note


def split_long_block(block: str, limit: int) -> list[str]:
    """Split a comment block into parts no longer than limit characters.

    Splits only on line boundaries so no line is cut in half. When a split
    falls inside a fenced code block, the fence is closed on the part that ends
    and reopened on the part that continues, so each part renders on its own.
    """
    if len(block) <= limit:
        return [block]
    fence = "```"
    # Reserve room to close and reopen a fence around a split.
    budget = limit - 2 * (len(fence) + 1)
    parts: list[str] = []
    lines: list[str] = []
    length = 0
    in_fence = False
    for line in block.split("\n"):
        if lines and length + len(line) + 1 > budget:
            ended = "\n".join(lines)
            if in_fence:
                ended += "\n" + fence
            parts.append(ended)
            lines = [fence] if in_fence else []
...
def pack_comment_bodies(header: str, blocks: list[str]) -> list[str]:
    """Pack per-comment blocks into as few Linear comment bodies as possible.

    Each block is one migrated GitHub comment. Blocks are concatenated under a
    repeated header until the next part would exceed LINEAR_MAX_BODY, then a new
    body starts. A block longer than the limit is split by split_long_block, so
    no comment text is lost.
    """
    sep = "\n\n---\n\n"
    limit = LINEAR_MAX_BODY - len(header) - len(sep)
    bodies: list[str] = []
    current = header
    for block in blocks:
        for part in split_long_block(block, limit):
            if len(current) + len(sep) + len(part) > LINEAR_MAX_BODY:
...
def migrate_issue(api_keys: dict[str, str], gh: dict[str, Any], dry_run: bool) -> None:
    ...
    desc = clamp_body(normalize_code_blocks(gh["body"]), gh["url"], "Description")
    inp: dict[str, Any] = {
        "teamId": team["id"],
        "title": gh["title"],
        "description": desc,
    ...
    if comments:
        header = f"**Migrated comments from GitHub#{gh['number']}:**"
        blocks = [
            f"**{c['author']}** ({c['date']}):\n\n{normalize_code_blocks(c['body'])}"
            for c in comments
        ]
        for body in pack_comment_bodies(header, blocks):
