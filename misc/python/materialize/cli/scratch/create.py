...
def configure_parser(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--subnet-id", type=str, default=DEFAULT_SUBNET_ID, help="EC2 Subnet ID. Defaults to Materialize scratch account.")
    parser.add_argument("--key-name", type=str, required=False, help="Optional EC2 Key Pair name")
    parser.add_argument(
        "--security-group-id", type=str, default=DEFAULT_SECURITY_GROUP_ID, help="EC2 Security Group ID. Defaults to Materialize scratch account."
    )
    parser.add_argument("--extra-tags", type=str, required=False, help="Additional EC2 tags for created instance. Format: {\"key\", \"value\"}")
    parser.add_argument(
        "--instance-profile", type=str, default=DEFAULT_INSTANCE_PROFILE_NAME, help="EC2 instance profile / IAM role. Defaults to `%s`." % DEFAULT_INSTANCE_PROFILE_NAME
    )
    parser.add_argument("--output-format", choices=["table", "csv"], default="table")
    parser.add_argument("--git-rev", type=str, default="HEAD", help="Git revision of `materialize` codebase to push to scratch instance. Defaults to `HEAD`")
    parser.add_argument(
        "--ssh",
        action="store_true",
        help=(
    ...
    parser.add_argument("--max-age-days", type=float, default=MAX_AGE_DAYS, help="Maximum age for scratch instance in days. Defaults to 1.5")
