...
def configure_parser(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "machine",
        help=(
            "Machine config name from misc/scratch (e.g. hetzner-dedi-48). "
...
def run(args: argparse.Namespace) -> None:
    # Read the machine config first to determine the provider
    ...
    try:
        if provider == "hetzner":
            from materialize.scratch_hetzner import get_instance
        else:
            from materialize.scratch import get_instance

        instance = get_instance("mine")
        say(f"Found existing instance {instance.instance_id} ({instance.name})")
