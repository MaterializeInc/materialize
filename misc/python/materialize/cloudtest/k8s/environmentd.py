...
class EnvironmentdStatefulSet(K8sStatefulSet):
    ...
    def args(self) -> list[str]:
        ...
        args = [
            ...
            f"--persist-consensus-url=postgres://postgres:postgres@postgres.{self.postgres_namespace}:5432?options=--search_path=consensus",
        ...
        if self._meets_minimum_version("0.79.0-dev"):
            args += [
                f"--timestamp-oracle-url=postgres://postgres:postgres@postgres.{self.postgres_namespace}:5432?options=--search_path=tsoracle"
            ]
        if not self._meets_minimum_version("0.105.0-dev"):
            args += [
                f"--storage-stash-url=postgres://postgres:postgres@postgres.{self.postgres_namespace}:5432?options=--search_path=storage"
            ]
        ...
        env = [
            ...
            V1EnvVar(
                name="MZ_ADAPTER_STASH_URL",
                 value=f"postgres://postgres:postgres@postgres.{self.postgres_namespace}:5432?options=--search_path=adapter",
            ),
        ]
