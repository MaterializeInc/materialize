...
class EnvironmentdStatefulSet(K8sStatefulSet):
    ...
    def args(self) -> list[str]:
        ...
        args = [
            ...
            f"--persist-consensus-url=postgres://materialize:materialize@postgres.{self.postgres_namespace}:5432?options=--search_path=consensus",
        ...
        if self._meets_minimum_version("0.79.0-dev"):
            args += [
                f"--timestamp-oracle-url=postgres://materialize:materialize@postgres.{self.postgres_namespace}:5432?options=--search_path=tsoracle"
            ]
        if not self._meets_minimum_version("0.105.0-dev"):
            args += [
                f"--storage-stash-url=postgres://materialize:materialize@postgres.{self.postgres_namespace}:5432?options=--search_path=storage"
            ]
    ...
    def env_vars(self) -> list[V1EnvVar]:
        ...
        env = [
            ...
            V1EnvVar(
                ...
                value=f"postgres://materialize:materialize@postgres.{self.postgres_namespace}:5432?options=--search_path=adapter",
            ),
        ]
