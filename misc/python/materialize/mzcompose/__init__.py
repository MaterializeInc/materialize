...
class UnknownCompositionError(UIError):
    """The specified composition is unknown."""

    def __init__(self, name: str):
        super().__init__(f"unknown composition {name!r}")


class UnknownWorkflowError(UIError):
    """The specified workflow is unknown."""
...
class LintError:
    """A lint rule violation returned by `Composition.lint`."""
...
def _lint_image_name(path: Path, spec: str, errors: List[LintError]) -> None:
    """Lint an image name in a `Service`."""
...
def _lint_materialized_service(
    ...
    """Lint the definition for the `materialized` service."""
...
class Composition:
    """A composition represents a set of Docker Compose service definitions
    and workflow functions that interact with those services.

    A composition is loaded from the mzcompose.yml and mzcompose.py files in a
    directory. For details, see the [user docs].
    ...
    def __init__(
        ...
        if mzcompose_py.exists():
            spec = importlib.util.spec_from_file_location("mzworkflows", mzcompose_py)
            assert spec
            ...
            for name, fn in getmembers(module, isfunction):
                if name.startswith("workflow_"):
                    ...
                    fn.workflow_name = name
                    self.workflows[name] = fn

            for service in getattr(module, "SERVICES"):
                compose["services"][service.name] = service.config
        ...
        for name, config in compose["services"].items():
            if "mzbuild" in config:
                ...
                self.images.append(self.repo.images[image_name])
    ...
    def lint(cls, repo: mzbuild.Repository, name: str) -> List[LintError]:
        """Check a composition for known errors."""
    ...
    def down(self, volumes: bool = True) -> None:
        """Stop and remove containers, networks, images, and optionally volumes.
        ...
        """
        self.invoke("down", *(["--volumes"] if volumes else []))

    def start(self, *services: str) -> None:
        """Start existing containers.

        Delegates to `docker-compose start`. See that command's help for
        details.

        Args:
            services: The names of services in the composition.
        """
        self.invoke("start", *services)

    def rm(self, *services: str, stop: bool = True) -> None:
        ...
        """
        self.invoke("rm", "--force", *(["--stop"] if stop else []), *services)
    ...
    def sleep(self, duration: float) -> None:
        """Sleep for the specified druation in seconds."""
    ...
    def workflow(self, name: str, args: List[str] = []) -> None:
        ...
        if len(inspect.signature(func).parameters) > 1:
            ...
            # If the workflow doesn't have an `args` parameter, parse with an
            # empty parser to reject bogus arguments and to handle the trivial
            # help message.
            parser.parse_args()
            func(self)
    ...
    def sql(self, sql: str) -> None:
        ...
        Args:
            sql: The SQL string to run. May contain multiple statements
                separated by semicolons.
        """
        port = self.port("materialized", 6875)
    ...
    def override(self, *services: "Service") -> Iterator[None]:
        ...
        The services must already exist in the composition. They are restored to
        ...
        Args:
            services: The service definitions to temporarily install.
        """
        ...
        for service in services:
            ...
            if "mzbuild" in service.config:
                image_name = service.config["mzbuild"]
                image = self.repo.images[image_name]
                deps = self.repo.resolve_dependencies([image])
                service.config["image"] = deps[image_name].spec()
                del service.config["mzbuild"]
    ...
    def invoke(self, *args: str, capture: bool = False) -> subprocess.CompletedProcess:
        """Invoke docker-compose on the composition.
...
class ServiceDependsOn(TypedDict):
    """A dependency in a `ServiceConfig`."""

    condition: str
    """The condition to wait for.

    Must be one of `service_started`, `service_healthy`, or
    `service_completed_successfully`.
    """


class ServiceHealthCheck(TypedDict, total=False):
    """A health check in a `ServiceConfig`."""

    test: str
    """A shell command to run to evaluate healthiness."""

    interval: str
    """The interval at which to check the service's health."""

    timeout: str
    """How long each health check test has to succeed."""

    retries: int
    """How many health check tests must fail consecutively before the service is
    marked unhealthy."""

    start_period: str
    """A grace period immediately after startup during which the service cannot
...
class ServiceConfig(TypedDict, total=False):
    ...
    depends_on: Union[List[str], Dict[str, ServiceDependsOn]]
    """The list of other services that must be started before this one."""

    healthcheck: ServiceHealthCheck
    """How to evaluate whether this service is healthy."""
...
class Service:
    ...
    def __init__(self, name: str, config: ServiceConfig) -> None:
        ...
        if isinstance(self.config.setdefault("depends_on", {}), list):
            depends_on = {}
            for name in self.config["depends_on"]:
                depends_on[name] = ServiceDependsOn(condition="service_healthy")
            self.config["depends_on"] = depends_on
