...
# S3 policy allowing all actions on all buckets
ALLOW_ALL_ACL = {
    "Version": "2012-10-17",
    "Statement": [
        {
...
def make_random_key(n: int) -> str:
    """Generate a random alphanumeric key of length n."""
    return "".join(
...
def create_minio_user(name: str, minio_alias: str, c: "Composition") -> str:
    """
    Create a MinIO user with S3 permissions and return the secret key.

    Args:
        name: Username to create
        minio_alias: The mc alias for the MinIO server
        c: The mzcompose Composition

    Returns:
        The generated secret key for the user
    """
...
def setup_minio_alias(
    ...
    """Configure the mc client with a MinIO alias."""
...
def get_polaris_access_token(c: "Composition") -> str:
    """
    Get an access token from Polaris OAuth endpoint.
...
def create_polaris_catalog(
    ...
    """
    Create a Polaris catalog with S3 storage configuration.

    Args:
        c: The mzcompose Composition
        access_token: Polaris OAuth access token
...
def create_polaris_namespace(
    ...
    """
    Create a namespace in a Polaris catalog.

    Args:
        c: The mzcompose Composition
        access_token: Polaris OAuth access token
        namespace: Name for the namespace
        catalog_name: Name of the catalog to create the namespace in
    """
...
def setup_polaris_for_iceberg(
    ...
    Args:
        ...
        minio_alias: Alias for the MinIO server in mc
        username: Username for the MinIO user
        catalog_name: Name for the Polaris catalog
        namespace: Name for the namespace in the catalog

    Returns:
        Tuple of (username, secret_key) for the created MinIO user
    """
