...
"""Pre-defined services for mzcompose."""
...
class Materialized(Service):
    def __init__(
        ...
        healthcheck = "NO_PROXY=* curl -f localhost:6875"
        if image:
            [_, tag] = image.split(":", maxsplit=2)
            if tag.startswith("v") and tag[1:] <= Version.parse("0.8.0"):
                # materialize v0.8.0 and earlier had wget rather than curl.
                healthcheck = "NO_PROXY=* wget --tries=1 localhost:6875"
...
class Squid(Service):
    ...
    def __init__(
        self,
        name: str = "squid",
        image: str = "ubuntu/squid:4.10-20.04_beta",
        port: int = 3128,
        ...
        super().__init__(
            ...
            config={
                ...
                "ports": [port],
                "volumes": volumes,
                # TODO(benesch): squid starts fast enough that this is fine in
                # practice, but ideally we'd use a container with `curl` or `nc`
                # so we could write a proper health check.
                "healthcheck": {
                    "test": "true",
                    "interval": "1s",
                },
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config={
                ...
                "volumes": volumes,
                "healthcheck": {
                    "test": "curl -f localhost:4566/health",
                    "interval": "1s",
                    "start_period": "30s",
                },
            },
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config={
                ...
                # We keep this service alive and trivially healthy forever to
                # make it easier to depend upon.
