...
class Clusterd(Service):
    def __init__(
        ...
        compute_timely_config = timely_config(process_names, 2102, workers)
        storage_timely_config = timely_config(process_names, 2103, workers)
...
def timely_config(process_names: list[str], port: int, workers: int) -> str:
    config = {
        "workers": workers,
        "process": 0,
