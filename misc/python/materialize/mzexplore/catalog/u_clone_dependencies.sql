...
-- 2. All source_ids representing views that transitively depend on source_ids.
...
WITH
  ...
  direct_view_dependencies(source_id, target_id) AS (
    ...
    FROM
      mz_internal.mz_object_dependencies od JOIN
      mz_catalog.mz_objects o ON(od.object_id = o.id)
    WHERE
      od.referenced_object_id ilike 'u%' AND o.type = 'view'
  ),
  ...
  dependency_indexes(id) AS (
    ...
    WHERE
      id <= (SELECT max(id) FROM unnest(array[{source_ids}]) AS src(id)) AND -- (1)
