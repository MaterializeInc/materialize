...
OCC_COUNTER_NAME = "materialize.public.pw_occ_counter"
...
# included. The frontend path only reports those for a write it knows did not
# land, but the same statement falls back to coordinator sequencing when
# `enable_adapter_frontend_occ_read_then_write` is off, and there a timeout or
# cancel can race a completed commit. A wrong entry here makes healthy runs
...
class OccCounter:
    """A single-row counter table that `OccCounterUpdateAction` increments with
    read-then-write UPDATEs, plus the tallies of the outcomes its workers saw.
    ...
    applied twice by an OCC retry, a success reported for a write that never
    landed, or an error reported for a write that did land. The lower bound is
    ...
    def validate(self, exe: Executor) -> None:
        ...
        print(f"OCC counter tallies: {tallies}")
        ...
        if len(rows) != 1 or rows[0][0] != 1:
            message = (
                f"OCC counter table should hold exactly the seeded row (1, v), "
        ...
        if not lower <= value <= upper:
            message = (
                f"OCC counter conservation invariant violated: observed v={value}, "
                f"expected {lower} <= v <= {upper} ({tallies})"
            )
            print(f"+++ {message}")
            raise ValueError(message)
        print(
            f"OCC counter conservation invariant holds: {lower} <= v={value} <= {upper}"
        )
