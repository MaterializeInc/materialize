...
def run(
    ...
    with system_conn.cursor() as system_cur:
        ...
        system_exe.execute(
            f"ALTER SYSTEM SET max_schemas_per_database = {MAX_SCHEMAS * 2 + 100}"
        )
        # The presence of ALTER TABLE RENAME can cause the total number of tables to exceed MAX_TABLES
        system_exe.execute(f"ALTER SYSTEM SET max_tables = {MAX_TABLES * 2 + 100}")
        system_exe.execute(f"ALTER SYSTEM SET max_materialized_views = {MAX_VIEWS * 2 + 100}")
        system_exe.execute(
            f"ALTER SYSTEM SET max_sources = {(MAX_WEBHOOK_SOURCES + MAX_KAFKA_SOURCES + MAX_POSTGRES_SOURCES) * 2}"
        )
        system_exe.execute(f"ALTER SYSTEM SET max_sinks = {MAX_KAFKA_SINKS * 2 + 100}")
        system_exe.execute(f"ALTER SYSTEM SET max_roles = {MAX_ROLES * 2 + 100}")
        system_exe.execute(f"ALTER SYSTEM SET max_clusters = {MAX_CLUSTERS * 2 + 100}")
        system_exe.execute(
            f"ALTER SYSTEM SET max_replicas_per_cluster = {MAX_CLUSTER_REPLICAS * 2 + 100}"
        )
