...
----------------+-------------------+-------------------+-----------+----------------------------
 prod_analytics | scale=1,workers=1 | scale=1,workers=2 | t         | 2026-06-11 01:44:58.189+00
(1 row)
```
...
against both indexes answer in single-digit milliseconds, served by the old
replica:
...
pending target is surfaced separately. About 50 seconds later the new replica
...
are recorded in `mz_audit_events`:
...
------------+-------------------+----------------------------
 started    | scale=1,workers=2 | 2026-06-10 01:44:58.189+00
 finalized  | scale=1,workers=2 | 2026-06-10 01:45:45.283+00
(2 rows)
...
shows one continuous operation spanning the crash (`started` at 01:46:11,
kill at ~01:46:20, `finalized` at 01:47:24):

```
 transition |    target_size    |        occurred_at
------------+-------------------+----------------------------
 finalized  | scale=1,workers=1 | 2026-06-10 01:47:24.582+00
 started    | scale=1,workers=1 | 2026-06-10 01:46:11.639+00
(2 rows)
```
...
level, and the replica create with reason `hydration-burst`:

```
 transition |    burst_size     |        occurred_at
------------+-------------------+----------------------------
 started    | scale=1,workers=8 | 2026-06-10 01:47:25.741+00
 finished   | scale=1,workers=8 | 2026-06-10 01:47:42.801+00
 started    | scale=1,workers=8 | 2026-06-10 01:47:52.838+00
 finished   | scale=1,workers=8 | 2026-06-10 01:49:09.988+00
(4 rows)
...
| `mz_catalog.mz_audit_events` | History. Reconfiguration transitions: `started` / `finalized` / `timed-out` / `cancelled`. Burst transitions: `started` / `finished`. Replica creates carry the responsible strategy as their reason (`reconfiguration`, `hydration-burst`). |
...
8. **Audit log shape changes.** Replica creates gain new reasons
   (`reconfiguration`, `hydration-burst`); new cluster-level transition
   events exist (see the observability table); scheduled-cluster on/off
   events no longer carry the legacy `scheduling_policies` detail blob.
...
- **Controller-initiated replica *drops* are attributed `manual` in the audit
  log** (visible in the transcripts above), because a drop happens exactly
  when no strategy wants the replica anymore. Creates carry the right reason;
  drops reading "manual" is misleading next to them.
...
| `default_cluster_reconfiguration_timeout` | 1 day | Deadline written when an `ALTER` gives no explicit timeout — no transition is ever unbounded. |
...
Local environment: `bin/environmentd --optimized --reset -- --unsafe-mode`
(`--unsafe-mode` only for `mz_unsafe.mz_sleep` used as a wait between
captures). Flags, as `mz_system`:
...
index: an md5 over ~10 kB per row, 2M rows, ~50s to hydrate on one worker.
The data is loaded in eight separate inserts so the work spreads across the
workers of a larger replica (which is what makes the 8-worker burst replica
in Scenario 2 hydrate ~8x faster):
