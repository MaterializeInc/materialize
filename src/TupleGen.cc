...
void TupleGen::openOutputFiles() {
    ofopen("warehouse", warehouseStream,
           Config::getInitialDbCreationPath() + "/WAREHOUSE.tbl");
    ofopen("district", districtStream,
           Config::getInitialDbCreationPath() + "/DISTRICT.tbl");
    ofopen("customer", customerStream,
           Config::getInitialDbCreationPath() + "/CUSTOMER.tbl");
    ofopen("history", historyStream,
           Config::getInitialDbCreationPath() + "/HISTORY.tbl");
    ofopen("neworder", neworderStream,
           Config::getInitialDbCreationPath() + "/NEWORDER.tbl");
    ofopen("order", orderStream,
           Config::getInitialDbCreationPath() + "/ORDER.tbl");
    ofopen("orderline", orderlineStream,
           Config::getInitialDbCreationPath() + "/ORDERLINE.tbl");
    ofopen("item", itemStream,
           Config::getInitialDbCreationPath() + "/ITEM.tbl");
    ofopen("stock", stockStream,
           Config::getInitialDbCreationPath() + "/STOCK.tbl");
    ofopen("nation", nationStream,
           Config::getInitialDbCreationPath() + "/NATION.tbl");
    ofopen("supplier", supplierStream,
           Config::getInitialDbCreationPath() + "/SUPPLIER.tbl");
    ofopen("region", regionStream,
           Config::getInitialDbCreationPath() + "/REGION.tbl");
}
...
void TupleGen::genCustomer(int& cId, int& dId, int& wId, string& customerTime) {
    ...
    customerStream
        << customerTime
        << Config::getCsvDelim(); // C_SINCE - date/time given by the os when
                                  // the CUSTOMER table was populated
    customerStream << (DataSource::randomTrue(0.1) ? "BC" : "GC")
                   << Config::getCsvDelim();               // C_CREDIT
...
void TupleGen::genHistory(int& cId, int& dId, int& wId) {
    ...
    historyStream << DataSource::getCurrentTimeString()
                  << Config::getCsvDelim(); // H_DATE - current date and time
...
void TupleGen::genOrder(int& oId, int& dId, int& wId, int& cId, int& olCount,
    ...
    orderStream << orderTime
                << Config::getCsvDelim(); // O_ENTRY_D - current date/ time
                                          // given by the os
...
void TupleGen::genOrderline(int& oId, int& dId, int& wId, int& olNumber,
    ...
    orderlineStream << (oId <= 2100 ? orderTime : "")
                    << Config::getCsvDelim(); // OL_DELIVERY_D = O_ENTRY_D if
