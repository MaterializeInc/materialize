...
impl CatalogState {
    ...
    > {
        ...
        for state_update in updates {
            ...
            // to this process must be a complete no-op: no in-memory entry, no
            // builtin table rows, no parsed updates for the controllers. This
            ...
            if self.is_nonlocal_temporary_item_update(&state_update) {
                tracing::debug!(
                    ?state_update,
    ...
    fn is_nonlocal_temporary_item_update(&self, update: &StateUpdate) -> bool {
        ...
        match update.diff {
            ...
            // bootstrap snapshot, which is consolidated and so contains no
            // retractions. On the writer, keying off the entry rather than
            // the ephemeral-owner mapping keeps this gate correct independent of
            // session teardown ordering, since wrongly skipping a retraction
