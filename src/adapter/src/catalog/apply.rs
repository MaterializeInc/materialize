...
use crate::coord::controller_commands::controller_state_updates::{self, ParsedStateUpdate};
        ...
        for StateUpdate { kind, ts, diff } in updates {
            ...
            match diff {
                StateDiff::Retraction => {
                    ...
                    // state of the catalog _before_ applying the update.
                    if let Some(update) =
                        controller_state_updates::parse_state_update(self, kind.clone(), ts, diff)
                    {
                ...
                StateDiff::Addition => {
                    ...
                    // We want the parsed controller state updates to match the
                    // state of the catalog _after_ applying the update.
                    if let Some(update) =
                        controller_state_updates::parse_state_update(self, kind.clone(), ts, diff)
                    {
                        controller_state_updates.push(update);
