...
impl ApplyTimings {
    ...
    fn merge(&mut self, other: Self) {
        self.role_duration += other.role_duration;
        self.database_duration += other.database_duration;
        self.schema_duration += other.schema_duration;
        self.default_privilege_duration += other.default_privilege_duration;
        self.system_privilege_duration += other.system_privilege_duration;
        self.system_configuration_duration += other.system_configuration_duration;
        self.cluster_duration += other.cluster_duration;
        self.introspection_source_index_duration += other.introspection_source_index_duration;
        self.cluster_replica_duration += other.cluster_replica_duration;
        self.source_references_duration += other.source_references_duration;
        self.system_object_mapping_duration += other.system_object_mapping_duration;
        self.temporary_item_duration += other.temporary_item_duration;
        self.item_duration.merge(other.item_duration);
        self.comment_duration += other.comment_duration;
        self.audit_log_duration += other.audit_log_duration;
        self.storage_collection_metadata_duration += other.storage_collection_metadata_duration;
        self.unfinalized_shard_duration += other.unfinalized_shard_duration;
        self.generate_builtin_table_updates += other.generate_builtin_table_updates;
    }
}
...
impl CatalogState {
    ...
    pub(crate) async fn apply_updates_for_bootstrap(
        ...
        for updates in groups {
            ...
            let (builtin_table_update, inner_apply_timings) =
        ...
        for StateUpdate { kind, ts: _, diff } in updates {
            match diff {
                StateDiff::Retraction => {
                    ...
                    let inner_apply_timings = self.apply_update(kind, diff, retractions)?;
                    apply_timings.merge(inner_apply_timings);
                }
                StateDiff::Addition => {
                    let inner_apply_timings = self.apply_update(kind.clone(), diff, retractions)?;
                    apply_timings.merge(inner_apply_timings);
    ...
    fn apply_update(
        ...
        match kind {
            ...
            StateUpdateKind::Item(item) => {
                let item_timings = self.apply_item_update(item, diff, retractions)?;
                apply_timings.item_duration.merge(item_timings);
            }
        ...
        match diff {
            StateDiff::Addition => {
                ...
                let entry = match retractions.items.remove(&key) {
                    ...
                    None => {
                        let (catalog_item, inner_item_timings) =
                            self.deserialize_item(id, &create_sql).unwrap_or_else(|e| {
                                panic!("{e:?}: invalid persisted SQL: {create_sql}")
                            });
                        item_timings.merge(inner_item_timings);
    ...
    async fn apply(
        ...
        match self {
            BootstrapApplyState::BuiltinViewAdditions(builtin_view_additions) => {
                ...
                let (builtin_table_updates, mut item_timings) =
    ...
    async fn step(
        ...
        match (self, next) {
            ...
            (apply_state, next_apply_state) => {
                ...
                let (builtin_table_update, inner_apply_timings) =
                    apply_state.apply(state, retractions).await;
                apply_timings.merge(inner_apply_timings);
