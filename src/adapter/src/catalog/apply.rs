...
impl CatalogState {
    ...
    fn apply_item_update(
        ...
        // Temporary items whose owning session is not served by this process
        // are skipped entirely, additions and retractions alike. This happens
        // when following the catalog read-only during a zero-downtime
        // deployment: the leader's temporary items arrive here, but their
        // owning sessions' connections live in the leader, so the items are
        // neither resolvable nor visible in this process.
        if let Some(owner) = &item.ephemeral_owner_session {
            if !self.session_conns_by_uuid.contains_key(owner) {
                return Ok(());
            }
        }
        match diff {
            StateDiff::Addition => {
                ...
                // owning session's mapping is registered at connect, before
                // the session can create any items, and checked above.
