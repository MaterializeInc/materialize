...
pub(crate) struct MigrateResult {
    ...
    pub(crate) side_effects: Vec<CatalogSideEffect>,
...
pub(crate) async fn migrate(
    ...
    let item_updates = item_updates
        ...
        .collect();
    let (mut ast_builtin_table_updates, mut ast_side_effects) = state
    ...
    let op_item_updates = tx.get_and_commit_op_updates();
    let (item_builtin_table_updates, item_side_effects) = state
