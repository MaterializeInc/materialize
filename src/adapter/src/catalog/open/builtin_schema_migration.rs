...
static MIGRATIONS: LazyLock<Vec<MigrationStep>> = LazyLock::new(|| {
    vec![
        ...
        // fingerprints, and mz_sessions gains a deploy_generation column, so
        // each needs an explicit replacement step. See the NOTE above: this
        // version must stay at the workspace's current dev version until the
        // change ships.
        MigrationStep::replacement(
            "26.35.0-dev.0",
            CatalogItemType::Table,
