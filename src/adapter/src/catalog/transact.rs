...
use mz_audit_log::{
    ...
    SchedulingDecisionsWithReasonsV1, VersionedEvent, VersionedStorageUsage,
};
use mz_catalog::builtin::BuiltinLog;
use mz_catalog::durable::Transaction;
use mz_catalog::memory::error::{AmbiguousRename, Error, ErrorKind};
use mz_catalog::memory::objects::{CatalogEntry, CatalogItem, ClusterConfig};
...
use mz_sql::catalog::{
    CatalogDatabase, CatalogError as SqlCatalogError, CatalogRole, CatalogSchema,
...
impl Catalog {
    ...
    async fn transact_inner(
        ...
        for op in ops {
            let (weird_builtin_table_update, temporary_item_actions) = Self::transact_op(
                oracle_write_ts,
                session,
                op,
                &temporary_ids,
            ...
            // work if we ever want multi-writer catalogs or high availability catalogs. If this
            // instance crashes after committing the durable catalog but before applying the weird
            // builtin table updates, then they'll be lost forever.
            ...
            for temporary_item_action in temporary_item_actions {
                match temporary_item_action {
                    TemporaryItemAction::Create {
                        id,
                        oid,
                        name,
                        item,
                        ...
                        state.insert_item(id, oid, name, item, owner_id, privilege_map);
                        let item_updates = state.pack_item_update(id, 1);
                        let item_updates = state.resolve_builtin_table_updates(item_updates);
                        builtin_table_updates.extend(item_updates);
                    }
                    TemporaryItemAction::Drop { id } => {
                        let item_updates = state.pack_item_update(id, -1);
                        let item_updates = state.resolve_builtin_table_updates(item_updates);
                        builtin_table_updates.extend(item_updates);
                        state.drop_item(id);
                    }
                    TemporaryItemAction::Update { id, entry } => {
                        let item_updates = state.pack_item_update(id, -1);
                        let item_updates = state.resolve_builtin_table_updates(item_updates);
                        builtin_table_updates.extend(item_updates);
                        state.drop_item(id);
    ...
    /// derived from the durable catalog state. These are all very weird tables and ideally these
    /// don't exist.
    #[instrument]
    async fn transact_op(
    ...
    ) -> Result<(Option<BuiltinTableUpdate>, Vec<TemporaryItemAction>), AdapterError> {
        let mut weird_builtin_table_update = None;
        let mut temporary_item_actions = Vec::new();
        ...
        match op {
            ...
            Op::CreateItem {
                ...
                if item.is_temporary() {
                    ...
                    temporary_item_actions.push(TemporaryItemAction::Create {
                        id,
                        oid,
                        name: name.clone(),
                        item: item.clone(),
                        owner_id,
                        privilege_map: PrivilegeMap::from_mz_acl_items(privileges),
                    });
                } else {
                    if let Some(temp_id) =
            ...
            Op::DropObjects(drop_object_infos) => {
                ...
                temporary_item_actions.extend(
                    temporary_items_to_drop
                        .into_iter()
                        .map(|id| TemporaryItemAction::Drop { id }),
                );
                ...
                match &target_id {
                    SystemObjectId::Object(object_id) => match object_id {
                        ...
                        ObjectId::Schema((database_spec, schema_spec)) => {
                            ...
                            tx.update_schema(schema_id, schema.into_durable_schema(database_id))?;
                        }
                        ObjectId::Item(id) => {
                            ...
                            if !entry.item().is_temporary() {
                                ...
                                temporary_item_actions
                                    .push(TemporaryItemAction::Update { id: *id, entry });
                            }
                        }
            ...
            Op::RenameItem {
                ...
                for id in entry.referenced_by() {
                    ...
                    if !new_entry.item().is_temporary() {
                        ...
                        temporary_item_actions.push(TemporaryItemAction::Update {
                            id: *id,
                            entry: to_entry,
                        });
                    }
            ...
            Op::UpdateOwner { id, new_owner } => {
                ...
                match &id {
                    ...
                    ObjectId::Schema((database_spec, schema_spec)) => {
                        ...
                        tx.update_schema(
                            schema_id,
                            schema.into_durable_schema(database_id.copied()),
                        )?;
                    }
                    ObjectId::Item(id) => {
                        let mut entry = state.get_entry(id).clone();
            ...
            Op::UpdateStorageUsage {
                ...
                let id = tx.get_and_increment_id(
                    mz_catalog::durable::STORAGE_USAGE_ID_ALLOC_KEY.to_string(),
                )?;
                let details =
        ...
        Ok((weird_builtin_table_update, temporary_item_actions))
    }
