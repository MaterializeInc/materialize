...
impl Coordinator {
    ...
    pub async fn controller_apply_catalog_updates(
        ...
        let mut controller_commands: BTreeMap<CatalogItemId, ObjectState> = BTreeMap::new();
        let mut cluster_commands: BTreeMap<ClusterId, ObjectState> = BTreeMap::new();
        let mut cluster_replica_commands: BTreeMap<(ClusterId, ReplicaId), ObjectState> =
            BTreeMap::new();
            ...
            match &update.kind {
                ParsedStateUpdateKind::Item {
                    ...
                    let entry = controller_commands
                        ...
                        .or_insert_with(|| ObjectState::None);
                    entry.absorb(update);
                }
    ...
    async fn controller_apply_commands(
        ...
        commands: Vec<(CatalogItemId, ObjectState)>,
        cluster_commands: Vec<(ClusterId, ObjectState)>,
        cluster_replica_commands: Vec<((ClusterId, ReplicaId), ObjectState)>,
    ) -> Result<(), AdapterError> {
        ...
        for (catalog_id, command) in commands {
            let controller_command = command.to_controller_command();

            tracing::trace!(?controller_command, "have to apply controller command");
        ...
        for (cluster_id, command) in cluster_commands {
            let controller_command = command.to_controller_command();
            tracing::trace!(?controller_command, "have cluster command to apply!");
        ...
        for ((cluster_id, replica_id), command) in cluster_replica_commands {
            ...
            tracing::trace!(
                ?controller_command,
                "have cluster replica command to apply!"
            );
...
/// A command that can be applied to the controller(s). These are derived from
/// the in-progress [ObjectState] variants.
#[derive(Debug, Clone)]
...
/// Represents the state machine for building controller commands from catalog
/// updates. Each variant represents a specific state in the lifecycle of an
/// object.
#[derive(Debug, Clone)]
enum ObjectState {
    None,
    Table(ObjectStateKind<Table>),
    Source(ObjectStateKind<(Source, Option<GenericSourceConnection>)>),
    Sink(ObjectStateKind<Sink>),
    Index(ObjectStateKind<Index>),
    MaterializedView(ObjectStateKind<MaterializedView>),
    View(ObjectStateKind<View>),
    ContinualTask(ObjectStateKind<ContinualTask>),
    Secret(ObjectStateKind<Secret>),
    Connection(ObjectStateKind<Connection>),
    Cluster(ObjectStateKind<Cluster>),
    ClusterReplica(ObjectStateKind<ClusterReplica>),
}

#[derive(Debug, Clone)]
enum ObjectStateKind<T> {
...
impl<T: Clone> ObjectStateKind<T> {
...
/// Macro to generate the conversion from [ObjectState] to [ControllerCommand].
macro_rules! impl_state_to_command {
    (
        $method_name:ident,
        $command_type:ident,
        $item_type:ty,
        add: $add_variant:ident($($add_field:ident)*),
        drop: $drop_variant:ident($($drop_field:ident)*),
        alter: $alter_variant:ident { prev: $prev_field:ident, new: $new_field:ident }
    ) => {
        fn $method_name(state: ObjectStateKind<$item_type>) -> $command_type {
            match state {
                ObjectStateKind::None => panic!("Cannot apply None state"),
                ObjectStateKind::Added(item) => $command_type::$add_variant(item $(.$add_field)*),
                ObjectStateKind::Dropped(item, name) => $command_type::$drop_variant(item $(.$drop_field)*, name),
                ObjectStateKind::Altered { prev, new } => $command_type::$alter_variant {
                    $prev_field: prev,
                    $new_field: new,
                },
            }
    ...
    // Variant for types that don't use the name in drop commands.
    (
        $method_name:ident,
        $command_type:ident,
        $item_type:ty,
        add: $add_variant:ident($($add_field:ident)*),
        drop_no_name: $drop_variant:ident($($drop_field:ident)*),
        alter: $alter_variant:ident { prev: $prev_field:ident, new: $new_field:ident }
    ) => {
        fn $method_name(state: ObjectStateKind<$item_type>) -> $command_type {
            match state {
                ObjectStateKind::None => panic!("Cannot execute None state"),
                ObjectStateKind::Added(item) => $command_type::$add_variant(item $(.$add_field)*),
                ObjectStateKind::Dropped(item, _name) => $command_type::$drop_variant(item $(.$drop_field)*),
                ObjectStateKind::Altered { prev, new } => $command_type::$alter_variant {
                    $prev_field: prev,
                    $new_field: new,
                },
    ...
    (
        ...
        fn $method_name(
            ...
            let state = match self {
                ObjectState::$variant(state) => state,
                ObjectState::None => {
                    *self = ObjectState::$variant(ObjectStateKind::None);
                    match self {
                        ObjectState::$variant(state) => state,
                        _ => unreachable!(),
                    }
...
impl ObjectState {
    /// Absorbs the given catalog update into this [ObjectState], causing a
    ...
    fn absorb_cluster(&mut self, cluster: Cluster, _ts: Timestamp, diff: StateDiff) {
        let state = match self {
            ObjectState::Cluster(state) => state,
            ObjectState::None => {
                *self = ObjectState::Cluster(ObjectStateKind::None);
                match self {
                    ObjectState::Cluster(state) => state,
                    _ => unreachable!(),
                }
    ...
    fn absorb_cluster_replica(
        ...
        let state = match self {
            ObjectState::ClusterReplica(state) => state,
            ObjectState::None => {
                *self = ObjectState::ClusterReplica(ObjectStateKind::None);
                match self {
                    ObjectState::ClusterReplica(state) => state,
                    _ => unreachable!(),
                }
    ...
    /// Convert the [ObjectState] to the final actionable [ControllerCommand].
    fn to_controller_command(self) -> ControllerCommand {
        match self {
            ObjectState::None => ControllerCommand::None,
            // ObjectState::Table(state) => convert!(state, Table, item_state_to_table_command),
            ObjectState::Table(state) => {
                ControllerCommand::Table(Self::item_state_to_table_command(state))
            }
            ObjectState::Source(state) => {
                ControllerCommand::Source(Self::item_state_to_source_command(state))
            }
            ObjectState::Sink(state) => {
                ControllerCommand::Sink(Self::item_state_to_sink_command(state))
            }
            ObjectState::Index(state) => {
                ControllerCommand::Index(Self::item_state_to_index_command(state))
            }
            ObjectState::MaterializedView(state) => {
                ControllerCommand::MaterializedView(Self::item_state_to_mv_command(state))
            }
            ObjectState::View(state) => {
                ControllerCommand::View(Self::item_state_to_view_command(state))
            }
            ObjectState::ContinualTask(state) => {
                ControllerCommand::ContinualTask(Self::item_state_to_ct_command(state))
            }
            ObjectState::Secret(state) => {
                ControllerCommand::Secret(Self::item_state_to_secret_command(state))
            }
            ObjectState::Connection(state) => {
                ControllerCommand::Connection(Self::item_state_to_connection_command(state))
            }
            ObjectState::Cluster(state) => {
                ControllerCommand::Cluster(Self::item_state_to_cluster_command(state))
            }
            ObjectState::ClusterReplica(state) => {
                ControllerCommand::ClusterReplica(Self::item_state_to_replica_command(state))
            }
        }
    }

    impl_state_to_command!(
        item_state_to_table_command,
        TableControllerCommand,
        Table,
        add: AddTable(),
        drop: DropTable(),
        alter: AlterTable { prev: prev_table, new: new_table }
    );

    impl_state_to_command!(
        item_state_to_sink_command,
        SinkControllerCommand,
        Sink,
        add: AddSink(),
        drop: DropSink(),
        alter: AlterSink { prev: prev_sink, new: new_sink }
    );

    impl_state_to_command!(
        item_state_to_index_command,
        IndexControllerCommand,
        Index,
        add: AddIndex(),
        drop: DropIndex(),
        alter: AlterIndex { prev: prev_index, new: new_index }
    );

    impl_state_to_command!(
        item_state_to_mv_command,
        MaterializedViewControllerCommand,
        MaterializedView,
        add: AddMaterializedView(),
        drop: DropMaterializedView(),
        alter: AlterMaterializedView { prev: prev_mv, new: new_mv }
    );

    impl_state_to_command!(
        item_state_to_view_command,
        ViewControllerCommand,
        View,
        add: AddView(),
        drop: DropView(),
        alter: AlterView { prev: prev_view, new: new_view }
    );

    impl_state_to_command!(
        item_state_to_ct_command,
        ContinualTaskControllerCommand,
        ContinualTask,
        add: AddContinualTask(),
        drop_no_name: DropContinualTask(),
        alter: AlterContinualTask { prev: prev_ct, new: new_ct }
    );

    impl_state_to_command!(
        item_state_to_secret_command,
        SecretControllerCommand,
        Secret,
        add: AddSecret(),
        drop_no_name: DropSecret(),
        alter: AlterSecret { prev: prev_secret, new: new_secret }
    );

    impl_state_to_command!(
        item_state_to_connection_command,
        ConnectionControllerCommand,
        Connection,
        add: AddConnection(),
        drop_no_name: DropConnection(),
        alter: AlterConnection { prev: prev_connection, new: new_connection }
    );

    impl_state_to_command!(
        item_state_to_cluster_command,
        ClusterControllerCommand,
        Cluster,
        add: AddCluster(),
        drop_no_name: DropCluster(),
        alter: AlterCluster { prev: prev_cluster, new: new_cluster }
    );

    impl_state_to_command!(
        item_state_to_replica_command,
        ClusterReplicaControllerCommand,
        ClusterReplica,
        add: AddClusterReplica(),
        drop_no_name: DropClusterReplica(),
        alter: AlterClusterReplica { prev: prev_replica, new: new_replica }
    );

    // Special case for source which has a tuple type
    fn item_state_to_source_command(
        state: ObjectStateKind<(Source, Option<GenericSourceConnection>)>,
    ) -> SourceControllerCommand {
        match state {
            ObjectStateKind::None => panic!("Cannot execute None state"),
            ObjectStateKind::Added((s, c)) => SourceControllerCommand::AddSource(s, c),
            ObjectStateKind::Dropped((s, c), name) => {
                SourceControllerCommand::DropSource(s, c, name)
            }
            ObjectStateKind::Altered { prev, new } => SourceControllerCommand::AlterSource {
                prev_source: prev.0,
                prev_connection: prev.1,
                new_source: new.0,
                new_connection: new.1,
            },
        }
    }
}
...
mod tests {
        ...
        Table {
            desc: VersionedRelationDesc::new(
                RelationDesc::builder()
                    .with_column(name, mz_repr::ScalarType::String.nullable(false))
                    .finish(),
            ),
            create_sql: None,
    ...
    fn test_item_state_transitions() {
        // Test None -> Added
        let mut state = ObjectStateKind::None;
        assert!(
            state
                .transition("item1".to_string(), None, StateDiff::Addition)
                .is_ok()
        );
        assert!(matches!(state, ObjectStateKind::Added(_)));

        // Test Added -> Altered (via retraction)
        let mut state = ObjectStateKind::Added("new_item".to_string());
        assert!(
            state
        ...
        match &state {
            ObjectStateKind::Altered { prev, new } => {
        ...
        assert!(matches!(state, ObjectStateKind::Dropped(_, _)));

        // Test Dropped -> Altered (via addition)
        let mut state = ObjectStateKind::Dropped("old_item".to_string(), "name".to_string());
        assert!(
            state
        ...
        let mut state = ObjectStateKind::Added("item".to_string());
        assert!(
            state
        ...
        let mut state = ObjectStateKind::Dropped("item".to_string(), "name".to_string());
        assert!(
            state
    ...
    fn test_table_absorb_state_machine() {
        ...
        match &cmd {
            ObjectState::Table(state) => match state {
                ObjectStateKind::Added(t) => {
        ...
        match &cmd {
            ObjectState::Table(state) => match state {
                ObjectStateKind::Dropped(t, name) => {
                    assert_eq!(t.desc.latest().arity(), table1.desc.latest().arity());
    ...
    #[mz_ore::test]
    fn test_to_command_conversion() {
        let table = create_test_table("test");

        // Test Added -> AddTable
        let cmd = ObjectState::Table(ObjectStateKind::Added(table.clone()));
        match cmd.to_controller_command() {
            ControllerCommand::Table(TableControllerCommand::AddTable(t)) => {
                assert_eq!(t.desc.latest().arity(), table.desc.latest().arity());
            }
            _ => panic!("Expected AddTable command"),
        }

        // Test Dropped -> DropTable
        let cmd = ObjectState::Table(ObjectStateKind::Dropped(
            table.clone(),
            "test.table".to_string(),
        ));
        match cmd.to_controller_command() {
            ControllerCommand::Table(TableControllerCommand::DropTable(t, name)) => {
                assert_eq!(t.desc.latest().arity(), table.desc.latest().arity());
        ...
        // Test Altered -> AlterTable
        let table2 = create_test_table("test2");
        let cmd = ObjectState::Table(ObjectStateKind::Altered {
            prev: table.clone(),
            new: table2.clone(),
        });
        match cmd.to_controller_command() {
            ControllerCommand::Table(TableControllerCommand::AlterTable {
                ...
                assert_eq!(
                    prev_table.desc.latest().arity(),
                    table.desc.latest().arity()
                );
                assert_eq!(
                    new_table.desc.latest().arity(),
                    table2.desc.latest().arity()
                );
            }
            _ => panic!("Expected AlterTable command"),
        }

        // Test None -> ExecutableCommand::None
        let cmd = ObjectState::None;
        match cmd.to_controller_command() {
            ControllerCommand::None => {}
            _ => panic!("Expected None command"),
        }
    }
}
