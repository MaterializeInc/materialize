...
impl Coordinator {
    ...
    pub(crate) async fn sequence_plan(
        ...
        match plan {
            ...
            Plan::CreateReplicaSet(plan) => {
                let result = self.sequence_create_replica_set(ctx.session(), plan).await;
                ctx.retire(result);
            }
            ...
            Plan::AlterReplicaSet(plan) => {
                let result = self.sequence_alter_replica_set(ctx.session(), plan).await;
                ctx.retire(result);
            }
