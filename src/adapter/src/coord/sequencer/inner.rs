...
impl Coordinator {
    ...
    ///
    /// The `CREATE METRIC SINK` `_finish` stage renders notices before the catalog transaction
    /// that creates the item, so the rendered text can already refer to the item by its
    /// intended name.
