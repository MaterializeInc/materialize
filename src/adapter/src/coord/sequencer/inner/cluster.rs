...
use mz_sql::plan::{AlterClusterPlan, AlterClusterStrategyCondition, AlterOptionParameter};
use mz_sql::{plan, session::metadata::SessionMetadata};
use tracing::{debug, Span};

use crate::catalog::ReplicaCreateDropReason;
use crate::catalog::{self, Op};
use crate::coord::FinalizeAlterCluster;
use crate::coord::{
...
impl Coordinator {
        ...
        match (&config.variant, new_config.variant) {
            (Managed(config), Managed(new_config)) => {
                ...
                if needs_finalization {
                    return match strategy.condition {
                        AlterClusterStrategyCondition::None => Ok(StageResult::Response(
                            ExecuteResponse::AlteredObject(ObjectType::Cluster),
                        )),
                        AlterClusterStrategyCondition::For(duration) => Ok(StageResult::Handle(
                            mz_ore::task::spawn(|| "Finalize Alter Cluster", async move {
    ...
    pub(crate) async fn finalize_alter_cluster_managed_to_managed(
        ...
        // Gather the ops to remove the non pending cluster
        // Also skip any billed_as free clusters
        let remove_replicas = cluster
            .replicas()
            .filter_map(|r| {
                if !r.config.location.pending() && r.config.location.billed_as().is_none() {
                    debug!("Dropping replica: {}", r.name);
                ...
                vec![
                    catalog::Op::RenameClusterReplica {
                        ...
                        to_name: replica
                            ...
                            .expect("Pending replicas must have names ending with -pending")
                            .to_owned(),
                    },
