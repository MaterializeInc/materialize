...
use mz_catalog::memory::objects::{
    ClusterConfig, ClusterVariant, ClusterVariantManaged, ManagedReplicaConfigShape,
    ReconfigurationState, ReconfigurationStatus, ReconfigurationTarget,
};
...
impl Coordinator {
    ...
    async fn sequence_alter_cluster_stage(
        ...
        if let Some(target) = folded_target {
            ...
            target_managed.size = target.size;
            target_managed.replication_factor = target.replication_factor;
            target_managed.availability_zones = target.availability_zones;
            target_managed.logging = target.logging;
            target_managed.arrangement_compression = target.arrangement_compression;
        }
