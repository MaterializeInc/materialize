...
impl From<mz_sql::session::vars::ConnectionError> for AdapterError {
    ...
    mz_sql::session::vars::ConnectionError::TooManyConnections{current, limit}            =>
      AdapterError::ResourceExhaustion { resource_type: "connection".into(),
    limit_name: "max_connections".into(), desired: (current+1).to_string(), limit: limit.to_string(), current: current.to_string() }
