...
use crate::active_compute_sink::format_subscribe_response;
...
use crate::{
    AdapterError, Client, CollectionIdBundle, ExecuteResponse, PeekResponseUnary, ReadHolds,
    statement_logging,
};
...
use mz_compute_client::protocol::command::{PeekDescription, PeekTarget};
...
impl PeekClient {
    ...
    /// peek target. For slow-path peeks (to be implemented later), we'll need to additionally call
    /// into the Controller to acquire a hold on the peek target after we create the dataflow.
    pub async fn implement_fast_path_subscribe_plan(
        &mut self,
        ...
        let arity = intermediate_result_type.arity();
        let finishing = RowSetFinishing::trivial(arity);
        let (rows_tx, rows_rx) = oneshot::channel();

        let (subscribe_tx, subscribe_rx) = tokio::sync::mpsc::unbounded_channel();

        let persist_client = self.persist_client.clone();
        mz_ore::task::spawn(|| "todo: make this method take a stream", async move {
            let response = match rows_rx.await {
                Err(recv_error) => PeekResponseUnary::Error(recv_error.to_string()),
                Ok(PeekResponse::Rows(rows)) => format_subscribe_response(
                    SubscribeBatch {
                        lower: Antichain::from_elem(as_of),
                        upper: Antichain::from_elem(up_to),
                        updates: Ok(rows),
                    },
                    arity,
                    as_of,
                    &subscribe_plan.output,
                    subscribe_plan.emit_progress,
                ),
                Ok(PeekResponse::Stashed(response)) => {
                    let rx = Coordinator::create_stash_stream(
                        persist_client,
                        *response,
                        peek_stash_read_batch_size_bytes,
                        peek_stash_read_memory_budget_bytes,
                    );
                    let rows: Vec<_> = ReceiverStream::new(rx).collect().await;
                ...
                Ok(PeekResponse::Error(e)) => PeekResponseUnary::Error(e),
                Ok(PeekResponse::Canceled) => PeekResponseUnary::Canceled,
            };
            let _ = subscribe_tx.send(response);
        });
        ...
        let (peek_target, target_read_hold, literal_constraints, mfp, _strategy) = match fast_path {
            FastPathPlan::PeekExisting(_coll_id, idx_id, literal_constraints, mfp) => {
                let peek_target = PeekTarget::Index { id: idx_id };
                let target_read_hold = input_read_holds
                    .compute_holds
        ...
        let peek_result = client
            .peek(
                ...
                literal_constraints,
                uuid,
                PeekDescription::subscribe(as_of, Some(up_to), subscribe_plan.with_snapshot),
                result_desc,
                finishing_for_instance,
                mfp,
                target_read_hold,
        ...
        let resp = ExecuteResponse::Subscribing {
            rx: subscribe_rx,
            instance_id: compute_instance,
            ctx_extra: Default::default(),
        };
        let resp = match subscribe_plan.copy_to {
            None => resp,
