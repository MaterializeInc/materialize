...
mod tests {
    ...
    impl QueryTrackerEffects for RecordingEffects {
        ...
        fn inc_canceled_peeks(&self, by: u64) {
            *self.canceled_metric.lock().expect("lock poisoned") += by;
        }
        ...
        fn retire_execute(
            ...
            self.retired.lock().expect("lock poisoned").push(reason);
        }
    }
    ...
    fn track_peek_is_canceled_if_cluster_was_dropped() {
        ...
        let canceled = effects.canceled.lock().expect("lock poisoned").clone();
