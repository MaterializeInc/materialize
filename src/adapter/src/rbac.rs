...
use crate::catalog::{Catalog, SYSTEM_USER};
use crate::command::Command;
use crate::session::vars::ENABLE_RBAC_CHECKS;
use crate::session::{Session, Var};
use crate::AdapterError;
use mz_sql::catalog::{CatalogRole, SessionCatalog};
use mz_sql::plan::{AlterSystemResetPlan, AlterSystemSetPlan, Plan};
use std::error::Error;
use std::fmt;
...
impl UnauthorizedError {
    fn superuser(action: String) -> UnauthorizedError {
        UnauthorizedError {
            action,
    ...
    fn attribute(action: String, attribute: Attribute) -> UnauthorizedError {
        UnauthorizedError {
            action,
    ...
    pub fn unstructured(action: String, msg: String) -> UnauthorizedError {
        UnauthorizedError {
            action,
            inner: UnauthorizedErrorInner::Unstructured(msg),
        }
    }
