...
fn generate_required_ownership(plan: &Plan) -> Vec<ObjectId> {
    match plan {
        ...
        Plan::CreateReplicaSet(plan) => vec![ObjectId::Cluster(plan.cluster_id)],
        ...
        Plan::AlterReplicaSet(plan::AlterReplicaSetPlan {
            id, replica_set_id, ..
        }) => {
            vec![
                ObjectId::Cluster(*id),
                ObjectId::ClusterItem((*id, *replica_set_id)),
            ]
        }
...
fn generate_required_privileges(
    ...
    match plan {
        ...
        | Plan::CreateReplicaSet(plan::CreateReplicaSetPlan {
            cluster_id: _,
            name: _,
            variant: _,
        })
...
pub(crate) const fn all_object_privileges(object_type: SystemObjectType) -> AclMode {
    ...
    match object_type {
        ...
        SystemObjectType::Object(ObjectType::ReplicaSet) => EMPTY_ACL_MODE,
