...
impl ShouldTerminateGracefully for mz_catalog::catalog::CatalogError {
    fn should_terminate_gracefully(&self) -> bool {
        match self {
            mz_catalog::catalog::CatalogError::Catalog(error) => {
                error.should_terminate_gracefully()
            }
            _ => false,
        }
    }
...
pub use mz_catalog::catalog::sort_topological;
