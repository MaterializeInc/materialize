    ...
    F: Fn(&SqlScalarType) -> Option<(DataType, String)>,
{
    ...
    let (data_type, extension_name) = match scalar_type {
        ...
        SqlScalarType::Range { element_type } => {
            ...
            //   - empty: bool
            let (inner_type, inner_name) =
                scalar_to_arrow_datatype_impl(element_type, overrides)?;
            ...
            let upper_field = Arc::new(field_with_typename(
                "upper",
                inner_type,
                true,
                &inner_name,
            ));
            let lower_inclusive_field = Arc::new(Field::new("lower_inclusive", DataType::Boolean, false));
            let upper_inclusive_field = Arc::new(Field::new("upper_inclusive", DataType::Boolean, false));
            let empty_field = Arc::new(Field::new("empty", DataType::Boolean, false));
            (
                DataType::Struct(
                    [lower_field, upper_field, lower_inclusive_field, upper_inclusive_field, empty_field].into(),
                ),
                "range",
            )
        }
...
fn builder_for_datatype(
    ...
    let builder = match &data_type {
        ...
        DataType::Interval(arrow::datatypes::IntervalUnit::MonthDayNano) => {
            ColBuilder::IntervalMonthDayNanoBuilder(
                IntervalMonthDayNanoBuilder::with_capacity(item_capacity),
            )
        }
                ...
                match range.inner {
                    None => {
                        ...
                        struct_builder
                            ...
                            .unwrap()
                            .append_datum(Datum::Null)?;
                        if let ColBuilder::BooleanBuilder(b) =
                            &mut struct_builder.field_builder::<ArrowColumn>(2).unwrap().inner
                        {
                            b.append_value(false);
                        }
                        if let ColBuilder::BooleanBuilder(b) =
                            &mut struct_builder.field_builder::<ArrowColumn>(3).unwrap().inner
                        {
                            b.append_value(false);
                        }
                        if let ColBuilder::BooleanBuilder(b) =
                            &mut struct_builder.field_builder::<ArrowColumn>(4).unwrap().inner
                        {
                            b.append_value(true);
                        }
                    }
