...
mod tests {
    ...
    fn test_mock_sasl_challenge() {
        let username = "alice";
        let client_nonce = "cnonce";
        let opts1 = mock_sasl_challenge(username, client_nonce);
        let opts2 = mock_sasl_challenge(username, client_nonce);
        assert_eq!(opts1.iterations, DEFAULT_ITERATIONS);
        assert_eq!(opts1.salt.len(), DEFAULT_SALT_SIZE);
        // Non-deterministic: salts should usually differ across invocations
        assert_ne!(opts1.salt, opts2.salt);
    }
}
