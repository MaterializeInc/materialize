...
mod tests {
    ...
    fn test_negative_timestamps() {
        assert_eq!(
            ...
            Value::Timestamp(NaiveDateTime::from_timestamp_millis(-1).unwrap())
        );
        // TODO[btv]: This fails, because Chrono doesn't consider secs=-2, nsecs=1B
        // to be equal to secs=-1, nsecs=0.
        //
        // Thus, if there's somewhere else where we convert timestamps
        // in seconds to Chrono `NaiveDateTime`s by calling
        // `from_timestamp_opt(secs, 0)`, those timestamps (if negative)
        // will fail to compare equal to ones generated by `build_ts_value`,
        // even if they represent the same time.
        //
        // I think it's better if we just inherit Chrono's behavior for now here, and think
        // about filing a bug with them.
        //
        // assert_eq!(
        //     NaiveDateTime::from_timestamp_opt(-1, 0),
        //     NaiveDateTime::from_timestamp_millis(-1000),
        // );
        assert_eq!(
            build_ts_value(-1000, TsUnit::Millis).unwrap(),
            Value::Timestamp(NaiveDateTime::from_timestamp_millis(-1000).unwrap())
        );
        assert_eq!(
...
fn build_ts_value(value: i64, unit: TsUnit) -> Result<Value, AvroError> {
    ...
    let mut seconds = value / units_per_second;
    if value < 0 {
        seconds -= 1;
    }
    let fraction = u32::try_from((value % units_per_second).abs()).unwrap();
    let mut nsecs = fraction * nanos_per_unit;
    if seconds < 0 {
        nsecs = NANOS_PER_SECOND - nsecs;
    }
    Ok(Value::Timestamp(
        NaiveDateTime::from_timestamp_opt(seconds, nsecs).ok_or(AvroError::Decode(
