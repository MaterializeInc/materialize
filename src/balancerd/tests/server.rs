...
/// The current value of a `source="pgwire"` metric, from balancerd's metrics endpoint.
async fn pgwire_metric(internal_http: SocketAddr, name: &str) -> Option<f64> {
    let body = reqwest::get(format!("http://{internal_http}/metrics"))
        ...
        .unwrap();
    body.lines()
        .find(|line| line.starts_with(name) && line.contains("source=\"pgwire\""))
...
async fn assert_pgwire_metric(internal_http: SocketAddr, name: &str, expected: f64) {
    Retry::default()
        .max_duration(Duration::from_secs(10))
        .retry_async(|_| async {
            match pgwire_metric(internal_http, name).await {
...
async fn test_connection_limit_covers_unresolved_connections() {
    ...
    assert_pgwire_metric(balancer.internal_http, ACTIVE, 1.0).await;

    // The next is refused at accept while the limit is reached.
    let mut refused = startup_header_only(balancer.pgwire, 1 << 10).await;
    assert!(
        was_closed(&mut refused, Duration::from_secs(10)).await,
        "a connection beyond the limit should be closed",
    );
    ...
    assert_pgwire_metric(balancer.internal_http, ACTIVE, 0.0).await;
    assert_pgwire_metric(balancer.internal_http, TIMEOUTS, 1.0).await;
