...
use mz_catalog::durable::debug::{
    AuditLogCollection, ClusterCollection, ClusterReplicaCollection, Collection, CollectionTrace,
    CollectionType, CommentCollection, ConfigCollection, DatabaseCollection, DebugCatalogState,
    DefaultPrivilegeCollection, IdAllocatorCollection, IntrospectionSourceIndexCollection,
    ItemCollection, NetworkPolicyCollection, RoleAuthCollection, RoleCollection, SchemaCollection,
    SettingCollection, SourceReferencesCollection, StorageCollectionMetadataCollection,
    SystemConfigurationCollection, SystemObjectMappingCollection, SystemPrivilegeCollection, Trace,
...
macro_rules! for_collection {
    ($collection_type:expr, $fn:ident $(, $arg:expr)*) => {
        match $collection_type {
            ...
            CollectionType::IntrospectionSourceIndex => $fn::<IntrospectionSourceIndexCollection>($($arg),*).await?,
            ...
            CollectionType::DefaultPrivilege => $fn::<DefaultPrivilegeCollection>($($arg),*).await?,
            CollectionType::IdAllocator => $fn::<IdAllocatorCollection>($($arg),*).await?,
            ...
            CollectionType::SystemObjectMapping => $fn::<SystemObjectMappingCollection>($($arg),*).await?,
            CollectionType::SystemPrivilege => $fn::<SystemPrivilegeCollection>($($arg),*).await?,
