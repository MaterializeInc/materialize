...
/// A session currently connected to an environmentd.
///
/// The envd incarnation owning the session is identified by
...
/// current one belongs to a dead envd and can be reclaimed.
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
#[cfg_attr(any(test, feature = "proptest"), derive(Arbitrary))]
