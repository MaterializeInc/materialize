...
FROM (VALUES {values}) AS v(oid, schema_name, name, definition, privileges, create_sql)"
    ...
    BuiltinView {
        ...
        desc: RelationDesc::builder()
            ...
            // NOTE: The declared keys must exactly match the keys the
            // optimizer derives from the generated VALUES list
            // (`verify_builtin_descs` enforces this). View names happen to be
            // unique across builtin schemas today. If a view is ever added
            // whose bare name collides with another schema's, drop the `name`
            // key here.
            .with_key(vec![0])
            .with_key(vec![2])
            .with_key(vec![3])
