...
// mz_object_dependencies_raw is generated dynamically in BUILTINS_STATIC via
// mz_internal::make_mz_object_dependencies_raw().

/// Generate the `mz_internal.mz_object_dependencies_raw` builtin view with
/// builtin dependency edges inlined as VALUES clauses.
///
/// The builtin edge set is known only at compile time, so it has to be baked
/// into SQL. It lives here rather than in `mz_object_dependencies` itself so
/// that the materialized view's own definition stays fixed across releases:
/// its fingerprint never moves, it keeps its persist shard, and its
/// self-correcting sink writes the difference once the edges change. A view
/// carries no durable state, so it is rebuilt from this definition on every
/// boot and never needs a `MigrationStep`.
///
...
/// - This view itself has no outgoing edges: it is generated here, so it is
///   absent from the builtin list the generator walks, and inlining its own
///   edge rows would require a fixpoint. `mz_object_dependencies` does get its
///   edge to this view, since that definition is static.
///
...
pub(super) fn make_mz_object_dependencies_raw(builtins: &[Builtin<NameReference>]) -> BuiltinView {
    ...
    let resolve = |object: &str, name: &UnresolvedItemName| -> (String, String) {
        match &name.0[..] {
            [.., schema, item] => (schema.as_str().to_string(), item.as_str().to_string()),
            [item] => {
                let item = item.as_str();
                let schema = schema_by_name.get(item).unwrap_or_else(|| {
                    panic!(
    ...
    for b in builtins {
        ...
        for item_name in &refs.named_relations {
            let (schema, name) = resolve(b.name(), item_name);
        ...
        for elem in &refs.named_array_elements {
            ...
            let (_, name) = resolve(b.name(), elem);
...
/// The `mz_internal.mz_object_dependencies` materialized view.
///
/// The definition is deliberately a bare read of
/// [`make_mz_object_dependencies_raw`]'s view, which holds the parts that
/// change from release to release. That keeps this SQL, and so this object's
/// catalog fingerprint, fixed: the persist shard survives upgrades and the
/// self-correcting sink reconciles the contents against the new edge set.
