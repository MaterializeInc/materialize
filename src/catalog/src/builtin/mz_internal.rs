...
pub(super) fn make_mz_object_dependencies(
    ...
    // Maps reconstructing what name resolution does with array types: `T[]`
    // additionally resolves the paired array type, and referencing an array
    // type by name additionally resolves its element type.
    let mut array_type_by_elem: BTreeMap<&str, (&str, &str)> = BTreeMap::new();
    let mut elem_by_array_type: BTreeMap<&str, &str> = BTreeMap::new();
    for b in builtins {
        if let Builtin::Type(t) = b {
            if let CatalogType::Array { element_reference } = &t.details.typ {
                ...
                elem_by_array_type.insert(t.name, element_reference);
            }
        }
    }
    ...
    for b in builtins {
        ...
        for parts in &refs.type_names {
            let (schema, name) = resolve(b.name(), parts);
            // Referencing an array type by name also resolves its element
            // type (see the `element_reference` handling in name resolution).
            if let Some(elem) = elem_by_array_type.get(name.as_str()) {
                let elem_schema = schema_by_name
                    .get(elem)
                    .unwrap_or_else(|| panic!("array element type {elem} must be a builtin"));
                push(elem_schema, elem, "type");
            }
            push(&schema, &name, "type");
        }
        for parts in &refs.array_elem_type_names {
            // `T[]` resolves the paired array type in addition to `T` itself
            // (which is already in `type_names`).
