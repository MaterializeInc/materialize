...
pub static MZ_REPLICA_HYDRATION_HISTORY: LazyLock<BuiltinTable> = LazyLock::new(|| BuiltinTable {
    ...
    column_comments: BTreeMap::from_iter([
        ...
        (
            "process_id",
            "The ID of a process within the replica. `NULL` identifies a replica-wide aggregate of the maximum peaks across processes rather than a per-process observation. Episode timing and object_count are replica-wide and repeated for each process.",
        ),
    ]),
    // Not a retained-metrics object: that would pin a 30 day compaction window,
    // and our history lives in the rows, which the retention sweep retracts on
