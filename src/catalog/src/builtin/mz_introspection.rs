...
pub static MZ_COMPUTE_EXPORT_ARRANGEMENTS: LazyLock<BuiltinView> = LazyLock::new(|| BuiltinView {
    ...
    desc: RelationDesc::builder()
        ...
        .with_key(vec![0])
        .finish(),
    // NOTE: these must match the column table in
    // `doc/user/content/reference/system-catalog/mz_introspection.md` exactly, or
    // `test/sqllogictest/autogenerated/mz_introspection.slt` fails. The rationale
    // for why this relation is needed lives in the prose there, not here.
    column_comments: BTreeMap::from_iter([
        (
            "export_id",
            "The ID of the index exported by the dataflow. Corresponds to \
             `mz_compute_exports.export_id`.",
        ),
        (
            "operator_id",
            "The ID of the operator holding the arrangement. Corresponds to \
