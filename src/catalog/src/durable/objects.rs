...
/// collection that backs cluster-coherent scoped feature flags (step 3 of the
/// scoped feature flags design). The collection — keyed by `(ClusterId, name)`
/// — is the analog of `system_configurations` (`ALTER SYSTEM`), but for
/// per-cluster values; it is written solely by the system-parameter sync loop
