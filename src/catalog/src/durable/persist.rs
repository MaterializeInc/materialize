...
impl DurableCatalogState for PersistCatalogState {
    ...
    async fn commit_transaction(
        ...
        async fn commit_transaction_inner(
            ...
            // If the transaction is empty then we don't error, even in read-only mode. This is
            // mostly for legacy reasons (i.e. with enough elbow grease this behavior can be
            // changed without breaking any fundamental assumptions).
