...
pub struct PersistHandle<T: TryIntoStateUpdateKind, U: ApplyUpdate<StateUpdate<T>>> {
    ...
    /// Applies custom processing and filtering for each individual update.
    update_applier: U,
...
#[derive(Debug)]
pub(crate) struct UnopenedCatalogUpdateApplier {
    /// A cache of the config collection of the catalog.
    configs: BTreeMap<String, u64>,
}

impl UnopenedCatalogUpdateApplier {
    fn new() -> UnopenedCatalogUpdateApplier {
        UnopenedCatalogUpdateApplier {
            configs: BTreeMap::new(),
        }
    }
}

impl ApplyUpdate<StateUpdate<StateUpdateKindRaw>> for UnopenedCatalogUpdateApplier {
    fn apply_update(
        &mut self,
...
pub struct UnopenedPersistCatalogState {
    /// A handle to the catalog persist shard.
    pub(crate) persist_handle: PersistHandle<StateUpdateKindRaw, UnopenedCatalogUpdateApplier>,
...
impl UnopenedPersistCatalogState {
    ...
    pub(crate) async fn new(
        ...
        let persist_handle = PersistHandle::new(
            ...
            version,
            UnopenedCatalogUpdateApplier::new(),
            metrics,
        )
        .await?;
    ...
    async fn open_inner(
        ...
        let mut catalog = PersistCatalogState {
            ...
            persist_handle: PersistHandle {
                ...
                // Initialize empty in-memory state.
                snapshot: Vec::new(),
                update_applier: CatalogUpdateApplier::new(),
        ...
        let txn = if is_initialized {
            ...
            soft_assert_eq_no_log!(
                ...
                catalog.persist_handle.snapshot
            );
            let mut txn = catalog.transaction().await?;
        ...
        if read_only {
            ...
            let updates = StateUpdate::from_txn_batch_ts(txn_batch, catalog.persist_handle.upper);
            catalog.persist_handle.apply_updates(updates)?;
        } else {
            txn.commit().await?;
        }
...
impl CatalogUpdateApplier {
    fn new() -> CatalogUpdateApplier {
        CatalogUpdateApplier {
...
impl ApplyUpdate<StateUpdate<StateUpdateKind>> for CatalogUpdateApplier {
    fn apply_update(
        &mut self,
...
pub struct PersistCatalogState {
    ...
    mode: Mode,
    /// A handle to the catalog persist shard.
    persist_handle: PersistHandle<StateUpdateKind, CatalogUpdateApplier>,
}
...
impl ReadOnlyDurableCatalogState for PersistCatalogState {
    ...
    async fn get_audit_logs(&mut self) -> Result<Vec<VersionedEvent>, CatalogError> {
        self.persist_handle.sync_to_current_upper().await?;
        let audit_logs = match self.persist_handle.update_applier.audit_logs.take() {
...
impl DurableCatalogState for PersistCatalogState {
    ...
    async fn transaction(&mut self) -> Result<Transaction, CatalogError> {
        self.persist_handle.metrics.transactions_started.inc();
    ...
    async fn commit_transaction(
        ...
        async fn commit_transaction_inner(
            ...
            if matches!(catalog.mode, Mode::Writable) {
                catalog.persist_handle.compare_and_append(updates).await?;
            } else if matches!(catalog.mode, Mode::Savepoint) {
                let ts = catalog.persist_handle.upper;
                let updates =
                    updates
                        .into_iter()
        ...
        self.persist_handle.metrics.transaction_commits.inc();
        let counter = self
            .persist_handle
            .metrics
