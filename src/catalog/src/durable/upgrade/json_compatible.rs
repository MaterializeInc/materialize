...
macro_rules! json_compatible {
    ($a:ident $(:: $a_sub:ident)* with $b:ident $(:: $b_sub:ident)*) => {
        ...
        // SAFETY: Below we assert that these types are JSON
        // compatible by generating arbitrary structs, encoding in
        // one, and then decoding in the other.
        unsafe impl $crate::durable::upgrade::json_compatible
            ::JsonCompatible< $b $(::$b_sub)* >
            for $a $(::$a_sub)* {}
        unsafe impl $crate::durable::upgrade::json_compatible
        ...
        ::paste::paste! {
            ::proptest::proptest! {
                ...
                fn [<proptest_json_compat_
                    $a:snake $(_$a_sub:snake)* _to_
                    $b:snake $(_$b_sub:snake)*
                >](a: $a $(::$a_sub)* ) {
                    let a_bytes = ::serde_json::to_vec(&a)
                        .expect("JSON serializable");
                    let b_decoded = ::serde_json::from_slice
                        ::<$b $(::$b_sub)*>(&a_bytes);
                    ::proptest::prelude::prop_assert!(
                        b_decoded.is_ok(),
                    );

                    // Maybe superfluous, but this is a method
                    // called in production.
                    let b_decoded = b_decoded.expect("asserted Ok");
                    use $crate::durable::upgrade::json_compatible
                        ::JsonCompatible;
                    let b_converted: $b $(::$b_sub)* =
                        JsonCompatible::convert(&a);
                    assert_eq!(b_decoded, b_converted);

                    let b_bytes = ::serde_json::to_vec(&b_decoded)
                        .expect("JSON serializable");
                    ::proptest::prelude::prop_assert_eq!(
                        a_bytes, b_bytes,
                        "a and b serialize differently",
                    );
                }
                ...
                #[cfg_attr(miri, ignore)] // slow
                fn [<proptest_json_compat_
                    $b:snake $(_$b_sub:snake)* _to_
                    $a:snake $(_$a_sub:snake)*
                >](b: $b $(::$b_sub)* ) {
                    let b_bytes = ::serde_json::to_vec(&b)
                        .expect("JSON serializable");
                    let a_decoded = ::serde_json::from_slice
                        ::<$a $(::$a_sub)*>(&b_bytes);
                    ::proptest::prelude::prop_assert!(
                        a_decoded.is_ok(),
                    );
                    ...
                    use $crate::durable::upgrade::json_compatible
                        ::JsonCompatible;
                    let a_converted: $a $(::$a_sub)* =
                        JsonCompatible::convert(&b);
