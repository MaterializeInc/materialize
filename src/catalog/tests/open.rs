...
async fn test_get_deployment_generation(
    ...
    let state = Box::new(openable_state1)
        .open(SYSTEM_TIME.clone(), &test_bootstrap_args(), Some(42))
        .await
        .unwrap();
    state.expire().await;
...
async fn test_open_savepoint(
...
) {
    {
        ...
        let err = Box::new(openable_state1)
            .open_savepoint(SYSTEM_TIME.clone(), &test_bootstrap_args(), None)
            .await
            .unwrap_err();
        match err {
...
async fn test_open_read_only(
    ...
    let err = Box::new(openable_state1)
        .open_read_only(SYSTEM_TIME.clone(), &test_bootstrap_args())
        .await
        .unwrap_err();
    match err {
        CatalogError::Catalog(_) => panic!("unexpected catalog error"),
...
async fn test_open(
    ...
    let (snapshot, audit_log) = {
        let mut state = Box::new(openable_state1)
            ...
            .open(NOW_ZERO.clone(), &test_bootstrap_args(), None)
            .await
            .unwrap();
