...
fn main() -> Result<(), Error> {
    ...
    if config.only_initialize {
        ...
        let _ = init_result;
        // runtime.enter(|| run_checks(&config));
        run_checks(&config);
        unreachable!()
    }
}
...
fn run_checks(config: &Args) {
    ...
    for c in &config.config.checks {
        peek_threads.extend(spawn_query_thread(config.mz_url.clone(), c.clone()))
    }
    info!(
...
/// the result vector. Once all row-checks within a check have passed, and there are no more rows in the
/// TODO(natacha): this requires knowing the number of rows returned a priori. We should expand the checker
/// to support unknown number of rows
fn validate_result(check: &Check, rows: &Vec<Row>) -> bool {
...
fn spawn_query_thread(mz_url: String, check: Check) -> Vec<JoinHandle<()>> {
    let mut cs = vec![];
