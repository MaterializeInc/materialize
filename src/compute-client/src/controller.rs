...
impl ComputeController {
    ...
    /// Metric sinks are sink exports too, but intentionally run untargeted on every replica so each
    /// renders into its own registry for per-replica introspection.
    ///
