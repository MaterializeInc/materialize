...
impl<T: ComputeControllerTimestamp> Instance<T> {
    ...
    fn refresh_wallclock_lag(&mut self) {
        ...
        // We keep a record of empty collections, so we can emit undefined lags for those here and
        // below when we collect history measurements.
        let mut empty_collections = BTreeSet::new();
        for (id, collection) in &mut self.collections {
            ...
            let collection_empty = PartialOrder::less_equal(&write_frontier, &read_frontier);
            if collection_empty {
                empty_collections.insert(id);
            }
            ...
            if let Some(stash) = &mut collection.wallclock_lag_histogram_stash {
                let bucket = if collection_empty {
        ...
        for replica in self.replicas.values_mut() {
            for (id, collection) in &mut replica.collections {
                let lag = if empty_collections.contains(&id) {
                    WallclockLag::Undefined
                } else {
...
impl<T: ComputeControllerTimestamp> ReplicaCollectionState<T> {
    fn new(
        ...
        Self {
            ...
            introspection,
            input_read_holds,
            wallclock_lag_max: Some(Default::default()),
        }
    }
