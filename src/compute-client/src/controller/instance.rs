...
pub(super) struct Instance<T: Timestamp> {
    ...
    ///
    /// Received updates are applied by [`Instance::apply_read_hold_updates`].
    ...
    ComputeGrpcClient: ComputeClient<T>,
{
    ...
    fn remove_peek(&mut self, uuid: Uuid) {
        ...
        // NOTE: We need to send the `CancelPeek` command _before_ we we release the peek's read
        // hold (by dropping it), to avoid the edge case that caused #16615.
...
struct ReplicaCollectionState<T: Timestamp> {
    ...
    ///
    /// These read holds are kept to ensure that the replica is always to read from storage inputs
    /// at all times it hasn't read yet. We only need install read holds for storage inputs since
