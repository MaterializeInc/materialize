...
use self::durable::{Command, DurableState};
    ...
    ComputeGrpcClient: ComputeClient<T>,
{
    ...
    pub async fn remove_replica(&mut self, id: ReplicaId) -> Result<(), ComputeError> {
        let cmd = Command::RemoveReplica { id };
        self.apply_durable_command(cmd).await
    }
    ...
    pub async fn create_dataflows(
        ...
        let cmd = Command::CreateDataflows { dataflows };
        self.apply_durable_command(cmd).await
    }

    /// Drops the read capability for the sinks and allows their resources to be reclaimed.
    pub async fn drop_sinks(&mut self, ids: Vec<GlobalId>) -> Result<(), ComputeError> {
        let cmd = Command::DropCollections { ids };
        self.apply_durable_command(cmd).await
    }
    ...
    ///     `drop_sinks`.
    pub async fn drop_sinks_unvalidated(&mut self, ids: Vec<GlobalId>) -> Result<(), ComputeError> {
        let cmd = Command::DropCollectionsUnvalidated { ids };
        self.apply_durable_command(cmd).await
    }

    /// Drops the read capability for the indexes and allows their resources to be reclaimed.
    pub async fn drop_indexes(&mut self, ids: Vec<GlobalId>) -> Result<(), ComputeError> {
        let cmd = Command::DropCollections { ids };
        self.apply_durable_command(cmd).await
    }
    ...
    pub async fn cancel_peeks(&mut self, uuids: BTreeSet<Uuid>) -> Result<(), ComputeError> {
        let cmd = Command::CancelPeeks { uuids };
        self.apply_durable_command(cmd).await
    }
    ...
    pub async fn set_read_policy(
        ...
        let cmd = Command::SetReadPolicy { policies };
        self.apply_durable_command(cmd).await
    }
        ...
        match self.compute.stashed_response.take() {
            ...
            Some(ActiveReplicationResponse::ComputeResponse(response)) => match response {
                ComputeResponse::FrontierUppers(updates) => {
                    let cmd = Command::UpdateWriteFrontiers { updates };
                    self.apply_durable_command(cmd).await?;
                    Ok(None)
                }
                ComputeResponse::PeekResponse(uuid, peek_response, otel_ctx) => {
                    let cmd = Command::RemovePeeks {
                        uuids: [uuid].into(),
                    };
                    self.apply_durable_command(cmd).await?;
...
where
    ...
    ComputeGrpcClient: ComputeClient<T>,
{
    async fn apply_durable_command(&mut self, cmd: Command<T>) -> Result<(), ComputeError> {
        self.compute
            .durable
