...
/// subscribe sinks, and `copy_to_ids` as copy-to-S3 sinks, over `[as_of, until)`.
///
/// Both frontiers are explicit because routing reads them together: `is_peek_dataflow` asks whether
/// `until` is exactly one step past `as_of`, so neither alone determines where a dataflow lands.
...
fn create_dataflow(index_ids: &[GlobalId], subscribe_ids: &[GlobalId]) -> ComputeCommand {
    create_dataflow_with(
        ...
        subscribe_ids,
        &[],
        Antichain::from_elem(Timestamp::from(99u64)),
...
async fn maintained_dataflow_routes_to_maintenance() {
    let mut h = harness();
    // A plain index never bounds `until`, so an unbounded (empty) `until` at a finite `as_of`
    // keeps the dataflow on maintenance regardless of the export id's namespace.
    let cmd = create_dataflow_with(
        &[GlobalId::User(7)],
        &[],
        &[],
        Antichain::from_elem(Timestamp::from(99u64)),
...
fn interactive_import_of(
    ...
    let until = Antichain::from_iter(as_of.iter().filter_map(|t| t.try_step_forward()));
    let mut cmd = create_dataflow_with(&[export], &[], &[], as_of.clone(), until);
...
async fn routing_excludes_copy_to_and_subscribe_and_unbounded() {
    ...
    // An unbounded-until (empty antichain) dataflow at a finite `as_of` routes to maintenance: it
    // never stops, so it is not a peek dataflow however transient its exports are.
    let mut h = harness();
    let cmd = create_dataflow_with(
        &[GlobalId::Transient(24)],
        &[],
        &[],
        as_of.clone(),
        Antichain::new(),
    ...
    // `as_of = Timestamp::MAX` is the one single-time shape whose `until` is empty, because there
    // is no time after MAX to bound it with (`optimize::peek` leaves `until` unset when
    // `try_step_forward` fails). It is a one-shot read of the final state, so it routes to
    // interactive: the emptiness of `until` here means end-of-time, not unbounded.
    let mut h = harness();
    let cmd = create_dataflow_with(
        &[GlobalId::Transient(26)],
        &[],
        &[],
    ...
    assert_eq!(
        inter_commands(&h).len(),
        1,
        "a read at MAX is single-time and goes to interactive"
    );
    assert!(maint_commands(&h).is_empty());
