...
//! sequencing.
//!
//! Commands in the command channel are tagged with an epoch identifying the iteration of the
//! compute protocol the command belongs to, allowing clients to recognize client reconnects that
//! require a reconsiliation.
...
pub struct Sender {
    tx: crossbeam_channel::Sender<(ComputeCommand, ClusterStartupEpoch)>,
    activator: Arc<Mutex<Option<SyncActivator>>>,
}

impl Sender {
    /// Broadcasts the given command to all workers.
    pub fn send(&self, message: (ComputeCommand, ClusterStartupEpoch)) {
...
pub struct Receiver {
    rx: crossbeam_channel::Receiver<(ComputeCommand, ClusterStartupEpoch)>,
}
...
impl Receiver {
    ...
    /// in the future.
    pub fn try_recv(&self) -> Option<(ComputeCommand, ClusterStartupEpoch)> {
            ...
            source(scope, "command_channel::source", |cap, info| {
                ...
                *activator.lock().expect("poisoned") = Some(sync_activator);
                ...
                move |output| {
                    ...
                    loop {
                        let (cmd, epoch) = match input_rx.try_recv() {
                            Ok(msg) => msg,
...
fn split_command(command: ComputeCommand, parts: usize) -> Vec<ComputeCommand> {
    match command {
