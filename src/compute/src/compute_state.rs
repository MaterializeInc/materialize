...
pub struct ComputeState {
    ...
    /// The handle to send heap size limit updates.
    pub heap_size_limits_handle: Option<Handle<Timestamp, Update<(usize, u64)>>>,
...
impl<'a, A: Allocate + 'static> ActiveComputeState<'a, A> {
    ...
    fn handle_create_dataflow(
        ...
        for object_id in dataflow.export_ids() {
            ...
            let mut collection =
                CollectionState::new(is_subscribe_or_copy, as_of.clone(), dataflow_id);
    ...
    fn handle_set_heap_size_limit(&mut self, id: GlobalId, limit: Option<u64>) {
        ...
        if let Some(handle) = self.compute_state.heap_size_limits_handle.as_mut() {
    ...
    fn drop_collection(&mut self, id: GlobalId) {
        ...
        if let (Some(handle), Some(limit)) = (
            self.compute_state.heap_size_limits_handle.as_mut(),
    ...
    pub fn initialize_logging(&mut self, config: LoggingConfig) {
        ...
        let (logger, traces) = logging::initialize(
            self.timely_worker,
            &config,
        ...
        for (log, (trace, dataflow_index)) in traces.logs {
            ...
            let mut collection = CollectionState::new(is_subscribe_or_copy, as_of, dataflow_index);
        ...
        self.compute_state.heap_size_limits_handle = Some(traces.heap_size_limits_handle);
...
impl CollectionState {
    fn new(is_subscribe_or_copy: bool, as_of: Antichain<Timestamp>, dataflow_id: usize) -> Self {
        Self {
