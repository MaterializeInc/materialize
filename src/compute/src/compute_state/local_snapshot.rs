...
//! peek's cursor walk, which runs on a blocking task off the timely worker's thread. A worker
//! reads its own traces through an `Rc`-based reader, so the walk needs a snapshot that owns the
//! `Arc` batches its cursor covers rather than a cursor borrowed from the trace.
