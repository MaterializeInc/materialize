...
//! peek's cursor walk, which runs on a blocking task off the timely worker's thread. The
//! interactive runtime reaches the same walk through a registry `SharedTraceHandle`, whose cursor
//! is already owned and `Send`, so this module serves the runtime that reads its own local traces.
    ...
    Tr: TraceReader<Batch: Navigable>,
{
    ...
    Ok(LocalSnapshot { cursor, storage })
}
...
mod tests {
    ...
    // can move to a thread other than the worker that owns the trace.
