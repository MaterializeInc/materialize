    ...
    Tr: TraceReader<Batch: Navigable>,
{
    ...
    target_id: GlobalId,
    cursor: CursorList<BatchCursor<Tr>>,
    storage: Vec<Tr::Batch>,
    ...
    BatchCursor<Tr>: Cursor<KeyContainer: BatchContainer<Owned: Ord>>,
{
    ...
    fn new(
        ...
        cursor: &mut CursorList<BatchCursor<Tr>>,
        storage: &Vec<Tr::Batch>,
    ) -> Self {
        ...
        literals.sort();
        let mut container =
            <<BatchCursor<Tr> as Cursor>::KeyContainer as BatchContainer>::with_capacity(
                literals.len(),
            );
