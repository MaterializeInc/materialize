    ...
    Tr: TraceReader,
{
    ...
    since_timestamp: mz_repr::Timestamp,
    until_timestamp: Option<mz_repr::Timestamp>,
    row_builder: Row,
    datum_vec: DatumVec,
    literals: Option<Literals<Tr>>,
    extracted_updates: Vec<(Row, mz_repr::Timestamp, Diff)>,
}
        ...
        >,
{
    pub fn new(
        ...
        Self {
            ...
            literals,
            extracted_updates: Vec::with_capacity(1),
        }
    }
        ...
        >,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let result = loop {
            if let Some(result) = self.extracted_updates.pop() {
                break Ok(result);
            }
    ...
    fn extract_current_row(&mut self) -> Result<(), String> {
            ...
            .map_err_to_string_with_causes()?
        {
            self.cursor.map_times(&self.storage, |time, diff| {
                let time = self.since_timestamp.max(*time);
                if self.until_timestamp.map_or(false, |until| until <= time) {
                    return;
                }
                self.extracted_updates.push((result.clone(), time, *diff))
            });
            // TODO: gross!
            consolidate_updates(&mut self.extracted_updates);
            self.extracted_updates.reverse();
