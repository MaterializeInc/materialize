            ...
            move |(input, frontier), output| {
                ...
                if let Some(cap) = cap.as_ref() {
                    ...
                    for chunk in peeled.into_iter().flat_map(|x| x.done()) {
                        ...
                        // consumes owned `(D, T, Diff)` tuples, so load each chunk's body and
                        // reconstitute each record from its columnar reference.
...
/// bucket chain and those arrangements share one merge-batcher
/// implementation. The choice is unconditional here: the bucket chain does
/// not consult the arrange batcher selector.
///
...
/// input into the [`Column`]s it wraps as chunks.
    ...
    for<'a> <(D, T, R) as Columnar>::Container: columnar::Push<&'a (D, T, R)>,
{
    ...
    fn push_container(&mut self, buffer: &mut Vec<(D, T, R)>) {
        ...
        // A chunker can hand back an empty column, which is not a valid chunk.
