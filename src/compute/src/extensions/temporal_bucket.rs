...
pub trait TemporalBucketing<T: Timestamp, O> {
    ...
    fn bucket<CB>(
        self,
        as_of: Antichain<T>,
        threshold: <T as Timestamp>::Summary,
...
impl<T, D> TemporalBucketing<T, (D, T, mz_repr::Diff)> for StreamVec<T, (D, T, mz_repr::Diff)>
where
