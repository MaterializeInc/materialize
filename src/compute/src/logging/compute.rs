                        ...
                        match datum {
                            ComputeEvent::Dataflow(id, is_create) => {
                                ...
                                if is_create {
                                    ...
                                    if let Some(source_map) = storage_sources.remove(key) {
                                        for (source_id, (_, delay_map, mut cur_time)) in source_map
                                        {
                                            ...
                                            // SourceFrontier events are generated by an inspect
                                            // operator that is dropped at the same time as the
                                            // dataflow so generate retractions for the last value
                                            // received.
                                            source_frontier_session.give_iterator(
                                                cur_time.drain().map(|(logical, delta)| {
                                                    (
                                                        Row::pack_slice(&[
                            ...
                            ComputeEvent::Frontier(name, logical, delta) => {
                                ...
                                if delta > 0 {
                                    ...
                                    {
                                        for (source_id, (time_deque, delay_map, _)) in source_map {
                            ...
                            ComputeEvent::SourceFrontier(dataflow, source_id, logical, delta) => {
                                ...
                                let source_map =
                                    storage_sources.entry((dataflow, worker)).or_default();
                                let (time_deque, _delays, cur_time) =
                                    source_map.entry(source_id).or_default();
                                cur_time.update(logical, i64::from(delta));
                                if delta > 0 {
