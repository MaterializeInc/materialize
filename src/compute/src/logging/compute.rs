...
//        let (mut source_frontier_delay_out, source_frontier_delay) = demux.new_output::<(GlobalId, GlobalId, usize, u128)>();
        ...
        demux.build(move |_capability| {
            ...
            let mut storage_sources = HashMap::<(GlobalId,usize),HashMap<(GlobalId,usize),(VecDeque<(u64, u128)>, HashSet<u128>)>>::new();
            move |_frontiers| {
                            ...
                            ComputeEvent::Frontier(name, logical, delta) => {
                                ...
                                if delta > 0 {
                                    ...
                                    if let Some(source_map) = storage_sources.get_mut(&dataflow_key) {
                                        ...
                                        for (source_key, time_entry) in source_map.iter_mut()  {
                                            ...
                                            while let Some(current_front) = time_deque.pop_front() {
                                                if logical >= current_front.0 {
                                                    ...
                                                    let delay_set = &mut time_entry.1;
                                                    delay_set.insert(delay_ns);
                                                    // TODO report delay!
                                                    println!(
                                                        "delay reported as:({}, {}, {}, {})",
                                                        name, source_key.0, worker, delay_ns
                                                    );
                                                } else  {
