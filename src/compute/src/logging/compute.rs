...
impl DemuxHandler<'_, '_, '_> {
    ...
    ///
    /// A stage already logged for this export is ignored, so a caller that cannot cheaply tell
    /// whether it has already reported one does not have to.
    fn log_lifecycle_at(
        &mut self,
        export_id: GlobalId,
        stage: LifecycleStage,
        ...
        let Entry::Vacant(entry) = export.lifecycle_rows.entry(stage) else {
            return;
        };
        let update = entry.insert(update);
