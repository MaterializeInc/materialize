...
use mz_timely_util::operator::{consolidate_pact, Bifurcate};
...
use crate::logging::{
    ...
    OutputSessionColumnar, PermutedRowPacker, SharedLoggingState, Update,
};
use crate::row_spine::RowRowBuilder;
use crate::typedefs::{KeyBatcher, RowRowSpine, VecKeyBatcher};
...
pub(super) fn construct<G: Scope<Timestamp = Timestamp>>(
    ...
    scope.scoped("differential logging", move |scope| {
        ...
        // Handle batcher_size differently because we need to fork it. Sorry for the wall of text.
        let (batcher_size_rows, batcher_size) = {
            let worker_id = scope.index();
            let log: LogVariant = BatcherSize.into();
            let c_name = &format!("Consolidate {log:?}");
            let u_name = &format!("ToRow {log:?}");
            let mut packer = PermutedRowPacker::new(log);
            let consolidated = consolidate_pact::<VecKeyBatcher<_, _, _>, _, _>(&batcher_size, Pipeline, c_name);
            let (consolidated, batcher_size_rows) = consolidated.bifurcate::<ColumnBuilder<_>, _>(u_name, move |data, session| {
            ...
            let batcher_size = consolidated.unary(Pipeline, "Unpack consolidated batcher_size", |_, _| move |input, output| {
                input.for_each(|time, data| {
                    for container in data.into_iter().flatten() {
                        output.session(&time).give_container(container);
                    }
                });
            });
            (batcher_size_rows, batcher_size)
        };
