...
pub(super) fn construct(
    ...
    scope.scoped("timely logging", move |scope| {
        ...
        // Like `channels`, summaries carry a `String` (which does not implement `Columnation`),
        // so we encode to `Row` via a columnar `unary` rather than `consolidate_and_pack`.
        let summaries = summaries.unary::<ColumnBuilder<_>, _, _, _>(
            Pipeline,
            "ToRow Summaries",
            |_cap, _info| {
                let mut packer = PermutedRowPacker::new(TimelyLog::Summary);
                move |input, output| {
                    input.for_each_time(|time, data| {
                        let mut session = output.session_with_builder(&time);
                        for d in data.flat_map(|c| c.borrow().into_index_iter()) {
                            let ((datum, ()), time, diff) = d;
                            let (id, input_port, output_port, outer_impact, detail) = datum;
                            let data = packer.pack_slice(&[
                                Datum::UInt64(u64::cast_from(id)),
                                Datum::UInt64(u64::cast_from(worker_id)),
                                Datum::UInt64(u64::cast_from(input_port)),
                                Datum::UInt64(u64::cast_from(output_port)),
                                Datum::from(outer_impact),
                                Datum::String(std::str::from_utf8(detail).expect("valid string")),
                            ]);
                            session.give((data, time, diff));
                        }
                    });
...
struct DemuxOutput<'a, 'b> {
    ...
    /// `(operator_id, input_port, output_port, outer_impact, summary_detail)`. Columnar (like
    /// `channels`) because the `String` detail does not implement `Columnation`.
    summaries: OutputSessionColumnar<'a, 'b, Update<(SummaryDatum, ())>>,
}

/// Per-operator internal connectivity summary row:
/// `(operator_id, input_port, output_port, outer_impact, summary_detail)`.
type SummaryDatum = (usize, usize, usize, Timestamp, String);
