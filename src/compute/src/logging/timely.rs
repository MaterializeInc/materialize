        ...
        for (variant, collection) in logs {
            if config.active_logs.contains_key(&variant) {
                ...
                let trace = rows
                    ...
                    .trace;
                result.insert(variant.clone(), (trace, dataflow_token.clone()));
            }

            if let Some((id, meta)) = config.sink_logs.get(&variant) {
                tracing::debug!("Persisting {:?} to {:?}", &variant, meta);
                persist_sink(*id, meta, compute_state, &dataflow_token, collection);
            }
        }
        result
    });
