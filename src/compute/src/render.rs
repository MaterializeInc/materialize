...
mod interactive_import_tests {
    ...
    use crate::arrangement::manager::TraceBundle;
    use crate::compute_state::CommandHold;
    use crate::extensions::arrange::{KeyCollection, MzArrange};
    use crate::shared_trace::PublishArrangement;
    ...
    /// Like [`publish_index_with_writer`], but returns the writer side as a [`TraceBundle`] over both
    /// the `oks` and `errs` traces, which is what `ComputeState::traces` holds for a maintained index
    /// and what a command-acquired hold clones from.
    ///
    /// Driving compaction through this bundle, rather than through a bare `oks` handle, is what makes
    /// the base's frontier the controller's frontier on both traces, which is the precondition
    /// `CommandHold::acquire` checks.
    fn publish_index_with_bundle(
        scope: Scope<'_, Timestamp>,
        registry: &ArrangementSharingRegistry,
        id: GlobalId,
        rows: Vec<(Row, Row)>,
        ...
        let bundle = TraceBundle::new(oks.trace.clone(), errs.trace.clone());
        ...
        (oks_input, errs_input, bundle)
    }

    /// Like [`tick`], but advances the `errs` input alongside the `oks` one.
    ///
    /// An index's two arrangements are sealed at the same frontier, so an import of one advances in
    /// step with an import of the other. Ticking `oks` alone leaves the `errs` import stuck at its
    /// `as_of`, which is invisible to a test that only reads the `oks` side but pins any hold that
    /// waits on both.
    fn tick_both(
        worker: &mut timely::worker::Worker,
        oks_input: &mut InputSession<Timestamp, (Row, Row), Diff>,
        errs_input: &mut InputSession<Timestamp, crate::render::errors::DataflowErrorSer, Diff>,
        at: Timestamp,
        next: Timestamp,
    ) {
        errs_input.advance_to(next);
        errs_input.flush();
    ...
    fn allow_compaction(
        ...
        base.errs_mut().set_physical_compaction(to.borrow());
    }

    /// A command-acquired hold admits a read the published `since` would otherwise refuse.
    ///
    /// This is the whole point of the mechanism, and the counterpart of
    /// [`import_asserts_since_at_most_as_of`]: that test drives the identical controller advance with
    /// no hold acquired and asserts the import panics. Acquiring the hold first, as the multiplexer
    /// arranges by putting `AcquireHolds` ahead of the `AllowCompaction` in this runtime's stream,
    /// must instead leave `as_of` readable and the rows at `as_of` accurate.
    ///
    /// A hold registered the ordinary way cannot do this. The publisher forwards the meet of the
    /// registrations into its own agent and that setter joins, so once the agent has followed the
    /// controller to 10 it can no longer represent 1. The acquired hold clones from the controller's
    /// own handle, which still sits at 0 when the acquisition is applied.
    #[mz_ore::test]
    fn command_hold_admits_read_below_writer_frontier() {
        let id = GlobalId::User(1);
        let rows = test_rows();
        let mut expected: Vec<(Row, Row)> = rows.clone();
        expected.sort();
        ...
        timely::execute_directly(move |worker| {
            let (mut oks_input, mut errs_input, mut base) =
                worker.dataflow::<Timestamp, _, _>(|scope| {
                    publish_index_with_bundle(scope, &registry, id, rows.clone())
                });

            // The acquisition, applied while the base still sits at the controller's initial
            // frontier. `_hold` must stay alive for the rest of the closure: it is the pin.
            let slot = registry.get_or_create_placeholder(id, 0, 1);
            let _hold = CommandHold::acquire(&mut base, slot, as_of.clone())
                .expect("the base has not compacted past as_of");

            // The controller now allows compaction well past `as_of`, exactly as in
            // `import_asserts_since_at_most_as_of`.
            let target = Antichain::from_elem(Timestamp::from(10_u64));
            allow_compaction(&registry, id, &mut base, &target);
            tick_both(
                worker,
                &mut oks_input,
                &mut errs_input,
            ...
            // The import succeeds where the same import panics without the hold, and the rows it
            // reconstructs are the accurate accumulation at `as_of` rather than coalesced data.
            //
            // `until` bounds the read to the single time `as_of`, as an interactive one-shot read
            // does, so the filler update `tick` writes above `as_of` stays out of the result and the
            // assertion is about the accumulation at `as_of` alone.
            let probe = ProbeHandle::new();
            let until = Antichain::from_elem(as_of_time.step_forward());
            worker.dataflow::<Timestamp, _, _>(|scope| {
                let (oks_arranged, _errs_arranged, _slot) =
                    import_shared_index(scope.clone(), &registry, id, "Index", &as_of, &until);
                let collected = Arranged::<SharedOksFrontier>::flat_map_batches(
                    oks_arranged.stream,
                    |k: DatumSeq, v: DatumSeq| {
                        let key = Row::pack_slice(&k.into_iter().collect::<Vec<_>>());
            ...
            let mut steps = 0;
            while probe.less_equal(&as_of_time) {
                worker.step();
                steps += 1;
                assert!(steps < 1_000, "the bounded read never completed");
            }
        });
    ...
    /// A command-acquired hold does not downgrade while the dataflow it was acquired for has not been
    /// built.
    ///
    /// The two runtimes' command streams are independent, so the acquisition routinely lands long
    /// before the importing dataflow exists, and for a cancelled or refused create it never exists.
    /// Treating "no registration" as "every reader has finished" would release the hold in exactly
    /// that window and make the whole mechanism inert: the controller's compaction would then run
    /// ahead and the import would find `as_of` gone.
    #[mz_ore::test]
    fn command_hold_waits_for_its_reader_to_register() {
        let id = GlobalId::User(1);
        let rows = test_rows();
        let as_of_time = Timestamp::from(1_u64);
        let as_of = Antichain::from_elem(as_of_time);
        ...
        timely::execute_directly(move |worker| {
            ...
            let slot = registry.get_or_create_placeholder(id, 0, 1);
            let mut hold = CommandHold::acquire(&mut base, Arc::clone(&slot), as_of.clone())
                .expect("the base has not compacted past as_of");
            ...
            // No import has registered, so there is no reader progress to follow. Asserted before any
            // probe handle is minted, since minting one registers a hold of its own.
            assert_eq!(
                hold.downgrade(),
                None,
                "a hold whose dataflow has not registered must not downgrade"
            );
            assert_eq!(hold.frontier(), &as_of);
            // The recorded frontier and the backing handle's must agree: the published `since` is
            // derived from the record, so a record that drifted would publish a frontier the trace
            // does not hold.
            assert_eq!(slot.oks.command_holds(), vec![as_of.clone()]);
            assert_eq!(slot.errs.command_holds(), vec![as_of.clone()]);

            let (probe_oks, _probe_errs) = registry.handles(&id, 0).expect("still published");
            assert!(
                probe_oks.snapshot_at(&as_of_time).is_some(),
                "the acquired hold must keep `as_of` readable across the controller's advance"
            );
        });
    }

    /// A command-acquired hold follows its reader's progress, and stops holding once the reader has
    /// advanced.
    ///
    /// Without this the hold is a permanent pin for the life of the importing dataflow. An interactive
    /// `SUBSCRIBE` lives as long as its client, so its index would never compact again, which is the
    /// same unbounded-growth failure that a reader hold nobody downgrades produces.
    ///
    /// The floor at the acquisition frontier is what makes following safe without tracking which
    /// registrations belong to which holder: the meet is at or below every registration, so flooring
    /// it cannot carry the hold past this dataflow's own reader.
    #[mz_ore::test]
    fn command_hold_downgrades_with_its_reader() {
        let id = GlobalId::User(1);
        let rows = test_rows();
        let as_of_time = Timestamp::from(1_u64);
        let as_of = Antichain::from_elem(as_of_time);
        ...
        timely::execute_directly(move |worker| {
            ...
            let slot = registry.get_or_create_placeholder(id, 0, 1);
            let mut hold = CommandHold::acquire(&mut base, slot, as_of.clone())
                .expect("the base has not compacted past as_of");

            allow_compaction(&registry, id, &mut base, &target);

            // The importing dataflow, keeping its traces so this is a downgrade rather than a release.
            let (mut oks_trace, mut errs_trace) = worker.dataflow::<Timestamp, _, _>(|scope| {
                let (oks_arranged, errs_arranged, _slot) = import_shared_index(
                    scope.clone(),
                    &registry,
            ...
            // Its reader has not advanced, so the hold stays and `as_of` stays readable.
            assert_eq!(hold.downgrade(), None);
            {
                let (probe_oks, _) = registry.handles(&id, 0).expect("still published");
                assert!(
                    probe_oks.snapshot_at(&as_of_time).is_some(),
                    "a hold whose reader has not advanced must keep `as_of` readable"
                );
            }

            // The reader advances, as a join does once its other input has. The import leaves two
            // registrations behind: the `Arranged`'s own trace, downgraded here, and the source
            // operator's, which follows the frontier it has acknowledged. The meet only moves once
            // both have, so the tick that carries the published seal past `target` is part of the
            // reader advancing, not incidental setup.
            oks_trace.set_logical_compaction(target.borrow());
            errs_trace.set_logical_compaction(target.borrow());
            tick_both(
                worker,
            ...
            assert_eq!(
                hold.downgrade(),
                Some(target.clone()),
                "the hold must follow its reader"
            );

            let (compacted_oks, _) = registry.handles(&id, 0).expect("still published");
            assert!(
                compacted_oks.snapshot_at(&as_of_time).is_none(),
                "after the hold followed its reader, the trace must compact past the original as_of"
            );
            assert!(
                compacted_oks
                    .snapshot_at(&Timestamp::from(10_u64))
                    .is_some(),
                "the frontier the hold moved to must still be readable"
            );
            drop((oks_trace, errs_trace));
        });
    }

    /// Dropping a command-acquired hold frees the collection to compact.
    ///
    /// The published `since` is derived from the recorded holds, so a hold that deregistered without
    /// its trace handle releasing, or the reverse, would leave `since` disagreeing with the trace.
    /// This pins the visible half of that: once the hold is gone, the frontier it protected is gone
    /// too.
    #[mz_ore::test]
    fn command_hold_release_frees_compaction() {
        let id = GlobalId::User(1);
        let rows = test_rows();
        let as_of_time = Timestamp::from(1_u64);
        let as_of = Antichain::from_elem(as_of_time);
        ...
        timely::execute_directly(move |worker| {
            ...
            let slot = registry.get_or_create_placeholder(id, 0, 1);
            let hold = CommandHold::acquire(&mut base, slot, as_of.clone())
                .expect("the base has not compacted past as_of");
            ...
            // Readable while held. Without this half the test would pass on a hold that never worked
            // at all, since a read that was never protected also fails after the drop.
            {
                let (probe_oks, _) = registry.handles(&id, 0).expect("still published");
                assert!(
                    probe_oks.snapshot_at(&as_of_time).is_some(),
                    "the acquired hold must keep `as_of` readable while it is held"
                );
            }
            ...
            let (released_oks, _) = registry.handles(&id, 0).expect("still published");
            assert!(
                released_oks.snapshot_at(&as_of_time).is_none(),
                "after the acquired hold drops, the trace must be free to compact past `as_of`"
            );
        });
    }
