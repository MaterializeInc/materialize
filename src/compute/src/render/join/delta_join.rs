...
use differential_dataflow::trace::cursor::{
    BatchCursor, BatchKey, BatchTimeGat, BatchVal, Navigable,
};
        ...
        let (oks, errs) = self.scope.clone().region_named("Join(Delta)", |inner| {
            ...
            for path_plan in join_plan.path_plans {
                ...
                let path_results = inner.clone().region_named(&name, |region| {
                    ...
                    update_stream.leave_region(inner)
                });
...
where
    ...
    BatchCursor<Tr>: Cursor<Time = T, Diff = Diff>,
    <BatchCursor<Tr> as Cursor>::KeyContainer: BatchContainer<Owned = Row>,
    for<'a> BatchVal<'a, Tr>: ExtendDatums,
    CF: Fn(BatchTimeGat<'_, Tr>, &T) -> bool + 'static,
{
...
where
    T: RenderTimestamp,
    for<'a, 'b> &'a T: PartialEq<BatchTimeGat<'b, Tr>>,
    Tr: TraceReader<Batch: Navigable, Time = T> + Clone + 'static,
    BatchCursor<Tr>: Cursor<Time = T, Diff = Diff>,
    for<'a> BatchKey<'a, Tr>: ExtendDatums,
    for<'a> BatchVal<'a, Tr>: ExtendDatums,
{
    let mut inner_as_of = Antichain::new();
    for event_time in as_of.elements().iter() {
        inner_as_of.insert(<T>::to_inner(event_time.clone()));
    }
