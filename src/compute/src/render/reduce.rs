...
use crate::typedefs::{
    ErrBatcher, RowAgent, RowErrBuilder, RowErrSpine, RowRowAgent, RowRowArrangement, RowRowSpine,
...
impl<'scope, T: RenderTimestamp> Context<'scope, T> {
    ...
    fn render_reduce_plan<'s>(
        ...
        CollectionBundle::from_columns(
            ...
            ArrangementFlavor::Local(
                arrangement,
                errs.mz_arrange::<ErrBatcher<_, _, ColumnationChunker<_>>, _>(
                    "Arrange bundle err",
                    MergeBatcher::new,
                ),
            ),
    ...
    fn build_basic_aggregates<'s>(
        ...
        let arranged = differential_dataflow::collection::concatenate(input.scope(), to_collect)
            .mz_arrange::<RowValBatcher<_, _, _, ColumnationChunker<_>>, RowValSpine<_, _, _>>(
        ...
        Arranged<'s, TraceAgent<Tr>>: ArrangementSize,
    {
        ...
        let arranged = input.mz_arrange::<RowBatcher<_, _, ColumnationChunker<_>>, RowSpine<_, _>>(
    ...
    fn build_monotonic<'s>(
        ...
        type MonoidSpine<T> = RowSpine<T, Vec<ReductionMonoid>>;
        let arranged = partial
            .mz_arrange::<RowBatcher<_, _, ColumnationChunker<_>>, MonoidSpine<_>>(
    ...
    fn build_accumulable<'s>(
        ...
        if ENABLE_COLUMNAR_ACCUMULABLE_DIFF.get(&self.config_set) {
            let arranged = collection.mz_arrange::<
                RowBatcher<_, _, ColumnationChunker<_>, Coltainer<_>>,
