...
impl DeferredStorageCommands {
    ...
    /// The number of commands currently held back.
    fn held(&self) -> usize {
...
impl<'w> Worker<'w> {
    ...
    fn dispatch_storage_internal_command(&mut self, cmd: InternalStorageCommand) {
        ...
        match guest.deferred_commands.admit(cmd) {
            Some(cmd) => self.handle_storage_internal_command(cmd),
            None => info!(
                worker = self.timely_worker.index(),
                held = guest.deferred_commands.held(),
    ...
    fn release_deferred_storage_commands(&mut self) {
        ...
        let released = guest.deferred_commands.release();
        info!(
            worker = self.timely_worker.index(),
            count = released.len(),
            "released storage commands deferred until compute logging initialization"
        );
        for cmd in released {
