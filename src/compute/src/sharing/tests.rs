...
fn shared_trace_flavor_feeds_join_and_reduce() {
    ...
    let b: Vec<Update> = vec![(1, "x", 0, 1), (2, "y", 1, 1), (3, "z", 2, 1)];
    let seal = 3;
    // Read as of `seal - 1`, one tick below the sealed upper `{seal}`: `import_frontier_core`
    ...
    timely::execute_directly(move |worker| {
        ...
        worker.dataflow::<Timestamp, _, _>(|scope| {
            ...
            let arr_a =
                oks_a.import_frontier_core(scope.clone(), "import A", as_of.clone(), until.clone());
            let err_a = errs_a.import_frontier_core(
                scope.clone(),
                "import A errs",
                as_of.clone(),
                until.clone(),
            ...
            let arr_b =
                oks_b.import_frontier_core(scope.clone(), "import B", as_of.clone(), until.clone());
            let err_b = errs_b.import_frontier_core(scope.clone(), "import B errs", as_of, until);
