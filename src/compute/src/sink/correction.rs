...
//! The `Correction` data structure used by the MV sink's `write_batches` operator to stash
//! updates before they are written.
//!
...
//!
//! [`Correction`] holds onto a list of `Chain`s containing `Chunk`s of stashed updates. Each
...
use mz_compute_types::dyncfgs::{CORRECTION_V2_CHAIN_PROPORTIONALITY, CORRECTION_V2_CHUNK_SIZE};
...
use crate::sink::correction::logging::{ChannelLogging, SizeMetrics};
...
///
/// Updates are stored in columnar chunks whose memory can be transparently spilled to disk.
#[derive(Debug)]
pub struct Correction<D: Data> {
...
impl<D: Data> Correction<D> {
    /// Construct a new [`Correction`] instance, tuned by the given configuration.
    pub fn new(
        metrics: SinkMetrics,
        worker_metrics: SinkWorkerMetrics,
        logging: Option<ChannelLogging>,
        ...
        let chain_proportionality = CORRECTION_V2_CHAIN_PROPORTIONALITY.get(config);
    ...
    /// Construct a new [`Correction`] instance with explicit tuning parameters.
    ///
    /// `chain_proportionality` is the size factor between subsequent chains, `chunk_size` the
...
impl<D: Data> Drop for Correction<D> {
    fn drop(&mut self) {
        for bucket in self.chain.buckets() {
...
struct Chunk<D: Data> {
    ...
    /// `Rc`, and the iterator returned by [`Correction::updates_before`] borrows them across
...
mod tests {
    ...
    /// A naive correction buffer, used as a reference for the real implementation.
    ///
    /// Keeps every update in a flat vector and does all the work (time advancement,
    /// consolidation, filtering) on read. Obviously too slow for production, but simple enough
    /// to be evidently correct.
    struct ReferenceCorrection {
        updates: Vec<(String, Timestamp, Diff)>,
        since: Antichain<Timestamp>,
    }

    impl ReferenceCorrection {
        fn new() -> Self {
            Self {
                updates: Vec::new(),
                since: Antichain::from_elem(Timestamp::MIN),
        ...
        fn insert(&mut self, updates: &mut Vec<(String, Timestamp, Diff)>) {
            let Some(since_ts) = self.since.as_option() else {
                updates.clear();
                return;
            };
            for (d, t, r) in updates.drain(..) {
                self.updates.push((d, std::cmp::max(t, *since_ts), r));
            }
        }

        fn insert_negated(&mut self, updates: &mut Vec<(String, Timestamp, Diff)>) {
            for (_, _, r) in &mut *updates {
                *r = -*r;
            }
            self.insert(updates);
        }
        ...
        fn advance_since(&mut self, since: Antichain<Timestamp>) {
            ...
            match since.as_option() {
                Some(ts) => {
                    for (_, t, _) in &mut self.updates {
                        *t = std::cmp::max(*t, *ts);
                    }
                }
                None => self.updates.clear(),
            }
            self.since = since;
        }

        fn updates_before(&self, upper: &Antichain<Timestamp>) -> Vec<(String, Timestamp, Diff)> {
            let mut out: Vec<_> = self
                .updates
                .iter()
                .filter(|(_, t, _)| !upper.less_equal(t))
                .cloned()
                .collect();
    ...
    /// Run the same stepwise-drain workload through the reference implementation and
    /// `Correction` and assert that they emit the same updates at every step.
    ///
    ...
    fn equivalence_with_reference() {
        ...
        let mut reference = ReferenceCorrection::new();
        let mut correction = Correction::<String>::with_params(
            sink_metrics.clone(),
        ...
        for t in 0..num_ts {
            reference.insert(&mut batch(t));
            correction.insert(&mut batch(t));
        }
        ...
        for t in 0..num_ts {
            ...
            let mut expected = reference.updates_before(&upper);
            let mut actual: Vec<_> = correction.updates_before(&upper).collect();
            expected.sort();
            actual.sort();
            assert_eq!(expected, actual, "diverged at t={t}");
            // Guard against the comparison passing because both sides emit nothing.
            assert!(!actual.is_empty(), "no updates emitted at t={t}");

            reference.insert_negated(&mut expected.clone());
            correction.insert_negated(&mut actual);
            reference.advance_since(upper.clone());
    ...
    fn since_jump() {
        ...
        let upper = Antichain::from_elem(Timestamp::from(num_ts + 1));
        let out: Vec<_> = correction.updates_before(&upper).collect();
    ...
    fn upper_not_beyond_since() {
        ...
        correction.advance_since(Antichain::from_elem(Timestamp::from(10_u64)));
        ...
        assert_eq!(correction.updates_before(&upper).count(), 0);
