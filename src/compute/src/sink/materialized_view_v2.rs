...
mod write {
    ...
    /// stops it from polling every other task scheduled on it, including the persist lease
    /// heartbeats whose expiry costs readers their leases. It therefore runs on a blocking
    ...
    mod tests {
        ...
        ///
        /// When it ran inline on the write task, a correction pass over a large buffer pinned a
        /// worker for its whole duration and stopped it polling everything else scheduled there,
        /// including the persist lease heartbeats whose expiry costs readers their leases. Under
        /// memory pressure the pass is not merely slow but blocked in the kernel, paging chunks
        /// back in, so it cannot be fixed by yielding.
        ///
        /// Stalls every worker thread inside `apply_batch` and asserts an unrelated task keeps
        /// getting polled throughout. Moving the `apply_batch` body back out of `spawn_blocking`
        /// fails this.
