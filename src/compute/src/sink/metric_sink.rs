...
impl<'scope> SinkRender<'scope> for MetricSinkConnection {
    fn render_sink(
        ...
        // That drop is what frees the `Desc` id for the next incarnation, which is retrying.
        None
    }
}
...
///
/// Fixed, no backoff: the collision is bounded by the old dataflow's teardown, so it converges
/// without an unbounded or growing wait.
...
impl PendingRegistration {
    /// Registers the collector if it is not registered yet, returning whether it is now registered.
    ///
    /// On a collision nothing is registered, the retry counter is bumped, and a later call may
    /// retry with the same collector.
    fn try_register(&mut self, registry: &MetricsRegistry, metrics: &WorkerMetrics) -> bool {
        ...
        match registry.try_register_collector_with_dropper(self.collector.clone()) {
            ...
            Err(err) => {
                ...
                // Only the first failure logs. The counter carries the ongoing signal, and a
                // contended registration retries once a second for as long as it takes.
                if !self.logged {
                    self.logged = true;
                    tracing::info!(
                        sink = %self.label,
                        "metric sink collector registration collided, retrying: {err}"
                    );
                }
                false
            }
