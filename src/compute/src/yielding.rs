...
impl Budget {
    ...
    pub fn allowance(&self) -> usize {
        // The time bound has to be checked here and not only in `is_spent`,
        // otherwise an expired budget keeps handing out full allowances. That
        // matters most for a shared budget: a nested allowance is the minimum
        // of the two, so items reached after the shared deadline passed would
        // each still get a full turn, and the time bound would not cap the
        // round at all.
        if self.expired {
            return 0;
        }
...
mod tests {
    ...
    #[mz_ore::test]
    fn expired_budget_hands_out_no_allowance() {
        let spec = YieldSpec {
            after_work: None,
            after_time: Some(Duration::ZERO),
        };
        ...
        // These two have to agree, so that a caller can drive off either one.
        assert!(budget.is_spent());
        assert_eq!(budget.allowance(), 0);
    }

    #[mz_ore::test]
    fn expired_shared_budget_leaves_nothing_to_nest() {
        let shared_spec = YieldSpec {
            after_work: None,
            after_time: Some(Duration::ZERO),
        };
        let own_spec = YieldSpec {
            after_work: Some(100),
            after_time: None,
        };

        let mut shared = Budget::new(&shared_spec);
        shared.charge(Budget::CLOCK_INTERVAL);
        assert!(shared.is_spent());

        // A nested allowance is the minimum of the two. If the shared one kept
        // reporting an allowance after its deadline passed, every item reached
        // afterwards would still get a full turn and the shared time bound
        // would not cap the round.
        let nested = shared.nest(&own_spec);
        assert!(nested.is_spent());
        assert_eq!(nested.allowance(), 0);
    }
