...
//! `mz_compute::sharing`'s tests), then panics on the interactive one. This stays hermetic, needing
//! no persist or network setup, while exercising the exact hook wiring `clusterd::main` uses.
