...
use expr::{
    GlobalId, Id, IdHumanizer, PartitionId, RelationExpr, RowSetFinishing, ScalarExpr,
    SourceInstanceId,
};
use ore::collections::CollectionExt;
use ore::thread::JoinHandleExt;
use repr::{ColumnName, Datum, RelationDesc, RelationType, Row, ScalarType};
    ...
    C: comm::Connection,
{
    ...
    fn sequence_peek(
        ...
        // HACK(benesch): look for a `mz_logical_time` placeholder, which
        // presents as a decimal with an impossible precision.
        source.visit_literals_mut(&mut |res, typ| {
            if let ScalarType::Decimal(255, _) = &typ.scalar_type {
                *res = Ok(Row::pack(&[Datum::from(timestamp as i128)]));
            }
        });
