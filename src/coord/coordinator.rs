    ...
    pub fn create_dataflows(&mut self, mut dataflows: Vec<DataflowDesc>) {
        for dataflow in dataflows.iter_mut() {
            ...
            for view in dataflow.views.iter_mut() {
                ...
                // In fact, we do all keys here. But if the view is a mirror of a source
                // there should just be a single key, which would be its primary key.
                for (index, key) in typ.keys.into_iter().enumerate() {
                    broadcast(
                        &mut self.broadcast_tx,
                        SequencedCommand::AppendLog(MaterializedEvent::PrimaryKey(
                            view.name.to_string(),
