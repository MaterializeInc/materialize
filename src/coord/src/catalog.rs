...
impl Catalog {
    ...
    ///
    /// A "time domain" is a set of relations (tables, views) that need to share
    /// time guarantees. Currently we assume time domains to mean "everything in a
    /// schema". For example, a read transaction on table A in some schema may also
    /// need to query table B in the same schema, so A and B (and all other tables
    /// and views in the schema) are in the same time domain.
    pub fn timedomain_for(&self, source: &MirRelationExpr, conn_id: u32) -> Vec<GlobalId> {
        ...
        for id in source.global_uses() {
            ...
            if let Some(schema) = self.get_schema(&name.database, &name.schema, conn_id) {
                for id in schema.items.values() {
                    if let SqlCatalogItemType::Table | SqlCatalogItemType::View =
                        self.get_by_id(id).item_type()
                    {
                        ids.insert(*id);
                    }
                }
