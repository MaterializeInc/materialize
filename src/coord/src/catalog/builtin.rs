...
JOIN (
    SELECT type_id, 'a' AS typtype FROM mz_catalog.mz_array_types
        UNION SELECT type_id, 'b' FROM mz_catalog.mz_base_types
        UNION SELECT type_id, 'l' FROM mz_catalog.mz_list_types
        UNION SELECT type_id, 'm' FROM mz_catalog.mz_map_types
    ) AS t
    ON mz_types.id = t.type_id",
