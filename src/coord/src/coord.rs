...
impl Coordinator {
    ...
    fn advance_local_inputs(&mut self) {
        ...
        if next_ts > self.closed_up_to {
            ...
            for res in rx {
                if let Err(err) = res {
                    // TODO: Linearizibility relies on this, bubble up the error instead.
    ...
    async fn send_builtin_table_updates_at_offset(
        ...
        for (id, updates) in &updates.into_iter().group_by(|u| u.id) {
            ...
            if let Some(persist) = persist {
                ...
                // Persistence takes care of introducing anything it writes to
                // the dataflow, so we only need a SequencedCommand::Insert for
                // the volatile updates.
...
pub fn serve_debug(
    ...
    let persist = PersistConfig {
        buffer_path: catalog_path.join("buffer"),
        blob_path: catalog_path.join("buffer"),
        user_table_enabled: false,
        lock_info: "serve_debug".to_owned(),
    };
    let persist = persist.init().unwrap();
