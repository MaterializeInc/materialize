...
impl Coordinator {
    ...
    async fn sequence_peek(
        ...
        let timestamp = if in_transaction && when == PeekWhen::Immediately {
            session.get_transaction_timestamp(|| {
                ...
                // and add all tables and views in those schemas to a set.
                let mut ids = HashSet::new();
                for id in source.global_uses().iter() {
                    let entry = self.catalog.get_by_id(&id);
                    let name = entry.name();
                    if let Some(schema) =
                        self.catalog
                            .get_schema(&name.database, &name.schema, conn_id)
                    {
                        for id in schema.items.values() {
                            if let CatalogItemType::Table | CatalogItemType::View =
                                self.catalog.get_by_id(id).item_type()
                            {
                                ids.insert(*id);
                            }
                        }
    ...
    async fn sequence_tail(
        ...
        // Ensure this is the only non AS OF TAIL.
        if ts.is_none() {
