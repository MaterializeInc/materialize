...
pub enum CoordError {
    ...
    /// The read transaction timestamps differed.
    ReadTimestampsDiffered,
...
impl fmt::Display for CoordError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            CoordError::ReadTimestampsDiffered => {
                f.write_str("differing read timestamps in transaction")
            }
            CoordError::SqlCatalog(e) => e.fmt(f),
            CoordError::TailOnlyTransaction => {
                f.write_str("TAIL in transactions must be only read statement")
            }
