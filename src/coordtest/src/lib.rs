...
//! and we aren't able to observe output of them until we bump a timestamp and
...
pub struct CoordTest {
    inner_feedback_tx: mpsc::UnboundedSender<WorkerFeedbackWithMeta>,
    client: Option<Client>,
    _handle: JoinOnDropHandle<()>,
    inner_feedback_rx: mpsc::UnboundedReceiver<WorkerFeedbackWithMeta>,
...
impl CoordTest {
    pub async fn new() -> anyhow::Result<Self> {
        ...
        let (handle, client, inner_feedback_tx, inner_feedback_rx, timestamp) =
    ...
    async fn drain_uppers(&mut self, exclude: &HashSet<GlobalId>) {
        loop {
            if let Some(Some(mut msg)) = self.inner_feedback_rx.recv().now_or_never() {
                ...
                self.inner_feedback_tx.send(msg).unwrap();
            } else {
                return;
            }
        }
