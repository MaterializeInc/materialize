...
pub mod tcp {
    ...
    ///
    /// The client opens a connection using the ProtoComputeClient stubs that
    /// are generated by tonic from the service definition in `client.proto`. After creation, the client
    /// is in disconnected state. To connect it, `connect` has to be called. Once the client
    /// is connected, it will call automatically the only RPC defined in the service description: `Handle`.
    /// Handle sets up two streams: A command stream going from this instance to the server and a response
    /// stream coming back. Both of these streams are backed by one tokio mpsc queue each.
    /// The recv and send functions interact with those two mpsc channels to send and receive data.
    #[derive(Debug)]
            ...
            loop {
                match &mut self.state {
                    GrpcComputedTcpConn::Disconnected => {
                        info!("Attempt to connect to {}", &self.addr);
                    ...
                    GrpcComputedTcpConn::AwaitResponse(client) => {
                        ...
                        match client.handle(ReceiverStream::new(rx)).await {
                            Ok(resp_rx_wrap) => {
                            ...
                            Err(err) => {
                                info!(
                                    "GrpcComputedClient: Connection to {} refused: {}",
                                    &self.addr, err
                                );
    ...
    where
        T: ProtoSerializableTimestamp + 'static + std::fmt::Debug + Send,
    {
        async fn send(&mut self, cmd: ComputeCommand<T>) -> Result<(), anyhow::Error> {
    ...
    pub struct GrpcComputedShared {
        ...
        // stream implementation. `queue` is None if no client is connected, otherwise
        // the endpoints are forwarded to the single client. If a client is connecting but a client
        // is already connected, this mutex is used to block. If the consumer calls send or recv and queue
    ...
    impl super::proto_compute_server::ProtoCompute for GrpcComputedServer {
        ...
        async fn handle(
            ...
            info!("GrpcComputedServer: remote client connected");

            let mut in_stream = req.into_inner();
            // The channel sizes are arbitrary, but it should be a
            // small bounded channel such that backpressure is applied early.
            ...
            mz_ore::task::spawn(|| "ProtoComputeServer: ErrorChecker", async move {
                while let Some(result) = in_stream.next().await {
                    match result {
                        ...
                        Err(e) => {
                            info!("Client disconnected with error {}", e);
                            break;
                        }
                    }
                }
