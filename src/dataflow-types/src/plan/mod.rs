...
This is not expected to cause incorrect results, but could indicate a performance issue in Materialize.", missing);
    ...
    pub fn finalize_dataflow(
        ...
        for index in desc.index_imports.values() {
            let key = index.key.clone();
            // TODO[btv] - We should be told the permutation by
            // `index`, and it should have been generated
            ...
            // and recover it here.
            let (permutation, thinning) = permutation_for_arrangement(&key, index.on_type.arity());
            arrangements
                .entry(Id::Global(index.on_id))
