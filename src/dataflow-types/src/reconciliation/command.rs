    ...
    async fn absorb_command(&mut self, command: StorageCommand<T>) -> Result<(), anyhow::Error> {
        ...
        match command {
            ...
            RenderSources(sources) => {
                // TODO(jkosh44): This is a COMPUTE-STORAGE command so we're punting the
                //  reconciliation until later
            ...
            Append(appends) => {
                let mut filtered_appends = Vec::with_capacity(appends.len());
                for (id, mut updates, timestamp) in appends {
                    let frontier = self.uppers.get(&id).expect("Append into unknown source");
                    if frontier.less_than(&timestamp) {
                        updates = updates
                            .into_iter()
                            .filter(|update| frontier.less_than(&update.timestamp))
                            .collect();
                        filtered_appends.push((id, updates, timestamp));
                    }
                }
                self.client.send(Append(filtered_appends)).await
            }
