...
pub fn construct<A: Allocate>(
    ...
    let traces = worker.dataflow_named("Dataflow: mz logging", move |scope| {
        let (compute_logs, compute_token) = Some(compute).mz_replay(
            scope,
            "materialized logs",
            Duration::from_nanos(config.granularity_ns as u64),
            activator.clone(),
        );
        let (storage_logs, storage_token) = Some(storage).mz_replay(
            scope,
            "materialized logs",
            Duration::from_nanos(config.granularity_ns as u64),
            activator,
        ...
        for (variant, collection) in logs {
            if config.active_logs.contains_key(&variant) {
                ...
                let trace = collection
                    ...
                    .trace;
                let token: Rc<dyn Any> =
                    Rc::new((Rc::clone(&storage_token), Rc::clone(&compute_token)));
                result.insert(variant, (trace, token));
            }
        }
        result
    });
