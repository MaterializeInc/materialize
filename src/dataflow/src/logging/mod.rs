...
/// half of the buffer's capacity, at which point the buffer will be filled, once more consolidated
/// and shipped.
///
    ...
    P: Push<Bundle<T, (D, T, R)>> + 'a,
{
    ...
    pub fn give(&mut self, cap: &CapabilityRef<T>, data: (D, T, R)) {
        ...
        if self.buffer.len() == self.buffer.capacity() {
            ...
            if self.clean < self.buffer.capacity() / 2 {
                consolidate_updates(&mut self.buffer);
                self.clean = self.buffer.len();
            } else {
                // Consolidate once more and flush
                consolidate_updates(&mut self.buffer);
                self.flush();
            }
        }
