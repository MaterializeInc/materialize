    ...
    G: ScopeParent<Timestamp = Timestamp>,
{
    ...
    match envelope.mode {
        DebeziumMode::Upsert => {
            ...
            let key_arity = key_arity.expect(
                "SourceEnvelope::Debezium(DebeziumMode::Upsert) to \
                require SourceDataEncoding::KeyValue",
            );
            super::upsert::upsert(
                "gus",
                input,
                ...
                source_arity,
                None,
                move |row, row_packer| match row.iter().nth(after_idx).unwrap() {
                    Datum::List(after) => {
                        row_packer.clear();
                        row_packer.extend(after.iter().skip(key_arity));
                        Some(row_packer.finish_and_reuse())
                    }
                    Datum::Null => None,
                    d => panic!("type error: expected record, found {:?}", d),
                },
                move |current_values_len| gauge.set(current_values_len),
            )
        }
        _ => input
