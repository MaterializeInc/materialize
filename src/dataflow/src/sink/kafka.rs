...
impl KafkaSink {
    fn transition_on_error(&self, current_state: SendState, e: KafkaError) -> SendState {
        error!(
            "encountered error during kafka interaction. {} in state {:?} : {}",
            &self.name, current_state, e
        );
...
enum SendState {
    // Initialize ourselves as a transactional producer with Kafka; only runes once
    Init,
    ...
    G: Scope<Timestamp = Timestamp>,
{
    ...
    let s = {
        ...
        KafkaSink {
            ...
            producer,
            activator,
            txn_timeout: Duration::from_secs(3),
        }
    };
                ...
                state = match state {
                    SendState::Init => {
                        ...
                        match result {
                            ...
                            Err(e) => s.transition_on_error(state, e),
                        }
                    }
                    SendState::BeginTxn => {
                        let result = if transactional {
