...
fn batch_upper(
    ...
    match batch {
        Some(batch) => {
            // Break the antichain abstraction here by fetching the first element because
            // we know there won't ever be 2.
            let upper = match batch.desc.upper().elements().get(0) {
                Some(upper) => upper,
                None => return None,
            };
            Some(*upper)
