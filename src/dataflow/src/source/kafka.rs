...
use crate::source::{
    ConsistencyInfo, PartitionMetrics, RecordIter, SourceConstructor, SourceInfo, SourceMessage,
};
...
impl SourceInfo<Vec<u8>> for KafkaSourceInfo {
    ...
    fn read_persisted_files(&self, files: Vec<PathBuf>) -> Vec<(Vec<u8>, Timestamp, i64)> {
        files
            .iter()
            .flat_map(|f| {
                // TODO: only process files for partitions we know about.
    ...
    fn persist_message(
        ...
        if let Some(persistence_tx) = persistence_tx {
            ...
            let data = if let Some(payload) = &message.payload {
                payload.clone()
            } else {
                // TODO fix this
                panic!("payload was null, expected something")
            };

            let data = WorkerPersistenceData {
            ...
            block_on(connector.send(data)).unwrap();
        }
    }
}
