...
pub struct PartitionMetrics {
    ...
    last_ingested_offset: i64,
}

impl PartitionMetrics {
    /// Record the latest offset received high-water mark
    pub fn record_offset_received(&mut self, offset: i64) {
        ...
        if let Some(logger) = self.logger.as_mut() {
            logger.log(MaterializedEvent::SourceInfo(
                ...
                offset - self.last_received_offset,
                0,
            ));
        }
        self.last_received_offset = offset;
    }

    /// Record the latest offset ingested high-water mark
    pub fn record_offset_ingested(&mut self, offset: i64) {
        ...
        if let Some(logger) = self.logger.as_mut() {
            logger.log(MaterializedEvent::SourceInfo(
                ...
                self.partition_id.clone(),
                0,
                offset - self.last_ingested_offset,
            ));
        }
        self.last_ingested_offset = offset;
    }
                ...
                match source_info.get_next_message(&mut consistency_info, &activator) {
                    Ok(NextMessage::Ready(message)) => {
                        ...
                        consistency_info
                            ...
                            .get_mut(&partition)
                            .unwrap()
                            .record_offset_received(offset.offset);
                        ...
                        match ts {
                            ...
                            Some(ts) => {
                                ...
                                let partition_metrics = consistency_info
                                    ...
                                    .get_mut(&partition)
                                    .unwrap();
                                partition_metrics.record_offset_ingested(offset.offset);
