...
use timely::dataflow::{
    ...
    operators::{self, Capability, Event},
};
...
/// A thread-safe timestamp provider that allows consumers to pause ticking of the clock
...
impl Timestamper {
    ...
    pub async fn start_tx<'a>(&'a self) -> Transaction<'a> {
        Transaction {
    ...
    /// Records an error
    pub async fn error(&self, err: SourceError) -> anyhow::Result<()> {
...
pub trait SimpleSource {
    ...
    async fn start(self, timestamper: &Timestamper) -> anyhow::Result<()>;
}
        ...
        tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                    ...
                    res = &mut source => {
                        if let Err(err) = res {
                            let err = SourceError::FileIO(err.to_string());
                            if let Err(_) = timestamper.error(err).await {
                                break;
                            }
                        }
                        break;
