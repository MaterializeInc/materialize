...
impl SourceConstructor<Vec<u8>> for S3SourceInfo {
    fn new(
        ...
        consistency_info.partition_metrics.insert(
            PartitionId::S3,
            PartitionMetrics::new(&source_name, source_id, &s3_conn.bucket, logger.clone()),
        );
    ...
    if let Some(body) = obj.body {
        ...
        match reader.read_to_end(&mut buf).await {
            Ok(_) => {
                ...
                let mut lines = 0;
                for line in VecLinesIter::new(buf) {
        ...
        log::info!("get object response had no body: {}", pattern.as_str());
    }
}
...
impl VecLinesIter {
    fn new(inner: Vec<u8>) -> VecLinesIter {
        VecLinesIter { inner, idx: 0 }
    }
}

impl Iterator for VecLinesIter {
    type Item = Vec<u8>;
    fn next(&mut self) -> Option<Vec<u8>> {
        match self.inner[self.idx..].iter().position(|c| *c == b'\n') {
            Some(end_idx) => {
                let end = self.idx + end_idx;
                let new_vec = Vec::from(&self.inner[self.idx..end]);
                self.idx = end + 1;
                Some(new_vec)
            }
            None if self.idx < self.inner.len() => {
                let new_vec = Vec::from(&self.inner[self.idx..]);
...
mod test {
    ...
    #[test]
    fn vec_iter_well() {
        let expected: Vec<Vec<u8>> = vec![Vec::from(*b"hello"), Vec::from(*b"vecs")];
        let well_formatted = Vec::from(*b"hello\nvecs\n");

        let iter1 = VecLinesIter::new(well_formatted);
        assert_eq!(iter1.collect::<Vec<_>>(), expected);
    }

    #[test]
    fn vec_iter_poor() {
        let expected: Vec<Vec<u8>> = vec![Vec::from(*b"hello"), Vec::from(*b"vecs")];
        let no_trailing = Vec::from(*b"hello\nvecs");

        let iter_no = VecLinesIter::new(no_trailing);
        assert_eq!(iter_no.collect::<Vec<_>>(), expected);
    }
}
