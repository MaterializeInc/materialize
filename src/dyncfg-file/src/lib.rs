...
pub async fn sync_file_to_configset(
    ...
    .await
    {
        tracing::warn!("SyncedConfigSet initialize on boot: {}", err);
        return Err(err.into());
    }
...
mod tests {
    ...
    async fn test_file_sync() {
        ...
        // Start sync with non-existent file (should create it)
        ...
        // Verify file was created with default values
        let contents = fs::read_to_string(&config_file).unwrap();
        let values: BTreeMap<String, JsonValue> = serde_json::from_str(&contents).unwrap();
        assert_eq!(values.get("test_bool").unwrap(), &JsonValue::Bool(true));
        assert_eq!(
            values.get("test_string").unwrap(),
