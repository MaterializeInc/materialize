        ...
        McpEndpointType::Agent => {
            let mut tools = vec![
                ...
                ToolDefinition {
                    ...
                    description: "Get the complete schema and structure of a specific data product, plus a `foreign_keys` object giving the declared join paths to other data products (`references` for the ones this product points at, `referenced_by` for the ones pointing at it, each edge naming the other relation and the corresponding column pairs) and a `hydration` object reporting whether the dataflow is ready across the cluster's replicas (`{hydrated, replica_count, hydrated_replica_count}`). This shows you exactly what fields are available, their types, and what data you can query. Reads never return partial data, so check `hydration` before reading: if `hydrated` is false and `replica_count` is greater than 0 the dataflow is still warming up (a read would block until it catches up, possibly hitting the request timeout), so wait and retry; if `replica_count` is 0 the cluster has no replicas and the read cannot make progress until one is added.".to_string(),
                    input_schema: json!({
                        "type": "object",
                        "properties": {
                            "name": {
