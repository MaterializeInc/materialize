...
async fn setup_group_sync_test() -> (
    ...
    let server = test_util::TestHarness::default()
        .with_tls(server_cert, server_key)
        .with_oidc_auth(Some(oidc_server.issuer.clone()), Some("sub".to_string()), None)
        .start()
        .await;
    ...
    admin_client
        .batch_execute("CREATE ROLE analytics; CREATE ROLE platform_eng; CREATE ROLE data_eng")
        .await
        .unwrap();
...
fn make_insecure_tls() -> tokio_postgres_openssl::MakeTlsConnector {
    make_pg_tls(Box::new(|b: &mut SslConnectorBuilder| {
        Ok(b.set_verify(SslVerifyMode::NONE))
    }))
}
...
async fn test_oidc_group_sync_first_login() {
    ...
    let token = jwt_with_groups(&oidc_server, serde_json::json!(["analytics", "platform_eng"]));
    ...
    // Verify audit log records grant events with sentinel grantor.
    let audit_rows = admin_client
        .query(
            "SELECT
                 event_type,
                 details::jsonb->>'grantor_id' as grantor_id,
                 details::jsonb->>'executed_by' as executed_by
             FROM mz_audit_events
             WHERE event_type = 'grant'
               AND object_type = 'role'
               AND details::jsonb->>'member_id' = (
                   SELECT id::text FROM mz_roles WHERE name = $1
               )
             ORDER BY id DESC
             LIMIT 2",
            &[&GROUP_SYNC_USER],
    ...
    assert_eq!(
        audit_rows.len(),
        2,
        "should have 2 grant audit events for the two group roles"
    );
    for row in &audit_rows {
        let event_type: String = row.get(0);
        let grantor_id: String = row.get(1);
        let executed_by: String = row.get(2);
        assert_eq!(event_type, "grant");
        // MZ_JWT_SYNC_ROLE_ID = RoleId::System(4) = "s4"
        assert_eq!(
            grantor_id, "s4",
            "audit log grantor_id should be MZ_JWT_SYNC_ROLE_ID (s4)"
        );
        // session=None means executed_by defaults to MZ_SYSTEM_ROLE_ID = "s1"
        assert_eq!(
            executed_by, "s1",
            "audit log executed_by should be MZ_SYSTEM_ROLE_ID (s1)"
        );
    }
}

/// Reconnecting with changed groups revokes removed roles and grants new ones.
#[mz_ore::test(tokio::test(flavor = "multi_thread", worker_threads = 2))]
#[cfg_attr(miri, ignore)] // unsupported operation: can't call foreign function `TLS_method`
async fn test_oidc_group_sync_changed_groups() {
    let (server, admin_client, oidc_server) = setup_group_sync_test().await;

    // Initial login with analytics + platform_eng.
    let token1 = jwt_with_groups(&oidc_server, serde_json::json!(["analytics", "platform_eng"]));
    let _client1 = oidc_connect(&server, &token1).await.expect("login should succeed");

    // Reconnect with analytics + data_eng (platform_eng removed, data_eng added).
    let token2 = jwt_with_groups(&oidc_server, serde_json::json!(["analytics", "data_eng"]));
    let _client2 = oidc_connect(&server, &token2).await.expect("login should succeed");

    let role_names = fetch_user_role_memberships(&admin_client, GROUP_SYNC_USER).await;
    assert_eq!(
        role_names,
        vec!["analytics", "data_eng"],
        "reconnect should revoke platform_eng and grant data_eng"
    );

    // Verify audit log records revoke events with sentinel grantor.
    let revoke_rows = admin_client
        .query(
            "SELECT
                 details::jsonb->>'grantor_id' as grantor_id
             FROM mz_audit_events
    ...
    assert_eq!(
        revoke_rows.len(),
        1,
        "should have 1 revoke audit event (platform_eng revoked)"
    );
    let revoke_grantor: String = revoke_rows[0].get(0);
    assert_eq!(
        revoke_grantor, "s4",
        "revoke audit log grantor_id should be MZ_JWT_SYNC_ROLE_ID (s4)"
    );
}

/// Empty groups claim revokes all sync-granted roles.
#[mz_ore::test(tokio::test(flavor = "multi_thread", worker_threads = 2))]
#[cfg_attr(miri, ignore)] // unsupported operation: can't call foreign function `TLS_method`
async fn test_oidc_group_sync_empty_groups() {
    let (server, admin_client, oidc_server) = setup_group_sync_test().await;

    // Grant some roles first.
    let token1 = jwt_with_groups(&oidc_server, serde_json::json!(["analytics", "data_eng"]));
    let _client1 = oidc_connect(&server, &token1).await.expect("login should succeed");

    // Reconnect with empty groups.
    let token2 = jwt_with_groups(&oidc_server, serde_json::json!([]));
    let _client2 = oidc_connect(&server, &token2).await.expect("login should succeed");

    let role_names = fetch_user_role_memberships(&admin_client, GROUP_SYNC_USER).await;
    assert!(
        role_names.is_empty(),
        "empty groups should revoke all sync-granted roles, got: {:?}",
        role_names
    );
}
...
#[mz_ore::test(tokio::test(flavor = "multi_thread", worker_threads = 2))]
#[cfg_attr(miri, ignore)] // unsupported operation: can't call foreign function `TLS_method`
async fn test_oidc_group_sync_disabled() {
    let (server, admin_client, oidc_server) = setup_group_sync_test().await;

    admin_client
        .batch_execute("ALTER SYSTEM SET oidc_group_role_sync_enabled = false")
        .await
        .unwrap();
    ...
    assert!(
        role_names.is_empty(),
        "sync disabled should not grant any roles, got: {:?}",
        role_names
    );
}
