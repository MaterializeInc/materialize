...
#[allow(clippy::disallowed_methods)]
fn get_counter_value(registry: &MetricsRegistry, name: &str, labels: &[(&str, &str)]) -> u64 {
...
#[cfg_attr(miri, ignore)] // too slow
#[allow(clippy::disallowed_methods)]
fn test_frontend_read_then_write_counts_query_total() {
    for frontend_occ in [false, true] {
        ...
        for (statement_type, sql) in [
            ...
            let before = get_counter_value(server.metrics_registry(), "mz_query_total", &labels);
            client.batch_execute(sql).unwrap();
            let after = get_counter_value(server.metrics_registry(), "mz_query_total", &labels);
            assert_eq!(
                after,
                before + 1,
