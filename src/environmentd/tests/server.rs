...
#[mz_ore::test]
#[cfg_attr(miri, ignore)] // too slow
fn test_ws_notifies_of_session_var_updates() {
    let server = util::start_server(util::Config::default()).unwrap();
    ...
    let json = "{\"query\":\"SET application_name = 'heheh test';\"}";
    let json: serde_json::Value = serde_json::from_str(json).unwrap();
    ws.write_message(Message::Text(json.to_string())).unwrap();
    ...
    let complete = read_msg();
    let param_update = read_msg();
    let ready = read_msg();

    assert!(matches!(complete, WebSocketResponse::CommandComplete(_)));
    assert!(matches!(
        param_update,
        WebSocketResponse::ParameterStatus(_)
    ));
    assert!(matches!(ready, WebSocketResponse::ReadyForQuery(_)));
}
