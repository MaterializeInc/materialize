...
fn test_webhook_request_compression() {
    ...
    // Wait a moment for the webhook to get appended.
    std::thread::sleep(std::time::Duration::from_secs(5));

    // Ensure the webhook payload got decompressed.
    let row = client
        .query_one("SELECT body FROM webhook_text", &[])
        .unwrap();
    let rnd_body: String = row.get("body");
    assert_eq!(rnd_body, og_body);
}
