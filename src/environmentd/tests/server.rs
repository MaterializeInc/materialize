    ...
    Retry::default()
        ...
        .retry(|_state| {
            ...
            let handle1 = thread::spawn(move || {
                let err =  client1
                    .batch_execute("insert into foo select a, case when mz_dangerous.mz_sleep(ts) > 0 then 0 end as ts from foo")
                    .unwrap_err();
                assert_contains!(
                    err.to_string(),
                    "statement timeout"
    ...
    client
        .execute(
            "CREATE SOURCE webhook_text IN CLUSTER webhook_cluster FROM WEBHOOK \
             BODY FORMAT TEXT \
             CHECK ( WITH(BODY) body IS NOT NULL AND mz_dangerous.mz_sleep(5) IS NULL )",
            &[],
        )
        .expect("failed to create source");
