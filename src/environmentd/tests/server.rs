...
fn test_timeout_read_then_write() {
    test_timeout_read_then_write_inner(false);
}

// Same as `test_timeout_read_then_write`, but exercising the frontend OCC
// read-then-write path: a statement timeout must abort the OCC loop without
// committing.
#[mz_ore::test]
#[cfg_attr(miri, ignore)] // unsupported operation: can't call foreign function `epoll_wait` on OS `linux`
fn test_timeout_frontend_read_then_write() {
    test_timeout_read_then_write_inner(true);
}

fn test_timeout_read_then_write_inner(frontend_occ: bool) {
    let mut harness = test_util::TestHarness::default().unsafe_mode();
    if frontend_occ {
        ...
        Err(_) => {
            panic!(
                "BUG: INSERT...SELECT from a far-future REFRESH AT MV did not return \
                 ...
                 ignores the statement timeout (hangs in its sleep loop)."
            );
        }
    }
}
        ...
        Err(_) => {
            panic!(
                "LEGACY INSERT...SELECT from a far-future REFRESH AT MV also did not \
                 return within 45s; the far-future hang is pre-existing, not an OCC regression."
            );
        }
    }
}
        ...
        Err(_) => {
            panic!(
                "BUG: an unrelated UPDATE on a different table did not return within 25s \
                 ...
                 statement_timeout is enforced, so the victim cannot time out either."
            );
        }
    }
}
