...
// #[mz_ore::test]
// fn test_cancel_peek_on_cluster_drop() {
//     mz_ore::test::init_logging();
//     let config = util::Config::default();
//     let server = util::start_server(config).unwrap();
//
...
//     let mut drop_client = server.connect(postgres::NoTls).unwrap();
//
//     peek_client.batch_execute("CREATE TABLE t (a INT)").unwrap();
//     let handle = std::thread::spawn(move || {
//         peek_client.batch_execute(&format!("SELECT * FROM generate_series(0, {i32::MAX}), t"))
//     });
//
//     {
//         let mut client1 = server.connect(postgres::NoTls).unwrap();
