...
/// it, among the methods `override_methods` emitted.
///
...
fn emit(e: &Emission, func: &syn::ItemFn, trait_impl: TokenStream) -> TokenStream {
    let Emission {
        struct_name,
        has_self,
        sqlname,
...
/// Emits the trait impl for one `#[sqlfunc]`-annotated function, together with the
/// items [`emit`] wraps around it.
///
...
pub(crate) fn generate(
    ...
    Ok(emit(&emission, func, trait_impl))
}
