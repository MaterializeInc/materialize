...
//! arity's table here, not writing expansion code.
...
/// explicitly because they feed `Display`, the output-type body, or the expansion
/// decision instead.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
// Table order matches the `#*_fn` interpolation sequence generated by the corresponding
// generator arm in `sqlfunc.rs` today. A later task drives expansion from these tables
...
mod tests {
    ...
    fn modifier_table_order_matches_todays_generated_code() {
        // Order is load-bearing: a later task generates override methods in table order
