...
//!  * `non_nullable_capture_groups`, the AST-walking analysis Materialize runs on
//!    every compiled regex (it re-parses the pattern with `regex_syntax`), which
//!    has its own recursion over groups/repetitions/alternations.
...
fn drive(pattern: &str, flags: &str, text: &str) {
    ...
    // The AST analysis Materialize runs on every compiled regex; re-parses the
    // pattern and walks groups/repetitions/alternations.
    let _ = regex.non_nullable_capture_groups();
}
