...
//! (map expressions can reference earlier columns; predicates and the projection
//! reference any column), with a scalar vocabulary that includes `int4`/`int8`
//! arithmetic and the `int4`↔`int8` casts (the latter is fallible, so it feeds
//! optimize's error-handling). The target runs in one of two modes:
//!
