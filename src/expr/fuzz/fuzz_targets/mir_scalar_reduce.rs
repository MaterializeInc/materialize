...
fn run(u: &mut Unstructured) -> arbitrary::Result<()> {
    ...
    for _ in 0..ROWS_PER_EXPR {
        ...
        // current exception in the other direction is non-strict AND/OR error
        // absorption, where a successful `false AND <error>` or
        // `true OR <error>` may reduce to the absorbed error.
        if original.is_ok() {
            if folded.is_err() && absorbs_and_or_operand_error(&expr, &datums, &arena) {
                continue;
            }
            assert_eq!(
                original, folded,
