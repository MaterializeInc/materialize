...
impl MirScalarExpr {
        ...
        match self {
            ...
            MirScalarExpr::CallBinary { func, expr1, expr2 } => {
                ...
                match func {
                    ...
                    BinaryFunc::AddInt16(_) | BinaryFunc::SubInt16(_) | BinaryFunc::MulInt16(_) => {
                        let a = expr1.eval(datums, temp_storage)?;
                        let b = expr2.eval(datums, temp_storage)?;
                        if a.is_null() || b.is_null() {
                            return Ok(Datum::Null);
                        ...
                        if let (Datum::Int16(a), Datum::Int16(b)) = (a, b) {
                            ...
                            r.map(Datum::Int16).ok_or(EvalError::NumericFieldOverflow)
                        } else {
                            Err(EvalError::Internal("invalid input type".into()))
                        }
                    }
                    BinaryFunc::AddInt32(_) | BinaryFunc::SubInt32(_) | BinaryFunc::MulInt32(_) => {
                        let a = expr1.eval(datums, temp_storage)?;
                        let b = expr2.eval(datums, temp_storage)?;
                        if a.is_null() || b.is_null() {
                            return Ok(Datum::Null);
                        ...
                        if let (Datum::Int32(a), Datum::Int32(b)) = (a, b) {
                            ...
                            r.map(Datum::Int32).ok_or(EvalError::NumericFieldOverflow)
                        } else {
                            Err(EvalError::Internal("invalid input type".into()))
                        }
                    }
                    BinaryFunc::AddInt64(_) | BinaryFunc::SubInt64(_) | BinaryFunc::MulInt64(_) => {
                        let a = expr1.eval(datums, temp_storage)?;
                        let b = expr2.eval(datums, temp_storage)?;
                        if a.is_null() || b.is_null() {
                            return Ok(Datum::Null);
                        ...
                        if let (Datum::Int64(a), Datum::Int64(b)) = (a, b) {
                            ...
                            r.map(Datum::Int64).ok_or(EvalError::NumericFieldOverflow)
                        } else {
                            Err(EvalError::Internal("invalid input type".into()))
                        }
                    }
                    ...
                    BinaryFunc::DivInt16(_) | BinaryFunc::DivInt32(_) | BinaryFunc::DivInt64(_) => {
                        let a = expr1.eval(datums, temp_storage)?;
                        let b = expr2.eval(datums, temp_storage)?;
                        if a.is_null() || b.is_null() {
                            return Ok(Datum::Null);
                    ...
                    BinaryFunc::ModInt16(_) | BinaryFunc::ModInt32(_) | BinaryFunc::ModInt64(_) => {
                        let a = expr1.eval(datums, temp_storage)?;
                        let b = expr2.eval(datums, temp_storage)?;
                        if a.is_null() || b.is_null() {
                            return Ok(Datum::Null);
    ...
    pub fn contains_column(&self) -> bool {
        match self {
            ...
            MirScalarExpr::CallVariadic { exprs, .. } => exprs.iter().any(|e| e.contains_column()),
            MirScalarExpr::If { cond, then, els } => {
