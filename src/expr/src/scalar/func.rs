...
fn sqrt_dec<'a>(a: Datum<'a>, scale: u8) -> Datum<'a> {
    ...
    let d_scaled = div_float64(d_f64, Datum::from(factor))
        .unwrap()
        .unwrap_float64();

    match cast_float64_to_decimal(Datum::from(d_scaled.sqrt()), Datum::from(i32::from(scale))) {
        Ok(d) => d,
        Err(_) => Datum::Null,
    }
}
...
impl UnaryFunc {
    pub fn eval<'a>(
        ...
        match self {
            ...
            UnaryFunc::SqrtDec(scale) => Ok(sqrt_dec(a, *scale)),
