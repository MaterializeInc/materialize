    ...
    Ok(Datum::String(
        temp_storage.push_string(
            str::from_utf8(&decrypted)
                .map_err(|e| EvalError::Internal(format!("making str: {}", e.to_string())))?
                .to_owned(),
        ),
    ))
}
...
pub fn pgp_pub_decrypt_inner(datums: &[Datum]) -> Result<Vec<u8>, EvalError> {
    ...
    let cert = sequoia_openpgp::cert::Cert::from_bytes(key)
        .map_err(|_| EvalError::InvalidPgpPrivateKey)?;

    let to_decrypt = datums[0].unwrap_bytes();
    let p = &sequoia_openpgp::policy::StandardPolicy::new();
    let mut decryptor = DecryptorBuilder::from_bytes(to_decrypt)
        .map_err(|e| EvalError::PgpDecryption(e.to_string()))?
        .with_policy(p, None, Helper::new(p, cert))
        .map_err(|e| EvalError::PgpDecryption(e.to_string()))?;
...
pub fn pgp_pub_encrypt_inner<'a>(
    ...
    let cert =
        sequoia_openpgp::cert::Cert::from_bytes(key).map_err(|_| EvalError::InvalidPgpPublicKey)?;
    ...
    if recipients.is_empty() {
        return Err(EvalError::InvalidPgpPublicKey);
    }
    ...
    let message = Encryptor::for_recipients(message, recipients)
        ...
        .build()
        .map_err(|e| EvalError::PgpEncryption(e.to_string()))?;
