...
use mz_sql_parser::ast::{
    AstInfo, AvroSchema, CreateSinkConnection, CreateSubsourceOptionName, Format, FormatSpecifier,
...
fn parse_catalog_create_sql<'a>(a: &'a str) -> Result<Jsonb, EvalError> {
    ...
    fn string_option<T: AstInfo>(value: Option<WithOptionValue<T>>) -> Option<String> {
        match value {
                ...
                match stmt.connection {
                    CreateSinkConnection::Kafka {
                        ...
                        let topic = options
                            ...
                            .and_then(|o| string_option(o.value))
                        ...
                        for option in options {
                            match option.name {
                                IcebergSinkConfigOptionName::Namespace => {
                                    namespace = string_option(option.value)
                                }
                                IcebergSinkConfigOptionName::Table => {
                                    table = string_option(option.value)
                                }
                            }
                        }
                        info.insert(
