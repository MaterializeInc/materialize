...
mod test {
    ...
    fn reduce(s: &str) -> Result<MirScalarExpr, String> {
        ...
        let repr_typ = typ.iter().map(ReprColumnType::from).collect::<Vec<_>>();
        let before = scalar.repr_typ(&repr_typ);
        scalar.reduce(&repr_typ);
        let after = scalar.repr_typ(&repr_typ);
    ...
    fn test_interpret(s: &str) -> Result<Vec<String>, String> {
        ...
        let relation = SqlRelationType::new(types);
