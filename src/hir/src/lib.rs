...
//! This crate houses HIR, a representation of a SQL plan that is parallel to MIR, but represents
...
use mz_expr::{CollectionPlan, Id, LetRecLimit, func};
...
// Display and ScalarOps impls for HIR types.
// These must live in the crate where the types are defined (orphan rule).
        ...
        match self {
            ...
            Windowing(expr, _name) => {
                ...
                let order_by = column_orders
                    ...
                    .map(|(column_order, expr)| ColumnOrderWithExpr {
                        expr: expr.clone(),
            ...
            Exists(expr, _name) => match expr.as_ref() {
                Get { id, .. } => write!(f, "exists(Get {})", id),
                _ => write!(f, "exists(???)"),
            },
            Select(expr, _name) => match expr.as_ref() {
                Get { id, .. } => write!(f, "select(Get {})", id),
                _ => write!(f, "select(???)"),
            },
        }
    }
...
/// Helper for Display of ORDER BY in window functions.
