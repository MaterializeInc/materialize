...
///
/// In standard operation, Debezium guarantees that messages are sent in position order, but may
/// be duplicated.
///
            ...
            TrackFull::SeenOffsets(seen_offsets) => {
                if let Some(seen_offsets) = seen_offsets.get_mut(file) {
                    ...
                    match (is_new, should_skip) {
                        // new item that correctly does not violate debezium guarantees
                        (true, None) => {}
                        // new item that violates debezium guarantees that the no new
                        ...
                        // already seen item debezium says it *may* repeat things, so this is fine
                        (false, Some(skipinfo)) => {
                            info!(
