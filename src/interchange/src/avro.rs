...
pub struct Encoder {
    ...
    key_schema_and_filter: Option<(Schema, Vec<bool>)>,
}
...
impl Encoder {
    fn key_filter(&self) -> Option<&[bool]> {
        self.key_schema_and_filter
            .as_ref()
            .map(|(_, filter)| filter.as_slice())
    }
    pub fn new(
        desc: RelationDesc,
        include_transaction: bool,
    ...
    where
        I: IntoIterator<Item = Datum<'a>>,
    {
        encode_datums_as_avro(row, &self.columns, self.key_filter())
    }
}
    ...
    I: IntoIterator<Item = Datum<'a>>,
{
    ...
    for (i, datum) in datums.into_iter().enumerate() {
        let (name, typ) = &names_types[i];
        let name = name.as_str().to_owned();
        use mz_avro::types::ToAvro;
        let field = (name, TypedDatum::new(datum, typ.clone()).avro());
    ...
    let k = if key_filter.is_some() {
        Some(Value::Record(key_fields))
    } else {
        None
    };
