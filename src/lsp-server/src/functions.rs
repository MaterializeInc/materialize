...
pub static FUNCTIONS: Lazy<Vec<CompletionItem>> = Lazy::new(|| vec![
        CompletionItem {
            ...
            insert_text: Some("CAST()".to_string()),
            ..Default::default()
        }
        ,CompletionItem {
    ...
    CompletionItem {
        ...
        ,    CompletionItem {
            label: "regexp_replace(source: str, pattern: str, replacement: str [, flags: str]]) -> str".to_string(),
            label_details: Some(CompletionItemLabelDetails {
                detail: None,
                description: Some("String function".to_string()),
            }),
            kind: Some(CompletionItemKind::FUNCTION),
            detail: Some("Replaces the first occurrence of `pattern` with `replacement` in `source`. No match will return `source` unchanged. If `flags` is set to `g`, all occurrences are replaced. If `flags` is set to `i`, matches case-insensitively. `$N` or `$name` in `replacement` can be used to match capture groups. `${N}` must be used to disambiguate capture group indexes from names if other characters follow `N`. A `$$` in `replacement` will write a literal `$`. See the [rust regex docs](https://docs.rs/regex/latest/regex/struct.Regex.html#method.replace) for more details about replacement.".to_string()),
            documentation: Some(DOCS_LINK.clone()),
            deprecated: Some(false),
            ..Default::default()
        }
        ,    CompletionItem {
            label: "regexp_split_to_array(text: str, pattern: str [, flags: str]]) -> str[]".to_string(),
            label_details: Some(CompletionItemLabelDetails {
                detail: None,
                description: Some("String function".to_string()),
            }),
            kind: Some(CompletionItemKind::FUNCTION),
            detail: Some("Splits `text` by the regular expression `pattern` into an array. If `flags` is set to `i`, matches case-insensitively.".to_string()),
            documentation: Some(DOCS_LINK.clone()),
            deprecated: Some(false),
            ..Default::default()
        }
...
".to_string()),
    ...
    CompletionItem {
        ...
        ,    CompletionItem {
            label: "regexp_split_to_table(text: str, pattern: str [, flags: str]]) -> Col<string>".to_string(),
            label_details: Some(CompletionItemLabelDetails {
                detail: None,
                description: Some("Table function".to_string()),
            }),
            kind: Some(CompletionItemKind::FUNCTION),
            detail: Some("Splits `text` by the regular expression `pattern`. If `flags` is set to `i`, matches case-insensitively.".to_string()),
            documentation: Some(DOCS_LINK.clone()),
            deprecated: Some(false),
            ..Default::default()
        }
    ...
    CompletionItem {
        ...
        ,    CompletionItem {
            label: "mz_is_superuser() -> bool".to_string(),
            label_details: Some(CompletionItemLabelDetails {
                detail: None,
                description: Some("Access privilege inquiry function".to_string()),
            }),
            kind: Some(CompletionItemKind::FUNCTION),
            detail: Some("Reports whether the `current_role` is a superuser.".to_string()),
            documentation: Some(DOCS_LINK.clone()),
            deprecated: Some(false),
            ..Default::default()
        }
