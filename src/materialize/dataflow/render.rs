...
pub fn build_dataflow<A: Allocate>(
    ...
    worker.dataflow::<Timestamp, _, _>(|scope| match dataflow {
        Dataflow::Source(src) => {
            ...
            manager.set_trace(
                &RelationExpr::Source(src.name.clone()),
                arrangement.trace,
                on_delete,
            );
        }
        Dataflow::Sink(sink) => {
            ...
            let (arrangement, _) = manager
                .get_trace(&RelationExpr::Source(sink.from.to_owned()))
        ...
        Dataflow::View(view) => {
            ...
            manager.set_trace(
                &RelationExpr::Source(view.name.clone()),
                arrangement.trace,
                on_delete,
            );
        }
...
fn build_relation_expr<S: Scope<Timestamp = Timestamp>>(
    ...
    match relation_expr {
        RelationExpr::Source(name) => {
            let (arrangement, button) = manager
                .get_trace(&RelationExpr::Source(name.to_owned()))
        ...
        RelationExpr::Filter { predicate, input } => {
            ...
            build_relation_expr(&input, manager, worker_index, scope, buttons).filter(move |data| {
        ...
        RelationExpr::Aggregate { key, aggs, input } => {
            ...
            if key.is_empty() {
                ...
                relation_expr = relation_expr.concat(&scope.new_collection_from(sentinel).1);
            }
            let aggs = aggs.clone();
            let relation_expr = relation_expr
        ...
        RelationExpr::Join {
            ...
            let left_relation_expr = left;
            let right_relation_expr = right;
            ...
            let left_arranged = if let Some(mut trace) = manager.get_keyed_trace(&left, &left_key) {
                ...
                let left_trace =
                    build_relation_expr(&left_relation_expr, manager, worker_index, scope, buttons)
                        .map(move |data| {
                            (
            ...
            let right_arranged =
                if let Some(mut trace) = manager.get_keyed_trace(&right, &right_key) {
                    ...
                    let right_trace = build_relation_expr(
                        &right_relation_expr,
                        manager,
                        worker_index,
                        scope,
        ...
        RelationExpr::Distinct(relation_expr) => {
            build_relation_expr(relation_expr, manager, worker_index, scope, buttons)
                .distinct_total()
        }
        RelationExpr::UnionAll(relation_exprs) => {
            assert!(!relation_exprs.is_empty());
            let mut relation_exprs = relation_exprs.iter().map(|relation_expr| {
                build_relation_expr(relation_expr, manager, worker_index, scope, buttons)
            });
            let relation_expr = relation_exprs.next().unwrap();
            relation_exprs.fold(relation_expr, |p1, p2| p1.concat(&p2))
        }
    }
}
