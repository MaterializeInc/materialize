...
impl<'a> NameResolver<'a> {
    ...
    pub fn resolve_column(&self, column_name: &str) -> Result<(usize, &Type), failure::Error> {
        let results = self
            ...
            .iter()
            .enumerate()
            .filter(|(_, (_, t))| t.name.as_deref() == Some(column_name))
            .collect::<Vec<_>>();
        match results.len() {
    ...
    pub fn resolve_table_column(
        ...
        let results = self
            ...
            .enumerate()
            .filter(|(_, (tn, t))| {
                tn.as_deref() == Some(table_name) && t.name.as_deref() == Some(column_name)
            })
            .collect::<Vec<_>>();
        match results.len() {
    ...
    pub fn resolve_column_on_side(
        ...
        let results = self
            ...
            .enumerate()
            .filter(|(i, (_, t))| {
                t.name.as_deref() == Some(column_name)
                    && match side {
    ...
    pub fn get_table_column_types(&self, table_name: &str) -> Vec<&Type> {
        self.columns
            .iter()
            .filter(|(tn, _)| tn.as_deref() == Some(table_name))
            .map(|(_, t)| t)
            .collect()
    }
