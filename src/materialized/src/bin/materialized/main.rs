    ...
    for (key, _val) in env::vars() {
        ...
        match key.as_str() {
            ...
            "DEFAULT_PROGRESS_MODE" => bail!(
                "Materialize no long respects the DEFAULT_PROGRESS_MODE environment variable\n\
...
swap: {swap_total}KB total, {swap_used}KB used",
    ...
    let timely_worker = {
        // This is a little goofy because `differential_dataflow::configure`
        // wants the full `timely::Config` struct, when we only want to deal
        // with `timely::WorkerConfig`. We just ignore the
        // `timely::CommunicationConfig` that we create here, because we
        // construct Timely's communication infrastructure separately.
        let mut config = timely::Config {
            communication: timely::CommunicationConfig::Thread,
            worker: timely::WorkerConfig::default().progress_mode(timely_progress_mode),
        };
