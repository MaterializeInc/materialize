        ...
        let svc = service::service_fn(move |req| {
            ...
            async move {
                ...
                match (req.method(), req.uri().path()) {
                    (&Method::GET, "/") => root::handle_home(req, coord_client).await,
                    (&Method::GET, "/metrics") => {
                        metrics::handle_prometheus(req, coord_client, start_time).await
                    }
                    (&Method::GET, "/status") => {
                        metrics::handle_status(req, coord_client, start_time).await
                    }
                    (&Method::GET, "/prof") => prof::handle_prof(req, coord_client).await,
                    (&Method::GET, "/memory") => memory::handle_memory(req, coord_client).await,
                    (&Method::POST, "/prof") => prof::handle_prof(req, coord_client).await,
                    (&Method::POST, "/sql") => sql::handle_sql(req, coord_client).await,
                    (&Method::GET, "/internal/catalog") => {
                        catalog::handle_internal_catalog(req, coord_client).await
                    }
                    _ => root::handle_static(req, coord_client).await,
                }
            }
        });
