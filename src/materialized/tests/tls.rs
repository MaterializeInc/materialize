...
fn test_tls() -> Result<(), Box<dyn Error>> {
    ...
    // Test TLS modes with a server that allows TLS.
    let config = util::Config::default().with_tls(TlsMode::Allow, &server_cert, &server_key);
    ...
    run_tests(
        ...
        &[
            ...
            TestCase::Pgwire {
                ...
                configure: Box::new(|_| Ok(())),
                assert: Assert::Err(Box::new(|err| {
                    assert!(err.to_string().contains("certificate verify failed"))
                })),
            },
            TestCase::Http {
                user: "mz_system",
