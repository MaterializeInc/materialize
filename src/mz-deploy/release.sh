...
for target in "${TARGETS[@]}"; do
    ...
    case "$target" in
        *-apple-darwin)
            cargo build --profile release-mz-deploy --target "$target" -p mz-deploy \
...
NATIVE_BINARY="$WORKSPACE_ROOT/target/aarch64-apple-darwin/release-mz-deploy/mz-deploy"
...
for target in "${TARGETS[@]}"; do
    ...
    BINARY="$WORKSPACE_ROOT/target/${target}/release-mz-deploy/mz-deploy"
