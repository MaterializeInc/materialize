...
pub async fn run(settings: &Settings, skip_secrets: bool, dry_run: bool) -> Result<(), CliError> {
    progress::info("Applying all infrastructure objects...");

    // 1. Clusters
    progress::info("--- Applying clusters ---");
    super::clusters::run(settings, dry_run).await?;

    // 2. Roles
    progress::info("--- Applying roles ---");
    super::roles::run(settings, dry_run).await?;

    // 3. Network Policies
    progress::info("--- Applying network policies ---");
    super::apply_network_policies::run(settings, dry_run).await?;
    ...
    if skip_secrets {
        ...
        super::apply_secrets::run(settings, dry_run).await?;
    }

    // 5. Connections
    progress::info("--- Applying connections ---");
    super::apply_connections::run(settings, dry_run).await?;

    // 6. Sources
    progress::info("--- Applying sources ---");
    super::apply_tables::apply_sources(settings, dry_run).await?;

    // 7. Tables
    progress::info("--- Applying tables ---");
    super::apply_tables::apply_tables(settings, dry_run).await?;
