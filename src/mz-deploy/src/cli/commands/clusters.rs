...
const CLUSTER_KIND: GrantNamedObjectKind = GrantNamedObjectKind::Cluster;
...
pub async fn plan(
    ...
    let (reconcile_state, live, default_replication_factor) = futures::try_join!(
        reconcile::ReconcileState::for_named_objects(client, &CLUSTER_KIND, &names),
...
async fn plan_cluster(
    ...
    grants::reconcile_named_object(executor, cluster_name, &def.grants, &CLUSTER_KIND, state)
        .await?;
    comments::reconcile(
        executor,
        &CommentObject::Cluster(cluster_name),
        &def.comments,
        state.comments(cluster_name),
    )
    .await?;
...
/// Each live cluster's configuration, as the canonical `CREATE CLUSTER`
/// statement the server renders from the catalog, keyed by cluster name.
///
