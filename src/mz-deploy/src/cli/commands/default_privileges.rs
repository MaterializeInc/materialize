...
use crate::client::DefaultPrivilege;
use crate::verbose;
use mz_sql_parser::ast::{
    AbbreviatedGrantOrRevokeStatement, AbbreviatedGrantStatement, AbbreviatedRevokeStatement,
    AlterDefaultPrivilegesStatement, Ident, ObjectType, Privilege, PrivilegeSpecification, Raw,
...
pub async fn reconcile(
    ...
    let current = current.iter().cloned().collect();
    ...
    grouped
        ...
        .filter_map(|((target_role, object_type, grantee), privileges)| {
            ...
            Some(AlterDefaultPrivilegesStatement {
                ...
                target_objects: scope
                    .default_privilege_scope()
                    .expect("default privileges require a database or schema target"),
                grant_or_revoke,
            })
        })
        .collect()
...
fn parse_object_type(object_type: &str) -> Option<ObjectType> {
    match object_type {
        ...
        "network policy" => Some(ObjectType::NetworkPolicy),
        _ => None,
    }
}
...
fn privilege_names(
    ...
    match privileges {
        ...
        PrivilegeSpecification::All => match object_type {
            ...
            | ObjectType::NetworkPolicy => Some(vec!["USAGE".into()]),
...
mod tests {
    ...
    const SCOPE: ReconcileTarget<'static> = ReconcileTarget::Schema {
        database: "db",
        schema: "app",
    };
    ...
    #[mz_ore::test]
    #[cfg_attr(miri, ignore)] // unsupported operation: can't call foreign function 'rust_psm_stack_pointer'
    fn test_missing_network_policy_rule_is_granted() {
        assert_eq!(
            changes(
                &["ALTER DEFAULT PRIVILEGES FOR ROLE owner IN SCHEMA db.app \
                   GRANT USAGE ON POLICIES TO analyst"],
                vec![],
            ),
            vec![
                "ALTER DEFAULT PRIVILEGES FOR ROLE owner IN SCHEMA db.app \
    ...
    fn test_database_scope_renders_in_database() {
        let rendered: Vec<String> = privilege_changes(
            ...
            &ReconcileTarget::Named {
                kind: crate::cli::commands::reconcile::ObjectKind::Database,
                name: "db",
            },
        )
        .iter()
