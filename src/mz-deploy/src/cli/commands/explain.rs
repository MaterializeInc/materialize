...
fn validate_target(
    planned_obj: &graph::DatabaseObject,
...
fn plan_dep(
    ...
    if project.external_dependencies.contains(dep_id) {
        let columns = get_columns_for_stub(dep_id, None, get_columns)?;
    ...
    if !matching_indexes.is_empty() {
        ...
        let columns =
            get_columns_for_stub(dep_id, Some(&planned_obj.typed_object.stmt), get_columns)?;
        actions.push(StagingAction::StubTable {
            object_id: dep_id.clone(),
            columns,
        });
        ...
        match planned_obj.typed_object.stmt.kind() {
            crate::types::ObjectKind::MaterializedView | crate::types::ObjectKind::Table => {
                ...
                let columns = get_columns_for_stub(
                    dep_id,
                    Some(&planned_obj.typed_object.stmt),
                    get_columns,
                )?;
                actions.push(StagingAction::StubTable {
                    object_id: dep_id.clone(),
...
fn get_columns_for_stub(
    ...
    Err(CliError::Message(format!(
        "no column schema available for '{}'. Run 'mz-deploy compile' to populate the type cache",
        object_id
    )))
}
