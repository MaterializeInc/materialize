...
pub async fn run(name: &str, init_git: bool) -> Result<(), CliError> {
    ...
    create_dir(project_dir, ".agent/skills/mz-deploy/references")?;
    ...
    add_file(project_dir, ".agent/skills/mz-deploy/SKILL.md", SKILL_MD)?;
    add_file(
        project_dir,
        ".agent/skills/mz-deploy/references/unit-tests.md",
        UNIT_TEST_REFERENCE_MD,
    )?;

    std::os::unix::fs::symlink(
        "../../.agent/skills/mz-deploy",
