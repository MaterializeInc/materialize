...
use crate::cli::commands::default_privileges::{self, DefaultPrivilegeScope};
use crate::cli::commands::grants::{self, GrantScope};
...
pub async fn reconcile_database(
    ...
    default_privileges::reconcile(
        client,
        executor,
        &DefaultPrivilegeScope::Database(database),
...
pub async fn reconcile_schema(
    ...
    default_privileges::reconcile(
        client,
        executor,
        &DefaultPrivilegeScope::Schema { database, schema },
        &config.default_privileges,
    )
    .await?;
    execute_other(executor, &config).await
