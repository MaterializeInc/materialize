...
2. **Stable schema** (`SET api = stable`) — thin materialized views that
   ...
   have a `COMMENT ON` describing its contract. Never use `SELECT *` —
   always list out all columns explicitly.
...
Example layout:
    ...
    models/mydb/public/customers.sql              # CREATE MATERIALIZED VIEW — explicit column list from internal view
    models/mydb/public/orders.sql                 # CREATE MATERIALIZED VIEW — explicit column list from internal view
