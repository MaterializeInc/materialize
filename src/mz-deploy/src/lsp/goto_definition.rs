...
mod tests {
    ...
    fn resolve_one_part_name() {
        ...
        let location =
            resolve_reference(&["foo".to_string()], &file_uri, root.path(), &cache).unwrap();
    ...
    fn resolve_unknown_name_returns_none() {
        ...
        let result =
            resolve_reference(&["nonexistent".to_string()], &file_uri, root.path(), &cache);
        assert!(result.is_none());
    }
    ...
    fn build_test_project_cross_schema_cache() -> (tempfile::TempDir, ProjectCache) {
        let root = tempfile::tempdir().unwrap();
