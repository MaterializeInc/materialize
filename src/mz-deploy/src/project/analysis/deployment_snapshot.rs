...
//! Instead of hashing source files, it hashes normalized compiled objects so
//! formatting and comment changes don't trigger unnecessary redeployments.
//!
...
use crate::client::{
    Client, ConnectionError, DeploymentKind, DeploymentMode, DeploymentObjectRecord,
    SchemaDeploymentRecord,
};
use crate::project::ir::{compiled, graph, object_id::ObjectId};
...
/// the normalized compiled representation (not source file contents).
...
pub(crate) fn build_snapshot_from_planned(
    ...
    for (object_id, typed_obj) in sorted_objects {
        ...
        // and should not participate in deployment hash change detection.
...
mod tests {
    ...
    /// Parse a single SQL statement into a compiled::DatabaseObject.
    fn make_typed_object(sql: &str) -> compiled::DatabaseObject {
    ...
    /// Build a graph::Project from (database, schema, name, typed_obj) tuples.
