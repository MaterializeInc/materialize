...
//!
//! This module extends [`graph::Project`](crate::project::ir::graph::Project)
//! with traversal and accessor methods. The core operation is a DFS-based
...
use crate::project::ir::{
    compiled,
    graph::{DatabaseObject, ModStatement, Project},
};
...
impl Project {
    ...
    /// Returns all table-like objects (`CREATE TABLE` and `CREATE TABLE ... FROM SOURCE`).
    ...
    /// Get all database objects in topological order with their compiled statements.
    ///
    /// Returns a vector of `(ObjectId, compiled object)` tuples in deployment order.
    /// This allows access to the fully qualified SQL statements for each object.
    pub fn get_sorted_objects(
        &self,
    ) -> Result<Vec<(ObjectId, &compiled::DatabaseObject)>, DependencyError> {
    ...
    ///
    /// # Returns
    /// Vector of `(ObjectId, compiled object)` tuples in deployment order
    ...
    pub fn validate_cluster_isolation(
        ...
        super::graph_validation::validate_cluster_isolation(self, sources_by_cluster)
    }
}
