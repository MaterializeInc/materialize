...
//!   extraction happens downstream in [`analysis::deps`](crate::project::analysis::deps),
//!   which produces the final [`graph::Project`](super::graph::Project).
...
use crate::project::resolve::normalize::{ClusterTransformer, NameTransformer, NormalizingVisitor};
...
///
/// This struct is created during object validation and is used to:
...
impl TryFrom<UnresolvedItemName> for FullyQualifiedName {
    ...
    fn try_from(value: UnresolvedItemName) -> Result<Self, Self::Error> {
        if value.0.len() < 3 {
            return Err("fully qualified names require database, schema, and object parts");
        }
        let id = ObjectId::new(
            value.0[0].to_string(),
            value.0[1].to_string(),
...
pub struct DatabaseObject {
    ...
    ///
    /// Carried from [`input::ObjectVariant::path`](crate::project::syntax::input::ObjectVariant)
...
/// ```
///
/// Note: `mod.sql` files are parsed as source inputs and carried here as
/// schema-level setup statements.
#[derive(Debug)]
pub struct Schema {
...
/// schemas, and objects. This is the top-level compiled project produced after
/// validating project source inputs.
///
/// Compiled projects are produced by [`crate::project::plan_sync`] and the
/// internal compiler entrypoints in [`crate::project::compiler`].
///
