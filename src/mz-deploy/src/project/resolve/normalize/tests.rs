...
fn test_staging_external_dependency_not_transformed() {
    ...
    external_deps.insert(ObjectId::new(
...
fn test_staging_cross_schema_objects_to_deploy_filter() {
    ...
    let fqn = FullyQualifiedName::try_from(UnresolvedItemName(vec![database, schema, object]))
        .expect("test FQN is fully qualified");
    ...
    replacement_objects.insert(ObjectId::new(
        "app".to_string(),
        "core".to_string(),
        "order_summary".to_string(),
    ));
