...
use crate::api::admin::AdminClient;
...
impl Context {
    ...
    pub async fn activate_profile(
        self,
        name: Option<String>,
    ) -> Result<ProfileContext, anyhow::Error> {
        let profile_name = name.unwrap_or_else(|| self.config_file.profile().into());
        let profile = self.config_file.load_profile(&profile_name)?;
        let admin_client = AdminClient::auth(
            profile.admin_endpoint().parse().unwrap(),
            profile.app_password(),
        )
        .await?;
        let cloud_client = CloudClient::new(profile.cloud_endpoint().parse().unwrap());
        Ok(ProfileContext {
            context: self,
            profile_name,
            admin_client,
...
impl ProfileContext {
    pub fn activate_region(self, name: Option<String>) -> Result<RegionContext, anyhow::Error> {
        let profile = self
            .context
            .config_file
