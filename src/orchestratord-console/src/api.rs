...
//! mutations (request/promote rollouts, edit the spec, edit system
//! parameters). The UI at `/` is a single self-contained HTML file.
//!
//! Everything here goes through the Kubernetes API. Nothing reaches into
//! orchestratord, which is what lets this ship and be upgraded on its own.
//!
//! There is no authentication. The server is reachable only over its ClusterIP
//! service (or a port-forward), and the design doc
...
/// What this console knows about itself, independent of the operator.
#[derive(Clone)]
...
/// Facts shown in the header, surfaced at `/api/info`.
///
/// The operator half is read back off the orchestratord Deployment rather than
/// configured here, so the two do not have to be kept in step by hand. It is
/// best effort: when the Deployment cannot be read, those fields are null and
/// the console still reports its own version.
#[derive(Serialize)]
...
struct Info {
    ...
    create_balancers: Option<bool>,
    create_console: Option<bool>,
}
...
impl Context {
    pub fn new(client: Client, info: ConsoleInfo) -> Self {
        Self { client, info }
    }
}
...
/// Label the operator's chart puts on its Deployment.
const OPERATOR_SELECTOR: &str = "app.kubernetes.io/name=materialize-operator";

async fn get_info(State(context): State<Arc<Context>>) -> Json<Info> {
    let mut info = Info {
        console_version: context.info.version.clone(),
        operator_version: None,
        cloud_provider: None,
        region: None,
        helm_chart_version: None,
        create_balancers: None,
        create_console: None,
    };

    let deployment_api: Api<Deployment> = match &context.info.operator_namespace {
        Some(namespace) => Api::namespaced(context.client.clone(), namespace),
        None => Api::all(context.client.clone()),
    };
    let Ok(deployments) = deployment_api
        .list(&ListParams::default().labels(OPERATOR_SELECTOR).limit(1))
        .await
    else {
        return Json(info);
    };
    let Some(container) = deployments
        .items
        .first()
    ...
    info.operator_version = container
        .image
        .as_deref()
        .and_then(|image| image.rsplit_once(':'))
        .map(|(_, tag)| tag.to_owned());

    // The operator's configuration is only visible as the flags it was started
    // with, so read them back rather than duplicating the values here.
    ...
    info.cloud_provider = flag_value("cloud-provider");
    info.region = flag_value("region");
    info.helm_chart_version = flag_value("helm-chart-version");
    info.create_balancers = Some(args.iter().any(|arg| arg == "--create-balancers"));
    info.create_console = Some(args.iter().any(|arg| arg == "--create-console"));
...
/// Flag the operator passes to `environmentd` for the config sync loop. Its
/// presence on the serving generation is what tells us system parameter edits
/// will be picked up without a rollout.
const CONFIG_SYNC_FLAG: &str = "--config-sync-file-path";

/// Whether system parameter edits reach the running environment, and if not,
/// why.
///
/// Reported from what is deployed rather than inferred from a version, so this
/// stays correct without the console having to know which `environmentd`
/// releases support the sync loop.
#[derive(Clone, Copy, PartialEq, Serialize)]
#[serde(rename_all = "camelCase")]
enum SyncState {
    /// The serving generation runs the sync loop. Edits apply on their own.
    Live,
    /// The ConfigMap is wired into the spec but the serving generation predates
    /// that change, so a rollout is needed to put the mount in place.
    PendingRollout,
    /// The operator settled on a generation that has no mount even though the
    /// spec asks for one, which means this `environmentd` is too old for system
    /// parameter sync. Saving here has no effect until it is upgraded.
    Unsupported,
    /// No ConfigMap is referenced by the spec yet.
    NotConfigured,
}

/// Determine the [`SyncState`] for an environment.
///
/// The serving generation's StatefulSet is the ground truth for whether the
/// sync loop is running. When the spec names a ConfigMap but that StatefulSet
/// has no mount, the environment is either mid-change or on an `environmentd`
/// that never gets one. Those are told apart by the controller's own verdict:
/// if it has finished reconciling with nothing outstanding, the omission was
/// deliberate.
async fn sync_state(client: &Client, mz: &Materialize, namespace: &str) -> SyncState {
    if mz.spec.system_parameter_configmap_name.is_none() {
        return SyncState::NotConfigured;
    }
    let Some(status) = mz.status.as_ref() else {
        return SyncState::PendingRollout;
    };
    let sts_api: Api<StatefulSet> = Api::namespaced(client.clone(), namespace);
    let serving = mz.environmentd_statefulset_name(status.active_generation);
    let mounted = match sts_api.get_opt(&serving).await {
        Ok(Some(sts)) => sts
            .spec
            ...
            .is_some_and(|pod| {
                pod.containers.iter().any(|container| {
                    container
                        ...
                        .iter()
                        .flatten()
                        .any(|arg| arg.starts_with(CONFIG_SYNC_FLAG))
                })
            }),
        // Treat an unreadable StatefulSet as "not yet", rather than claiming
        // the stronger `Unsupported`.
        _ => return SyncState::PendingRollout,
    };
    if mounted {
        return SyncState::Live;
    }
    // The condition has to describe the spec as it is now. Wiring the ConfigMap
    // bumps the resource's generation, and until the controller reconciles that
    // it still reports the previous spec as applied. Reading that stale verdict
    // would call a brand new mount a deliberate omission.
    let settled = !mz.rollout_requested()
        && status.conditions.iter().any(|condition| {
            condition.type_ == "UpToDate"
                && condition.status == "True"
                && condition.observed_generation == mz.meta().generation
        });
    if settled {
        SyncState::Unsupported
    } else {
        SyncState::PendingRollout
    }
}
...
struct SystemParamsWriteResponse {
    ...
    rollout_required: bool,
    /// State observed before this write, so the caller can tell a save that
    /// applies now from one that is waiting on a rollout or will never apply.
    sync_state: SyncState,
...
async fn get_system_params(
    ...
    let mz = get_mz(&context, &namespace, &name).await?;
    let sync_state = sync_state(&context.client, &mz, &namespace).await;
...
async fn put_system_params(
    ...
    let pods_refreshed = if sync_state == SyncState::Live {
