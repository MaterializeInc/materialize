...
pub fn merge_iters_by<I: Iterator, F: Fn(&I::Item, &I::Item) -> Ordering>(
    ...
    impl<I: Iterator, F: Fn(&I::Item, &I::Item) -> Ordering> Ord for RunIter<I, F> {
        fn cmp(&self, other: &Self) -> Ordering {
            // Flip the arguments, since we're using this in a max-heap.
            (self.merge_by)(&other.peek, &self.peek)
        }
    }
    ...
    iter::from_fn(move || {
        ...
        let next = match peek_mut.iter.next() {
            None => PeekMut::pop(peek_mut).peek,
            Some(next) => std::mem::replace(&mut peek_mut.peek, next),
        };
        Some(next)
    })
}
