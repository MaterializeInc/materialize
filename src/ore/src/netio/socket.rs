...
impl SocketAddrType {
    ...
    pub fn guess(s: &str) -> SocketAddrType {
        if s.starts_with("turmoil://") {
            return SocketAddrType::Turmoil;
        }
...
impl Listener {
    ...
    fn poll_accept(
        ...
        match self.get_mut() {
            ...
            #[cfg(feature = "turmoil")]
            Listener::Turmoil(_) => {
                unimplemented!("`turmoil::net::TcpListener` does not provide `poll_accept`");
            }
        }
    }
}
