...
impl Args {
    ...
    pub fn from_cli() -> Result<Args, Box<dyn std::error::Error>> {
        ...
        match std::fs::read_to_string(&config_file) {
            Ok(contents) => {
                conf = toml::from_str::<InnerConfig>(&contents)?;
...
///
/// This is a normalized version of [`InnerConfig`], which is what is actually parsed
...
impl From<InnerConfig> for Config {
    fn from(conf: InnerConfig) -> Config {
