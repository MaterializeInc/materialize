...
impl<K: Codec, V: Codec, T: Timestamp + Lattice + Codec64, D> FetchedBlob<K, V, T, D> {
    /// Returns whether this blob was fetched as part of a filter pushdown audit.
    /// Callers should refrain from applying optimizations like projection pushdown
    /// when this is `true`, as the audit needs the full row to re-evaluate the MFP.
    pub fn is_filter_pushdown_audit(&self) -> bool {
