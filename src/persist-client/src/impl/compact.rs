...
impl Compactor {
        ...
        D: Semigroup + Codec64,
    {
        ...
        let mut should_compact =
            req.inputs.len() >= 16 || req.inputs.iter().map(|x| x.len).sum::<usize>() >= 128;
        // TODO: Enable the heuristics after we've gotten a sense for where
        // we're starting.
        should_compact = true;
        if !should_compact {
            return;
        ...
        D: Semigroup + Codec64,
    {
        let compact_blocking = move || {
            ...
            for ((k, v), t, d) in updates.iter() {
                ...
                if let Some(chunks) = builder.take_filled() {
                    for chunk in chunks {
                        handle.block_on(parts.write(chunk, req.desc.upper().clone()));
                    }
                }
            }
