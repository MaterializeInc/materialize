...
fn id_range(ids: Vec<SpineId>) -> CompactionInput {
    ...
    D: Semigroup + Ord + Codec64 + Send + Sync,
{
    ...
    pub async fn compact_all(
        ...
        let batches = req.inputs.iter().map(|x| x.id).collect::<Vec<_>>();
        let input = id_range(batches);
        ...
        async_stream::stream! {
                ...
                chunked_runs
            {
                ...
                let input = if incremental_enabled {
                    let batch_ids = req.inputs.iter().map(|x| x.id).collect::<BTreeSet<_>>();
                    if batch_ids.len() == 1 {
                        CompactionInput::PartialBatch(
                            batch_ids.iter().next().cloned().unwrap(),
                            runs.iter()
                                .map(|(run_id, _, _, _)| run_id.1.expect("run_id must be present"))
                                .collect::<BTreeSet<_>>(),
                        )
                    } else {
                        id_range(batch_ids.into_iter().collect())
                    }
                } else {
                    let runs = runs.iter()
