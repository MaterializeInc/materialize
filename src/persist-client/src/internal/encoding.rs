...
mod tests {
    ...
    fn state_diff_migration_rollups() {
        let r1key = PartialRollupKey("foo".to_owned());
        let r1 = StateFieldDiff {
            key: SeqNo(1),
            val: StateFieldValDiff::Insert(HollowRollup {
                key: r1key.clone(),
                encoded_size_bytes: None,
            }),
        };
        ...
        field_diffs_into_proto(
            ...
            &[StateFieldDiff {
                key: r1.key,
                val: StateFieldValDiff::Insert(r1key),
            }],
            proto.field_diffs.as_mut().unwrap(),
            |k| k.into_proto().encode_to_vec(),
            |v| v.into_proto().encode_to_vec(),
        );

        let diff = StateDiff::<u64>::from_proto(proto).unwrap();
        let expected = vec![r2, r1];
        assert_eq!(diff.rollups.into_iter().collect::<Vec<_>>(), expected);
    }
}
