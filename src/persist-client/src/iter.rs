...
impl<T: Codec64 + Timestamp + Lattice> FetchData<T> {
    async fn fetch(
        ...
        match self.part {
            ...
            RunPart::Many(runs_ref) => {
                let runs = runs_ref.get(shard_id, blob).await?;
                Ok(Err(runs))
            }
        }
    }
            ...
            .map(|run| async {
                ...
                'fetch_loop: loop {
                    ...
                    match fetch_result {
                        Err(err) => {
                            run.push_front((part, size));
                            break 'fetch_loop Err(err);
                        }
                        Ok(Err(run_part)) => {
                            eprintln!("WHAO: {} {:?}", self.context, run_part.parts);
