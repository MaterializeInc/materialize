    ...
    G: Scope<Timestamp = T>,
{
    ...
    let shutdown_button = builder.build(move |mut capabilities| async move {
        let mut cap = capabilities.pop().expect("not enough inputs");
        assert!(capabilities.pop().is_none(), "too many inputs");
...
impl DataSubscribe {
    ...
    pub fn new(
        ...
        let (data, txns, capture, tokens) =
            worker.dataflow::<u64, _, _>(|scope| {
                let (data_stream, token0) = scope.scoped::<u64, _, _>("hybrid", |scope| {
                ...
                let tokens: Rc<dyn Any> = Rc::new((token0, token1));
