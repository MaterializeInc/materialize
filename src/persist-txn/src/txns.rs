    ...
    C: TxnsCodec,
{
    ...
    pub async fn forget(&mut self, forget_ts: T, data_id: ShardId) -> Result<Tidy, T> {
        ...
        op.run(async {
            ...
            loop {
                ...
                let update = self.txns_cache.data_schemas_at(&data_id, &txns_upper).map(
                ...
                let updates = if let Some((key, val)) = update.as_ref() {
    ...
    pub async fn apply_le(&mut self, ts: &T) -> Tidy {
        ...
        op.run(async {
            ...
            for (x, unapplied, unapplied_ts) in self.txns_cache.unapplied() {
                ...
                // TODO: Support schema changes within persist-txn. In the
                // meantime, assert that we got zero or one real schemas.
    ...
    pub async fn apply_eager_le(&mut self, ts: &T) -> Tidy {
        ...
        op.run(async {
            ...
            for x in self.txns_cache.all_registered_at(ts) {
                let mut data_write = self
                    .datas
                    .take_write(&x.data_id, &x.key_schema, &x.val_schema)
                    .await;
