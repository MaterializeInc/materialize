...
pub trait Schema<T>: Debug + Send + Sync {
    ...
    /// Like [`Self::decoder_any`], but the caller may pass a `demand` set listing the
    /// input column indices the decoder must produce values for. Schemas that support
    /// projection pushdown may use this to skip materializing un-demanded columns;
    /// schemas that don't will produce the same decoder as `decoder_any`.
    ///
    /// The slice is sorted ascending and contains no duplicates. A `None` demand
    /// means "all columns are demanded" (no projection).
    fn decoder_any_with_demand(
        &self,
        col: &dyn arrow::array::Array,
        _demand: Option<&[usize]>,
    ) -> Result<Self::Decoder, anyhow::Error> {
        self.decoder_any(col)
    }
...
/// A shared, immutable set of input column indices that a decoder must materialize.
///
/// Carried alongside read-path operations to enable column projection pushdown:
/// schemas that honor [`Schema::decoder_any_with_demand`] use this to skip decoding
