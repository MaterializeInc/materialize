...
/// Backoff between sibling open attempts. The ceiling bounds a long outage to
/// one attempt per minute per process (a credential resolution, a health-check
/// get and a warning) and still re-arms within a minute of recovery.
...
/// Opens the sibling handle for a store. [HedgedBlob::new_arming] calls it
/// until it returns something other than [HedgeSibling::Unavailable], and
/// relies on it to log the cause of each failed attempt. See
/// [crate::cfg::open_hedge_sibling].
...
pub struct HedgedBlob {
    ...
    budget: HedgeBudget,
    /// Opens the sibling, then keeps it warm. Aborted on drop. Only the
    /// test constructor `new` leaves it out, when there is nothing to warm.
...
async fn warm(hedge: Arc<dyn Blob>, cfg: Arc<ConfigSet>, metrics: BlobHedgeMetrics) {
    loop {
        ...
        // Ping first, sleep second, so the pool is warm as soon as the
        // sibling is armed. As many concurrent pings as hedges can run at once
    ...
    let hedge = loop {
        ...
        match opener().await {
            ...
            HedgeSibling::Unavailable => {
                ...
                // Retries are sibling traffic, so they wait for `enabled`
                // like the warmer. The first attempt above is not gated, so
                // that a process is usually armed before hedging is enabled
                // instead of one recheck after it.
...
impl HedgedBlob {
    ...
    ///
    /// Dropping the returned [HedgedBlob] aborts the task, including an open
    /// in progress, so `opener` must be safe to cancel.
    ///
...
mod tests {
    ...
    #[mz_ore::test(tokio::test(start_paused = true))]
    async fn dropping_blob_stops_arming() {
        let primary: Arc<dyn Blob> = TestBlob::new(SECS(0), Ok(None));
        let hedge = TestBlob::new(SECS(0), Ok(None));
        let attempts = Arc::new(AtomicUsize::new(0));
        let blob = HedgedBlob::new_arming(
            primary,
            flaky_opener(&hedge, usize::MAX, &attempts),
            test_cfg(|_| {}),
            metrics(),
        );
    ...
    #[mz_ore::test(tokio::test(start_paused = true))]
    async fn dropping_blob_cancels_open() {
        let primary: Arc<dyn Blob> = TestBlob::new(SECS(0), Ok(None));
        // Held by the opener and by its one in-flight open, which never ends.
        ...
        let opener: HedgeSiblingOpener = {
            ...
            Box::new(move || {
                ...
                Box::pin(async move {
                    let _held = held;
                    std::future::pending::<HedgeSibling>().await
                })
            })
        };
        let blob = HedgedBlob::new_arming(primary, opener, test_cfg(|_| {}), metrics());
        tokio::task::yield_now().await;
    ...
    #[mz_ore::test(tokio::test(start_paused = true))]
    async fn arming_backoff_doubles_up_to_the_ceiling() {
        let primary: Arc<dyn Blob> = TestBlob::new(SECS(0), Ok(None));
        ...
        let opener: HedgeSiblingOpener = {
            ...
            Box::new(move || {
                ...
                Box::pin(async { HedgeSibling::Unavailable })
            })
        };
        let _blob = HedgedBlob::new_arming(primary, opener, test_cfg(|_| {}), metrics());
        tokio::time::sleep(SECS(300)).await;
        let attempt_times = attempt_times.lock().expect("lock").clone();
