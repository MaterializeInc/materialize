...
impl From<ProtoArrangement> for ArrangementMeta {
    fn from(x: ProtoArrangement) -> Self {
        ArrangementMeta {
            id: Id(x.id),
            seal: x
                .seal
...
impl From<ProtoStreamRegistration> for StreamRegistration {
    fn from(x: ProtoStreamRegistration) -> Self {
        StreamRegistration {
            name: x.name,
            id: Id(x.id),
...
impl From<&BlobMeta> for ProtoMeta {
    fn from(x: &BlobMeta) -> Self {
        ProtoMeta {
            seqno: x.seqno.0,
            id_mapping: x.id_mapping.iter().map(|x| x.into()).collect(),
            graveyard: x.graveyard.iter().map(|x| x.into()).collect(),
            arrangements: x.arrangements.iter().map(|x| x.into()).collect(),
