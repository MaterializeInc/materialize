...
//! of inputs in the original "any OID, raw bytes" mode so the not-implemented
//! branches and the syntax-error paths stay covered.
//!
...
fn gen_value(u: &mut Unstructured, depth: u8) -> arbitrary::Result<(Type, String)> {
    ...
    Ok(match u.int_in_range(0u8..=3)? {
        ...
        // be a totally-ordered scalar; restrict to one of the supported domains.
        _ => {
