...
//! (so any POSIX DST-rule tail is silently discarded — fuzzing that grammar is
//! dead weight), while everything else flows through `parse_num`, which splits
//! long all-digit runs into `[..hhhh]mm` chunks unless a `:` is present, plus
//! the punctuation-as-delimiter trimming and the `z`/`Z`-only-at-end rule.
