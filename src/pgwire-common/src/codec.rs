...
/// four-byte length field. It is checked before the body buffer is sized, so the
/// buffer follows the caller's ceiling rather than the protocol maximum. Pass
/// [`MAX_STARTUP_FRAME_SIZE`], or [`MAX_FORWARDED_STARTUP_FRAME_SIZE`] if a
/// balancer may have appended parameters in transit.
...
mod tests {
    ...
    async fn test_forwarded_startup_params_fit_the_allowance() {
        ...
        assert!(
            ...
            "FORWARDED_STARTUP_PARAM_ALLOWANCE is too small: {} bytes forwarded \
             against a {MAX_FORWARDED_STARTUP_FRAME_SIZE} byte bound",
            forwarded.len(),
        );
        assert!(
    ...
    /// A client that declares a frame within budget and then stops sending
    /// leaves the server holding a buffer sized for that declaration, so the
    /// budget is what bounds it.
    #[mz_ore::test(tokio::test(start_paused = true))]
    async fn test_startup_buffer_is_bounded_by_the_budget() {
