        ...
        match msg {
            ...
            BackendMessage::AuthenticationSASLFinal(data) => {
                ...
                let res = match data.kind {
                    SASLServerFinalMessageKinds::Error(err) => {
                        format!("e={}", err)
                    }
...
pub fn decode_sasl_client_first_message(mut buf: Cursor) -> Result<SASLInitialResponse, io::Error> {
    println!("Decoding SASL client first message");
    println!("Raw message: {:?}", buf);
    ...
    // 3) Optional reserved "m=" extension before n= (rare; keep if you want)
