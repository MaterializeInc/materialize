...
use tokio::time::{self, timeout_at, Duration};
    ...
    A: AsyncRead + AsyncWrite + Send + Unpin,
{
            ...
            Some(Statement::Fetch(stmt)) => {
                ...
                let timeout_secs = match options.timeout {
                    Some(timeout) => {
                        ...
                        // bounds of a tokio::time::Duration.
    ...
    async fn send_rows(
        ...
        let deadline =
            timeout_secs.map(|t| tokio::time::Instant::now() + Duration::from_secs_f64(t));
        ...
        async fn fetch_batch(
            ...
            match deadline {
                ...
                Some(deadline) => match timeout_at(deadline, rows.try_next()).await {
                    Ok(batch) => batch,
