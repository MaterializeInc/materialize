...
//! source bodies) into Materialize's JSONB `Row` representation — recursive
...
/// as an object — an asymmetry that defeats the strict re-parse oracle but must
...
fn check_no_panic(data: &[u8]) {
    if let Ok(j) = Jsonb::from_slice(data) {
        // Force the full Display rendering; must not panic.
        let _ = j.to_string();
    }
}
...
fn check(data: &[u8]) {
    ...
    // limitation, not a JSONB round-trip bug — `from_slice` itself never panics
    // on it — so skip the re-parse check whenever the rendering leans on
