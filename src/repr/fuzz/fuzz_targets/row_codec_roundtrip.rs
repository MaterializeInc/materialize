...
//! Codec::encode`, which is the path persist actually uses on read/write —
...
//! runs — it produces an empty `Row` for any input, so the round trip was
//! trivially satisfied and the schema threading the target exists to test was
//! dead code.
//!
...
//! exactly the behavior the empty schema skipped. The invariant under test is
//! that decode(encode(decode(bytes))) == decode(bytes) for a fixed schema.
