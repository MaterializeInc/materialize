...
//! map literal — balanced braces, quoted keys/values with `\"`/`\\` escapes,
//! NULL values, and (in nested-value mode) embedded maps — so the key/value
//! split, the quote/escape state machine, and the recursive value parse all run
//! deep. A minority of inputs get structural noise spliced in so the parser's
