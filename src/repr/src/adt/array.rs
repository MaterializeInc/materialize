...
impl Iterator for ArrayDimensionsIter<'_> {
    ...
    fn next(&mut self) -> Option<ArrayDimension> {
        if self.data.is_empty() {
            ...
            let lower_bound = unsafe { (self.data.as_ptr() as *const usize).read_unaligned() };
            self.data = &self.data[mem::size_of::<usize>()..];
            let length = unsafe { (self.data.as_ptr() as *const usize).read_unaligned() };
            self.data = &self.data[mem::size_of::<usize>()..];
            Some(ArrayDimension {
                lower_bound,
                length,
