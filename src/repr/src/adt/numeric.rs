...
pub fn write_numeric_standard_notation<W: fmt::Write>(w: &mut W, n: &Numeric) -> fmt::Result {
    write_decimal_standard_notation(w, n)
}
...
fn write_decimal_standard_notation<const N: usize>(
    ...
    let ndigits = usize::try_from(d.digits()).expect("Decimal digits fits in usize");
    ...
    if msu_ndigits >= 3 {
        digit_buf[pos] = u8::try_from(msu / 100).expect("decimal digit fits in u8");
        pos += 1;
    }
    if msu_ndigits >= 2 {
        digit_buf[pos] = u8::try_from((msu / 10) % 10).expect("decimal digit fits in u8");
        pos += 1;
    }
    digit_buf[pos] = u8::try_from(msu % 10).expect("decimal digit fits in u8");
    pos += 1;

    // Remaining units (full 3-digit groups), most-significant first
    for i in (0..nunit.saturating_sub(1)).rev() {
        let u = units[i];
        digit_buf[pos] = u8::try_from(u / 100).expect("decimal digit fits in u8");
        digit_buf[pos + 1] = u8::try_from((u / 10) % 10).expect("decimal digit fits in u8");
        digit_buf[pos + 2] = u8::try_from(u % 10).expect("decimal digit fits in u8");
        pos += 3;
    }
    ...
    let sig_digits = i32::try_from(digits.len()).expect("Decimal digits fits in i32");
    ...
    if exponent >= 0 {
        ...
        let before_point =
            usize::try_from(sig_digits + exponent).expect("before_point is non-negative");
