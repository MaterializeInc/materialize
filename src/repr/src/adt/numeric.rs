...
/// Maximum number of coefficient digits in a [`Numeric`] value (13 units * 3 digits/unit).
const NUMERIC_MAX_DIGITS: usize = NUMERIC_DATUM_WIDTH_USIZE * 3;

/// Maximum output length for a [`Numeric`] in standard notation:
/// sign (1) + digits (39) + decimal point (1) + leading zeros from negative exponent (39).
const NUMERIC_MAX_OUTPUT: usize = 1 + NUMERIC_MAX_DIGITS + 1 + NUMERIC_MAX_DIGITS;

/// Writes a [`Numeric`] value in standard (non-scientific) notation directly to
/// a [`fmt::Write`], without any heap allocation.
///
/// This is functionally equivalent to [`Decimal::to_standard_notation_string`]
/// but avoids allocating both a `Vec<u8>` for the coefficient digits and a
/// `String` for the result. Instead, it extracts digits from the internal
/// [`Decimal::coefficient_units`] representation (a `&[u16]` slice requiring no
/// allocation) into a fixed-size stack buffer and writes the entire result with
/// a single `write_str` call.
pub fn write_numeric_standard_notation<W: fmt::Write>(w: &mut W, n: &Numeric) -> fmt::Result {
    if !n.is_finite() {
        return write!(w, "{}", n);
    }
    ...
    // Extract coefficient digits from the internal unit representation.
    // Units are stored LSB-first with DECDPUN=3 digits per u16.
    let units = n.coefficient_units();
    let ndigits = n.digits() as usize;

    // Build the entire output in a stack buffer, then write it all at once.
    let mut out = [0u8; NUMERIC_MAX_OUTPUT];
    let mut len = 0;
    ...
    // Decode unit digits into a temporary array (MSB-first).
    let mut digit_buf = [0u8; NUMERIC_MAX_DIGITS];
    let mut pos = ndigits;
    for &unit in units.iter() {
        let d0 = (unit % 10) as u8;
        let d1 = ((unit / 10) % 10) as u8;
        let d2 = (unit / 100) as u8;
        if pos >= 3 {
            pos -= 3;
            digit_buf[pos] = d2;
            digit_buf[pos + 1] = d1;
    ...
    let first_nonzero = digit_buf[..ndigits]
        .iter()
        .position(|d| *d != 0)
        .unwrap_or(ndigits - 1);
    let digits = &digit_buf[first_nonzero..ndigits];
    ...
    // SAFETY: we only pushed ASCII bytes (digits 0-9, '.', '-').
    let s = unsafe { std::str::from_utf8_unchecked(&out[..len]) };
    w.write_str(s)
}
...
mod tests {
    ...
    fn test_write_numeric_standard_notation_matches() {
        fn check(s: &str) {
            let mut cx = cx_datum();
            let n: Numeric = cx.parse(s).unwrap();
            let expected = n.to_standard_notation_string();
            let mut got = String::new();
            write_numeric_standard_notation(&mut got, &n).unwrap();
            assert_eq!(
                expected, got,
                "mismatch for input {s:?} (digits={}, exp={})",
                n.digits(), n.exponent(),
            );
        }
        // Basic values
        ...
        check("1e10");
        check("1e-10");
        // Values with trailing zeros in exponent
    ...
    fn proptest_write_numeric_standard_notation() {
        proptest!(|(num in arb_numeric())| {
            if num.is_finite() {
                let expected = num.to_standard_notation_string();
                let mut got = String::new();
                write_numeric_standard_notation(&mut got, &num).unwrap();
                assert_eq!(expected, got,
                    "mismatch for num with digits={}, exp={}",
                    num.digits(), num.exponent());
            }
        });
    }
}
