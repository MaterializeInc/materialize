...
impl<const N: usize> IntoRowByTypes for BytesN<N> {
    /// Single-element iterator for single-column `BytesN`.
    type DatumIter<'a> = Once<Datum<'a>>;

    /// Constructs an iterator of datums from the `BytesN` instance given by `self`.
    ///
