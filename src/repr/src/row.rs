...
mod tests {
    ...
    #[test]
    fn test_array() {
        // Construct an  array using `Row::pack_array` and verify that it
        // unpacks correctly.
    ...
    fn test_array_max_dimensions() {
        let mut packer = RowPacker::new();
        let ndims = usize::from(MAX_ARRAY_DIMENSIONS) + 1;
        let res = packer.push_array(
            ...
            vec![Datum::Int32(4); ndims],
        );
        assert_eq!(res, Err(InvalidArrayError::TooManyDimensions(ndims)));
