    ...
    fn get<'a>(&'a self, idx: usize, packer: &'a mut RowPacker) {
        ...
        match self {
            ...
            DatumColumnDecoder::Time(array) => {
                if array.is_valid(idx) {
                    let packed = PackedNaiveTime::from_bytes(array.value(idx)).expect("failed to roundtrip time");
            ...
            DatumColumnDecoder::Uuid(array) => {
                if array.is_valid(idx) {
                    let uuid = Uuid::from_slice(array.value(idx)).expect("failed to roundtrip uuid");
