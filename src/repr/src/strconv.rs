...
#[doc(hidden)]
pub fn parse_timestamptz_general(
    s: &str,
...
mod tests {
    ...
    #[mz_ore::test]
    fn test_parse_timestamp_fast_path() {
        // Standard ISO 8601 formats that should hit the fast path
        let fast_cases = vec![
            "2024-06-15 14:30:25",
            "2024-06-15 14:30:25.1",
            "2024-06-15 14:30:25.12",
            "2024-06-15 14:30:25.123",
            "2024-06-15 14:30:25.1234",
            "2024-06-15 14:30:25.12345",
            "2024-06-15 14:30:25.123456",
            "2024-06-15 14:30:25.1234567",
            "2024-06-15 14:30:25.12345678",
            "2024-06-15 14:30:25.123456789",
            "2024-06-15T14:30:25",
            "2024-06-15T14:30:25.123456",
            "1970-01-01 00:00:00",
            "2000-12-31 23:59:59",
            "0001-01-01 00:00:00",
        ...
        for s in &fast_cases {
            // Verify fast path returns a result
            let fast = try_parse_timestamp_fast(s);
            assert!(fast.is_some(), "fast path should handle: {}", s);
            // Verify fast path agrees with general parser
            let general = parse_timestamp(s).unwrap();
            let fast_checked =
                CheckedTimestamp::from_timestamplike(fast.unwrap()).unwrap();
            assert_eq!(
                *fast_checked, *general,
                "fast path disagrees with general parser for: {}",
                s
            );
        }

        // Formats that should fall back to the general parser
        let fallback_cases = vec![
            "epoch",
            "2024-6-15 14:30:25",  // single-digit month
        ];
        for s in &fallback_cases {
            let fast = try_parse_timestamp_fast(s);
            assert!(fast.is_none(), "fast path should NOT handle: {}", s);
            // But general parser should still work
            assert!(parse_timestamp(s).is_ok(), "general parser should handle: {}", s);
        }
    }

    #[mz_ore::test]
    fn test_parse_timestamptz_fast_path() {
        let fast_cases = vec![
            ("2024-06-15 14:30:25+00", "2024-06-15 14:30:25+00"),
            ("2024-06-15 14:30:25+00:00", "2024-06-15 14:30:25+00:00"),
            ("2024-06-15 14:30:25-05", "2024-06-15 14:30:25-05"),
            ("2024-06-15 14:30:25-05:30", "2024-06-15 14:30:25-05:30"),
            (
                "2024-06-15 14:30:25.123456+00",
                "2024-06-15 14:30:25.123456+00",
            ),
            (
                "2024-06-15T14:30:25+00:00",
                "2024-06-15T14:30:25+00:00",
            ),
        ];
        for (s, _) in &fast_cases {
            let fast = try_parse_timestamptz_fast(s);
            assert!(fast.is_some(), "fast path should handle: {}", s);
            let general = parse_timestamptz(s).unwrap();
            let fast_checked =
                CheckedTimestamp::from_timestamplike(fast.unwrap()).unwrap();
            assert_eq!(
                *fast_checked, *general,
    ...
    fn test_parse_date_fast_path() {
        ...
        for s in &fast_cases {
            let fast = try_parse_date_fast(s);
            assert!(fast.is_some(), "fast path should handle: {}", s);
            let general = parse_date(s).unwrap();
            let fast_date = Date::try_from(fast.unwrap()).unwrap();
            assert_eq!(
                fast_date, general,
                "fast path disagrees with general parser for: {}",
                s
    ...
    fn test_parse_time_fast_path() {
        ...
        for s in &fast_cases {
            let fast = try_parse_time_fast(s);
            assert!(fast.is_some(), "fast path should handle: {}", s);
            let general = parse_time(s).unwrap();
            assert_eq!(
                fast.unwrap(),
                general,
                "fast path disagrees with general parser for: {}",
