...
impl<Out: Message, In: Message> Client<Out, In> {
    ...
    pub async fn connect(
        ...
        // The connection tasks inherit this span, so their log lines name the peer. A controller
        // does not otherwise put its replica in scope, and under Kubernetes the address names the
        // cluster, replica, generation, and process.
        let span = info_span!("ctp", %address);
        let conn = Connection::start(stream, version, dest_host, idle_timeout, metrics)
            .instrument(span)
            .await?;
        Ok(Self { conn })
    }
...
impl ErrorTx {
    ...
    fn report(&self, error: String) {
        ...
        self.0.send_if_modified(|slot| match slot {
            Some(_) => false,
            None => {
                *slot = Some(error);
                true
