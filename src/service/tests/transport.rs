...
/// Helper for connecting to a CTP server that retried until it encounters the expected error.
...
#[cfg_attr(miri, ignore)] // unsupported operation: can't call foreign function `deflateInit2_`
...
fn test_metrics() {
    ...
    sim.host("server", move || async {
        ...
        assert!(metrics.bytes_sent.load(Ordering::SeqCst) >= 73);
        assert!(metrics.bytes_received.load(Ordering::SeqCst) >= 54);
    ...
    sim.client("client", async move {
        ...
        assert!(metrics.bytes_sent.load(Ordering::SeqCst) >= 54);
        assert!(metrics.bytes_received.load(Ordering::SeqCst) >= 73);
        assert_eq!(metrics.messages_sent.load(Ordering::SeqCst), 1);
        assert_eq!(metrics.messages_received.load(Ordering::SeqCst), 1);
