...
/// A borrowed identifier that properly quotes itself when displayed.
///
/// Uses the same quoting rules as [`Ident`]: identifiers matching
/// `[a-z_][a-z0-9_]*` that are not reserved keywords are printed bare;
/// all others are wrapped in double quotes with internal `"` doubled.
///
/// [`Ident`]: crate::ast::Ident
pub struct EscapedIdentifier<'a>(&'a str);

impl<'a> EscapedIdentifier<'a> {
    fn can_be_printed_bare(&self) -> bool {
...
impl<'a> fmt::Display for EscapedIdentifier<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
pub fn escaped_identifier(s: &str) -> EscapedIdentifier<'_> {
    EscapedIdentifier(s)
}
