...
pub struct ItemReferences {
    ...
    /// Name-only references in data-type position.
    pub type_names: BTreeSet<Vec<String>>,
    /// The subset of `type_names` that appear directly under `[]` array
    /// syntax. Name resolution resolves `T[]` to the paired array type in
    /// addition to `T` itself, so callers reconstructing resolution behavior
    /// need to know which type names were array-wrapped.
    pub array_elem_type_names: BTreeSet<Vec<String>>,
}
            ...
            RawItemName::Name(UnresolvedItemName(parts)) => {
                ...
                match position {
                    ...
                    Position::Type { in_array } => {
                        if in_array {
                            self.refs.array_elem_type_names.insert(parts.clone());
                        }
...
mod tests {
    ...
    fn nested_data_types() {
        ...
        // Only int4 sits directly under `[]` syntax.
        assert_eq!(
            refs.array_elem_type_names,
