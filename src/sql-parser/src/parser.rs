...
impl<'a> Parser<'a> {
    ...
    fn parse_create_sink(&mut self) -> Result<Statement<Raw>, ParserError> {
        ...
        self.expect_keyword(FROM)?;
        let from = RawName::Name(self.parse_object_name()?);
    ...
    fn parse_create_views(&mut self) -> Result<Statement<Raw>, ParserError> {
        ...
        let definitions = if self.parse_keywords(&[FROM, SOURCE]) {
            let name = RawName::Name(self.parse_object_name()?);
    ...
    fn parse_create_index(&mut self) -> Result<Statement<Raw>, ParserError> {
        ...
        let on_name = RawName::Name(self.parse_object_name()?);
    ...
    fn parse_drop(&mut self) -> Result<Statement<Raw>, ParserError> {
        ...
        let if_exists = self.parse_if_exists()?;
        // TODO(jkosh44) Must be a cleaner way
        let names = self.parse_comma_separated(Parser::parse_object_name)?;
        let names = names.into_iter().map(RawName::Name).collect();
        let cascade = matches!(
            self.parse_at_most_one_keyword(&[CASCADE, RESTRICT], "DROP")?,
            Some(CASCADE),
        );
    ...
    /// TODO(Joe) I think this should return RawName not UnresolvedName?? Though maybe caller should handle that
    ...
    fn parse_tail(&mut self) -> Result<Statement<Raw>, ParserError> {
        let relation = if self.consume_token(&Token::LParen) {
            ...
            TailRelation::Name(RawName::Name(self.parse_object_name()?))
        };
    ...
    fn parse_explain(&mut self) -> Result<Statement<Raw>, ParserError> {
        ...
        let explainee = if self.parse_keyword(VIEW) {
            Explainee::View(RawName::Name(self.parse_object_name()?))
        } else {
