...
use crate::Pretty;
use crate::util::{comma_separate, comma_separated};
...
impl Pretty {
    ...
    pub(crate) fn doc_create_source<'a, T: AstInfo>(
        ...
        if !names.is_empty() {
            doc = self.nest(doc, self.bracket("(", comma_separated(names), ")"));
        }
        docs.push(self.nest_title(title, doc));
        if let Some(cluster) = &v.in_cluster {
            docs.push(self.nest_title("IN CLUSTER", self.doc_display_pass(cluster)));
        }
        docs.push(self.nest_title("FROM", self.doc_display_pass(&v.connection)));
        if let Some(format) = &v.format {
            docs.push(self.doc_format_specifier(format));
        }
        if !v.include_metadata.is_empty() {
            docs.push(self.nest_title(
                "INCLUDE",
                comma_separate(|im| self.doc_display_pass(im), &v.include_metadata),
            ));
        }
        if let Some(envelope) = &v.envelope {
            docs.push(self.nest_title("ENVELOPE", self.doc_display_pass(envelope)));
        }
        ...
        if let Some(progress) = &v.progress_subsource {
            docs.push(self.nest_title("EXPOSE PROGRESS AS", self.doc_display_pass(progress)));
        }
        if !v.with_options.is_empty() {
            docs.push(self.bracket(
                "WITH (",
    ...
    pub(crate) fn doc_create_webhook_source<'a, T: AstInfo>(
        ...
        docs.push(self.nest_title(title, self.doc_display_pass(&v.name)));
        ...
        docs.push(self.nest_title("BODY FORMAT", self.doc_display_pass(&v.body_format)));
    ...
    fn doc_webhook_check<'a, T: AstInfo>(
        ...
        if let Some(options) = &v.options {
            ...
            if !with_items.is_empty() {
                inner.push(self.bracket("WITH (", comma_separated(with_items), ")"));
    ...
    pub(crate) fn doc_create_table<'a, T: AstInfo>(
        ...
        let table_def = self.nest(
            self.doc_display_pass(&v.name),
            self.bracket("(", comma_separated(col_items), ")"),
        );
        docs.push(self.nest_title(title, table_def));
    ...
    pub(crate) fn doc_create_table_from_source<'a, T: AstInfo>(
        ...
        if has_columns_or_constraints {
            ...
            if !items.is_empty() {
                table_def = self.nest(table_def, self.bracket("(", comma_separated(items), ")"));
            }
        }
        ...
        // FROM SOURCE
        let mut from_source = self.nest_title("FROM SOURCE", self.doc_display_pass(&v.source));
        if let Some(reference) = &v.external_reference {
            from_source = self.nest(
                from_source,
                self.bracket("(REFERENCE = ", self.doc_display_pass(reference), ")"),
            );
        }
    ...
    pub(crate) fn doc_create_connection<'a, T: AstInfo>(
        ...
        let connection_with_values = self.nest(
            RcDoc::concat([
                RcDoc::text("TO "),
    ...
    pub(crate) fn doc_create_sink<'a, T: AstInfo>(
        ...
        if let Some(name) = &v.name {
            docs.push(self.nest_title(title, self.doc_display_pass(name)));
        } else {
        ...
        docs.push(self.nest_title("FROM", self.doc_display_pass(&v.from)));
        docs.push(self.nest_title("INTO", self.doc_display_pass(&v.connection)));
        ...
        if let Some(mode) = &v.mode {
            docs.push(self.nest_title("MODE", self.doc_display_pass(mode)));
        }
    ...
    pub(crate) fn doc_create_subsource<'a, T: AstInfo>(
        ...
        if let Some(of_source) = &v.of_source {
            docs.push(self.nest_title("OF SOURCE", self.doc_display_pass(of_source)));
        }
    ...
    pub(crate) fn doc_create_cluster<'a, T: AstInfo>(
        ...
        // CREATE CLUSTER name
        docs.push(self.nest_title("CREATE CLUSTER", self.doc_display_pass(&v.name)));
    ...
    pub(crate) fn doc_create_cluster_replica<'a, T: AstInfo>(
        ...
        docs.push(self.nest_title("CREATE CLUSTER REPLICA", replica_name));
    ...
    pub(crate) fn doc_create_network_policy<'a, T: AstInfo>(
        ...
        let docs = vec![
            ...
            self.nest_title("CREATE NETWORK POLICY", self.doc_display_pass(&v.name)),
            // OPTIONS (...)
            self.bracket(
                "(",
                comma_separate(|o| self.doc_display_pass(o), &v.options),
    ...
    pub(crate) fn doc_create_index<'a, T: AstInfo>(
        ...
        docs.push(self.nest_title("ON", on_clause));
    ...
    fn doc_format_specifier<T: AstInfo>(&self, v: &FormatSpecifier<T>) -> RcDoc<'_> {
        match v {
            FormatSpecifier::Bare(format) => {
                self.nest_title("FORMAT", self.doc_display_pass(format))
            }
            FormatSpecifier::KeyValue { key, value } => {
                let docs = vec![
                    self.nest_title("KEY FORMAT", self.doc_display_pass(key)),
                    self.nest_title("VALUE FORMAT", self.doc_display_pass(value)),
                ];
                RcDoc::intersperse(docs, Doc::line()).group()
            }
        }
    ...
    fn doc_external_references<'a>(&'a self, v: &'a ExternalReferences) -> RcDoc<'a> {
        match v {
            ExternalReferences::SubsetTables(subsources) => self.bracket(
                "FOR TABLES (",
                comma_separate(|s| self.doc_display_pass(s), subsources),
                ")",
            ),
            ExternalReferences::SubsetSchemas(schemas) => self.bracket(
                "FOR SCHEMAS (",
    ...
    pub(crate) fn doc_copy<'a, T: AstInfo>(&'a self, v: &'a CopyStatement<T>) -> RcDoc<'a> {
        let relation = match &v.relation {
            CopyRelation::Named { name, columns } => {
                ...
                if !columns.is_empty() {
                    relation = self.bracket_doc(
                        self.nest(relation, RcDoc::text("(")),
            ...
            CopyRelation::Select(query) => {
                self.bracket("COPY (", self.doc_select_statement(query), ")")
            }
            CopyRelation::Subscribe(query) => {
                self.bracket("COPY (", self.doc_subscribe(query), ")")
            }
        };
        let mut docs = vec![
            relation,
    ...
    pub(crate) fn doc_subscribe<'a, T: AstInfo>(
        ...
        let doc = match &v.relation {
            SubscribeRelation::Name(name) => {
                self.nest_title("SUBSCRIBE", self.doc_display_pass(name))
            }
            SubscribeRelation::Query(query) => {
                self.bracket("SUBSCRIBE (", self.doc_query(query), ")")
            }
        };
        ...
        if let Some(up_to) = &v.up_to {
            docs.push(self.nest_title("UP TO", self.doc_expr(up_to)));
        }
        match &v.output {
    ...
    fn doc_as_of<'a, T: AstInfo>(&'a self, v: &'a AsOf<T>) -> RcDoc<'a> {
        ...
        self.nest_title(title, self.doc_expr(expr))
    }
    ...
    pub(crate) fn doc_create_view<'a, T: AstInfo>(
        ...
        self.intersperse_line_nest(docs)
    }
    ...
    pub(crate) fn doc_create_materialized_view<'a, T: AstInfo>(
        ...
        docs.push(self.nest_title("AS", self.doc_query(&v.query)));
    ...
    pub(crate) fn doc_insert<'a, T: AstInfo>(&'a self, v: &'a InsertStatement<T>) -> RcDoc<'a> {
        ...
        let mut doc = self.intersperse_line_nest([self.intersperse_line_nest(first), sources]);
    ...
    pub(crate) fn doc_select_statement<'a, T: AstInfo>(
        ...
        if let Some(as_of) = &v.as_of {
            doc = self.intersperse_line_nest([doc, self.doc_as_of(as_of)]);
        }
        doc.group()
    }

    fn doc_order_by<'a, T: AstInfo>(&'a self, v: &'a [OrderByExpr<T>]) -> RcDoc<'a> {
        self.title_comma_separate("ORDER BY", |o| self.doc_order_by_expr(o), v)
    }

    fn doc_order_by_expr<'a, T: AstInfo>(&'a self, v: &'a OrderByExpr<T>) -> RcDoc<'a> {
        let doc = self.doc_expr(&v.expr);
        let doc = match v.asc {
            Some(true) => self.nest(doc, RcDoc::text("ASC")),
            Some(false) => self.nest(doc, RcDoc::text("DESC")),
            None => doc,
        };
        match v.nulls_last {
            Some(true) => self.nest(doc, RcDoc::text("NULLS LAST")),
            Some(false) => self.nest(doc, RcDoc::text("NULLS FIRST")),
            None => doc,
        }
    }
    ...
    fn doc_query<'a, T: AstInfo>(&'a self, v: &'a Query<T>) -> RcDoc<'a> {
        ...
        if !v.ctes.is_empty() {
            match &v.ctes {
                CteBlock::Simple(ctes) => {
                    docs.push(self.title_comma_separate("WITH", |cte| self.doc_cte(cte), ctes))
                }
        ...
        let offset = if let Some(offset) = &v.offset {
            vec![RcDoc::concat([
                self.nest_title("OFFSET", self.doc_expr(offset))
            ])]
        } else {
            vec![]
        };
    ...
    fn doc_cte<'a, T: AstInfo>(&'a self, v: &'a Cte<T>) -> RcDoc<'a> {
        RcDoc::concat([
            ...
            RcDoc::line(),
            self.bracket("(", self.doc_query(&v.query), ")"),
        ])
    }
    ...
    fn doc_mutually_recursive<'a, T: AstInfo>(&'a self, v: &'a CteMutRec<T>) -> RcDoc<'a> {
        ...
        docs.push(self.bracket("AS (", self.doc_query(&v.query), ")"));
        self.nest(
            self.doc_display_pass(&v.name),
            RcDoc::intersperse(docs, Doc::line()).group(),
        )
    ...
    fn doc_set_expr<'a, T: AstInfo>(&'a self, v: &'a SetExpr<T>) -> RcDoc<'a> {
        match v {
            ...
            SetExpr::Query(v) => self.bracket("(", self.doc_query(v), ")"),
            ...
            SetExpr::Table(v) => self.nest(RcDoc::text("TABLE"), self.doc_display_pass(v)),
        }
        .group()
    }
    ...
    fn doc_values<'a, T: AstInfo>(&'a self, v: &'a Values<T>) -> RcDoc<'a> {
        let rows =
            v.0.iter()
                .map(|row| self.bracket("(", comma_separate(|v| self.doc_expr(v), row), ")"));
    ...
    fn doc_join<'a, T: AstInfo>(&'a self, v: &'a Join<T>) -> RcDoc<'a> {
        ...
        let constraint = match constraint {
            JoinConstraint::On(expr) => self.nest_title("ON", self.doc_expr(expr)),
            JoinConstraint::Using { columns, alias } => {
                let mut doc = self.bracket(
                    "USING(",
                    comma_separate(|c| self.doc_display_pass(c), columns),
                    ")",
                );
                if let Some(alias) = alias {
                    doc = self.nest(doc, self.nest_title("AS", self.doc_display_pass(alias)));
                }
                doc
            }
            JoinConstraint::Natural => return self.doc_display(v, "join constraint"),
        };
        self.intersperse_line_nest([
            RcDoc::text(name),
    ...
    fn doc_table_factor<'a, T: AstInfo>(&'a self, v: &'a TableFactor<T>) -> RcDoc<'a> {
        match v {
            TableFactor::Derived {
                ...
                let mut docs = vec![self.bracket(prefix, self.doc_query(subquery), ")")];
                if let Some(alias) = alias {
            ...
            TableFactor::NestedJoin { join, alias } => {
                let mut doc = self.bracket("(", self.doc_table_with_joins(join), ")");
                if let Some(alias) = alias {
                    doc = self.nest(doc, RcDoc::text(format!("AS {}", alias)));
                }
                doc
            }
    ...
    fn doc_select<'a, T: AstInfo>(&'a self, v: &'a Select<T>) -> RcDoc<'a> {
        ...
        if let Some(distinct) = &v.distinct {
            select = self.nest(select, self.doc_distinct(distinct));
        }
        docs.push(self.nest_comma_separate(select, |s| self.doc_select_item(s), &v.projection));
        if !v.from.is_empty() {
            docs.push(self.title_comma_separate("FROM", |t| self.doc_table_with_joins(t), &v.from));
        }
        if let Some(selection) = &v.selection {
            docs.push(self.nest_title("WHERE", self.doc_expr(selection)));
        }
        if !v.group_by.is_empty() {
            docs.push(self.title_comma_separate("GROUP BY", |e| self.doc_expr(e), &v.group_by));
        }
        if let Some(having) = &v.having {
            docs.push(self.nest_title("HAVING", self.doc_expr(having)));
        }
        if let Some(qualify) = &v.qualify {
            docs.push(self.nest_title("QUALIFY", self.doc_expr(qualify)));
        }
        if !v.options.is_empty() {
            docs.push(self.bracket(
                "OPTIONS (",
        ...
        match v {
            Expr::Op { op, expr1, expr2 } => {
                if let Some(expr2) = expr2 {
                    RcDoc::concat([
                        ...
                        self.doc_expr(expr2).nest(self.config.indent),
                    ])
                } else {
            ...
            Expr::Case {
                ...
                for (c, r) in conditions.iter().zip_eq(results) {
                    let when = self.nest_title("WHEN", self.doc_expr(c));
                    let then = self.nest_title("THEN", self.doc_expr(r));
                    docs.push(self.nest(when, then));
                }
                if let Some(else_result) = else_result {
                    docs.push(self.nest_title("ELSE", self.doc_expr(else_result)));
                }
                let doc = self.intersperse_line_nest(docs);
                self.bracket_doc(RcDoc::text("CASE"), doc, RcDoc::text("END"), RcDoc::line())
            }
            ...
            Expr::Nested(ast) => self.bracket("(", self.doc_expr(ast), ")"),
            Expr::Function(fun) => self.doc_function(fun),
            Expr::Subquery(ast) => self.bracket("(", self.doc_query(ast), ")"),
            Expr::Identifier(_)
            | Expr::Value(_)
            | Expr::QualifiedWildcard(_)
            | Expr::WildcardAccess(_)
            | Expr::FieldAccess { .. } => self.doc_display_pass(v),
            Expr::And { left, right } => self.bracket_doc(
                self.doc_expr(left),
                RcDoc::text("AND"),
                self.doc_expr(right),
                RcDoc::line(),
            ),
            Expr::Or { left, right } => self.bracket_doc(
                self.doc_expr(left),
                RcDoc::text("OR"),
                self.doc_expr(right),
                RcDoc::line(),
            ),
            Expr::Exists(s) => self.bracket("EXISTS (", self.doc_query(s), ")"),
            Expr::IsExpr {
                expr,
                negated,
                construct,
            ...
            Expr::Row { exprs } => {
                self.bracket("ROW(", comma_separate(|e| self.doc_expr(e), exprs), ")")
            }
            Expr::NullIf { l_expr, r_expr } => self.bracket(
                "NULLIF (",
                comma_separate(|e| self.doc_expr(e), [&**l_expr, &**r_expr]),
                ")",
            ),
            Expr::HomogenizingFunction { function, exprs } => self.bracket(
                format!("{function}("),
                comma_separate(|e| self.doc_expr(e), exprs),
                ")",
            ),
            Expr::ArraySubquery(s) => self.bracket("ARRAY(", self.doc_query(s), ")"),
            Expr::ListSubquery(s) => self.bracket("LIST(", self.doc_query(s), ")"),
            Expr::Array(exprs) => {
                self.bracket("ARRAY[", comma_separate(|e| self.doc_expr(e), exprs), "]")
            }
            Expr::List(exprs) => {
                self.bracket("LIST[", comma_separate(|e| self.doc_expr(e), exprs), "]")
            }
    ...
    fn doc_function<'a, T: AstInfo>(&'a self, v: &'a Function<T>) -> RcDoc<'a> {
        match &v.args {
            ...
            FunctionArgs::Args { args, order_by } => {
                ...
                self.bracket(name, comma_separate(|e| self.doc_expr(e), args), ")")
            }
        }
    }
}
