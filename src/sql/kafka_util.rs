...
// TODO(sploiselle): Support default values.
struct Config {
    name: &'static str,
    val_type: ValType,
    rewrite_key: Option<&'static str>,
}

impl Config {
    fn new(name: &'static str, val_type: ValType, rewrite_key: Option<&'static str>) -> Self {
        Config {
            name,
            val_type,
            rewrite_key,
    ...
    fn get_key(&self) -> String {
        if let Some(key) = self.rewrite_key {
            key.to_string()
        } else {
            self.name.replace("_", ".")
        }
