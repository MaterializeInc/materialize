...
pub trait Params {
    fn as_params(self) -> Vec<ParamType>;
}
...
impl<'a> ArgImplementationMatcher<'a> {
    ...
    pub fn select_implementation(
        ...
        let impls = impls
            ...
            .cloned()
            .collect();
        let mut m = Self::new(ident, ecx, impls)?;
    ...
    fn coerce_arg_to_type(
        ...
        if self.ecx.scalar_type(&hinted_expr) == s {
            ...
            if self.ident.contains("jsonb") {
                super::query::plan_cast_internal(self.ecx, Explicit, hinted_expr, s)
            } else if ParamType::StringAny == *typ || arg.is_arg_string_literal() {
        ...
        func_impls! {
            ...
            "jsonb_build_object" => {
                impls! {
                    ...
                    Repeat(vec![StringAny, JsonbAny]) => Variadic(VariadicFunc::JsonbBuildObject)
                }
            },
            "jsonb_pretty" => {
                impls! {
