...
//!     result in catalog corruption in Materialize.
//!
//! Purification is the escape hatch. It is a transformation from SQL AST to SQL
//! AST that "inlines" any external state. For example, the query above would be
//! purified by fetching the schema from the schema registry and inlining it.
//!
//! ```sql
