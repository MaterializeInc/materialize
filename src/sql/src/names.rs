...
// original paths.
pub use mz_sql_types::names::{
    CommentObjectId, DatabaseId, ItemQualifiers, ObjectId, PUBLIC_ROLE_NAME,
...
/// Converts a [`ResolvedObjectName`] into an [`ObjectId`].
///
/// This was an `impl TryFrom<ResolvedObjectName> for ObjectId`, but `ObjectId` now lives in
/// `mz-sql-types` while `ResolvedObjectName` stays in `mz-sql`, so the conversion is a free
/// function here to satisfy the orphan rule.
pub fn object_id_from_resolved(name: ResolvedObjectName) -> Result<ObjectId, anyhow::Error> {
    match name {
