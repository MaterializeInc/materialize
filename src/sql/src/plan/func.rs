            ...
            match raw_arg_type {
                ...
                None => {
                    for c in candidates.iter() {
                        ...
                        if c.fimpl.params[i].accepts(&ScalarType::String) {
                    ...
                    candidates.retain(|c| {
                        if let Some(typ) = &preferred_type {
                            found_preferred_type_candidate =
                                c.fimpl.params[i].accepts(typ) || found_preferred_type_candidate;
                        }
                    ...
                    if found_preferred_type_candidate {
                        ...
                        candidates.retain(|c| c.fimpl.params[i].accepts(&preferred_type));
                    }
                }
                Some(typ) => {
                    found_known = true;
        ...
        if found_known && types_match {
            ...
            for (i, raw_arg_type) in types.iter().enumerate() {
                if raw_arg_type.is_none() {
                    candidates.retain(|c| c.fimpl.params[i].accepts(&common_type));
                }
            }
