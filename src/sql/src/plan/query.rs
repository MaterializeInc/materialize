    ...
    let order_by = {
        ...
        match distinct {
            ...
            Some(Distinct::On(exprs)) => {
                ...
                // expressions (though the order of expressions does not
                // matter). This matches PostgreSQL and leaves the door open to
                // a future optimization where the `DISTINCT ON` and `ORDER BY`
                // operations happen in one pass.
                //
                ...
                // `DISTINCT ON` is semantically a TopK with limit 1. Note that
                // we use the whole `ORDER BY` key, as the PostgreSQL semantics
                // dictate that the `ORDER BY` key determines which record is
                // retained within each group, if it is specific enough.
                relation_expr = RelationExpr::TopK {
                    input: Box::new(relation_expr.map(map_exprs)),
                    group_key: distinct_key,
                    order_key: order_by.clone(),
                    limit: Some(1),
                    offset: 0,
                }
            }
