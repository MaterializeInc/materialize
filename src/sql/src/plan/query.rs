...
fn plan_using_constraint(
    ...
    for column_name in column_names {
        ...
        let l_col_orig_idx = match lhs {
            ColumnRef {
                level: 0,
                column: l_col_orig_idx,
            } => l_col_orig_idx,
            _ => bail!(
        ...
        let r_col_orig_idx = match r {
            ColumnRef {
                level: 0,
                column: r_col_orig_idx,
            } => r_col_orig_idx,
            _ => bail!(
                "Internal error: name {} in USING resolved to outer column",
                column_name
        ...
        let lhs = ScalarExpr::Column(lhs);

        // Adjust the RHS reference to its post-join location.
        let rhs = ScalarExpr::Column(ColumnRef {
            level: 0,
            column: left_scope.len() + r_col_orig_idx,
        });

        // Join keys must be resolved to same type.
        let (expr1, expr2) = match coerce_homogeneous_exprs(
            "USING clause",
            &ecx,
            vec![
                CoercibleScalarExpr::Coerced(lhs.clone()),
                CoercibleScalarExpr::Coerced(rhs.clone()),
            ],
            None,
        ) {
            Ok(mut exprs) => (exprs.remove(0), exprs.remove(0)),
            Err(_) => {
                bail!(
                    "{} and {} are not comparable (in NATURAL/USING join on {})",
                    ecx.humanize_scalar_type(&ecx.scalar_type(&lhs)),
                    ecx.humanize_scalar_type(&ecx.scalar_type(&rhs)),
                    column_name
                )
            }
        };
        ...
        let mut names = left_scope.items[l_col_orig_idx].names.clone();
        names.extend(right_scope.items[r_col_orig_idx].names.clone());
        new_items.push(ScopeItem {
            names,
            expr: None,
            nameable: true,
        });
        dropped_columns.insert(l_col_orig_idx);
        dropped_columns.insert(left_scope.len() + r_col_orig_idx);
    }
    let project_key =
