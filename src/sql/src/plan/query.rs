...
/// Determines the mapping from input source data to the destination table.
///
...
> {
    ...
    // Create an MFP to modify the incoming data.
    let mut table_defaults = item
        .writable_table_details()
        .ok_or(PlanError::InvalidTable {
            name: fullname.item,
        })?
        .to_vec();
    ...
    let mut default_exprs = Vec::with_capacity(table_desc.arity() - source_column_names.len());
    ...
    let source_desc = if columns.is_empty() {
        let indexes = (0..table_desc.arity()).map(|idx| ColumnIndex::from_raw(idx));
...
pub fn plan_copy_from_rows(
    ...
    // projection).
    let default: Vec<_> = (0..desc.arity())
        .map(|x| ColumnIndex::from_raw(x))
        .collect();
    if columns == default {
        return Ok(expr);
    }
