...
fn plan_expr_or_col_index<'a>(ecx: &ExprContext, e: &'a Expr) -> Result<ScalarExpr, anyhow::Error> {
    let expr = match e {
        Expr::Value(Value::Number(n)) => {
            let n = n.parse::<usize>().with_context(|| {
                format!("unable to parse column reference in {}: {}", ecx.name, n)
            })?;
