...
pub fn scalar_type_from_pg(ty: &mz_pgrepr::Type) -> Result<ScalarType, PlanError> {
    match ty {
        ...
        pgrepr::Type::Numeric { max_scale, .. } => Ok(ScalarType::Numeric { scale: Some(*max_scale as u8) }),
        ...
        pgrepr::Type::Char  { length } => Ok(ScalarType::Char { length: *length }),
        pgrepr::Type::VarChar { max_length } => Ok(ScalarType::VarChar { length: *max_length }),
