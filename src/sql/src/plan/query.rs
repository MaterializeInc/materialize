    ...
    let (expr, typ) = match source {
            ...
            && fetch.is_none() =>
        {
            ...
            if columns.is_empty() {
                ...
                let typ = qcx.relation_type(&expr);
                (expr, typ)
            } else if columns.len() < desc.arity() {
                unsupported!("INSERT statement with incomplete column set")
            } else {
                let column_to_index: HashMap<&str, usize> = desc
                    .iter_names()
                    .filter_map(|x| x)
                    .enumerate()
                ...
                for c in &columns {
                    if let Some(idx) = column_to_index.get(c.as_str()) {
                        ordering.push(*idx);
                    } else {
                        bail!(
                            "INSERT statement specified column {}, but it is not present in table",
                            c.as_str(),
                        );
                    }
                }
