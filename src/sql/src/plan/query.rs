...
pub fn scalar_type_from_sql(
    ...
    Ok(match data_type {
        ...
        DataType::Other { name, typ_mod } => {
            let canonical_name = match data_type.internal_canonical_name() {
                Some(name) => name,
                None => name.clone(),
            };
            let canonical_name = normalize::object_name(canonical_name)?;
            match scx.catalog.try_get_scalar_type_by_name(&canonical_name) {
                Some(t) => match t {
                    ScalarType::Decimal(..) => {
                    ...
                    ScalarType::String => {
                        match name.to_string().as_str() {
                            n @ "char" | n @ "varchar" => {
                                validate_typ_mod(n, &typ_mod, &mut [(1, 10_485_760)])?
                            }
                            _ => {}
                        }
                        ScalarType::String
                    ...
                    t => {
                        validate_typ_mod(&name.to_string(), &typ_mod, &mut [])?;
                        t
                    }
                },
...
pub fn unwrap_numeric_typ_mod(typ_mod: &[u64]) -> Result<(u8, u8), anyhow::Error> {
    ...
    validate_typ_mod(
        "numeric",
        &typ_mod,
        &mut [(1, max_precision), (0, max_precision)],
    )?;
...
fn validate_typ_mod(
    ...
    if typ_mod
        .iter()
        .zip(val_ranges.iter())
        .any(|(v, range)| v < &range.0 || v > &range.1)
    {
        bail!(
            "{} type mods must be within ({})",
            name,
            Itertools::join(
                &mut val_ranges.iter().map(|r| format!("[{}-{}]", r.0, r.1)),
                ","
            )
        )
    }
