...
pub struct ScopeItem {
    ...
    /// If true, then this column represents an "exists" flag for the entire
    /// row. In that case, this column must be excluded from `*` expansion. If the
    /// corresponding datum is `NULL`, then `*` expansion should yield a single
    /// `NULL` instead of a record with various datums.
