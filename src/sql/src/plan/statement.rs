        ...
        (CopyDirection::TO, CopyTarget::STDOUT) => {
            match relation {
                CopyRelation::Table { name, columns } => {
                    ...
                    for col in columns {
                        let name: ColumnName = col.as_str().into();
                        if desc.get_by_name(&name).is_none() {
                            bail!("unknown column {}", col);
                        }
                        column_names.push(name);
                    }
                    bail!("table with COPY TO unsupported");
                    /*
                    CopyRelationKind::Table {
                        id: entry.id(),
                        column_names,
                    }
                    */
