...
fn handle_copy(
    ...
    let options = CopyOptions::new(options)?;
...
/// This macro accepts a struct definition and will generate it and a `new`
...
macro_rules! with_options {
  (struct $name:ident {
        ...
        impl $name {
            fn new(mut options: Vec<WithOption>) -> Result<Self, anyhow::Error> {
                let v = Self {
                    $($field_name: {
