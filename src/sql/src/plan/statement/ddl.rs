...
fn plan_sink(
    ...
    let envelope = match envelope {
        ...
        Some(ast::SinkEnvelope::Append) => SinkEnvelope::Append,
    ...
    let key = match &connection {
        CreateSinkConnection::Kafka { key, .. } => key,
        CreateSinkConnection::Iceberg { key, .. } => key,
    };
    let key_indices = if let Some(key) = key.clone() {
        let key_columns = key
            .key_columns
            .into_iter()
            .map(normalize::column_name)
        ...
        let indices = key_columns
            ...
            .map(|col| -> anyhow::Result<usize> {
                let name_idx = desc
                    .get_by_name(col)
                    .map(|(idx, _type)| idx)
                    .ok_or_else(|| sql_err!("column referenced in KEY does not exist: {}", col))?;
                if desc.get_unambiguous_name(name_idx).is_none() {
    ...
    let headers_index = match &connection {
        CreateSinkConnection::Kafka {
            ...
            match envelope {
                ...
                SinkEnvelope::Append => {
                    sql_bail!("HEADERS option is not supported with ENVELOPE APPEND")
                }
            };
    ...
    let partition_by = match &partition_by {
        Some(partition_by) => {
            ...
            match envelope {
                ...
                SinkEnvelope::Append => {
                    sql_bail!("Kafka sink does not support ENVELOPE APPEND")
                }
            };
