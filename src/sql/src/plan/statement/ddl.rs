...
fn kafka_sink_builder(
    ...
    let partition_count = match with_options.remove("partition_count") {
        ...
        Some(_) => bail!("partition count for sink topics has to be an integer"),
    };

    if partition_count == 0 || partition_count < -1 {
        bail!(
            "partition count for sink topics has to be a positive integer or -1 for broker default"
        );
    }
    ...
    let replication_factor = match with_options.remove("replication_factor") {
        None => -1,
        Some(Value::Number(n)) => n.parse::<i32>()?,
        Some(_) => bail!("replication factor for sink topics has to be an integer"),
    };

    if replication_factor == 0 || replication_factor < -1 {
        bail!(
            "replication factor for sink topics has to be a positive integer or -1 for broker default"
        );
    }
