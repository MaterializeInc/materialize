...
use crate::query_model::{
    BoxId, BoxScalarExpr, BoxType, Model, Quantifier, QuantifierId, QueryBox,
};
...
impl<'a> DotGenerator<'a> {
    ...
    fn get_box_head(&self, model: &Model, b: &QueryBox, with_types: bool) -> String {
        ...
        let typed_expr = |e: &BoxScalarExpr| -> String {
            if with_types {
                format!(
                    "{} ({})",
                    e,
                    self.expr_humanizer.humanize_column_type(&e.typ(model))
                )
            } else {
                format!("{}", e)
            }
        ...
        for (i, c) in b.columns.iter().enumerate() {
            if let Some(alias) = &c.alias {
                ...
                rows.push(format!("{}: {}", i, typed_expr(&c.expr)));
            }
        }
