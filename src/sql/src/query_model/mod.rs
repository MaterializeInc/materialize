...
impl Model {
    ...
    /// on a parent.
    pub(super) fn try_visit_pre_post<'a, F, G, E>(
        &'a self,
        pre: &mut F,
        post: &mut G,
    ) -> Result<(), E>
    ...
    /// Same as [`try_visit_pre_post`], but permits mutating the model.
    pub(super) fn try_visit_mut_pre_post<'a, F, G, E>(
        &'a mut self,
        pre: &mut F,
        post: &mut G,
    ) -> Result<(), E>
    ...
    /// Same as [`try_visit_pre_post_descendants`], but permits mutating the model.
