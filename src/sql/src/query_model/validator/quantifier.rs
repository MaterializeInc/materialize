...
///   type `Foreach`.
/// - An `OuterJoin` box must have at most two input quantifiers of
...
mod tests {
    ...
    /// Tests the "a Grouping box must have a single input quantifier of type Foreach" case .
    #[test]
    ...
    /// Tests constraints for quantifiers incident with [`BoxType::Select`] boxes.
    #[test]
    fn test_invalid_select() {
        ...
        model.make_quantifier(QuantifierType::Foreach, box_select, box_dst);
        model.make_quantifier(QuantifierType::Foreach, box_select, box_dst);
    ...
    /// Tests constraints for quantifiers incident with [`BoxType::OuterJoin`] boxes.
    #[test]
    fn test_outer_join() {
        let mut model = Model::new();
        ...
        let box_outer_join = model.make_box(OuterJoin::default().into());

        model.make_quantifier(QuantifierType::Existential, box_lhs, box_outer_join);
        model.make_quantifier(QuantifierType::Scalar, box_rhs, box_outer_join);
        ...
        let errors_act: HashSet<_> = result.unwrap_err().drain(..).collect();
        let errors_exp = HashSet::from([
            ValidationError::InvalidInputQuantifiers(box_outer_join, ONE_OR_TWO_PRES_FOREACH),
            ValidationError::InvalidInputQuantifiers(box_outer_join, TWO_OUTER_JOIN_INPUTS),
            ValidationError::InvalidInputQuantifiers(box_outer_join, ZERO_NOT_OUTER_JOIN_INPUTS),
        ]);
        assert_eq!(errors_act, errors_exp);
    }
