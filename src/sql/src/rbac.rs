...
fn rbac_preamble(
    ...
    // Skip RBAC checks on user items if RBAC is disabled. However, we never skip RBAC checks for system
    ...
    if is_rbac_disabled || is_superuser {
        // We never skip RBAC checks for system objects.
        return Ok(rbac_requirements.filter_to_system_objects());
    }
...
impl RbacRequirements {
    ...
    fn filter_to_system_objects(self) -> RbacRequirements {
