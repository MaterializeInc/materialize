...
const UPSERT_SOURCE_DISK_AVAILABLE: ServerVar<bool> = ServerVar {
    name: UncasedStr::new("upsert_source_disk_available"),
    value: &false,
    description: "Whether or not the `DISK` option in available `UPSERT` sources.",
    internal: true,
    safe: true,
};
...
impl Default for SystemVars {
    fn default() -> Self {
        SystemVars::empty()
            ...
            .with_var(&UPSERT_SOURCE_DISK_AVAILABLE)
...
impl SystemVars {
    ...
    /// Returns the `upsert_source_disk_available` configuration parameter.
    pub fn upsert_source_disk_available(&self) -> bool {
        *self.expect_value(&UPSERT_SOURCE_DISK_AVAILABLE)
    }
