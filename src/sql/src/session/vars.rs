...
pub const CLIENT_ENCODING: ServerVar<ClientEncoding> = ServerVar {
...
const DATE_STYLE: ServerVar<DateStyle> = ServerVar {
...
pub const INTERVAL_STYLE: ServerVar<IntervalStyle> = ServerVar {
...
static SEARCH_PATH: Lazy<ServerVar<Vec<Ident>>> = Lazy::new(|| ServerVar {
    ...
    value: vec![Ident::new(DEFAULT_SCHEMA)],
    description:
...
mod upsert_rocksdb {
    ...
    pub const UPSERT_ROCKSDB_COMPACTION_STYLE: ServerVar<CompactionStyle> = ServerVar {
    ...
    pub const UPSERT_ROCKSDB_COMPRESSION_TYPE: ServerVar<CompressionType> = ServerVar {
    ...
    pub const UPSERT_ROCKSDB_BOTTOMMOST_COMPRESSION_TYPE: ServerVar<CompressionType> = ServerVar {
    ...
    pub const UPSERT_ROCKSDB_BATCH_SIZE: ServerVar<usize> = ServerVar {
    ...
    pub const UPSERT_ROCKSDB_RETRY_DURATION: ServerVar<Duration> = ServerVar {
...
pub const ENABLE_LAUNCHDARKLY: ServerVar<bool> = ServerVar {
...
const UNSAFE_MOCK_AUDIT_EVENT_TIMESTAMP: ServerVar<Option<mz_repr::Timestamp>> = ServerVar {
    ...
    ($(($name:expr, $feature_desc:literal)),+ $(,)?) => {
        paste::paste!{
            $(
                ...
                const [<$name:upper _VAR>]: ServerVar<bool> = ServerVar {
    ...
    V::Owned: Debug + Send + Sync + PartialEq,
{
    ...
    /// Reset the stored value to the default.
    fn reset(&mut self, local: bool) {
        let value = self.parent.value.to_owned();
        if local {
