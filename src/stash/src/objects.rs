        ...
        proto::StatementLoggingEvent {
            value: Some(match self {
                ...
                StatementLoggingEvent::EndedExecution(ee) => {
                    ...
                    use proto::statement_logging_event::statement_ended_execution::ExecutionStrategy as ProtoExecutionStrategy;
                        ...
                        proto::statement_logging_event::StatementEndedExecution {
                            ...
                            reason: Some(match reason {
                                StatementEndedExecutionReason::Success {
                                    ...
                                    execution_strategy: execution_strategy.map(
                                        |execution_strategy| match execution_strategy {
                                            StatementExecutionStrategy::Standard => {
                                                ProtoExecutionStrategy::Standard.into()
                                            }
                                            StatementExecutionStrategy::FastPath => {
                                                ProtoExecutionStrategy::FastPath.into()
                                            }
                                            StatementExecutionStrategy::Constant => {
                                                ProtoExecutionStrategy::Constant.into()
                                            }
                                        },
                                    ),
                                }),
                    ...
                    Some(ProtoReason::Success(ProtoSuccess {
                        ...
                        execution_strategy: execution_strategy
                            .map(|execution_strategy| {
                                match ProtoExecutionStrategy::from_i32(execution_strategy) {
                                    Some(ProtoExecutionStrategy::Standard) => {
                                        Ok(StatementExecutionStrategy::Standard)
                                    }
                                    Some(ProtoExecutionStrategy::FastPath) => {
                                        Ok(StatementExecutionStrategy::FastPath)
                                    }
                                    Some(ProtoExecutionStrategy::Constant) => {
                                        Ok(StatementExecutionStrategy::Constant)
                                    }
