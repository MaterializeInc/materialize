        ...
        I::IntoIter: Send,
    {
        ...
        self.transact(move |stmts, tx| {
            ...
            Box::pin(async move {
                ...
                // Pipeline all batches and consolidations with a mess of futures.
                ...
                {
                    consolidate.push(collection_id);
                    let batch_futures = future::try_join4(
                        async move {
                            let upper = Self::upper_tx(stmts, tx, collection_id).await?;
                            if upper != lower {
                        ...
                        Self::update_many_tx(stmts, tx, collection_id, entries.into_iter()),
                        async move {
                            Self::seal_batch_tx(stmts, tx, std::iter::once((collection_id, &upper)))
                                .await
                        },
                        async move {
                            Self::compact_batch_tx(
                                stmts,
                                tx,
                                std::iter::once((collection_id, &compact)),
                            )
                            .await
                        },
                    );
                ...
                let futures = future::try_join(
                    ...
                    Self::consolidate_batch_tx(stmts, tx, consolidate.into_iter()),
                );
                futures.await?;
                Ok(())
            })
