...
///
/// The enum representation reflects the fact that wallclock lag is undefined for empty
/// collections, i.e. collections that contain no readable times.
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord)]
pub enum WallclockLag {
    /// Lag value in seconds, for non-empty collections.
    Seconds(u64),
    /// Undefined lag, for empty collections.
    Undefined,
}

impl Default for WallclockLag {
    fn default() -> Self {
        Self::Seconds(0)
    }
}
