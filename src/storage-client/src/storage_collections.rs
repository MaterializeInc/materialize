    ...
    async fn create_collections_for_bootstrap(
        ...
        let to_register: Vec<_> = futures::stream::iter(enriched_with_metadata)
            .map(|data: Result<_, StorageError<Self::Timestamp>>| {
                ...
                let (write, mut since_handle, leased_dur, critical_dur, write_dur, upper_dur) = this
                    .open_data_handles(
                        &id,
                        metadata.data_shard,
                ...
                Ok::<_, StorageError<Self::Timestamp>>((id, description, write, since_handle, metadata, open_handle_dur, leased_dur, critical_dur, write_dur, upper_dur, downgrade_dur))
            }})
