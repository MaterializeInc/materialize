    ...
    async fn create_collections(
        ...
        let enriched_with_metadata = collections
            ...
            .map(|(id, description)| {
                ...
                let get_shard = |id| -> Result<ShardId, StorageError> {
                    let shard = match self.collections.get(&id) {
                        Some(col) => col.collection_metadata.data_shard,
                        None => storage_metadata.get_collection_shard(id)?,
                    };
                    Ok(shard)
                };
    ...
    Self: StorageController<Timestamp = T>,
{
    ...
    async fn finalize_shards(&mut self) {
        ...
        let finalized_shards: BTreeSet<ShardId> = futures::stream::iter(
            self.finalizable_shards.clone(),
        )
                ...
                mz_ore::task::spawn(|| format!("finalize_shard({shard_id})"), async move {
                    let finalize = || async move {
                        ...
                        if !upper.is_empty() {
                            ...
                            if let Err(e) = append {
                                warn!("tried to finalize a shard with an advancing upper: {e:?}");
                                return Ok(());
                            }
                        }
