    ...
    StorageResponse<T>: RustType<ProtoStorageResponse>,
{
    ...
    fn initialization_complete(&mut self) {
        ...
        self.storage_collections.initialization_complete();
    }
    ...
    > {
        self.storage_collections.collection_frontiers(id)
    }
    ...
    fn collections_frontiers(
        ...
        self.storage_collections.collections_frontiers(ids)
    }
    ...
    async fn create_collections(
        ...
        for (id, mut description, write, metadata) in to_register {
            ...
            let mut collection_state = CollectionState {
                ...
                extra_state: CollectionExtraState::None,
            };
            ...
            match &collection_state.data_source {
                ...
                DataSource::IngestionExport {
                    ...
                    collection_state.extra_state = CollectionExtraState::Ingestion(ingestion_state);
    ...
    async fn alter_ingestion_source_desc(
        ...
        self.storage_collections
            .alter_ingestion_source_desc(ingestion_id, source_desc.clone())
            .await?;

        let collection = self.collection(ingestion_id).expect("validated exists");
        let curr_ingestion = match &collection.data_source {
        ...
        for export_id in curr_ingestion.source_exports.keys() {
            ...
            let DataSource::IngestionExport {
                ingestion_id,
                external_reference,
            } = &self.collection(*export_id)?.data_source
            else {
                ...
                for id in ids.iter() {
                    if let Some(collection) = self.collections.remove(id) {
                        ...
                        match collection.extra_state {
                            CollectionExtraState::Ingestion(mut ingestion) => {
                                tracing::debug!(
        ...
        for compaction_command in self.pending_compaction_commands.drain(..) {
            ...
            if read_frontier.is_empty() {
                if client.is_some() && self.collections.contains_key(&id) {
                    ...
                    match collection.extra_state {
                        CollectionExtraState::Ingestion(_) => {
    ...
    async fn record_replica_frontiers(
        ...
        for (object_id, collection_state) in self.collections.iter() {
            let ingestion_state = match &collection_state.extra_state {
                CollectionExtraState::Ingestion(ingestion) => ingestion,
                CollectionExtraState::None => continue,
            };
...
struct CollectionState<T: TimelyTimestamp> {
    ...
    pub extra_state: CollectionExtraState<T>,
}
...
#[derive(Debug)]
enum CollectionExtraState<T: TimelyTimestamp> {
