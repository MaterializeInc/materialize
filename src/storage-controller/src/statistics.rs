    ...
    mz_ore::task::spawn(|| "statistics_scraper", async move {
        ...
        let mut correction = Vec::new();
        {
            ...
            for row in previous_values {
                ...
                shared_stats
                    .as_mut_stats()
                    .insert(current.0, StatsState::Initialized(current.2));
            }
...
pub(super) struct SourceStatistics {
    ...
    /// Per-replica source statistics that will be emitted to the raw statistics
    /// table, same as the above.
    pub per_replica_source_statistics:
        BTreeMap<(GlobalId, ReplicaId), StatsState<ControllerSourceStatistics>>,
            ...
            tokio::select! {
                ...
                _ = interval.tick() => {
                    ...
                    // be fine!
                    {
                        ...
                        for (_, stats) in shared_stats.per_replica_source_statistics.iter_mut() {
                            let stat = stats.stat_to_emit();
                            stat.pack(row_buf.packer());
                            correction.push((row_buf.clone(), 1));
                            stats.mark_initialized();
