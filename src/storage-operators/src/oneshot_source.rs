...
/// for fetching.
pub fn render_discover_objects<S>(
    scope: Scope<Timestamp>,
    collection_id: GlobalId,
    source: S,
    filter: ContentFilter,
) -> (
    StreamVec<Timestamp, Result<(S::Object, S::Checksum), StorageErrorX>>,
    PressOnDropButton,
)
where
...
pub fn render_split_work<S, F>(
    scope: Scope<Timestamp>,
    collection_id: GlobalId,
    objects: StreamVec<Timestamp, Result<(S::Object, S::Checksum), StorageErrorX>>,
    source: S,
    format: F,
) -> (
    StreamVec<Timestamp, Result<F::WorkRequest<S>, StorageErrorX>>,
    PressOnDropButton,
)
where
...
pub fn render_fetch_work<S, F>(
    scope: Scope<Timestamp>,
    collection_id: GlobalId,
    source: S,
    format: F,
    work_requests: StreamVec<Timestamp, Result<F::WorkRequest<S>, StorageErrorX>>,
) -> (
    StreamVec<Timestamp, Result<F::RecordChunk, StorageErrorX>>,
    PressOnDropButton,
)
where
...
pub fn render_decode_chunk<F>(
    scope: Scope<Timestamp>,
    format: F,
    record_chunks: StreamVec<Timestamp, Result<F::RecordChunk, StorageErrorX>>,
    mfp: SafeMfpPlan,
) -> (
    StreamVec<Timestamp, Result<Row, StorageErrorX>>,
    PressOnDropButton,
)
where
...
pub fn render_stage_batches_operator(
    scope: Scope<Timestamp>,
    collection_id: GlobalId,
    collection_meta: &CollectionMetadata,
    persist_clients: Arc<PersistClientCache>,
    rows_stream: StreamVec<Timestamp, Result<Row, StorageErrorX>>,
) -> (
    StreamVec<Timestamp, Result<ProtoBatch, StorageErrorX>>,
    PressOnDropButton,
)
where
{
...
pub fn render_completion_operator<F>(
    scope: Scope<Timestamp>,
    results_stream: StreamVec<Timestamp, Result<ProtoBatch, StorageErrorX>>,
    worker_callback: F,
) where
    F: FnOnce(Result<Option<ProtoBatch>, String>) -> () + 'static,
{
