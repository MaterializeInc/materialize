...
use crate::oneshot_source::{
    OneshotFormat, OneshotSource, StorageErrorX, StorageErrorXKind,
};
...
impl OneshotFormat for CsvDecoder {
    type WorkRequest<S> = CsvWorkRequest<S::Object, S::Checksum>
        where 
            S: OneshotSource;
    ...
    fn process_work<'a, S: OneshotSource + Sync>(
        ...
        csv_async::AsyncReader::from_reader(stream_reader)
            ...
            .map_ok(|record| {
                ...
                let ranges = (0..record.len()).map(|idx| record.range(idx).expect("known to exist")).collect();
