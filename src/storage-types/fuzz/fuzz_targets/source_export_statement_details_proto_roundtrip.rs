...
//!    malformed/adversarial wire input (including the SQL Server `Lsn`
//!    `try_from` length guard, which is only reachable from raw bytes since a
//!    re-encoded `Lsn` is always exactly 10 bytes).
//!
