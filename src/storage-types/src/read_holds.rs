...
pub struct ReadHold<T: TimelyTimestamp> {
    ...
    pub holds_tx: UnboundedSender<(GlobalId, ChangeBatch<T>)>,
}
...
// NOTE: This Serialize impl is only used for dumping coordinator state, for
// debugging.
impl<T: TimelyTimestamp> Serialize for ReadHold<T> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("ReadHold", 2)?;

        state.serialize_field("id", &self.id)?;
        state.serialize_field("since", &self.since)?;
...
impl<T: TimelyTimestamp> ReadHold<T> {
    ...
    pub fn since(&self) -> AntichainRef<T> {
        self.since.borrow()
    }
    ...
    pub fn try_downgrade(&mut self, frontier: Antichain<T>) -> Result<(), anyhow::Error> {
        if PartialOrder::less_than(&frontier, &self.since) {
            return Err(anyhow::anyhow!(
                "new frontier {:?} is beyond current since {:?}",
                frontier,
                self.since
            ));
        }
...
impl<T: TimelyTimestamp> Clone for ReadHold<T> {
    fn clone(&self) -> Self {
        if self.id.is_user() {
            tracing::debug!("cloning ReadHold on {}: {:?}", self.id, self.since);
        }
...
impl<T: TimelyTimestamp> Drop for ReadHold<T> {
    fn drop(&mut self) {
        if !self.since.is_empty() {
            if self.id.is_user() {
                tracing::debug!("dropping ReadHold on {}: {:?}", self.id, self.since);
            }
            let mut changes = ChangeBatch::new();
