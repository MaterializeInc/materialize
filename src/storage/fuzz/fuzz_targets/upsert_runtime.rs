...
//! — the per-timestamp `multi_get` -> order-keyed per-command processing ->
...
use mz_storage::upsert::fuzz_drain_staged_input;
use mz_storage::upsert::types::FuzzUpsertParts;
...
// iteration — re-registering its Prometheus metrics each call was the
