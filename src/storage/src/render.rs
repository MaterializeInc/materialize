...
pub fn build_ingestion_dataflow(
    ...
    timely_worker.dataflow_core(&name, worker_logging, Box::new(()), |_, root_scope| {
        ...
        root_scope.clone().scoped(&name, |mz_scope| {
            ...
            // The persist sink commits a ceiling a wall-clock lookahead ahead of a snapshotting
            // export's frontier. A CDCv2 export takes its MZ times from the data rather than from
            // reclocking, so no lookahead relates to the clock and a ceiling the frontier never
            // reaches would hold the shard upper forever.
