...
/// persist.
///
/// With a `lookahead`, and while the frontier sits at `snapshot_time`, it also commits to a
/// ceiling that far past the data and broadcasts it on the second output. A
/// ceiling is not a description: it gives the writers a bound to group updates under before the
/// frontier certifies anything, and it binds this operator, which mints nothing below an
/// outstanding ceiling. So the whole snapshot and the catch-up behind it become one description,
/// emitted when the frontier reaches the ceiling. See [`next_mint`].
///
...
fn mint_batch_descriptions<'scope>(
    ...
    let shutdown_button = mint_op.build(move |capabilities| async move {
        ...
        // The data-passthrough output uses the data capabilities, so we drop its capability here.
        ...
        // The wait sits at the bottom of the loop, so a mint pass runs once before the first
        // event as well as after every event. A fresh export's snapshot lands at the minimum and
        // its frontier never moves during it, so no event announces the pin. Its first ceiling
        // has to go out ahead of the snapshot's rows rather than alongside them.
        loop {
            // `desired_frontier` starts at the minimum. A fresh export's as_of is the minimum too,
            // so it reads as snapshotting from the first activation. A later as_of waits for the
            // progress statement that moves the frontier there. A row can arrive ahead of that
            // statement, and committing on it would anchor the ceiling at the shard upper instead
            // of the snapshot's time.
            let snapshot_in_progress = snapshot_time
                .as_ref()
                .is_some_and(|time| *time == desired_frontier);
            ...
            tokio::select! {
                event = desired_input.next() => match event {
                    ...
                    Some(Event::Progress(frontier)) => desired_frontier = frontier,
...
fn write_batches<'scope>(
    ...
    let shutdown_button = write_op.build(move |_capabilities| async move {
        ...
        // timestamp entirely or not at all. It is finished only once its description is ready,
        // since rows at its timestamp can still be staged after that description is processed.
                ...
                )
            {
                ...
                for batch_description in ready_batches {
                    ...
                    let covered: Vec<_> = uncovered_builders
                        ...
                        .collect();
                    let mut batch_tokens = Vec::with_capacity(covered.len() + 1);
                    for ts in covered {
...
mod tests {
    ...
    async fn run_sink_pipeline(
        ...
        run_worker(move |worker| {
            ...
            // Closing makes every outstanding description ready. `step_or_park` reports whether
            // any dataflow remains, and the operators retire once their inputs are exhausted, so
            // stepping until it reports none drains everything they still owe.
            data_input.close();
            for _ in 0..20_000 {
                if !worker.step_or_park(Some(Duration::from_millis(1))) {
                    break;
    ...
    /// The writer drains descriptions ahead of data in each round, and input keeps queueing while
    /// it awaits persist, so rows at a timestamp can be staged after the description covering them
    /// was processed. They belong in the batch that timestamp already has.
    #[mz_ore::test(tokio::test(flavor = "multi_thread"))]
    #[cfg_attr(miri, ignore)] // unsupported operation: returning ready events from epoll_wait
    async fn write_batches_keeps_one_batch_per_timestamp_for_rows_after_the_description() {
        const ROWS: usize = 4;
        const DONE: u64 = 4;
        ...
        .await;
        assert_eq!(total, i64::try_from(ROWS * 2).expect("small"));
    }
    ...
    /// An export that does not snapshot in this incarnation is given no snapshot time and never
    /// commits ahead of its frontier, so a restart of a caught up source has no window where it
    /// might.
    #[mz_ore::test(tokio::test(flavor = "multi_thread"))]
    #[cfg_attr(miri, ignore)] // unsupported operation: returning ready events from epoll_wait
    async fn mint_batch_descriptions_does_not_commit_without_a_snapshot_time() {
        ...
        assert_eq!(
            ...
            "the same script commits when the export snapshots at 1",
        );
    }
    ...
    /// minimum. Nothing is committed until the frontier reaches the snapshot time, so the ceiling
    /// is anchored there and not at the shard upper the row was seen under.
    #[mz_ore::test(tokio::test(flavor = "multi_thread"))]
    #[cfg_attr(miri, ignore)] // unsupported operation: returning ready events from epoll_wait
    async fn mint_batch_descriptions_waits_for_the_frontier_to_reach_the_snapshot_time() {
        let emitted = run_mint_batch_descriptions(
            test_target(),
            test_persist_clients(),
            Some(4),
        ...
        assert_eq!(
            ...
            Minted {
                descriptions: vec![(0, 1), (1, 6)],
                // Anchored at the snapshot time, then re-committed a lookahead past the row at 2.
                commitments: vec![(1, 5), (1, 6)],
            },
        );
    }

    /// A fresh source's snapshot lands at the minimum and its frontier never leaves it during the
    /// snapshot, so no progress statement announces the pin. The first ceiling goes out on the
    /// first activation, ahead of every row, and is re-committed as the data advances.
    #[mz_ore::test(tokio::test(flavor = "multi_thread"))]
    #[cfg_attr(miri, ignore)] // unsupported operation: returning ready events from epoll_wait
    async fn mint_batch_descriptions_commits_on_the_first_activation_at_a_minimum_snapshot_time() {
        // Replication rows land first, the snapshot's own rows at the pinned time arrive behind
        // them, and the frontier never moves.
        let script = vec![
            Step::Updates(5, 4),
            Step::Updates(0, 4),
            Step::Updates(9, 2),
        ...
        assert_eq!(
            ...
            .await,
            Minted {
                // The frontier never moves, so nothing is derived from it.
                descriptions: vec![],
                // Before any row, then a lookahead past each timestamp the data reached. The rows
                // at the pinned time sit below the outstanding ceiling and re-commit nothing.
                commitments: vec![(0, 4), (0, 9), (0, 13)],
            },
        );
        ...
        assert_eq!(
            ...
            .await,
            Minted::default(),
            "a snapshot time the frontier has not reached commits nothing, however far the data runs",
        );
    }
    ...
    /// Rows staged before a ceiling covers their timestamp stay in that timestamp's own builder.
    /// Rows arriving at it once a ceiling covers it join the open builder instead, and both are
    /// finished under the description once it is ready.
    #[mz_ore::test(tokio::test(flavor = "multi_thread"))]
    #[cfg_attr(miri, ignore)] // unsupported operation: returning ready events from epoll_wait
    async fn write_batches_keeps_an_uncovered_batch_apart_from_the_open_builder() {
        const PINNED: u64 = 2;
        ...
        assert_eq!(
            ...
            "rows staged before the ceiling should keep their own batch, and later rows at that \
            timestamp should join the open builder"
        );
        ...
        assert_eq!(
            ...
            "splitting a timestamp across two batches must not change what the shard ends up holding"
        );
    }
