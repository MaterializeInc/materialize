    ...
    writer::{
        ...
        base_writer::{
            ...
            equality_delete_writer::{
                EqualityDeleteFileWriter, EqualityDeleteFileWriterBuilder,
                EqualityDeleteWriterConfig,
            },
            position_delete_writer::{PositionDeleteFileWriterBuilder, PositionDeleteWriterConfig},
        },
        combined_writer::delta_writer::{DeltaWriter, DeltaWriterBuilder},
        file_writer::{ParquetWriterBuilder, location_generator},
    },
};
...
use mz_storage_types::{
    StorageDiff,
    configuration::StorageConfiguration,
    controller::CollectionMetadata,
    errors::DataflowError,
    sinks::{IcebergSinkConnection, StorageSinkDesc},
    sources::SourceData,
};
...
use timely::{
    ...
    dataflow::{
        ...
        operators::{CapabilitySet, Concatenate, Map, ToStream},
    },
    progress::{Antichain, Timestamp as _},
};

use crate::{
    healthcheck::{HealthStatusMessage, HealthStatusUpdate, StatusNamespace},
    render::sinks::SinkRender,
...
const INITIAL_DESCRIPTIONS_TO_MINT: usize = 3;
...
async fn load_or_create_table(
    ...
    match catalog.load_table(&table_ident).await {
        ...
        Err(err) => {
            ...
            if err_string.contains("The specified table does not exist.")
                || err_string.contains("Tried to load a table that does not exist")
            {
...
/// Initialize the Iceberg table and extract its schema.
/// This operator loads or creates the table on first data and stores
/// the authoritative Arrow schema (with correct field IDs) in shared state.
fn init_table_and_extract_schema<G>(
    name: String,
    ...
    G: Scope<Timestamp = Timestamp>,
{
    ...
    let mut builder = OperatorBuilder::new(name, input.inner.scope());
...
/// Batches are minted with 10-second windows to balance write efficiency with latency.
        ...
        Box::pin(async move {
            ...
            *data_capset = CapabilitySet::new();
            ...
            let table = catalog
                ...
                .await
                .context("Failed to load Iceberg table (should have been created by init operator)")?;
            ...
            loop {
                ...
                if !initialized {
                    ...
                    // Mint initial future batch descriptions at 10-second intervals
                    for i in 1..INITIAL_DESCRIPTIONS_TO_MINT + 1 {
                        let duration_ts = Timestamp::new(Duration::from_secs(10 * i as u64).as_millis() as u64);
                    ...
                    let (_, new_upper) = batch_descriptions.pop().unwrap();
                    ...
                    while let Some(oldest_desc) = minted_batches.front() {
                        ...
                        let duration_ts = Timestamp::new(Duration::from_secs(10).as_millis() as u64);
    ...
    let (button, errors) = builder.build_fallible(|caps| {
        Box::pin(async move {
            ...
            *capset = CapabilitySet::new();
            ...
            let namespace_ident = NamespaceIdent::new(connection.namespace.clone());
            let table_ident = TableIdent::new(namespace_ident, connection.table.clone());
            let table = catalog.load_table(&table_ident).await.context(
                "Failed to load Iceberg table (should have been created by init operator)",
            )?;

            let table_metadata = table.metadata().clone();
            let current_schema = table_metadata.current_schema().clone();
            ...
            let equality_ids = equality_indices
                .iter()
                .map(|u| (*u as i32) + 1)
                .collect::<Vec<_>>();
            ...
            while !(batch_description_frontier.is_empty() && input_frontier.is_empty()) {
                ...
                ) || PartialOrder::less_than(&processed_input_frontier, &input_frontier)
                {
                    ...
                    let ready_batch_descriptions: Vec<_> = in_flight_batches
                        .keys()
                        .cloned()
                        .filter(|(lower, upper)| {
                        ...
                        .collect();
                    for desc in ready_batch_descriptions {
                        let (cap, mut delta_writer) = in_flight_batches.remove(&desc).unwrap();
    ...
    G: Scope<Timestamp = Timestamp>,
{
    ...
    let (button, errors) = builder.build_fallible(move |_caps| {
        Box::pin(async move {
            ...
            // Load the table (it should already exist, created by the init operator)
            let namespace_ident = NamespaceIdent::new(connection.namespace.clone());
            let table_ident = TableIdent::new(namespace_ident, connection.table.clone());
                ...
                for batch in done_batches {
                    ...
                    if !data_files.is_empty() || !delete_files.is_empty() {
                        ...
                        if let Some((last_frontier, _last_version)) = last {
                            if PartialOrder::less_than(&frontier, &last_frontier) {
                                anyhow::bail!(
                                    "Iceberg table '{}' has been modified by another writer. Current frontier: {:?}, last frontier: {:?}. Schema or partition spec evolution is not supported.",
                                    connection.table,
                                    frontier,
                                    last_frontier,
                                );
...
impl<G: Scope<Timestamp = Timestamp>> SinkRender<G> for IcebergSinkConnection {
    ...
    fn render_sink(
        ...
        // Convert the sink RelationDesc to an Arrow Schema for validation with Iceberg
        let arrow_schema = match mz_arrow_util::builder::desc_to_schema(&sink.from_desc) {
            Ok(schema) => schema,
            Err(err) => {
                // Return an error stream if we cannot convert the schema
                let error_stream = std::iter::once(HealthStatusMessage {
                    id: None,
                    update: HealthStatusUpdate::halting(
                        format!("Failed to convert sink schema to Arrow: {}", err),
                        None,
                    ),
                    namespace: StatusNamespace::Iceberg,
                })
        ...
        // Add field IDs to the Arrow schema for Iceberg compatibility
        let arrow_schema_with_ids = add_field_ids_to_arrow_schema(arrow_schema);

        // Convert Arrow Schema to Iceberg Schema
        let iceberg_schema = match arrow_schema_to_schema(&arrow_schema_with_ids) {
            ...
            Err(err) => {
                ...
                let error_stream = std::iter::once(HealthStatusMessage {
                    id: None,
                    update: HealthStatusUpdate::halting(
                        format!("Failed to convert Arrow schema to Iceberg: {}", err),
                        None,
                    ),
                    namespace: StatusNamespace::Iceberg,
                })
        ...
        let (initialized_input, init_status, init_button) = init_table_and_extract_schema(
            format!("{sink_id}-iceberg-init"),
            &input,
            connection_clone,
        ...
        let statuses = scope.concatenate([
            running_status,
            mint_status,
            init_status,
            write_status,
        ...
        (
            statuses,
            vec![mint_button, init_button, write_button, commit_button],
        )
    }
}
