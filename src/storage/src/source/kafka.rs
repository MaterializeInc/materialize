    ...
    let button = builder.build(move |caps| {
        SignaledFuture::new(busy_signal, async move {
            ...
            loop {
                ...
                tokio::select! {
                    ...
                    event = metadata_input.next() => match event {
                        Some(Event::Data(_, mut updates)) => {
                            // Ignore late updates. (Not sure if this is necessary?)
                            updates.retain(|u| prev_metadata_ts.map_or(true, |ts| ts < u.probe_ts));
                            // Only consider the most recent update.
                            updates.sort_unstable_by_key(|u| u.probe_ts);
                ...
                if let Some(update) = metadata_update.take() {
                    match update.result {
                        Ok(partitions) => {
                            ...
                            if !PartialOrder::less_equal(data_cap.time(), &future_ts) {
                                let prev_pid_count =
                                    prev_pid_info.map(|info| info.len()).unwrap_or(0);
                        ...
                        Err(status) => {
                            if let Some(status) = status.kafka {
                    ...
                    prev_metadata_ts = Some(update.probe_ts);
                }
...
struct MetadataUpdate {
    ...
    result: Result<BTreeMap<PartitionId, HighWatermark>, HealthStatus>,
}

impl MetadataUpdate {
    fn to_probe(&self) -> Option<Probe<KafkaTimestamp>> {
        let Ok(partition_info) = &self.result else {
            return None;
        };

        let max_pid = partition_info.keys().last().copied();
        let lower = max_pid
            .map(RangeBound::after)
            .unwrap_or(RangeBound::NegInfinity);
        ...
        for (pid, high_watermark) in partition_info {
    ...
    let button = builder.build(move |caps| async move {
        ...
        let consumer = match consumer {
            ...
            Err(e) => {
                ...
                let status = if matches!(e, ContextCreationError::Ssh(_)) {
                    HealthStatus {
                        ssh: Some(update),
                        kafka: None,
                    }
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            loop {
                ...
                let update = MetadataUpdate { probe_ts, result };
