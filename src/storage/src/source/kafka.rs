...
use timely::dataflow::operators::{Broadcast, Capability, Concat};
...
impl SourceRender for KafkaSourceConnection {
    ...
    fn render<G: Scope<Timestamp = KafkaTimestamp>>(
        ...
        let (metadata, probes, metadata_health, metadata_token) =
            render_metadata_fetcher(scope, self.clone(), config.clone());
        let (data, progress, reader_health, stats, reader_token) =
            render_reader(scope, self, config, resume_uppers, metadata, start_signal);

        (
            data,
            Some(progress),
            metadata_health.concat(&reader_health),
            stats,
            Some(probes),
...
fn render_reader<G: Scope<Timestamp = KafkaTimestamp>>(
    ...
    let button = builder.build(move |caps| {
        SignaledFuture::new(busy_signal, async move {
            ...
            let mut prev_metadata: Option<MetadataUpdate> = None;
            ...
            loop {
                ...
                tokio::select! {
                    ...
                    event = metadata_input.next() => match event {
                        Some(Event::Data(_, mut updates)) => {
                            // Ignore late updates. (This might not be necessary?)
                            let prev_ts = prev_metadata.as_ref().map(|m| m.timestamp);
                            updates.retain(|u| prev_ts.map_or(true, |ts| ts < u.timestamp));

                            // Only consider the most recent update.
                            updates.sort_unstable_by_key(|u| u.timestamp);
                            metadata_update = updates.pop();
                        }
                        Some(Event::Progress(_)) => {}
                        None => panic!("metadata stream ended unexpectedly"),
                    }
                }

                if let Some(metadata) = metadata_update.take() {
                    let max_pid = metadata.partitions.keys().last().cloned();
                    let lower = max_pid
                        .map(RangeBound::after)
                        .unwrap_or(RangeBound::NegInfinity);
                    let future_ts =
                    ...
                    if !PartialOrder::less_equal(data_cap.time(), &future_ts) {
                        let prev_pid_count = prev_metadata.map(|p| p.partitions.len()).unwrap_or(0);
                        let pid_count = metadata.partitions.len();
                    ...
                    if let Some(prev_metadata) = &prev_metadata {
                        for (pid, prev_high_watermark) in &prev_metadata.partitions {
                            let high_watermark = metadata.partitions[pid];
                    ...
                    for (&pid, &high_watermark) in &metadata.partitions {
                        if responsible_for_pid(&config, pid) {
...
impl MetadataUpdate {
    fn to_probe(&self) -> Probe<KafkaTimestamp> {
        let max_pid = self.partitions.keys().last().copied();
        let lower = max_pid
            .map(RangeBound::after)
            .unwrap_or(RangeBound::NegInfinity);
        ...
        for (pid, high_watermark) in &self.partitions {
...
fn render_metadata_fetcher<G: Scope<Timestamp = KafkaTimestamp>>(
    ...
    let button = builder.build(move |caps| async move {
        ...
        let [metadata_cap, probe_cap, health_cap] = caps.try_into().unwrap();

        let output_indexes: Vec<_> = config
            .source_exports
            .values()
            .map(|e| e.ingestion_output)
            .collect();
        let emit_health = |namespace, update| {
            for (&index, update) in output_indexes.iter().repeat_clone(update) {
                let msg = HealthStatusMessage {
                    index,
                    namespace,
                    update,
                };
                health_output.give(&health_cap, msg);
            }
        };
        ...
        let consumer = match consumer {
            ...
            Err(e) => {
                ...
                let namespace = match e {
                    ContextCreationError::Ssh(_) => StatusNamespace::Ssh,
                    _ => StatusNamespace::Kafka,
                };
                emit_health(namespace, HealthStatusUpdate::halting(msg, None));
        ...
        while let Some(update) = rx.recv().await {
            match update {
                Ok(metadata) => {
                    probe_output.give(&probe_cap, metadata.to_probe());
                    metadata_output.give(&metadata_cap, metadata);
                    ...
                    emit_health(StatusNamespace::Kafka, HealthStatusUpdate::running());
                    emit_health(StatusNamespace::Ssh, HealthStatusUpdate::running());
                }
                Err(status) => {
                    if let Some(update) = status.kafka {
                        emit_health(StatusNamespace::Kafka, update);
                    }
                    if let Some(update) = status.ssh {
                        emit_health(StatusNamespace::Ssh, update);
                    }
                }
            }
        }
...
fn spawn_metadata_thread<C: ConsumerContext>(
    ...
    tx: mpsc::UnboundedSender<Result<MetadataUpdate, HealthStatus>>,
) {
    thread::Builder::new()
