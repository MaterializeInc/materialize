...
pub enum SourceMessageType {
    /// A source message
    Message(
        Result<SourceMessage<Option<Vec<u8>>, Option<Vec<u8>>>, SourceReaderError>,
        Capability<Partitioned<PartitionId, MzOffset>>,
        Diff,
    ),
...
impl KafkaSourceReader {
    async fn from_connection(
        source_connection: KafkaSourceConnection,
        source_name: String,
        source_id: GlobalId,
        worker_id: usize,
        worker_count: usize,
        ...
        resume_upper: Antichain<Partitioned<PartitionId, MzOffset>>,
        _: SourceDataEncoding,
        metrics: crate::source::metrics::SourceBaseMetrics,
        connection_context: ConnectionContext,
    ) -> Result<(KafkaSourceReader, KafkaOffsetCommitter), anyhow::Error> {
        ...
        let consumer: BaseConsumer<_> = connection
            .create_with_context(
                &connection_context,
                GlueConsumerContext {
        ...
        // from.
        let mut start_offsets: BTreeMap<_, i64> = source_connection
            .start_offsets
            .into_iter()
            .filter(|(pid, _offset)| {
        ...
        Ok((
            KafkaSourceReader {
                ...
                stats_rx,
                partition_info,
                include_headers: source_connection.include_headers.is_some(),
...
impl SourceRender for KafkaSourceConnection {
    ...
    fn render<G: Scope<Timestamp = Self::Time>>(
        ...
        let (mut _progress_output, progress) = builder.new_output();
        let (mut health_output, health_stream) = builder.new_output();

        let button = builder.build(move |caps| async move {
            let [data_cap, upper_cap, health_cap]: [_; 3] = caps.try_into().unwrap();

            let resume_upper = Antichain::from_iter(
                config
                    .source_resume_upper
                    .iter()
                    .map(<Self::Time as SourceTimestamp>::decode_row),
            );

            let (mut reader, offset_committer) = KafkaSourceReader::from_connection(
                self,
                config.name,
                config.id,
                config.worker_id,
            ...
            let mut batch = vec![];
            let ingestion_loop = async move {
                loop {
                    let mut emit_cap = None;
                    while let Poll::Ready(msg_type) = reader.get_next_message() {
                        match msg_type {
                            SourceMessageType::Message(message, cap, diff) => {
                                let time = cap.time().clone();
                                match emit_cap.as_mut() {
                                    // If cap is not beyond emit_cap we can't re-use emit_cap so
                                    // flush the current batch
                                    Some(emit_cap) => {
                                        if !PartialOrder::less_equal(emit_cap, &cap) {
                                            data_output
                                                .give_container(&*emit_cap, &mut batch)
                                                .await;
                                            batch.clear();
                                            *emit_cap = cap;
                                        }
                            ...
                            SourceMessageType::SourceStatus(new_status) => {
                                health_output.give(&health_cap, new_status).await;
                            }
                        }
                    }
                    if let Some(emit_cap) = emit_cap.take() {
                        data_output.give_container(&emit_cap, &mut batch).await;
                        batch.clear();
                    }
            ...
            futures::future::join(ingestion_loop, offset_commit_loop).await;
        });
...
impl KafkaSourceReader {
    ...
    fn get_next_message(&mut self) -> Poll<SourceMessageType> {
        ...
        let mut next_message = Poll::Pending;
        ...
        if let Some(result) = self.consumer.poll(Duration::from_secs(0)) {
            match result {
                Err(e) => {
                    ...
                    next_message =
                        Poll::Ready(SourceMessageType::status(HealthStatus::StalledWithError {
                            error: message,
                            hint: None,
                        }))
                }
        ...
        while attempts < consumer_count {
            ...
            if let Poll::Ready(_) = next_message {
            ...
            .take()
        {
            ...
            if let Poll::Pending = next_message {
                next_message = Poll::Ready(SourceMessageType::status(status))
            }
        }
    ...
    fn handle_message(
        ...
        if offset_as_i64 <= last_offset {
            ...
            *last_offset_ref = offset_as_i64;
            ...
            Poll::Ready(SourceMessageType::Message(message, cap, 1))
        }
    }
}
