...
async fn compute_pk_splits(
    ...
    // Collations with contractions (Czech ch), expansions (ß), or ignorable
    // characters (NUL) break prefix probing, so only split under collations
    // known to weigh one character at a time.
    let collation: Option<String> = conn
        ...
        .await?;
    let supported = collation
        .as_deref()
        .is_some_and(|c| c == "binary" || c.ends_with("_bin") || c.ends_with("_general_ci"));
    if !supported {
