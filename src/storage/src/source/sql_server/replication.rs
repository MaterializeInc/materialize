        ...
        Box::pin(SignaledFuture::new(busy_signal, async move {
            ...
            let snapshot_lsn = {
                ...
                while let Some((capture_instance, data)) = snapshot_streams.next().await {
                    let sql_server_row = data.map_err(|e| TransientError::from(e))?;
            ...
            while let Some(event) = cdc_stream.next().await {
                let event = event.map_err(|e| TransientError::from(e))?;
                tracing::info!(?event, "got replication event");
                ...
                let (capture_instance, commit_lsn, changes) = match event {
                    ...
                    CdcEvent::Progress { next_lsn } => {
                        tracing::info!(?next_lsn, "got a closed lsn");
                    ...
                    CdcEvent::Data {
                        ...
                        lsn,
                        changes,
                        // TODO(sql_server1): Communicate with the upstream that we finished.
                        mark_complete: _,
