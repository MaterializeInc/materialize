...
pub struct StorageState {
    ...
    pub oneshot_ingestions: BTreeMap<GlobalId, OneshotIngestionDescription>,
    /// Undocumented
    pub now: NowFn,
        ...
        for (ingestion_id, ingestion_state) in &mut self.storage_state.oneshot_ingestions {
            loop {
                match ingestion_state.results.try_recv() {
                    Ok(Ok((batch, row_count))) => {
                        let staged_batches =
                            BTreeMap::from([(*ingestion_id, vec![(batch, row_count)])]);
                        let _ = response_tx.send(StorageResponse::StagedBatches(staged_batches));
                    }
                    Ok(Err(err)) => {
                        tracing::error!(?err, "OH NO");
