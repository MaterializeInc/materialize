            ...
            while let Some(command) = command_rx.recv().await {
                match command {
                    ...
                    AsyncStorageWorkerCommand::UpdateSinkFrontiers(id, mut description) => {
                        ...
                        let upper = write_handle.fetch_recent_upper().await;
                        description.as_of = Antichain::from_iter(
                            upper
                                .iter()
                                .map(|t| t.step_back().unwrap_or_else(T::minimum)),
                        );
                        description.with_snapshot =
                            description.with_snapshot && **upper == [T::minimum()];
                        let res =
                            response_tx.send(AsyncStorageWorkerResponse::SinkFrontiersUpdated {
                                id,
                                description,
                            });
