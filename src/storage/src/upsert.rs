    ...
    FromTime: Ord,
{
    ...
    stash.extend(data.drain(..).map(|((key, value, order), time, diff)| {
        ...
        // WIP: Don't retain a cap per update but instead bunch them up.
    ...
    E: UpsertErrorEmitter<G>,
{
    ...
    tracing::debug!(?drain_style, updates = idx, "draining stash in upsert");
    ...
    E: UpsertErrorEmitter<G>,
{
    ...
    tracing::debug!(
        ?persist_upper,
        updates = idx,
        "ingesting state updates in upsert"
    );
    ...
    FromTime: Debug + timely::ExchangeData + Ord,
{
    ...
    let shutdown_button = builder.build(move |caps| async move {
        ...
        // For when we rebase on top of the with-permit code.
        // Only acquire the permit that blocks reading source updates when we're
        // expecting an actual snapshot of our previous state from the persist
        // input. And are only doing that when the resume upper is > MIN.
        //let mut permit = if upsert_config.delay_sources_past_rehydration && !resume_upper.less_equal(&G::Timestamp::minimum()) {
        //    // Grab the busy signal to avoid the upstream operators from doing any work
        //    // while we are rehydrating.
        //    tracing::debug!(?resume_upper, "acquiring backpressure permit");
            ...
            tokio::select! {
                ...
                input_event = input.next() => {
                    ...
                    for (i, event) in events {
                        tracing::debug!(?event, "input events");
                        ...
                        match event {
                            ...
                            AsyncEvent::Progress(upper) => {
                                //tracing::error!(?upper, "received progress in upsert");
                                ...
                                if PartialOrder::less_than(&upper, &resume_upper) {
                                    //tracing::error!(?upper, ?resume_upper, "continue");
                                    continue;
                                }

                                //tracing::error!("draining staged input");
                                ...
                                //tracing::error!(?output_updates, "output updates");
                                ...
                                if let Some(ts) = upper.as_option() {
                                    //tracing::error!(?ts, "downgrade cap");
                                    output_cap.try_downgrade(ts);
                                }
                                input_upper = upper;
                            }
                        }
                    ...
                    if let Some(partial_drain_time) = partial_drain_time {
                        ...
                        //.await;
                        //
                        ////tracing::error!(?output_updates, "output updates, because of partial");
                        //
                        //for (update, cap, diff) in output_updates.drain(..) {
                        //    let time = cap.time().clone();
                        //    output_handle.give(&cap, (update, time, diff));
                        //}
        ...
        // WIP: For rebasing on top of the with-permit code.
        // Never drop it! TODO: Don't need the token.
    });
