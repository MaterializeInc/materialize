...
use sql::normalize;
use sql::plan::{
    scalar_type_from_sql, unwrap_numeric_typ_mod, MutationKind, Plan, PlanContext,
        ...
        Ok(match stmt {
            Statement::CreateTable(CreateTableStatement {
                ...
                for (index, column) in columns.iter().enumerate() {
                    let ty = scalar_type_from_sql(&scx, &column.data_type)?;
    ...
    match sql_type {
        DataType::Other {
            ...
            match name.to_string().as_str() {
                ...
                "numeric" => {
                    let (_, desired_scale) = unwrap_numeric_typ_mod(typ_mod)?;
