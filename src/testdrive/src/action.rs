...
impl State {
    pub(crate) async fn reset_materialized(&mut self) -> Result<(), Error> {
        for row in self
            .pgclient
    ...
    pub(crate) async fn reset_kinesis(&mut self) -> Result<(), Error> {
    ...
    pub(crate) async fn reset_s3(&mut self) -> Result<(), Error> {
        if self.s3_buckets_created.is_empty() {
            return Ok(());
        }

        let buckets: Vec<String> = self.s3_buckets_created.iter().cloned().collect();

        let mut errors: Vec<Box<dyn std::error::Error + Send + Sync>> = Vec::new();
        for bucket in buckets.into_iter() {
            // you can't delete a bucket that has stuff in it
            ...
            loop {
                ...
                match response {
                    ...
                    Err(e) => {
                        errors.push(
                            Error::with_cause(format!("Listing objects for bucket {}", bucket), e)
                                .into(),
                        );
                    }
                }
...
pub async fn create_state(
    ...
    let state = State {
        ...
        aws_credentials: aws_info.credentials.unwrap().into(),
        kinesis_client,
