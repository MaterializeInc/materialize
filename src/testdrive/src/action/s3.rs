...
//
// Use of this software is governed  filter: (), id: () filter: (), id: ()by the Business Source License
...
pub fn build_create_bucket(mut cmd: BuiltinCommand) -> Result<CreateBucketAction, String> {
    Ok(CreateBucketAction {
        ...
        cleanup: cmd.args.opt_bool("cleanup")?,
    })
}
...
impl Action for AddBucketNotifications {
    ...
    async fn redo(&self, state: &mut State) -> Result<(), String> {
        ...
        // Allow the S3 bucket to write to this queue
        ...
        let mut writer = xml::EventWriter::new_with_config(
            Vec::new(),
            xml::EmitterConfig::new().perform_indent(true),
        );
        rusoto_s3::NotificationConfigurationSerializer::serialize(
            &mut writer,
            "NotificationConfiguration",
            &config,
        )
        .map_err(|e| format!("serializing notification configuration: {}", e))?;
        println!(
            "config: {:#?}\nserialized: {}",
            config,
...

        {
            ...
            // // rusoto reserializes empty Filters incorrectly
            // for qc in queue_configs.iter_mut() {
            //     qc.filter = None;
            // }
        ...
        .map_err(|e| format!("serializing notification configuraiont: {}", e))?;
        println!(
            "config modified: {:#?}\nserialized modified: {}",
            config,
            std::str::from_utf8(&writer.into_inner()).unwrap()
        ...
        eprintln!("created bucket notification configuration: {:?}", result);
