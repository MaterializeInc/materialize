...
pub fn build_add_notifications(mut cmd: BuiltinCommand, state: &State) -> Result<AddBucketNotifications, String> {
    let events = cmd
        .args
        .opt_string("events")
    ...
    Ok(AddBucketNotifications {
        ...
        sqs_validation_timeout: cmd
            ...
            .transpose()?
            .unwrap_or_else(|| Duration::from_secs_f64(if state.default_timeout > 120.0 { state.default_timeout } else { 120.0 })),
    })
}
