...
use crate::action::{Action, State, SQLContext};
...
pub fn build_fail_sql(cmd: FailSqlCommand, sql_context: SQLContext) -> Result<FailSqlAction, String> {
    Ok(FailSqlAction { cmd, sql_context: sql_context })
}
                ...
                match err.source().and_then(|err| err.downcast_ref::<DbError>()) {
                    Some(err) => {
                        ...
                        if let Some(regex) = &self.sql_context.regex {
                            err_string = regex.replace_all(&err_string, self.sql_context.regex_replacement.as_str()).to_string();
                        }
...
fn decode_row(row: Row, sql_context: SQLContext) -> Result<Vec<String>, String> {
    ...
    for (i, col) in row.columns().iter().enumerate() {
        ...
        if let Some(regex) = &sql_context.regex {
           value = regex.replace_all(&value, sql_context.regex_replacement.as_str()).to_string();
        }
