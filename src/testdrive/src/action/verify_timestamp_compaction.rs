        ...
        if let Some(path) = &state.materialized_data_path {
            ...
            Retry::default()
                ...
                .retry_async_canceling(|retry_state| {
                    ...
                    async move {
                        let catalog = Catalog::open_debug(&path.join("catalog"), &path, NOW_ZERO.clone())
                            .await?;
                        let item_id = catalog
                        ...
                        let stash = Stash::open(&path.join("storage/timestamp_bindings"))?;
                        let collection = stash
                            .collection::<PartitionId, ()>(&item_id.to_string())?;
