...
impl Origin {
    ...
    /// Whether a body of this origin is in flight on a dataflow edge, which is
    /// the population edge paging covers.
    ///
    /// A retained body is excluded because its holder moves it into a store of
    /// its own: the correction buffer hands its chunks to the column pager,
    /// which would copy a paged body straight back out of the pool to take it.
    pub const fn is_edge(self) -> bool {
        matches!(self, Origin::Ship | Origin::Consolidate)
    }
...
impl AlignBuffer {
    ...
    /// body is not an edge body, is not worth a slot, or no pool is installed.
    ///
    ...
    where
        A: AsBytes<'a>,
    {
        if !origin.is_edge() || words * 8 < PAGE_MIN_BYTES {
            return None;
        }
        let pool = edge_pool()?;
