...
pub trait UnloadBatch<C: UnloadChunk> {
    ...
    /// [`extract_into`](UnloadChunk::extract_into)) only the chunks whose key
    /// span contains a probe. Probes that fall in the gap between two chunks'
    /// spans are consumed against resident metadata alone. A probe left
    /// unconsumed at a chunk's last key is re-offered to the next chunk,
