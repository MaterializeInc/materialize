...
//!
//! Chunk spill consumers ([`crate::columnar::chunk`] and compute's
//! correction buffers) resolve their backing through [`active_pool`] at each
//! spill decision, so live reconfiguration takes effect on the next call.
...
/// budgeted it; never initializes anything. Chunk spill consumers resolve
/// their backing through this at each spill decision, so an unconfigured
/// process keeps every chunk resident.
...
/// spill mechanism. Returns `false` (and changes nothing) if the pool is
/// unavailable because its virtual reservation failed.
///
