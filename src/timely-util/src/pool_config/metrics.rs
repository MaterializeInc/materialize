...
pub fn register(registry: &MetricsRegistry) {
    ...
    REGISTERED.get_or_init(|| {
        ...
        // monotonic.
        gauge(registry, metric!(name: "mz_column_pool_spill_reads_total", help: "Nonresident pool reads queued on the spill threads."), |s| s.spill_reads);
        gauge(registry, metric!(name: "mz_column_pool_spill_reads_in_flight", help: "Queued pool reads whose copy has not completed."), |s| s.spill_reads_in_flight);
