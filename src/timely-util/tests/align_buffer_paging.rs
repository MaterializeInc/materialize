...
fn edge_paging() {
    ...
    // A retained body stays on the heap: its holder moves it into a store of
    // its own, which would copy a paged body straight back out.
    let c = container(RECORDS);
    let view = c.borrow();
    let retained =
        AlignBuffer::encode(Origin::Correction, usize::try_from(RECORDS).unwrap(), &view);
    assert!(!retained.is_paged(), "only edge bodies are paged");
