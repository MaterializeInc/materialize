...
// No unit tests live here: this is a build-script module (`#[path]`-included by
// `build.rs`) and `chumsky` is a build-only dependency, so the parser is not
// reachable from the crate's lib/test target. The grammar is covered instead by
// `cargo check` failing to build when a real `.rewrite` rule fails to parse.
