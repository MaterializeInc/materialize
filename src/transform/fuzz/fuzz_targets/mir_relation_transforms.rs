...
//!     as folding the original — a genuine correctness check.
//!
...
//!
//! Generation richness (vs. the original shallow shapes):
//!
...
fn gen_rel(u: &mut Unstructured, depth: u32) -> arbitrary::Result<(MirRelationExpr, Vec<Ty>)> {
    ...
    Ok(match u.int_in_range(0u8..=5)? {
        ...
        _ => {
            ...
            // of `inner`; an extra Negate goes on exactly one of them so the pair
