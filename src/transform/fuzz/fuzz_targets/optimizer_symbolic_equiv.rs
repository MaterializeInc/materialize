...
//! an optimizer panic is a genuine finding — and now it covers the symbolic-input
...
enum Collapse {
    ...
    StuckFixpoint,
    /// Hit the iteration budget without reaching either a constant or a fixpoint.
    /// The plan was still simplifying — we simply ran out of passes. This is the
    /// case the original fixed cap silently treated as a skip; we surface it so a
    /// heavily-`Let`-nested optimized plan that *would* fold isn't masked.
    BudgetExhausted,
}
...
///
/// Unlike a fixed small cap, this loops to a genuine fixpoint (stops only when a
/// pass leaves the plan unchanged), so a plan that just needs a few more passes
/// converges instead of being silently dropped. The budget is a generous guard
/// against a non-terminating rewrite.
...
fn run(u: &mut Unstructured) -> arbitrary::Result<()> {
    ...
    match collapse(substitute(optimized, &data)) {
        ...
        // The optimized plan reached a non-constant fixpoint: a genuine fold
        // limitation (e.g. an operator `FoldConstants` does not evaluate). The
        // baseline folded, so this is a conservative skip, not a divergence.
        Collapse::StuckFixpoint => {}
        // The baseline folded but the *optimized* plan was still simplifying when
        // the (generous, 64-pass) budget ran out. Because the input collapsed in
        // the same harness, the optimizer reshaped the plan into something that
        // fold + normalize-lets cannot drive to a fixpoint here — a coverage gap
        // (or a fold/normalize blowup) worth surfacing rather than silently
        // passing, which is exactly what the old fixed cap did.
        Collapse::BudgetExhausted => panic!(
            "optimized plan did not converge within the fold/normalize budget while \
             the input did; investigate (fold-limitation or non-termination)\n\
             plan = {plan:?}\ndata = {data:?}"
        ),
    }
    Ok(())
