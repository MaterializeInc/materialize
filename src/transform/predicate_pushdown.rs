...
impl PredicatePushdown {
    ...
    /// Defines a criteria for inlining scalar expressions.  A scalar expression is "safe" to
    /// inline if it only refers to non-computed columns, and:
    /// * It's a leaf, or
    /// * it's a unary function of a safe expression, or
    /// * it's a binary function having one argument be safe and one argument be a leaf.
    fn is_safe_inline(s: &ScalarExpr, input_arity: usize) -> bool {
        Self::is_safe_leaf(s, input_arity)
            || match s {
                ScalarExpr::CallUnary { func: _, expr } => Self::is_safe_inline(expr, input_arity),
                ScalarExpr::CallBinary {
                    func: _,
                    expr1,
                    expr2,
                } => {
                    Self::is_safe_leaf(expr1, input_arity)
                        && Self::is_safe_inline(expr2, input_arity)
                        || Self::is_safe_leaf(expr2, input_arity)
                            && Self::is_safe_inline(expr1, input_arity)
                }
        ...
        if let RelationExpr::Filter { input, predicates } = relation {
            match &mut **input {
                ...
                RelationExpr::Map { input, scalars } => {
                    ...
                    // expressions "safe." See the comment above is_safe_inline for more details.
                    ...
                    for mut predicate in predicates.drain(..) {
                        ...
                        if predicate.support().iter().all(|c| {
                            *c < input_arity
                                || PredicatePushdown::is_safe_inline(
                            ...
                            predicate.visit_mut(&mut |e| {
                                if let ScalarExpr::Column(c) = e {
                                    // NB: this inlining would be invalid if is_safe_inline did not
