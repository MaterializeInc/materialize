...
impl CostModel {
    ...
    ///
    /// This is the notion of "arranged" the cost model uses through
    /// [`Self::input_already_arranged`]. A single arranged-ness check keeps the
    /// cost model's memory estimate aligned with the arrangements the commit
    /// actually builds.
    ///
    ...
    /// authoritative commit-time arrangement recognizer, so the reuse decisions
    /// line up with the arrangements the commit actually builds.
