...
use mz_expr::{
    ...
    RECURSION_LIMIT, TableFunc, func,
};
        ...
        self.checked_recur(|_| {
            ...
            let actual_projection = match relation {
                ...
                MirRelationExpr::FlatMap { input, func, exprs } => {
                    ...
                    // When the values generated by a `generate_series` are not
                    // demanded, only its cardinality matters. Collapse it into a
                    // `RepeatRowNonNegative`, which encodes that cardinality in a
                    // single row's diff rather than emitting one row per value.
                    if !desired_projection.iter().any(|c| *c >= inner_arity) {
                        collapse_unused_generate_series(func, exprs);
                    }
...
/// rows.
fn collapse_unused_generate_series(func: &mut TableFunc, exprs: &mut Vec<MirScalarExpr>) {
    // Only integer `generate_series` is handled here.
    if !matches!(
        func,
        TableFunc::GenerateSeriesInt32 | TableFunc::GenerateSeriesInt64
    ) {
        return;
    }
