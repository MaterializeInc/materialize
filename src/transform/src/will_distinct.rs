...
impl WillDistinct {
    fn apply(&self, expr: &mut MirRelationExpr, derived: DerivedView) {
        ...
        while let Some((expr, derived, distinct_by)) = todo.pop() {
            ...
            if let (MirRelationExpr::Reduce { input, group_key, aggregates, .. }, Some(columns)) = (&*expr, distinct_by) {
                if aggregates.is_empty() && columns.iter().enumerate().all(|(c, e)| e == &MirScalarExpr::Column(c)) && columns.len() == group_key.len() {
                    let arity = input.arity();
                    *expr = input.clone().map(group_key.clone()).project((arity .. arity + group_key.len()).collect::<Vec<_>>());
                }
            } else {
                match (expr, distinct_by) {
                    // A distinct expression whose keys are only column references can be communicated onward.
                    (MirRelationExpr::Reduce { input, group_key, aggregates, .. }, _) => {
                        if aggregates.is_empty() && group_key.iter().all(|e| if let MirScalarExpr::Column(_) = e { true } else { false }) {
                    ...
                    (MirRelationExpr::Union { base, inputs }, exprs) => {
                        if derived.children_rev().all(|v| *v.value::<NonNegative>().unwrap()) {
                            let children_rev = inputs.iter_mut().rev().chain(Some(&mut **base));
                            todo.extend(children_rev.zip(derived.children_rev()).map(|(x,y)| (x,y,exprs.clone())));
                        } else {
                            let children_rev = inputs.iter_mut().rev().chain(Some(&mut **base));
                            todo.extend(children_rev.zip(derived.children_rev()).map(|(x,y)| (x,y,None)));
                        }
                    }
                    (x, _) => {
                        todo.extend(x.children_mut().rev().zip(derived.children_rev()).map(|(x,y)| (x,y,None)));
                    }
                }
            }
        }
