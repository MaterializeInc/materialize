...
use crate::{BinaryFunc, EvalEnv, GlobalId, Id, RelationExpr, ScalarExpr, TransformState};
...
impl FilterEqualLiteral {
    pub fn transform(&self, relation: &mut RelationExpr, state: &mut TransformState) {
        relation.visit_mut(&mut |e| {
            self.action(e, state);
        });
    }

    pub fn action(&self, relation: &mut RelationExpr, state: &mut TransformState) {
...
impl crate::Transform for FilterLifting {
    fn transform(&self, relation: &mut RelationExpr, state: &mut TransformState) {
        self.transform(relation, state);
    }
}
