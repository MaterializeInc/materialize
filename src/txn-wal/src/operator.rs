...
pub fn txns_progress<K, V, T, D, P, C, F>(
    passthrough: StreamVec<T, P>,
    name: &str,
    ctx: &TxnsContext,
    client_fn: impl Fn() -> F,
    txns_id: ShardId,
...
) -> (StreamVec<T, P>, Vec<PressOnDropButton>)
where
...
fn txns_progress_source_global<K, V, T, D, P, C>(
    scope: Scope<T>,
    name: &str,
    ctx: TxnsContext,
    ...
    data_val_schema: Arc<V::Schema>,
    unique_id: u64,
) -> (StreamVec<T, DataRemapEntry<T>>, PressOnDropButton)
where
    K: Debug + Codec + Send + Sync,
    V: Debug + Codec + Send + Sync,
    T: Timestamp + Lattice + TotalOrder + StepForward + Codec64 + Sync,
...
fn txns_progress_frontiers<K, V, T, D, P, C>(
    remap: StreamVec<T, DataRemapEntry<T>>,
    passthrough: StreamVec<T, P>,
    name: &str,
    data_id: ShardId,
    until: Antichain<T>,
    unique_id: u64,
) -> (StreamVec<T, P>, PressOnDropButton)
where
    K: Debug + Codec,
    V: Debug + Codec,
...
impl DataSubscribe {
    ...
    pub fn new(
        ...
        let mut worker = Worker::new(
            ...
            timely::communication::allocator::generic::Allocator::Thread(
                timely::communication::allocator::Thread::default(),
            ),
            Some(std::time::Instant::now()),
        );
        let (data, txns, capture, tokens) = worker.dataflow::<u64, _, _>(|outer_scope| {
            let mut outer_scope_for_shard = outer_scope.clone();
            let (data_stream, shard_source_token) =
                outer_scope.scoped::<u64, _, _>("hybrid", |scope| {
                    let client = client.clone();
                    let (data_stream, token) = shard_source::<String, (), u64, u64, i64, _, _>(
                        &mut outer_scope_for_shard,
                        scope,
                        name,
                    ...
                    (data_stream.leave(&outer_scope_for_shard), token)
                });
