...
# How long we let librdkafka work on draining a full producer queue
# before retrying the rejected produce.  `Local: Queue full` means the
# in-process queue hit `queue.buffering.max.messages` (default 100k)
# because the broker is unreachable (fault window) or because our
# produce rate outruns the network for a moment.  Polling with a
# non-zero timeout yields to librdkafka's background sender so it has
# a chance to either deliver messages or fail them with permanent
# errors that surface via the delivery callback.  100ms is a balance
# between sensitivity to shutdown and not spinning hot on poll.
QUEUE_FULL_POLL_S = 0.1

# How long to wait on shutdown for the producer to drain.  Sized to
# span at least one full fault-orchestrator MAX_ON window (~40s) plus
# margin: under sustained faulting at SIGTERM time, librdkafka still
# has up to 180s of `delivery.timeout.ms` to deliver-or-fail each
# pending message; we want to give that umbrella most of its budget
# before the container exits and prints the "TERMINATE: N messages
# still in queue" warning.
SHUTDOWN_FLUSH_TIMEOUT_S = 120
...
def _run_one_session(tombstone_prob: float) -> None:
    ...
    while not _shutdown_requested:
        ...
        # under fault injection — the broker is unreachable and
        # librdkafka's in-process queue (default 100k messages) fills
        # before the next quiet window opens.  Yielding to
        # `producer.poll(QUEUE_FULL_POLL_S)` gives the background
        # sender a chance to either deliver or fail messages; once
        # there's headroom in the queue the produce succeeds.  Letting
        # BufferError escape would crash this session and tear down
        # the producer for no reason — the queue would have drained on
        # its own once the fault window closed.
        while not _shutdown_requested:
            try:
                producer.produce(
                ...
                producer.poll(QUEUE_FULL_POLL_S)
        else:
            # Shutdown signaled while we were waiting for queue space.
            # Don't count an unproduced message.
            break
    ...
    # if the broker is gone for the entire flush window, librdkafka
    # will still print the "TERMINATE: N messages still in queue"
    # warning at destruct time — informational, not an error.
    LOG.info("hammer[%s]: draining producer before exit", INSTANCE_ID)
    producer.flush(timeout=SHUTDOWN_FLUSH_TIMEOUT_S)
