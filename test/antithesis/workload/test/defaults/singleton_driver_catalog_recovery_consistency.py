...
def _run_cycle(
    ...
    if expected and helper_random.random_bool(drop_probability):
        ...
        try:
            # IF EXISTS makes the DROP idempotent so `execute_retry`'s
            # OperationalError retries are safe: if the original DROP
            # committed pre-crash, the retry against the new incarnation
            # is a no-op success instead of a ProgrammingError
            # ("unknown catalog item") that would otherwise force the
            # driver to leave `expected` un-discarded.
        ...
        try:
            # IF NOT EXISTS: mirror of the DROP fix. If the original
            # CREATE committed pre-crash, an OperationalError retry
            # would otherwise hit "relation already exists" and the
            # driver would fail to add the (committed) table to its
            # expected set, producing a false `unexpected_in_catalog`
            # assertion later.
            execute_retry(f"CREATE TABLE IF NOT EXISTS {table} (id BIGINT NOT NULL)")
        except Exception as exc:  # noqa: BLE001
            LOG.info(
