...
from materialize import buildkite, MZ_ROOT
...
class Benchbase:
    ...
    def run_inner(cls, c: Composition, config) -> None:
        ...
        load_started = datetime.datetime.now(pytz.UTC)
        # Generate initial data.
        cls.benchbase(c, config, load=True)

        waiting_started = datetime.datetime.now(pytz.UTC)
