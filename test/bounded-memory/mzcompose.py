...
SCENARIOS = [
    PgCdcScenario(
        disabled=True,  # https://github.com/MaterializeInc/materialize/issues/15044
        ...
        + "\n".join(
            [
                dedent(
                    f"""
                    $ postgres-execute connection=postgres://postgres:postgres@postgres
                    INSERT INTO t1 (f3) SELECT REPEAT('{letter}', {PAD_LEN}) FROM generate_series(1, {REPEAT});
                    """
                )
        ...
        post_restart=PgCdcScenario.PG_POST_RESTART,
    ),
    PgCdcScenario(
        disabled=True,  # https://github.com/MaterializeInc/materialize/issues/15044
        name="pg-cdc-update",
