...
def is_dropped_element_ref_edge(
    ...
    The materialized-view conversion drops the edge to the element type of an
    array type: name resolution injects the element-type id without printing
    it in create_sql, so it cannot be recovered. The edge to the array type
    itself survives, so an old-only edge to type X is expected exactly when
...
CORPUS = [
    ...
    # baseline's injected element-type edge is not shadowed by a direct
    # reference. This row exercises the is_dropped_element_ref_edge allowlist.
