...
# Buildkite artifact that carries the minimized corpus between nightly runs so
...
# subdirectory; `bin/ci-builder` and the nightly pipeline build the same set.
FUZZ_CRATES = [
    "src/sql-parser/fuzz",
    "src/expr/fuzz",
    "src/transform/fuzz",
...
# encode/decode round-trips, simple jsonb access) are dropped — they've
...
FRUITFUL = [
    # SQL-parser round-trip oracles — ~60% of all findings.
    "parse_expr_roundtrip",
    "grammar_roundtrip",
...
def crash_input_lines(path: Path, max_bytes: int = 4096) -> list[str]:
    ...
    lines = [
        ...
        f"input base64: {base64.b64encode(shown).decode('ascii')}",
    ]
    if len(data) > max_bytes:
        lines.append(f"  (showing first {max_bytes} bytes; full input in the artifact)")
    return lines
...
class FuzzRunner:
    ...
    def _spawn(self, job: Job) -> None:
        ...
        # `cargo fuzz run`. The binary was already produced by `build()`;
    ...
    def _new_artifacts(self, job: Job) -> list[str]:
        ...
        This — not the exit code — is the source of truth for whether a crash was
        found: fork mode (`-fork=N`, the default under `--jobs auto`) is
        crash-resilient, writing the artifact and continuing to fuzz, then exiting
        0 at the time limit. Stale artifacts from earlier runs are excluded by the
        mtime check.
        """
        ...
        return sorted(
            ...
            if p.name.startswith(("crash-", "oom-", "timeout-", "slow-unit-", "leak-"))
    ...
    def _failure_block(self, job: Job, secs: int) -> str:
        ...
        lines.append(
            f"reproduce: cd {job.crate} && cargo fuzz run {san}"
            f"{job.target} artifacts/{job.target}/{repro}"
        )
    ...
    def build(self) -> None:
        ...
        for i, crate in enumerate(crates, 1):
            ...
            if subprocess.run(cmd, cwd=MZ_ROOT / crate, env=self.env).returncode != 0:
                say(f"  build FAILED for {crate} — skipping its targets")
                failed.append(crate)
        if failed:
            # Don't let one unbuildable crate sink the rest (e.g. a macOS-only
            # cdylib link failure shouldn't stop fuzzing the SQL parser).
            self.jobs = [j for j in self.jobs if j.crate not in failed]
            say(f"skipped {len(failed)} unbuildable crate(s): {', '.join(failed)}")
            if not self.jobs:
                raise ui.UIError("all fuzz crates failed to build")
    ...
    def _minimize(self) -> None:
        ...
        say(
            ...
            f"(cargo fuzz cmin, up to {limit} at once)…"
        )
        ...
        try:
            ...
            # Kill whatever's still merging and drop their scratch dirs; the
            # original corpora are untouched until a merge succeeds, so this is
            # always safe.
