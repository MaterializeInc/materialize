...
mz::createSource(pqxx::connection &c, const std::string& kafkaUrl, const std::string& registry, const std::string& source, const std::string& consistencySource, bool materialized) {
    ...
    try {
		...
		std::string kafka_conn_creation = "CREATE CONNECTION IF NOT EXISTS kafka_conn FROM KAFKA BROKER '" + kafkaUrl + "', SECURITY PROTOCOL PLAINTEXT";
		w.exec0(kafka_conn_creation);
		std::string source_creation = "CREATE " + mat + " SOURCE IF NOT EXISTS " + source + " FROM KAFKA CONNECTION kafka_conn (TOPIC '" + topic + "') " + consistency + " FORMAT AVRO USING CONFLUENT SCHEMA REGISTRY CONNECTION csr_conn ENVELOPE DEBEZIUM";
