...
def test_create_drop_source(mz: MaterializeApplication) -> None:
    ...
    mz.testdrive.run(
        input=dedent(
            ...
            >[retry] SELECT COUNT(*) FROM mz_internal.mz_source_status_history JOIN ( SELECT source_id AS src, max(occurred_at) AS ocr FROM mz_internal.mz_source_status_history GROUP BY source_id ) AS newest ON newest.src = source_id AND newest.ocr = occurred_at WHERE status = 'running';
            0
            """
        ),
        no_reset=True,
