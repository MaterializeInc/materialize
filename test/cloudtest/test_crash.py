...
def populate(mz: MaterializeApplication, seed: int) -> None:
    mz.testdrive.run(
        input=dedent(
            ...
            > CREATE MATERIALIZED VIEW v1 WITH (RETAIN HISTORY FOR '30s') AS SELECT COUNT(*) FROM t1 UNION ALL SELECT COUNT(*) FROM s1;
...
def test_crash_clusterd(mz: MaterializeApplication) -> None:
    ...
    mz.environmentd.sql(
        "CREATE MATERIALIZED VIEW crash_view WITH (RETAIN HISTORY FOR '30s') AS SELECT mz_unsafe.mz_panic(f1) FROM crash_table"
    )
