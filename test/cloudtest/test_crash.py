...
def populate(mz: MaterializeApplication, seed: int) -> None:
    mz.testdrive.run(
        input=dedent(
            ...
            > CREATE MATERIALIZED VIEW v1 WITH (REFRESH EVERY '2 seconds') AS SELECT COUNT(*) FROM t1 UNION ALL SELECT COUNT(*) FROM s1;
...
def test_crash_clusterd(mz: MaterializeApplication) -> None:
    ...
    mz.environmentd.sql(
        "CREATE MATERIALIZED VIEW crash_view WITH (REFRESH EVERY '2 seconds') AS SELECT mz_unsafe.mz_panic(f1) FROM crash_table"
    )
