...
def test_crash_environmentd(mz: MaterializeApplication) -> None:
    def epoch(p: V1StatefulSet) -> int:
        epoch_label = "environmentd.materialize.cloud/epoch"
        assert p.metadata is not None
        assert p.metadata.labels is not None
        return int(p.metadata.labels[epoch_label])
