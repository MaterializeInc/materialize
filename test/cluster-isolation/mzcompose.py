...
> CREATE MATERIALIZED VIEW sleep_view WITH (RETAIN HISTORY FOR '30s') AS SELECT mz_unsafe.mz_sleep(sleep) FROM sleep_table;
...
> CREATE MATERIALIZED VIEW v2 WITH (RETAIN HISTORY FOR '30s') AS SELECT COUNT(*) AS c1 FROM t1;

> SELECT * FROM v2;
3

> CREATE MATERIALIZED VIEW v1mat WITH (RETAIN HISTORY FOR '30s') AS SELECT * FROM v1;
