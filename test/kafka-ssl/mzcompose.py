...
def workflow_default(c: Composition, parser: WorkflowArgumentParser) -> None:
    ...
    client_cert = c.exec(
        "materialized", "cat", "/share/secrets/materialized.crt", capture=True
    )
    client_key = c.exec(
        "materialized", "cat", "/share/secrets/materialized.key", capture=True
    )
    test_args = [
        f"--var=ca-cert={ca_cert.stdout}",
        f"--var=client-cert={client_cert.stdout}",
        f"--var=client-key={client_key.stdout}",
        *args.files,
    ]
