...
class PostgresSource(Object):
    ...
    def manipulate(self, kind: int) -> str:
        manipulations = [
            ...
            lambda: dedent(
                f"""
                > ALTER TABLE {self.name} RENAME TO {self.name}_tmp;
                > ALTER TABLE {self.name}_tmp RENAME TO {self.name};
                """
            ),
        ]
        return manipulations[kind % len(manipulations)]()
...
class LoadGeneratorSource(Object):
    ...
    def manipulate(self, kind: int) -> str:
        manipulations = [
            ...
            lambda: dedent(
                f"""
                > ALTER SOURCE {self.name} RENAME TO {self.name}_tmp;
                > ALTER SOURCE {self.name}_tmp RENAME TO {self.name};
                """
            ),
        ]
        return manipulations[kind % len(manipulations)]()
...
class KafkaSink(Object):
    ...
    # TODO: Reenable when database-issues#9048 is fixed
    enabled: bool = False
    ...
    def create(self) -> str:
        ...
        cmds.append(
            ...
            > CREATE SINK {self.name}
              ...
              INTO KAFKA CONNECTION kafka_conn (TOPIC '{self.name}')
              FORMAT {self.format}
              ENVELOPE DEBEZIUM"""
            )
        )
    ...
    def manipulate(self, kind: int) -> str:
        manipulations = [
            ...
            lambda: dedent(
                f"""
                > ALTER SINK {self.name} RENAME TO {self.name}_tmp;
                > ALTER SINK {self.name}_tmp RENAME TO {self.name};
                """
            ),
        ]
        return manipulations[kind % len(manipulations)]()
...
class View(Object):
    ...
    def manipulate(self, kind: int) -> str:
        manipulations = [
            ...
            lambda: dedent(
                f"""
                > ALTER VIEW {self.name} RENAME TO {self.name}_tmp;
                > ALTER VIEW {self.name}_tmp RENAME TO {self.name};
                """
            ),
        ]
        return manipulations[kind % len(manipulations)]()
...
class MaterializedView(Object):
    ...
    def manipulate(self, kind: int) -> str:
        manipulations = [
            ...
            lambda: dedent(
                f"""
                > ALTER MATERIALIZED VIEW {self.name} RENAME TO {self.name}_tmp;
                > ALTER MATERIALIZED VIEW {self.name}_tmp RENAME TO {self.name};
                """
            ),
        ]
        return manipulations[kind % len(manipulations)]()
        ...
        for i in range(tries):
            try:
                ...
                if i == tries - 1:
                    ...
                    raise
                print(f"Failed to run fragment, retrying ({i}/{tries})")
