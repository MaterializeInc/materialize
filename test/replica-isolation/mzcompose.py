...
def populate(c: Composition) -> None:
    ...
    c.testdrive(
        dedent(
            ...
            > CREATE MATERIALIZED VIEW v1 WITH (REFRESH EVERY '2 seconds') AS SELECT COUNT(*) AS c1 FROM t1;
            > CREATE TABLE ten (f1 INTEGER);
            > INSERT INTO ten SELECT * FROM generate_series(1, 10);
            > CREATE MATERIALIZED VIEW expensive WITH (REFRESH EVERY '2 seconds') AS SELECT (a1.f1 * 1) +
              (a2.f1 * 10) +
              (a3.f1 * 100) +
              (a4.f1 * 1000) +
              (a5.f1 * 10000) +
            ...
            > CREATE SOURCE source1
              ...
              FORMAT BYTES
            > CREATE MATERIALIZED VIEW v2 WITH (REFRESH EVERY '2 seconds') AS SELECT COUNT(*) FROM source1
            """
        ),
    )
...
def validate(c: Composition) -> None:
    ...
    c.testdrive(
        dedent(
            ...
            > CREATE MATERIALIZED VIEW v3 WITH (REFRESH EVERY '2 seconds') AS SELECT COUNT(*) AS c1 FROM t1;
            > SELECT * FROM v3;
            11
