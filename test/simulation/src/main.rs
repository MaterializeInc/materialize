...
use mz_build_info::{DUMMY_BUILD_INFO, build_info};
...
use mz_server_core::listeners::ListenersConfig;
...
fn envd_startup(mut sim: Sim) {
    sim.host("envd", || async {
        ...
        let persist_config =
            PersistConfig::new(&DUMMY_BUILD_INFO, now.clone(), mz_dyncfgs::all_dyncfgs());
    ...
    while sim.elapsed() < Duration::from_secs(60) {
        sim.step().unwrap();
    }
}
