...
def workflow_backup_restore(c: Composition) -> None:
    ...
    with c.override(
        ...
        SqlServer(volumes_extra=["mssqldata:/var/lib/mssql/data", "tmp:/scratch"]),
    ):
        for scenario in scenarios:
...
def backup_restore_sql_server(c: Composition) -> None:
    ...
    c.exec(
        ...
        "bash",
        "-c",
        f"mkdir {backup_dir} && chown -R sql-server:sql-server {backup_dir}",
    )
    c.exec(
        "sql-server",
        ...
        "localhost,1433",
        "-U",
        f"${SqlServer.DEFAULT_USER}",
        "-P",
        f"${SqlServer.DEFAULT_SA_PASSWORD}",
        "-Q",
    ...
    # What are we testing by doing this in postgres? Is there missing coverage if I don't perform
    # file manipulation for the sql server tests?
    #
    # Perform a point-in-time recovery from the backup to sql server
    ...
    # Wait for sql server to become usable again
    c.up("sql-server")
    c.exec(
        "sql-server",
        "/opt/mssql-tools/bin/sqlcmd",
        ...
        f"${SqlServer.DEFAULT_SA_PASSWORD}",
        "-Q",
        "RESTORE DATABASE test FROM DISK = N'/var/opt/mssql/data/test.bak' WITH MOVE N'test' TO N'/var/opt/mssql/data/test.mdf', REPLACE, STATS = 5",
        backup_dir,
    )
    c.run_testdrive_files(
        f"--var=default-sql-server-user={SqlServer.DEFAULT_USER}",
    ...
    # Check state of the sql server source
    c.run_testdrive_files(
        f"--var=default-sql-server-user={SqlServer.DEFAULT_USER}",
        f"--var=default-sql-server-password={SqlServer.DEFAULT_SA_PASSWORD}",
