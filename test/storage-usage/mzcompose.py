...
database_objects = [
    ...
    DatabaseObject(
        ...
        testdrive=dedent(
            """
            > CREATE MATERIALIZED VIEW obj WITH (RETAIN HISTORY FOR '30s') AS SELECT generate_series::text , REPEAT('x', 1024) FROM generate_series(1, 1024)
            """
        ),
        expected_size=1024 * 1024,
    ),
    ...
    DatabaseObject(
        ...
        testdrive=dedent(
            ...
            > CREATE MATERIALIZED VIEW obj WITH (RETAIN HISTORY FOR '30s') AS SELECT COUNT(*) FROM t1;
            """
        ),
