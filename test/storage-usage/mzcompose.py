...
database_objects = [
    ...
    DatabaseObject(
        ...
        testdrive=dedent(
            """
            > CREATE MATERIALIZED VIEW obj WITH (REFRESH EVERY '2 seconds') AS SELECT generate_series::text , REPEAT('x', 1024) FROM generate_series(1, 1024)
            """
        ),
        expected_size=1024 * 1024,
    ),
    ...
    DatabaseObject(
        ...
        testdrive=dedent(
            ...
            > CREATE MATERIALIZED VIEW obj WITH (REFRESH EVERY '2 seconds') AS SELECT COUNT(*) FROM t1;
            """
        ),
